...
\begin{proposition}[Reverse triangle inequality] \label{prop:reverseTriangleInequality}
...
\end{proposition}
\begin{proof}
The reverse triangle inequality, $|\norm{v}-\norm{w}| \leq \norm{v-w}$, implies that the norm is Lipschitz continuous with Lipschitz constant $1$, so we can use \ref{lemma:LipschitzcontinuousContinuous}.
\end{proof}
...
&= (\norm{v}+\norm{w})^2.
\end{align*}
The other inequalities are the reverse triangle inequalities \ref{prop:reverseTriangleInequality}.
\end{proof}
...
\begin{proposition} \label{prop:ominusUnderIsometry}\label{prop:ominusSubspace}
...
\end{proof}
\begin{corollary} \label{corollary:perpToDenseSet}
...
\end{proof}
\begin{corollary}[Bessel inequality] \label{corollary:BesselInequality}
...
\begin{proposition} \label{prop:exitenceMaximalOrthonormalSet}
\begin{itemize}
...
Maximal orthonormal families are easy to construct, but do not have the nice properties of orthonormal bases (see below). We would really like the concepts of orthonormal basis and maximal orthonormal family to coincide. We will see they coincide exactly in Hilbert spaces (see \ref{theorem:criterionHilbertSpace}).
...
\begin{lemma} \label{lemma:quadraticFormInverseOperator}
...
\end{lemma}
\begin{proof}
Immediate from \ref{lemma:quadraticFormInverseOperator}.
\end{proof}
\begin{corollary}
Let $T$ be an invertible operator. Then $T$ is positive (definite) \textup{if and only if} $T^{-1}$ is positive (definite).
\end{corollary}

\begin{lemma} \label{lemma:positiveOperatorPositiveEnergyForm}
...
\begin{proposition} \label{prop:normNumRadius}
...
\end{proposition}
\begin{proof}
(1) The first claim follows simply from the Cauchy-Schwarz inequality \ref{theorem:CauchySchwarz}
\[ |J(u)| \leq \frac{\norm{u}\,\norm{Tu}}{\norm{u}^2} = \frac{\norm{Tu}}{\norm{u}} \leq \frac{\norm{T}\norm{u}}{\norm{u}} = \norm{T}. \]

(2) For the second claim we need to also show the inverse inequality. By \ref{lemma:operatorNormInnerProduct} it is enough to show that $|\inner{w,Tv}| \leq \nr(T)$ for all $v\in \dom(T)$ and $w\in\im(T)$ with $\norm{v} = 1 = \norm{w}$.
