...
\end{itemize}
A \udef{seminorm} is a function $V\to \R$ that is subadditive and absolutely homogeneous.
...
\begin{enumerate}
\item If $p: V\to \R$ is a seminorm and $\lambda\in \F$, then
\[ \setbuilder{v\in V}{p(v) < \lambda} \qquad\text{and}\qquad \setbuilder{v\in V}{p(v) \leq \lambda} \]
are absolutely convex and absorbent.
\item For each absolutely convex and absorbent subset $A\subseteq V$ there exists a seminorm $p$ defined by
\[ p_A: V\to \R: v\mapsto \inf\setbuilder{\lambda\in \R^{\geq 0}}{v\in \lambda A} \]
with the property that
\[ \setbuilder{v\in V}{p(v) < \lambda} \subseteq A \subseteq \setbuilder{v\in V}{p(v) \leq \lambda}. \]
\end{enumerate}
\end{proposition}
\begin{proof}
...
(2) We first show that $p$ is a seminorm.
\end{proof}

\begin{definition}
The seminorm $pc_A$ defined in \ref{gaugeSeminorms} is called the \udef{gauge} of $A$.
\end{definition}
