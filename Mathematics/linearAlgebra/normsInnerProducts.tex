...
\begin{theorem}[Polarisation identities]
...
Set $M = \sup_{w\in \im(T),v \in \dom(T)} \frac{|\inner{w,Tv}|}{\norm{w}\,\norm{v}}$. Then $M\leq \norm{T}$ follows from the Cauchy-Schwarz inequality \ref{theorem:CauchySchwarz}:
\[ \frac{|\inner{w,Tv}|}{\norm{w}\,\norm{v}} \leq \frac{\norm{w}\,\norm{Tv}}{\norm{w}\,\norm{v}} = \frac{\norm{Tv}}{\norm{v}} \leq \frac{\norm{T}\,\norm{v}}{\norm{v}} = \norm{T}. \]
For the opposite inequality, we have
\[ \setbuilder{|\inner{Tv,Tv}|}{v\in\dom{T}\land \norm{v} = 1} \subseteq \setbuilder{|\inner{w,Tv}|}{w\in \im(T)\land v\in\dom{T}\land \norm{w} = 1 = \norm{v}} \]
so $M \geq \sup\setbuilder{|\inner{Tv,Tv}|}{v\in\dom{T}\;\land\; \norm{v} = 1} = \norm{T}$, by \ref{prop:operatorNorm}.
\end{proof}
...
\[ Q: \dom(T)\to \F: u\mapsto \inner{u,Tu}. \]
\end{definition}
\begin{lemma}
...
\[ Q(u) = \inner{u,Tu} = \inner{Tu,u} = \overline{\inner{u,Tu}} = \overline{Q(u)}. \]
\end{proof}
...
\[ J: \dom(T)\setminus\{0\}\to \F: u\mapsto \frac{Q(u)}{\norm{u}^2} = \frac{\inner{u,Tu}}{\norm{u}^2}. \]
\end{definition}
...
\[ \nr(T) \defeq \sup_{u\in \dom(T)\setminus\{0\}} |J(u)| \]
is the \udef{numerical radius}.
\end{definition}

\begin{proposition}
Let $T$ be a bounded operator on an inner product space $V$, then
\[ |J(u)| \leq \norm{T} \qquad \forall u\in \dom(T)\setminus\{0\}. \]
If $T$ is also symmetric, then
\[ \norm{T} = \sup_{u\in \dom(T)\setminus\{0\}} |J(u)|. \]
\end{proposition}
\begin{proof}
The first claim follows simplif from the Cauchy-Schwarz inequality \ref{theorem:CauchySchwarz}
\[ |J(u)| \leq \frac{\norm{u}\,\norm{Tu}}{\norm{u}^2} = \frac{\norm{Tu}}{\norm{u}} \leq \frac{\norm{T}\norm{u}}{\norm{u}} = \norm{T}. \]
For the second claim we need to also show the inverse inequality. Set $M = \sup_{u\in \dom(T)\setminus\{0\}} |J(u)|$ and take $x\in\dom(T)$, $y\in V$.

Since the associated quadratic form $Q$ is real-valued we have the inequalities
\begin{align*}
\inner{u+v}
\end{align*}

{lemma:operatorNormInnerProduct}
\end{proof}