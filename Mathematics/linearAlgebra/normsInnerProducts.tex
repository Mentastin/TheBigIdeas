...
\end{proposition}
\begin{proof}
As both $V$ and $\R$ are sequential spaces, it is enough, by \ref{prop:sequentialContinuity}, to show that $x_i\to x$ implies $\norm{x_i}\to \norm{x}$. Take such a sequence $x_i\to x$. Let $\epsilon> 0$. There exists an $N\in\N$ such that for all $n\geq N: \norm{x_n - x}\leq \epsilon$. Then
...
\begin{definition}
An operator $L$ between normed vector spaces is called \udef{bounded} if $\exists M>0$ such that $\forall v\in \dom(L)$
\[ \norm{L(v)} \leq M \norm{v}. \]
...
\item[$\boxed{(2) \Rightarrow (3)}$] From continuity at zero, there exists a $\delta>0$ such that $\norm{L(h)} = \norm{L(h)-L(0)} \leq 1$ for all $h\in \dom(L)$ with $\norm{h}\leq \delta$. Thus for all nonzero $v\in \dom(L)$
\[ \norm{L(v)} = \norm{\frac{\norm{v}}{\delta}L(\delta \frac{v}{\norm{v}})} = \frac{\norm{v}}{\delta}\norm{L(\delta \frac{v}{\norm{v}})}\leq \frac{\norm{v}}{\delta}. \]
\item[$\boxed{(3) \Rightarrow (1)}$] For all non-zero vectors $v,h\in \dom(L)$ we have
\[ \norm{L(v+h)-L(v)} = \norm{L(h)}\leq M \norm{h} = M\norm{(v+h) - v}. \]
\end{itemize}
\end{proof}
...
\end{align*}
\end{proposition}
This norm is called the \udef{operator norm}.
...
\subsection{Open and closed operators}
TODO definition: closed meaning closed graph or closed map?
