...
\begin{lemma}
Let $V$ be a vector space and $B\subseteq V$ a subset. Then $\convex(B) = \sSet{rx + (1-r)y}{0\leq r \leq 1, x,y\in B}$.
\end{lemma}
...
\begin{definition}
A subset $B$ of a vector space $V$ over a field $\F$ with valuation $|\cdot|$ is called \udef{balanced} if for all real $|r|\leq 1$, $rC \subseteq C$.

The closure of a set $X\subseteq V$ into the lattice of balanced sets is called the \udef{balanced hull} of $X$ and is denoted $\balanced(X)$.
\end{definition}
Note that $0$ is an element of any balanced set.

\begin{lemma}
Let $V$ be a vector space and $B\subseteq V$ a subset. Then $\balanced(B) = \bigcup_{|r|\leq 1}rB$.
\end{lemma}
...
\begin{lemma}
Let $V$ be a vector space and $X\subseteq V$ a subset. Then $X$ is absolutely convex \textup{if and only if} for all $x,y\in X$ and $|r| \leq 1$:  $rx + (1-r)y\in X$.
\end{lemma}
