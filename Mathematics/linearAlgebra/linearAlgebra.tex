...
\begin{lemma}
Let $V$ be a Riesz space, $\epsilon > 0$, $v\in V$ and $x\in V$. Then
\begin{enumerate}
...
\end{lemma}
\begin{proof}
(1) Since $S_v$ is absolutely convex by \ref{principalSolidSetsAbsolutelyConvex}, it is in particular semibalanced. Take arbitrary $y\in V$. We calculate, using \ref{gaugeLemma},
\begin{align*}
y \in \ball_v(x, \epsilon) &\iff p_{S_v}(x-y) < \epsilon \\
&\implies \epsilon^{-1}(y-z) \in S_v \\
&\implies p_{S_v}(y-z) \leq \epsilon \\
&\iff y \in \cball_v(x, \epsilon).
\end{align*}
The inclusions follow, since
\[ \epsilon^{-1}(y-z) \in S_v \quad\iff\quad \epsilon^{-1}|y-z| \leq |v| \quad\iff\quad |y-z| \leq \epsilon |v|, \]
by \ref{principalSolidSets}.

(2) We have that $(y,z) \in \entourage_v(\epsilon/2)$ implies $z\in \ball_v(y,\epsilon /2) \subseteq \cball_v(y,\epsilon)$.

(3) Take arbitrary $y\in V$. Since $S_v$ is convex and algebraically closed, by \ref{principalSolidSetsAbsolutelyConvex} and \ref{principalSolidSetsClosedInArchimedeanRieszSpaces}, we can calculate, using \ref{gaugeScaling} and \ref{gaugeInherenceAdherence},
\begin{align*}
y \in \cball_v(x, \epsilon) &\iff p_{S_v}(x-y) \leq \epsilon \\
&\iff \epsilon^{-1}p_{S_v}(x-y) \leq 1 \\
&\iff p_{S_v}\big(\epsilon^{-1}(x-y)\big) \leq 1 \\
&\iff \epsilon^{-1}(x-y) \in S_v \\
&\iff \epsilon^{-1}|x-y| \leq |v| \\
&\iff |x-y| \leq \epsilon|v|.
\end{align*}

(4) Take arbitrary $y,z \in V$. We calculate
\begin{align*}
