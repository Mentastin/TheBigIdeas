...
\section{Upsets, downsets and bounds}
...
Compare with \ref{scalarMultipleUpperLowerBounds}, which still holds if $A \nsubseteq V^+$.
\begin{proof}
(1) By the lemma (and reflexivity), we have $\upset(\alpha A) \supseteq \upset\{\alpha\}\cdot \upset A \supseteq \alpha \upset A$. The equality of the first and last sets follows from \ref{scalarMultipleUpperLowerBounds}.

The other point is dual.

(2) By the lemma (and reflexivity), we have $\upset(\alpha A) \subseteq \upset\{\alpha\}\cdot \downset A \subseteq \alpha \downset A$. The equality of the first and last sets follows from \ref{scalarMultipleUpperLowerBounds}.

The other point is dual.
\end{proof}

\section{Riesz decomposition property}
\begin{definition}
...
\end{theorem}
\begin{proof}
We use \ref{RieszDecompositionLemma}.
\end{proof}
...
\end{align*}
\end{proof}
\begin{corollary}[Birkhoff inequalities]
Let $V$ be a Riesz space and $u,v,w\in V$, then
\begin{enumerate}
...
\begin{definition}
A Riesz space $V$ is called \udef{Archimedean} if $\bigwedge_{\epsilon > 0}\epsilon v = 0$ for all $v\in V^+$.
\end{definition}
...
For all $v\in V$ with decomposition $v= v_1 +v_2 \in B\oplus B^{\perp_r}$, the vector $v_1$ is called the \udef{component} of $v$ in $B$.
\end{definition}
...
\item $V$ has the \udef{principal projection property} if every principal band in $V$ is a projection band.
\end{itemize}
\end{definition}
...
\subsubsection{Projection bands in Archimedean Riesz spaces}

\section{Order and Riesz space convergence}
...
\item the restriction $\cdot: \R^+ \times V^+ \to V^+$ of scalar multiplication to positive scalars and vectors is continuous.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) Suppose $F\overset{\mathfrak{o}}{\longrightarrow} x$ and $G\overset{\mathfrak{o}}{\longrightarrow} y$ in $V$. Then we calculate using \ref{sumUpDownsets}, \ref{orderConvergenceEquivalents} and \ref{sumUpperLowerBounds},
\begin{align*}
\upset\{x + y\} &= \upset\{x\} + \upset\{y\} \\
&\subseteq \Big(\bigcup_{A\in F} A^\leq\Big) + \Big(\bigcup_{B\in G} B^\leq\Big) \\
&= \bigcup_{\substack{A\in F \\ B\in G}} A^\leq + B^\leq \\
&\subseteq \bigcup_{\substack{A\in F \\ B\in G}} \big(A + B\big)^\leq.
\end{align*}
Dually, we have $\downset\{x + y\} \subseteq \bigcup_{\substack{A\in F \\ B\in G}} \big(A + B\big)^\geq$.
Then, by \ref{orderConvergenceEquivalents}, we have that $F + G \overset{\mathfrak{o}}{\longrightarrow} x + y$.

(2) Suppose $F\overset{\mathfrak{o}}{\longrightarrow} v$ in $V$.
First suppose $\lambda = 0$. Then, for all $A\in F$, we have $\upset\{0\} = \{0\}^\leq = (0\cdot A)^\leq$. Thus $\upset\{0\} = \bigcup_{A\in F}(0\cdot A)^\leq$ and, similarly, $\downset\{0 \cdot v\} = \bigcup_{A\in F}(0\cdot A)^\geq$. Then, by \ref{orderConvergenceEquivalents}, we have that $0\cdot F \overset{\mathfrak{o}}{\longrightarrow} 0$.

Next, suppose $\lambda > 0$. Then we calculate using \ref{scalarMultipleUpDownSets}, \ref{orderConvergenceEquivalents} and \ref{scalarMultipleUpperLowerBounds},
\begin{align*}
\upset\{\lambda \cdot v\} &= \lambda\upset\{v\} \\
&\subseteq \lambda\Big(\bigcup_{A\in F} A^\leq\Big) \\
&= \bigcup_{A\in F} \lambda A^\leq \\
&= \bigcup_{A\in F} (\lambda A)^\leq.
\end{align*}
Dually, we have $\downset \{\lambda \cdot v\} \subseteq \bigcup_{A\in F} (\lambda A)^\geq$.
Then, by \ref{orderConvergenceEquivalents}, we have that $\lambda\cdot F \overset{\mathfrak{o}}{\longrightarrow} \lambda v$.

Finally, suppose $\lambda < 0$. Then we calculate using \ref{scalarMultipleUpDownSets}, \ref{orderConvergenceEquivalents} and \ref{scalarMultipleUpperLowerBounds},
\begin{align*}
\upset\{\lambda \cdot v\} &= \lambda\downset\{v\} \\
&\subseteq \lambda\Big(\bigcup_{A\in F} A^\geq\Big) \\
&= \bigcup_{A\in F} \lambda A^\geq \\
&= \bigcup_{A\in F} (\lambda A)^\leq.
\end{align*}
Dually, we have $\downset \{\lambda \cdot v\} \subseteq \bigcup_{A\in F} (\lambda A)^\geq$.
Then, by \ref{orderConvergenceEquivalents}, we have that $\lambda\cdot F \overset{\mathfrak{o}}{\longrightarrow} \lambda v$.

(3) Suppose $F\overset{\R}{\longrightarrow} \lambda$ in $\R^+$ and $G\overset{\mathfrak{o}}{\longrightarrow} x$ in $V^+$. We note that the convergence on $\R^+$ is the order convergence, see \ref{realConvergenceOrderConvergence}. Then we calculate using \ref{scalarMultipleUpDownSetPositiveSet}, \ref{orderConvergenceEquivalents} and \ref{scalarMultipleSetsUpperLowerBounds},
\begin{align*}
\upset\{\lambda x\} &= \lambda\upset\{x\} \\
&= \upset\{\lambda\} \cdot \upset\{x\} \\
&\subseteq \Big(\bigcup_{A\in F} A^\leq\Big) \cdot \Big(\bigcup_{B\in G} B^\leq\Big) \\
&= \bigcup_{\substack{A\in F \\ B\in G}} A^\leq \cdot B^\leq \\
&\subseteq \bigcup_{\substack{A\in F \\ B\in G}} \big(A \cdot B\big)^\leq.
\end{align*}
Dually, we have $\downset\{\lambda x\} \subseteq \bigcup_{\substack{A\in F \\ B\in G}} \big(A \cdot B\big)^\geq$.
Then, by \ref{orderConvergenceEquivalents}, we have that $F \cdot G \overset{\mathfrak{o}}{\longrightarrow} \lambda x$.
\end{proof}
...
\begin{enumerate}
\item $\downset\{0\} \subseteq \bigcup_{A\in F}A^\geq$;
...
\end{lemma}
\begin{proof}
(1) For all $A\in F$, we have $A\subseteq \upset \{0\}$, so $\downset \{0\} = \big(\upset \{0\}\big)^\geq \subseteq A^\geq$ and $\downset\{0\} \subseteq \bigcup_{A\in F}A^\geq$. 

(2) Take $v\in \upset\{0\}$. Since $\upset \{0\} \subseteq \bigcup_{B\in G}B^\leq$, there exists $B\in G$ such that $v\in B^\leq$. Take $A\in F$ such that $A\subseteq \downset B$. Then $B^\leq = (\downset B)^\leq \subseteq A^\leq$, so $v\in \bigcup_{A\in F}A^\leq$ and $\upset \{0\} \subseteq \bigcup_{A\in F}A^\leq$.

We have $\downset\{0\} \subseteq \bigcup_{A\in F}A^\geq$ from (1).
We conclude $F \overset{\mathfrak{o}}{\longrightarrow} 0$ by \ref{orderConvergenceEquivalents}.

(3) The direction $\Leftarrow$ is given by \ref{continuityOrderConvergenceOrderedVectorSpaces}.

For the direction $\Rightarrow$, assume $F+G \overset{\mathfrak{o}}{\longrightarrow} 0$. We show $F \overset{\mathfrak{o}}{\longrightarrow} 0$. The argument for $G$ is the same. 

We have $\upset \{0\} \subseteq \bigcup_{\substack{A\in F\\B\in G}}(A+B)^\leq$ by \ref{orderConvergenceEquivalents}. Now take arbitrary $v\in \upset\{0\}$. There exists $A\in F$ and $B\in G$ such that $v\in (A+B)^\leq$. Take arbitrary $a\in A$ and $b\in B$. Since $0\leq b$, we have $a \leq a+b \leq v$. Thus $v\in A^\leq \bigcup_{A\in F}A^\leq$ and so $\upset \{0\} \subseteq \bigcup_{A\in F}A^\leq$.

We have $\downset\{0\} \subseteq \bigcup_{A\in F}A^\geq$ from (1).
We conclude $F \overset{\mathfrak{o}}{\longrightarrow} 0$ by \ref{orderConvergenceEquivalents}.
\end{proof}

\begin{lemma} \label{RieszAbsoluteValueOrderContinuous}
Let $V$ be a Riesz space then the absolute value function $|-|$ is continuous when $V$ is equipped with the order convergence.
\end{lemma}
\begin{proof}
Since $|v| = v\vee (-v)$ for all $v\in V$, the result follows from \ref{continuityOrderConvergenceOrderedVectorSpaces} and \ref{latticeOperationsContinuousInOrderConvergence}.
\end{proof}

\subsection{Riesz space convergence}
\begin{definition}
Let $V$ be a Riesz space, $v\in V$ and $F\in\powerfilters(V)$. Then $F\to v$ in the \udef{Riesz space convergence} if $|F-v|  \overset{\mathfrak{o}}{\longrightarrow} 0$.
\end{definition}
Here $|F-v|$ is the image filter of $F-v$ under the absolute value function $|-|$.

\begin{lemma} \label{RieszAbsoluteValueRieszContinuous}
Let $V$ be a Riesz space then the absolute value function $|-|$ is continuous when $V$ is equipped with the Riesz convergence.
\end{lemma}
\begin{proof}
Let $F\to v \in V$. Then $|F-v|\overset{\mathfrak{o}}{\longrightarrow} 0$. Take arbitrary $|A-v|\in |F-v|$ and $\big||a| - |v|\big|\in \big||A|-|v|\big|$. Then $\big||a| - |v|\big| \leq |a-v| \in |A-v|$ by the reverse triangle inequality \ref{triangleInequalityRieszSpaces}. Thus $\big||F| - |v|\big| \overset{\mathfrak{o}}{\longrightarrow} 0$ by \ref{orderConvergencePositiveFilterLemma}. This means that $|F| \to |v|$.
\end{proof}

\begin{lemma} \label{RieszConvergenceVersusOrderConvergence}
Let $V$ be a Riesz space, $v\in V$ and $F\in\powerfilters(V)$. Then
\begin{enumerate}
\item if $F \overset{\mathfrak{o}}{\longrightarrow} v$, then $F\to v$;
\item if $F\to v$, then $|F|\overset{\mathfrak{o}}{\longrightarrow} |v|$.
\end{enumerate}
\end{lemma}
Thus (1) the Riesz convergence is weaker than the order convergence and (2) the absolute value function is Riesz-to-order-convergence-continuous.
\begin{proof}
(1) If $F \overset{\mathfrak{o}}{\longrightarrow} v$, then $F-v \overset{\mathfrak{o}}{\longrightarrow} 0$ by \ref{continuityOrderConvergenceOrderedVectorSpaces} and $|F-v| \overset{\mathfrak{o}}{\longrightarrow} 0$ by \ref{RieszAbsoluteValueOrderContinuous}. This means that $F \to v$.

(2) If $F\to v$, then $|F-v| \overset{\mathfrak{o}}{\longrightarrow} 0$. Take arbitrary $w\in \upset\{|v|\}$, so $0\leq w - |v|$ and $w-|v| \in \upset \{0\} \subseteq \bigcup_{A\in F}|A-v|^\leq$. Take $A\in F$ such that $w-|v| \in |A-v|^\leq$. Take arbitrary $a\in A$. Then $|a-v| \leq w - |v|$, so
\[ |a| = |a-v+v| \leq |a-v|+|v| \leq w. \]
Thus $w\in |A|^\leq$ and $\upset\{|v|\} \subseteq |A|^\leq \subseteq \bigcup_{A\in F}|A|^\leq$.

Now take arbitrary $w\in \downset\{|v|\}$, so $0\leq |v| - w$ and $|v|-w \in \upset \{0\} \subseteq \bigcup_{A\in F}|A-v|^\leq$. Take $A\in F$ such that $|v|-w \in |A-v|^\leq$. Take arbitrary $a\in A$. Then $|a-v| \leq |v|-w$, so
\[ w \leq |v| - |a-v| \leq \big||v| - |a-v|\big| \leq |v + a-v| = |a| \]
by the reverse triangle inequality \ref{triangleInequalityRieszSpaces}.
Thus $w\in |A|^\geq$ and $\downset\{|v|\} \subseteq |A|^\geq \subseteq \bigcup_{A\in F}|A|^\geq$.

We conclude $|F| \overset{\mathfrak{o}}{\longrightarrow} |v|$ by \ref{orderConvergenceEquivalents}.
\end{proof}


\begin{proposition} \label{RieszSpaceConvergenceVectorConvergence}
The Riesz space convergence is a vector space convergence.
\end{proposition}
\begin{proof}
We first show continuity of vector addition. Let $F\to v$ and $G \to w$, so $|F-v|\overset{\mathfrak{o}}{\longrightarrow} 0$ and $|G-w| \overset{\mathfrak{o}}{\longrightarrow} 0$. Then $|F-v|+|G-w|\overset{\mathfrak{o}}{\longrightarrow} 0$ by \ref{continuityOrderConvergenceOrderedVectorSpaces}. Take arbitrary $|A-v| + |B-w|\in |F-v|+|G-w|$. Take arbitrary $|a+b-v-w| \in |A+B - v-w|$. Then $|a+b-v-w| \leq |a-v|+|b-w| \in |A-v|+|B-w|$ by the triangle inequality \ref{triangleInequalityRieszSpaces}, so $|A+B - v-w| \subseteq \downset |A-v| + |B-w|$. This means we can apply \ref{orderConvergencePositiveFilterLemma} to obtain $|F + G-v - w|\overset{\mathfrak{o}}{\longrightarrow} 0$ and thus $F+G \to v+w$.

Now we show continuity of scalar multiplication. Let $F\to v$ and $H \to \lambda\in \R$. Then $|H|\overset{\mathfrak{o}}{\longrightarrow} |\lambda|$, $|H-\lambda|\overset{\mathfrak{o}}{\longrightarrow} 0$ and $|F-v| \overset{\mathfrak{o}}{\longrightarrow}$.
...
\begin{proposition}
Let $V$ be a Riesz space equipped with the Riesz convergence. Then the lattice operations $\wedge: V\times V \to V$ and $\vee: V\times V \to V$ are continuous.
\end{proposition}
\begin{proof}
Since, for all $v,w\in V$, we have $v\vee w = \frac{1}{2}(v+w + |v-w|)$ and $v\wedge w = \frac{1}{2}(v+w - |v-w|)$, \ref{absoluteValueMeetJoin}, the continuity of the lattice operations follows from \ref{RieszAbsoluteValueRieszContinuous} and \ref{RieszSpaceConvergenceVectorConvergence}.
\end{proof}


\begin{lemma}
TODO: Is the Riesz space convergence compatible with the weak order???
\end{lemma}
