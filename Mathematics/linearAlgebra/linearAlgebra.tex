...
\end{proposition}
\begin{proof}
We calculate using \ref{sumMultipleUpDownsets} and \ref{sumAsMeetJoin}
...
\begin{lemma} \label{PositiveNegativeElements}
Let $V$ be a Riesz space and $v,w\in V$. Then
\begin{enumerate}
\item $v^+, v^- \in V^+$;
\item $(-v)^- = v^+$ and $(-v)^+ = v^-$;
...
\begin{proposition} \label{minimalPositiveDecomposition}
...
(2) The direction $\Rightarrow$ is part of \ref{PositiveNegativeElements}. For the other direction, assume $p\wedge q = 0$. Then, using \ref{MeetJoinAsPositiveNegative},
