...
\begin{proposition} \label{additionLatticeOperationsInequalities}
...
\end{proof}
\begin{corollary} \label{positiveVectorHasPositiveComponents}
Let $V$ be a Riesz space and $E_1, E_2$ Riesz ideals in $V$ such that the sum $E_1\oplus E_2$ is direct. For all $0\leq v = v_1 + v_2 \in E_1\oplus E_2$, we have $0\leq v_1$ and $0\leq v_2$.
\end{corollary}
...
Now take $u\in B^{\perp_r}$. Since $V = A\oplus B$, we can write $|u| = u_1 + u_2$ for some $u_1\in A$ and $u_2\in B$. By \ref{positiveVectorHasPositiveComponents}, $0\leq u_1$ and $0\leq u_2$, so $u_2 \leq |u|$. By solidity, $u_2\in B^{\perp_r}$, so $u_2\in B\cap B^{\perp_r} = \{0\}$. Thus $|u| = u_1\in A$ and $u\in A$ by solidity, so $B^{\perp_r} \subseteq A$.
...
To that end, take $v\in B$ such that $0\leq v\leq u$. Then $u-v = (u_1 - v) + u_2 \in B\oplus B^{\perp_r}$. By \ref{positiveVectorHasPositiveComponents}, $0\leq u_1 - v$, so $v\leq u_1$. This shows that $u_1$ is an upper bound.
...
\begin{proof}
$(1) \Rightarrow (2)$ Linearity and idempotency are immediate from \ref{projectorComplementarySubspaces}. Take $u\in V^+$, then $Pu = \bigvee \setbuilder{v\in B}{0\leq v\leq u} \leq u$.
