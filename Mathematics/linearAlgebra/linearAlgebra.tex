...
\begin{definition}
Let $\sSet{\C, V, +}$ and $\sSet{\C, W, +}$ be complex vector spaces. A function $A: V\to W$ is called \udef{antilinear} (or \udef{conjugate-linear}) if
\begin{itemize}
...
\begin{definition}
Let $V,W$ be complex vector spaces. A function $T: V\to W$ is called \udef{real-linear} if it is linear as a function $T: V_\R \to W_\R$.

A linear function $L: V\to W$ may be called \udef{complex-linear} in order to emphases that it is linear and not real-linear.
\end{definition}
...
This space is called the \udef{complexification} of $V$ and is denoted $V_\C$.
\end{definition}
...
\begin{definition}
Let $V$ be a complex vector space. A \udef{real structure} on $V$ is an antilinear involution $\sigma: V\to V$.
...
\end{align*}
and call $\Re_\sigma(v)$ the \udef{real part} of $v$ w.r.t.\ $\sigma$ and $\Im_\sigma(v)$ the \udef{imaginary part} of $v$ w.r.t.\ $\sigma$.
\end{definition}
Involution means that $\sigma^2 = \id_V$.

\begin{proposition} \label{realImaginaryPartsProjectors}
...
We have that $\Re_\sigma$ is the projector on $\Re_\sigma^\imf(V)$ along $\Im_\sigma^\imf(V)$.
\begin{proof}
...
We define the \udef{real subspace} of $V$ w.r.t.\ $\sigma$ as
\[ \Re_\sigma^\imf(V) = \setbuilder{v\in V}{\sigma(v) = v} \]
and the \udef{imaginary subspace} of $V$ w.r.t.\ $\sigma$ as
...
\subsubsection{Complex structure on a real vector space}
\begin{definition}
Let $W$ be a real vector space. A \udef{complex structure} on $W$ is a linear map $J: W\to W$ such that $J^2 = -\id_W$.
\end{definition}
...
\end{lemma}
We call this the \udef{canonical complex structure} on $W\oplus W$.
\begin{proof}
...
\end{align*}
\end{lemma}
We call this real structure the \udef{canonical real structure} on $(W\oplus W)_J$.
\begin{proof}
...
\begin{lemma} \label{convexityPropertyOfRealVectorSpaces}
...
\begin{definition}
A subset $B$ of a vector space $V$ over a field $\F$ with valuation $|\cdot|$ is called \udef{absolutely convex} or \udef{disked} if for all $x,y\in B$ and $r,r'\in \F$ such that $|r| + |r'| \leq 1$, $rx + r'y\in B$.
...
\subsubsection{Riesz decomposition property}
\begin{definition}
...
\subsection{Linear functions on ordered vector spaces}
\subsubsection{Positive linear functions}
\begin{definition}
Let $V,W$ be ordered vector spaces and $f: V\to W$ a linear function. Then $f$ is called \udef{positive} if $f^\imf(V^+) \subseteq W^+$.
...
\begin{proposition} \label{positiveOperatorExtension}
...
\begin{definition}
Let $V,W$ be ordered vector spaces and $f: V\to W$ a linear function. We call $f$ \udef{order-bounded} if
...
\begin{proposition} \label{positivePartOrderBoundedOperator}
...
\begin{lemma} \label{absoluteValueOrderBoundedOperatorLemma}
...
\begin{lemma} \label{positiveNegativePartRieszSpace}
Let $V$ be an ordered vector space. Then the the following are equivalent:
\begin{enumerate}
...
\begin{proposition} \label{differenceOfSupremaLessThanSupremumOfDifferences}
...
\begin{lemma} \label{positiveMultipleSolid}
...
\begin{proposition} \label{DedekindCompleteProjectionPropertieArchimedean}
...
\section{Convergences on ordered vector spaces}
\begin{definition}
Let $\sSet{V, \leq}$ be a partially ordered vector space and $\xi$ a convergence on $V$. We call $\xi$ \udef{compatible} with $\sSet{V,\leq}$ if $\xi$ is compatible with the weak order.
\end{definition}
...
\subsection{Order convergence in ordered vector spaces}
...
\subsubsection{Order convergence in Riesz spaces}
...
\begin{definition}
Let $V$ be a Riesz space and $v\in V$. We call the seminorm uniformity generated by $p_{S_v}$ the \udef{$v$-uniformity}. In this case we call $v$ the \udef{regulator}.
\end{definition}
...
\begin{proposition} \label{OrderConvergenceWeakerThenUniformConvergenceArchimedeanRieszSpace}
...
\begin{lemma} \label{regulatorGaugeLemma}
...
\begin{definition}
Let $V$ be a Riesz space and $v\in V^+$. Let $F\in \powerfilters(V)$ and $x\in V$. Then we say $F$ converges \udef{$v$-uniformly} to $x$ if it converges to $x$ in the uniform convergence $\Gamma(\mathcal{U}_v)$. We write $F\overset{\text{$v$-un}}{\longrightarrow} x$.
\end{definition}
...
\begin{definition}
Let $V$ be a Riesz space. The \udef{Riesz uniformity} on $V$ is the uniformity $\bigvee_{v\in V} \mathcal{U}_v$, where the join is taken in the poset of uniformities on $V$.
\end{definition}

\begin{lemma} \label{RieszEntouragesCompositionLemma}
...
\begin{definition}
Let $V$ be a complex vector space with real structure $\sigma$ and ordering on $\Re^\imf_\sigma(V)$ that makes it a Riesz space. For all $z\in V$, we define the \udef{modulus}
\[ |z| \defeq \sup_{\theta\in S^1} \Re(ze^{i\theta}), \]
if this supremum exists.

If $|z|$ exists for all $z\in V$, we call $V$ a \udef{complex Riesz space}.
\end{definition}
...
\begin{lemma} \label{modulusConstructionLemma}
...
\begin{lemma} \label{modulusTriangleInequalities}
...
\begin{lemma} \label{scalarOutOfModulus}
...
\subsubsection{Sufficient conditions for complex Riesz space}
\begin{proposition}
