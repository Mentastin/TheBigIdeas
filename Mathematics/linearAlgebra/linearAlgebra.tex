...
\begin{theorem}[Steinitz exchange lemma] \label{theorem:SteinitzExchange}
...
\begin{definition}
Let $\seq{\R, V, +}$ be a real vector space and $\precsim$ a preorder on the set $V$. Then $\precsim$ is a \udef{vector preorder} if it is compatible with the vector space structure as follows: $\forall x,y,z\in V, \lambda\in\R$
\begin{enumerate}
...
\item If $\seq{V, \precsim}$ is a lattice, we call $(\R, V, +, \precsim)$ a \udef{vector lattice} or a \udef{Riesz space}.
\end{itemize}
\end{definition}

\begin{lemma}
Let $\seq{\R, V, +}$ be a real vector space and $\precsim$ a preorder on the set $V$. The compatibility of the order can equivalently be expressed by:
...
\end{lemma}
\begin{proof}
We apply \ref{corollary:bijectiveOrderEmbeddingBounds} to
...
\begin{proposition} \label{prop:sumAsMeetJoin}
...
\begin{lemma} \label{lemma:PositiveNegativeElements}
Let $V$ be a Riesz space and $v,w\in V$. Then
\begin{enumerate}
\item $v^+, v^- \in V^+$;
\item $(-v)^- = v^+$ and $(-v)^+ = v^-$;
...
\begin{proposition} \label{prop:triangleInequalityPositiveNegativeElements}
Let $V$ be a Riesz space and $v,w\in V$. Then
\begin{enumerate}
\item $(v+w)^+ \leq v^+ + w^+$;
\item $(v+w)^- \leq v^- + w^-$.
...
Let $V$ be a Riesz space and $v\in V$. Assume $p,q\in V^+$ are such that $v = p-q$. Then
\begin{enumerate}
...
\begin{lemma} \label{lemma:absoluteValue}
...
(3) This is equivalent to (1) by the replacements $v \leftrightarrow v+w$ and $w \leftrightarrow v-w$.

(2) We use (1), (3) and \ref{corollary:meetJoinAbsoluteValues} to calculate
\begin{align*}
|v|\wedge |w| &= |v| + 0\wedge \big(|w| - |v|\big) = |v| + |w| + \big(-|w|\big) \wedge \big(- |v|\big) = |v| + |w| - \big(|v|\vee |w|\big) \\
&= |v| + |w| - \frac{1}{2}\Big( |v+w| + |v - w| \Big) = |v+w|\vee |v-w| - \frac{1}{2}\Big( |v+w| + |v - w| \Big) \\
&= \frac{1}{2}\Big(|v+w|+|v-w| + \big||v+w| - |v-w|\big|\Big) - \frac{1}{2}\Big( |v+w| + |v - w| \Big) \\
&= \frac{1}{2}\Big| |v+w| - |v - w| \Big|.
\end{align*}

(4) This is equivalent to (2) by the replacements $v \leftrightarrow v+w$ and $w \leftrightarrow v-w$.
\end{proof}
\begin{corollary}
