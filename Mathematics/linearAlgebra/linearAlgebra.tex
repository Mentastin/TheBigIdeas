...
\begin{definition}
A subset $B$ of a vector space $V$ over a field $\F$ with valuation $|\cdot|$ is called \udef{balanced} if for all $|r|\leq 1$, $rB \subseteq B$.
...
\item $x_1 - x_2\in \Delta^\imf(A) \implies (x_1, x_2)\in A$;
\item $x_1 - x_2\in \Delta^\imf(A) \iff (x_1, x_2)\in A$.
\end{enumerate}
\end{lemma}
\begin{proof}
$\boxed{(1) \Leftrightarrow (2)}$ Restatement of definition.

$\boxed{(1) \Leftrightarrow (3)}$ First suppose $A$ translation invariant and take arbitrary $x_2 - x_1\in \Delta^\imf(A)$. Then there exists $(y_1,y_2)\in A$ such that $x_2 - x_1 = y_2 - y_1$. Then we can set $a \defeq x_1 - y_1 = x_2 - y_2$ and we have $(x_1, x_2) = (y_1 + a, y_2 + a)$, which is an element of $A$ by translation invariance.

Now assume $x_1 - x_2\in \Delta^\imf(A) \implies (x_1, x_2)\in A$, take $(a_1,a_2)\in A$ and $z\in V$. Then $(a_2 + z) - (a_1 + z) = a_2 - a_1\in \Delta^\imf(A)$, so $(a_1 + z, a_2+ z)\in A$.

$\boxed{(3) \Leftrightarrow (4)}$ Then implication $(x_1, x_2)\in A \implies x_1 - x_2\in \Delta^\imf(A)$ is immediate by the definition of $\Delta^\imf(A)$.
\end{proof}
...
Let $\sSet{V}$ be a convergence vector spaces, $A\subseteq V\oplus V$, $x\in V$ and $m\in \N$. Then
\begin{enumerate}
\item $Ax = x + \Delta^\imf(A)$;
...
\begin{proof}
(1) We have
\[ y\in Ax \iff (y,x) \in A \iff y-x \in \Delta^\imf(A) \iff y\in x + \Delta^\imf(A). \]

(2) Take $x\in m\Delta^{\imf}(A)$, then there exists $(y,z)\in A$ such that $x= m(y-z) = my - mz$. By translation invariance, we have
\[ my \mathrel{A}\big((m-1)y + z\big)\mathrel{A}\big((m-2)y + 2z\big)\mathrel{A}\ldots\mathrel{A}\big(y + (m-1)z\big)\mathrel{A}mz.  \]
So $(my, mz)\in A^m$ and thus $x\in \Delta^\imf(A^m)$.
