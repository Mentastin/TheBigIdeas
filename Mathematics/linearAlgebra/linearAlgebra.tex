...
\end{lemma}
\begin{proof}
We apply \ref{orderPreservingReversingBounds} to
...
\begin{proposition} \label{PositiveNegativeElements} \label{minimalPositiveDecomposition} \label{MeetJoinAsPositiveNegative}
Let $V$ be a Riesz space and $v,w\in V$. Then
\begin{enumerate}
\item $v^+, v^- \in V^+$;
\item $v= v^+ - v^-$;
...
(6, 7) We calculate $v\vee w = (v-w)\vee 0 + w = (v-w)^+ + w$. The other equalities are similar.

(4) From $v\leq p$ and $0\leq p$, we get $v^+ = v \vee 0 \leq p$. Then we also have $v^- = v^+ - v \leq p - v = q$.

(5) Assume $p,q\in V$ satisfy (2) and (3), then (1) automatically follows from (3). Using point (7), we calculate
\[ 0 = p\wedge q = p - (p-q)^+ = p - v^+. \]
So $p = v^+$ and $q = p - v = v^+ - v = v^-$.

(8) It is evident that $(-v)^- = (--v)\vee 0 = v\vee 0$.

(9) We calculate $\alpha v^+ = \alpha (v \vee 0) = (\alpha v) \vee 0 = (\alpha v)^+$; the calculation for $\alpha v^-$ is similar.

(10) This is clear from $-v^- = v\wedge 0 \;\leq\; v \;\leq\; v \vee 0 = v^+$.

(11) $v\leq w$ implies $v^+ = v\vee 0 \leq w\vee 0 = w^+$ and $-v^- = v\wedge 0 \leq w\wedge 0 = - w^-$.
...
\end{proposition}
\begin{proof}
The first equality follows from \ref{MeetJoinAsPositiveNegative} and \ref{absoluteValue}.

The next two follow from the first and \ref{sumAsMeetJoin}.
...
\begin{itemize}
\item a \udef{Riesz subspace} if it is both a subspace and a sublattic;
