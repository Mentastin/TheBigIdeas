...
\begin{lemma}
Let $f: V\to \F$ be a linear functional and $x\notin \ker(f)$. Let $A\subseteq V$ be a balanced set. Then $(x+U)\perp \ker(f)$ \textup{if and only if} $U \subseteq f^{\preimf}(\ball(0,|f(x)|))$.
\end{lemma}
\begin{proof}
Suppose $U \subseteq f^{\preimf}(\ball(0,|f(x)|))$. Then for all $u\in U$: $f(x+u) = f(x) + f(u) \neq 0$.

Conversely, suppose $U \not\subseteq f^{\preimf}(\ball(0,|f(x)|))$, i.e.\ there exists $u\in U$ such that $|f(u)| \geq |f(x)|$. Then $v= -\frac{f(x)}{f(u)}u\in U$, because $U$ is balanced and so $f(x+ v) = f(x)-\frac{f(x)}{f(u)}f(u) = 0$ and so $x+U \mesh \ker(f)$.
\end{proof}
...
Let $V$ be a vector space and $A\subseteq V$ an absorbent subset and $\lambda\in \R^{\geq 0}$. Then
\begin{enumerate}
