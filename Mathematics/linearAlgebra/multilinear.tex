...
\[ \setbuilder{v\otimes v - q(v) \vec{1}}{v\in V}. \]
...
TODO
\end{proof}
Clearly $\pi_q|_V(v)^2 = q(v) \vec{1}$ for all $v\in V$.

\begin{lemma} \label{vectorInverseCliffordAlgebra}
Let $\Cl(V,q)$ be a Clifford algebra. Then
\[ \Cl^\times(V,q) \cap V = \setbuilder{v\in V}{q(v) \neq 0}. \]
The inverse of $v\in \Cl^\times(V,q) \cap V$ is $v^{-1} = v/q(v)$.
\end{lemma}
\begin{proof}
First take $v\in \setbuilder{v\in V}{q(v) \neq 0}$. This definition of $v^{-1}$ is indeed a multiplicative inverse: $vv^{-1} = v^2/q(v) = q(v)/q(v)\vec{1} = \vec{1}$.

Conversely, take $v\in \Cl^\times(V,q) \cap V$. If $q(v) = 0$, then $v$ would be a zero divisor (as $vv = q(v)\vec{1} = 0$). No zero divisor can have an inverse (\ref{inverseZeroDivisor}), 
\end{proof}
...
\[ vv = q(v)\vec{1} \qquad \forall v\in V. \]
\end{lemma}
...
\begin{lemma}
Let $\Cl(V,q)$ be a Clifford algebra. Then $vw + wv$ is a scalar multiple of the identity.
\end{lemma}
\begin{proof}
We can calculate
...
\begin{definition}
Let $\F$ be a field whose characteristic is not $2$ and $\Cl(V,q)$ a Clifford algebra over $\F$. We can then write
\begin{align*}
vw &= \frac{vw+wv}{2} + \frac{vw-wv}{2} \\
&\defeq v\cdot w + v\wedge w.
\end{align*}
We call the symmetric part $v\cdot w$ the \udef{scalar product} (sometimes also called the \udef{inner product}),
and the antisymmetric part $v\wedge w$ the \udef{outer product}.
\end{definition}
...
\begin{itemize}
\item \udef{orthogonal} or \udef{perpendicular} if $v\cdot w = 0$;
\item \udef{parallel} if $v\wedge w = 0$.
\end{itemize}
\end{definition}
\begin{lemma}
...
\subsection{Grades and the grade operator}
\begin{definition}
