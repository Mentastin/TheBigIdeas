...
The final equality follows from \ref{ominusUnderIsometry}, using the fact that $\begin{pmatrix}
0 & -\id \\ \id & 0
...
\[ T^{n}\big[T[\mathcal{H}]^\perp\big] \subset T^{n}[\mathcal{H}] = T[T^{n-1}[\mathcal{H}]] \subset T[\mathcal{H}] \perp T[\mathcal{H}]^\perp. \]
...
\end{theorem}
\begin{proof}
The subspace $\mathcal{V} = T[\mathcal{H}]^\perp$ is wandering by \ref{WoldLemma}. Then $T$ is a unilateral shift in the subspace
...
\end{align*}
using \ref{ominusUnderIsometry} and \ref{cancellationOminus}, which is applicable because $T^i[\mathcal{V}]$ is closed by \ref{isometryClosed}. So $\mathcal{H}_0\subseteq T^n[\mathcal{H}]$ for all $n$.
