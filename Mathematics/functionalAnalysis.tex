...
The function $\F \to \Span\{v\}: \lambda \mapsto \lambda\cdot v$ is a homeomorphism iff $\Span\{v\}$ has the final convergence w.r.t. this function. Call this convergence $\nu$. Then $\sSet{\Span\{v\}, \nu}$ is Hausdorff, locally compact and Choquet.

Now assume it is not a homeomorphism. Then the convergence on $\Span\{v\}$, say $\xi$, is strictly weaker than $\nu$, so $\vicinity_\xi(0) \subsetneq \vicinity_\nu(0)$. The convergence on $\Span\{v\}$ is not Hausdorff by \ref{maximalHausdroffMinimalCompactChoquet}.
\end{proof}
