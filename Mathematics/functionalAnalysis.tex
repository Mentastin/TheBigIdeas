...
Let $\sSet{V}$ be a convergence vector spaces, $A\subseteq V^2$, $x\in V$ and $m\in \N$. Then
\begin{enumerate}
\item $Ax = x + \Delta^\imf(A)$;
\item if $\id_V+A \subseteq A$, then $m\Delta^{\imf}(A) \subseteq \Delta^{\imf}(A^m)$;
...
(2) Take $x\in m\Delta^{\imf}(A)$, then there exists $(y,z)\in A$ such that $x= m(y-z) = my - mz$. By assumption, we have
...
Now assume $B$ is bounded as a subset of a uniform space. Take arbitrary $U\in \neighbourhood_\xi(0)$, then by \ref{entourageConvergenceGroup}, we can find $A\in \entourage_V$ such that $A\subseteq \Delta^{\preimf}(U)$, i.e.\ $\Delta^{\imf}(A) \subseteq U$. Then there exists a finite subset $S\subseteq V$ such that $B \subseteq \bigcup_{x\in S}A^mx$. By \ref{vectorDeltaLemma}, we have
