...
\begin{lemma} \label{lemma:HilbertOnBasisMaximal}
An orthonormal subset of a Hilbert space is an o.n. basis \textup{if and only if} it is maximal.
\end{lemma}
\begin{proof}
TODO
\end{proof}
\begin{corollary}
Every Hilbert space has an o.n. basis.
\end{corollary}
...
\begin{lemma}
Let $\{\phi_n(x)\}^\infty_{n=0}$ be an orthonormal basis of $L^2(\Omega)$, then $\{\phi_n(x)phi_m(y)\}^\infty_{n,m=0}$ is an orthonormal basis of $L^2(\Omega\times\Omega)$.
\end{lemma}
\begin{proof}
The set $\{\phi_n(x)phi_m(y)\}^\infty_{n,m=0}$ is orthonormal:
\[ \iint_{\Omega\times\Omega} \phi_n(x)\phi_m(y)\overline{\phi_{n'}(x)\phi_{m'}(y)}\diff{x}\diff{y} = \int_\Omega\phi_n(x)\overline{\phi_{n'}(x)}\diff{x} \cdot \int_\Omega\phi_m(y)\overline{\phi_{m'}(y)}\diff{y} = \delta_{n,n'}\delta_{m,m'}, \]
using Fubini's theorem and the Hölder inequality (TODO refs).

For completeness we may simply verify the Bessel equality by \ref{prop:totalONBParsevalEquivalence} and \ref{lemma:HilbertOnBasisMaximal}. TODO
\end{proof}
...
\[ T^* \defeq \bigcup\setbuilder{S\in (K\not\to H)}{\text{$S$ is an adjoint of $T$}}. \]
\end{definition}
