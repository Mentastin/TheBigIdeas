...
\end{proof}
\begin{corollary}[Neumann series] \ref{corollary:NeumannSeries}
...
\begin{definition}
Let $T: H\not\to K$ be a densely defined operator between Hilbert spaces. We define the adjoint
\[ T^* = \bigcup\setbuilder{S\in (K\not\to H)}{\text{$S$ is an adjoint of $T$}}. \]
\end{definition}
...
\begin{proposition}
Let $T: H\not\to K$ be a densely defined operator between Hilbert spaces. Then $\ker(T^*) = \im(T)^\perp$.
\end{proposition}
...
\[ \sigma(L) = \{ \lambda \in \mathbb{F} \;|\; \lambda \id_V - L: V\to V \; \text{has no bounded inverse}.  \} \]
By ``having no bounded inverse'', we mean there is no bounded operator $R_\lambda: V\to \dom(L)$ such that
\[ R_\lambda(\lambda \id_V - L) = \id_{\dom(L)} \qquad (\lambda \id_V - L)R_\lambda = \id_V. \]
\item The \udef{point spectrum} $\sigma_\text{p}(L)$ is the set of all its eigenvalues, i.e. the values of $\lambda$ where $\lambda \id_V - L$ fails to be injective.
\item The \udef{continuous spectrum} $\sigma_\text{c}(L)$ is the set of all values of $\lambda$ such that $\lambda \id_V - L$ is injective, but not surjective, and has an image dense in $V$.
\item The \udef{residual spectrum} $\sigma_\text{r}(L)$ is the set of all values of $\lambda$ such that $\lambda \id_V - L$ is injective, but not surjective, and does not even have an image dense in $V$.
\end{itemize}
\end{definition}
...
\[ R_\lambda(L) = (\lambda \id_V - L)^{-1}. \]
Note that $R_\lambda(L)$ is a linear operator on $V$ with $\dom(R_\lambda(L)) \subset V$.
\end{definition}

\begin{proposition}
Let $L$ be a bounded operator on a Banach space $X$. For $|\lambda|>\norm{L}$ the resolvent $R_\lambda(L)$ is bounded and given by
\[ R_\lambda(L) = (\lambda \id_X - L)^{-1} = \sum_{n=0}^\infty\frac{L^n}{\lambda^{n+1}} \]
with uniform convergence. The norm is bounded by
\[ \norm{R_\lambda(L)} = \norm{(\lambda \id_X - L)^{-1}} \leq \frac{1}{|\lambda|-\norm{T}}. \]
\end{proposition}
\begin{proof}
...
\begin{proposition}
Let $X$ be a Banach space and $T:X\to X$ a bounded linear operator. Then $\lambda \in \sigma(T)$ \textup{if and only if} $\lambda \id_V - T$ is not bijective.
\end{proposition}
\begin{proof}
If $T$ is bounded, then $\lambda \id_V - T$ is bounded for all $\lambda\in\mathbb{F}$. By corollary \ref{corollary:boundedInverse} to the open mapping theorem, if $\lambda \id_V - T$ is bijective, then it is bounded.
\end{proof}
