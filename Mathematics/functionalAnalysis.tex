...
To show that the vector addition is continuous, take $F\to (v_1, v_2)$. Then $p_1(F) = F_1\to v_1$ and $p_2(F) = F_2 \to v_2$, i.e.\ $F_1-v_1 \in \mathcal{F}$ and $F_2-v_2 \in \mathcal{F}$. By (1), $(F_1-v_1) + (F_2-v_2) = (F_1+F_2) - (v_1 + v_2) \in \mathcal{F}$, so $F_1+F_2 = +[F] \to v_1 + v_2$.
...
\begin{enumerate}
\item $N$ has an absorbent and balanced base;
...
(1) Immediate because $\mathcal{F} = \pfilter{N}$.

(2) Take $F,G\in \pfilter{N}$. We need to show that $\upset (F + G) \supseteq N$, which means that for all $A \in N$ there exist $B\in F$ and $C\in G$ such that $B+C\subseteq A$. Take some balanced subset $A'$ of $A$ and set $B = \frac{1}{2}A' = C$.  TODO

(3) Take $F\in \pfilter{N}$. We need to show that $\upset(\vicinity_\F(0)\cdot F) \supseteq N$, which means that for all $A\in N$ there exists a $\Gamma\in \vicinity_\F(0)$ and $B\in F$ such that $\Gamma \cdot B\subseteq A$. We can take $B = \balancedCore(A) \in N \subseteq F$ and $\Gamma = B(0,1)$.

(4) Take $v\in V$. We need to show that all $A\in N$ contain $\Gamma\cdot v$ for some $\Gamma \in \vicinity_\F(0)$. Because $A$ is absorbent, there exists an $r>0$ such that $v\in cA$ for all $|c|\geq r$. Conversely $c^{-1}v \in A$ for all $|c^{-1}| \leq r^{-1}$. So $\ball(0,r^{-1})\cdot v \subseteq A$ and $B(0,r^{-1}) \in \vicinity_\F(0)$.

(5) Take $F\in \pfilter{N}$ and $\lambda\in \F$. We need to show that for all $A\in N$ there exists a $B\in F$ such that $\lambda\cdot B\subseteq A$. We can take $B = \lambda^{-1}A \in N\subseteq F$.
\end{proof}


\begin{proposition}
Let $\sSet{V,\xi}$ be a TVS and $U\in\neighbourhoods_\xi(0)$. Then
\begin{enumerate}
\item $U$ is absorbent;
\item $\balancedCore(U)\in\neighbourhoods_\xi(0)$.
\end{enumerate}
\end{proposition}
This means $\neighbourhoods_\xi(0)$ is based in the balanced sets.
\begin{proof}
(1) Fix $x\in V$. We need to show that for some $r>0$ and all $|c|\geq r$, we have $x\in cU$.

Consider the function $\lambda\mapsto \lambda x$. Because this function is continuous at $0$ and $U \in \neighbourhoods_\xi(0) = \neighbourhoods_\xi(0\cdot x)$, there exists $W\in \neighbourhoods_\F(0)$ such that $W\cdot x \subseteq U$ by \ref{pretopologicalContinuityVicinities}. Now $W$ contains an open ball $B(0,\epsilon)$. We may take $r = \epsilon^{-1}$. Indeed, take $|c|\geq \epsilon^{-1}$. Then $c^{-1}\in B(0,\epsilon) \subseteq W$ and thus $1\in cW$, so $x\in cW\cdot x \subseteq U$.

(2) Consider the function $\F\times V\to V: (\lambda,x)\mapsto \lambda x$. Because this function is continuous at $(0,0)$ and $U \in \neighbourhoods_\xi(0) = \neighbourhoods_\xi(0\cdot 0)$, there exists $W_s\times W_v\in \neighbourhoods_\F(0)\times \neighbourhoods_\xi(0)$ such that $W_s\cdot W_v \subseteq U$ by \ref{pretopologicalContinuityVicinities}. Then $W_s$ contains an open ball $B(0,\epsilon)$, so $\lambda W_v \subseteq U$ for all $|\lambda| \leq \epsilon$. Thus $\epsilon W_v \subseteq \frac{\epsilon}{\lambda} U$ and, setting $\mu = \epsilon/\lambda$, we have that $|\mu|\geq 1$ iff $|\lambda| \leq \epsilon$. Thus $\epsilon W_v \subseteq \bigcap_{|\mu|\geq 1}\mu U = \balancedCore(U)$ and $\epsilon W_v\in \neighbourhoods(0)$, so $\balancedCore(U) \in \neighbourhoods(0)$.
\end{proof}
Point (2) is partially implied by \ref{symmetricBase}.

\subsubsection{Locally convex convergence}
\begin{definition}
Let $\sSet{V,\xi}$ be a TVS. If $\xi$ is based in the convex sets, then $\xi$ is called \udef{locally convex}.
\end{definition}
\begin{lemma}
...
\end{lemma}
\begin{proof}
$(1) \Leftrightarrow (2)$ One direction is immediate, for the other it is enough to note that if $U$ is convec, then so is the translated set $x+U$ for all $x\in V$.
