...
Let $A$ be an operator. Then $\sigma_p(A)\subseteq W(A)$ and $\sigma(A) \subseteq \overline{W(A)}$.
\end{proposition}
...
\[ \Phi_x: \cont^\infty(\spec(x),\C) \to A: f\mapsto f(x)\defeq \int_\Gamma f(z)R_x(z)\diff{z}. \]
...
Functional calculus applied to indicator functions.

\begin{itemize}
\item In $C^*$ case immediately orthogonal projectors by spectral mapping.
\item 
\end{itemize}

sum to identity immediate.
