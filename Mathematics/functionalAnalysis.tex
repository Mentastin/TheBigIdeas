...
Let $V$ be a vector space and $A\subseteq V$. If $A$ is absorbent, then $p_A(v)$ is finite for all $v\in V$.
\end{corollary}
...
\end{lemma}
\begin{proof}
$(1) \Leftrightarrow (2)$ Immediate from \ref{boundedSetsInitialTopology} and \ref{weakTopologyLCTVS}.

$(2) \Leftrightarrow (3)$ Take $x\in X$ and let $r$ be a finite upper bound of $\setbuilder{\abspair{x, y}}{y\in A}$. Then $\sup\setbuilder{\abspair{x, r^{-1}y}}{y\in A} = \sup\setbuilder{\abspair{x, y}}{y\in r^{-1}A} \leq 1$, so $x\in (r^{-1}A)^\pol = rA^\pol$, by \ref{polarLemma}. We also have $x\in cA^\pol$ for all $|c|\geq r$ because $A^\pol$ is balanced, by \ref{polarPropertiesLemma}.

Conversely, take $x\in X$ and assume $A^\pol$ absorbs $x$. Then $x\in rA^\pol = (r^{-1}A)^\pol$, so $\sup_{y\in r^{-1}A}\abspair{x, y} \leq 1$ and thus $\sup_{y\in A}\abspair{x, y} \leq r$.
\end{proof}
