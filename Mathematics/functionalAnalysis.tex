...
\[ f(\lambda v) = f\big((a+ib)v\big) = af(v) + bf(iv) = af(v) + b\big(g(iv) - ig(i^2v)\big) = af(v) + b\big(g(iv) + ig(v)\big) = af(v) + ib\big(-ig(iv) + g(v)\big) = af(v) + ibf(v) = (a+ib)f(v) = \lambda f(v). \]
\end{proof}
...
In this case
\begin{align*}
\neighbourhoods(0) &= \Closure_\cap\setbuilder{p^\preimf(U)}{p\in S, U\in \neighbourhoods_\R(0)} \\
&= \mathfrak{F}\setbuilder{p^\preimf([0,\epsilon[)}{p\in S, \epsilon\in \R} \\
&= \Closure_\cap\setbuilder{p^\preimf([0,\epsilon])}{p\in S, \epsilon\in \R}.
\end{align*}
\end{proposition}
\begin{proof}
...
By \ref{pretopologicalInitialFinalConvergence} the initial convergence on $V$ w.r.t. $S$ is pretopological and the vicinities 

and \ref{seminormContinuity} each $W$ in the neighbourhood of the origin in the initial topology w.r.t. $S$ contains a scaled version of some $K$ in $\mathcal{B}$. So the topology on $V$ is coarser than the initial topology, meaning they are equal.
...
\begin{definition}
A \udef{pairing} is a triple $\sSet{V,W, b}$ where $V,W$ are vector spaces over $\mathbb{F}$ and $b: V\times W\to \mathbb{F}$ is a bilinear form. Often we will write the pairing as just $\sSet{V,W}$.

We say $W$ \udef{distinguishes} points of $V$ or is \udef{separating} on $V$ if $b(v,\cdot)$ is injective for all $v\in V$.
...
\begin{example}
\begin{itemize}
\item Let $V$ be a vector space. Then $\sSet{V^*, V, b}$ with $b:V^*\times V: (f,v)\mapsto f(v)$ is a dual pair. In this case we write $\pair{f, v}$ instead of $b(f,v)$. 
...
\begin{lemma}
Let $\sSet{V,W,b}$ be a dual pair. Then $\sSet{W,V,b^d}$ is also a dual pair.
\end{lemma}

\subsection{Weak topologies}
\begin{definition}
Let $(X,Y,b)$ be paired vector spaces. Then for each $y\in Y$, the map
\[ p_x: Y\to \R_{\geq 0} y\mapsto |b(x,y)| \]
determines a seminorm on $Y$.

The initial topology on $Y$ w.r.t. $\setbuilder{p_x: Y \to \R}{x\in X}$ is called the \udef{weak topology} $\sigma(X,Y)$ on $Y$ for the pair $\sSet{X,Y}$.
\end{definition}
\begin{lemma}
Let $\sSet{X,Y,b}$ be a pairing. The weak topology $\sigma(X,Y)$ on $Y$ is locally convex.
\end{lemma}
\begin{proof}
Immediate by \ref{locallyConvexSeminormTopology}.
\end{proof}
