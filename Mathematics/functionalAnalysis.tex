...
\begin{lemma}[Fredholm alternative] \label{lemma:FredholmAlternative}
...
\begin{proposition} \label{prop:adjointSpectrumNoResidual}
Let $L$ be a densely defined operator on a Hilbert space $H$. Assume $L$ and $L^*$ have empty residual spectra. Then 
\begin{enumerate}
\item $\sigma_\text{p}(L) = \overline{\sigma_\text{p}(L^*)}$;
\item if $\lambda\in \sigma_\text{p}(L)$ with eigenvector $x$ and $\overline{\mu}\in \overline{\sigma_\text{p}(L^*)}$ with eigenvector $y$, then
\[ (\lambda-\mu)\inner{y,x} = 0. \]
\end{enumerate}
\end{proposition}
\begin{proof}
(1) Take $\lambda\in \sigma_\text{p}(L)$ with eigenvector $x$. Then for all $y\in\dom(L^*)$ we have
\[ 0 = \inner{(\lambda\id-L)x,y} = \inner{x,(\overline{\lambda}\id-L^*)y}. \]
Because there is no residual spectrum, $\im(\overline{\lambda}\id-L^*)$ is either dense in $H$ (if $\overline{\lambda}\in \rho(L)$ or $\overline{\lambda}\in\sigma_\text{c}(L)$) or $\overline{\lambda}$ is an eigenvalue of $L^*$. The first option is impossible, because then $x\perp \im(\overline{\lambda}\id-L^*)$, which would mean $x = 0$ by \ref{corollary:perpToDenseSet}. This contradicts the fact that $x$ is an eigenvector.

(2) Now take non-zero vectors $x,y$ such that $Ax = \lambda x$ and $A^*y = \overline{\mu}y$. Then
\[ 0 = \inner{y, Ax} - \inner{y, Ax} = \inner{y, Ax} - \inner{A^*y,x} = \lambda\inner{y,x} - \mu\inner{y,x} = (\lambda-\mu)\inner{y,x} = 0. \]
\end{proof}
\begin{corollary}
If $\lambda \neq \mu$, then $\ker(\lambda\id-L)\perp\ker(\overline{\mu}\id-L^*)$.
\end{corollary}
...
\begin{enumerate}
\item $L$ is normal;
\item $\ker(\lambda\id-L)\perp\ker(\overline{\mu}\id-L^*)$ if $\lambda \neq \mu$;
\item the residual spectrum of both $L$ and $L^*$ is empty.
\end{enumerate}
In any of these cases we have
\[ \sigma_\text{p}(L) = \overline{\sigma_\text{p}(L^*)}. \]
\end{proposition}
\begin{proof}
...
\end{proposition}
\begin{proof}
(1) Let $\lambda$ be an eigenvalue of $T$ with eigenvector $x$. Then
\[ \lambda\inner{x,x} = \inner{x,\lambda x} = \inner{x,Tx} = \inner{Tx,x} = \inner{\lambda x,x} = \overline{\lambda}\inner{x,x}. \]

(2) Let $\lambda_1,\lambda_2$ be distinct eigenvalues of $T$ with eigenvectors $x_1,x_2$. Then
\[ \lambda_1\inner{x_1,x_2} = \overline{\lambda_1}\inner{x_1,x_2} = \inner{\lambda_1 x_1, x_2} = \inner{Tx_1, x_2} = \inner{x_1, Tx_2} = \inner{x_1,\lambda_2x_2} = \lambda_2\inner{x_1,x_2}, \]
which we can rewrite as
\[ (\lambda_1-\lambda_2)\inner{x_1,x_2} = 0. \]
\end{proof}
Compare this to \ref{prop:adjointSpectrumNoResidual}.
