...
\begin{lemma}
Let $\sSet{X,Y,\pair{\cdot,\cdot}}$ be a pairing. Then
\[ f\in \sSet{Y, \sigma(X,Y)}^* \iff \exists A\subseteq X: \forall y\in Y: \; |f(y)| \leq \sup_{x\in A}\abspair{x,y}.  \]
\end{lemma}
\begin{proof}
The direction $\Leftarrow$ is immediate from \ref{continuityDominatedFunctional}.

Now take $f\in \sSet{Y, \sigma(X,Y)}^*$
\end{proof}
...
\begin{align*}
f\in \sSet{V, \sigma(V^*, V)}^* &\iff \exists C>0: \exists U\in \neighbourhood_V(0):\; f^\imf[U] \subseteq \ball(0,C) \\
&\iff \exists C>0: \exists g\in V^*: f^\imf\big[ \setbuilder{v\in V}{\abspair{g,v} = |g(v)| \leq 1} \big] \subseteq \ball(0,C) \\
&\iff \exists C>0: \exists g\in V^*: \forall v\in V: \; |g(v)| \leq 1 \implies |f(v)| \leq C \\
&\iff \exists g\in V^*: \forall v\in V: \; |g(v)| \leq 1 \implies |f(v)| \leq 1 \\
&\iff \exists g\in V^*: \forall v\in V: \; |f(v)| \leq |g(v)|.
\end{align*}
