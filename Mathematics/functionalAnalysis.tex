...
\subsubsection{Symmetric versus self-adjoint operators}
...
\begin{enumerate}
\item If $\exists z \in \C\setminus\R: \; \im(A+z\id) = H$, then $A$ is densely defined.
\item If $\exists z \in \C: \; \im(A+z\id) = H$ and $\im(A+\overline{z}\id) = H$, then $A$ is self-adjoint.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) Let $A+z\id$ be surjective and suppose, towards a contradiction that there exists an $y\perp \dom(A)$. Then $y = (A-z\id)x$ for some $x\in\dom(A)$ by surjectivity. Then
...
(2) From \ref{symmetricOperatorAdjointInclusion} and point (5) of \ref{adjointAlgebraicProperties}, we have $A+z\id = (A^* + z\id)\cap(A+z\id) = (A+\overline{z}\id)^*\cap (A+z\id)$.
...
\end{itemize}
\item The \udef{continuous spectrum} $\cspec(L)$ is the set of all values of $\lambda\in\spec(L)$ such that the resolvent $R_L(\lambda)$ exists and has dense range.
\item The \udef{residual spectrum} $\rspec(L)$ is the set of all values of $\lambda\in\spec(L)$ such that the resolvent $R_L(\lambda)$ exists and does not have dense range.
...
\begin{proposition}
Let $S$ be a closed, injective linear operator on a Banach space with closed range. Let $T$ be a bounded operator with $\norm{T} < \norm{S^{-1}}^{-1}$, then $S-T$ is invertible and
\[ (S-T)^{-1} = S^{-1}\sum_{n=0}^\infty \left(S^{-1}T\right)^n. \]
\end{proposition}
The assumptions are enough to guarantee $S^{-1}$ is bounded, so $\norm{S^{-1}}$ makes sense.
\begin{proof}
...
\[ \frac{1}{\lambda^{-1} - T^{-1}} = \frac{\lambda T}{\lambda T}\frac{1}{\lambda^{-1} - T^{-1}} = \frac{\lambda T}{T - \lambda} = \frac{\lambda T - \lambda^2 + \lambda^2}{T - \lambda} = \frac{\lambda\cancel{(T - \lambda)}}{\cancel{T - \lambda}} + \frac{\lambda^2}{T - \lambda}. \]
TODO: make rigourous!!
\end{proof}
\begin{corollary}
Let $T$ be an injective operator with dense range. Then for all $\lambda\neq 0$
...
\end{proof}
TODO: redefine continuous spectrum?? Such that $\spec(T) = \pspec(T) \cup \cspec(T) \cup \rspec(T)$ by definition.
\begin{corollary}
...
Let $T$ be an operator on a Banach space $X$.
\begin{enumerate}
\item If there exists a sequence $\seq{x_n} \subseteq X$ such that
\[ \forall n: \; \norm{x_n} = 1 \qquad\text{and}\qquad \norm{(\lambda\id - T)x_n}\to 0, \]
then $\lambda\in\spec(T)$.
\item If $\lambda \in \sigma(T)\cap \overline{\res(T)}$, then there exists such a sequence for $\lambda$.
\end{enumerate}
\end{proposition}
A sequence as described in \ref{WeylSequence} is called a \udef{Weyl sequence}.
\begin{proof}
(1) If $\lambda\in\res(T)$, then
\[ 1 = \norm{x_n} = \norm{R_T(\lambda)(\lambda\id-T)x_n} \leq \norm{R_T(\lambda)}\;\norm{(\lambda\id-T)x_n} \not\to 0, \]
which is a contradiction.

(2) Assume $\lambda \in \sigma(T)\cap \overline{\res(T)}$. Then we can find a sequence $\seq{\lambda_n}\subseteq \res(T)$ such that $\lambda_n \to \lambda$.
...
\begin{lemma}
Let $L$ be an operator on a vector space $V$. If $\lambda \id_V - T$ is not bounded from below, then $\lambda \in \spec(T)$.
\end{lemma}
\begin{proof}
If $\lambda \notin \spec(T)$, then $R_\lambda(L)$ is bijective and bounded. By lemma \ref{boundedBelow}, $\lambda \id_V - T$ is bounded below.
\end{proof}
...
\begin{proposition}[Spectral inclusion property of numerical range]
Let $A$ be an operator. Then $\pspec(A)\subseteq W(A)$ and $\spec(A) \subseteq \overline{W(A)}$.
\end{proposition}


\url{https://mathoverflow.net/questions/264740/non-empty-resolvent-set-then-operator-closed}
\url{https://math.stackexchange.com/questions/3262168/closed-operator-with-trivial-resolvent-set}
...
\end{definition}
\begin{lemma}
Let $T$ be an operator on a vector space $V$. Then $\lambda \in \apspec(T)$ \textup{if and only if} there exists a sequence of unit vectors $(e_n)_{n\in\N}$ for which
...
\begin{lemma}
Let $T$ be an operator. Then $\pspec(T)\cup\cspec(T)\subset\apspec(T) \subset \spec(T)$.
\end{lemma}
\begin{proof}
First assume $\lambda\notin \apspec(T)$, 
...
\begin{proposition}
Let $T$ be a symmetric operator. Then $\spec(T)$ is one of the following:
\begin{itemize}
\item $\C$;
\item $\setbuilder{z\in \C}{\Im(z) \geq 0}$;
\item $\setbuilder{z\in \C}{\Im(z) \leq 0}$;
\item a subset of $\R$.
\end{itemize}
If $T$ is densely-defined, then either the first or the last case holds.

If $T$ is self-adjoint, then the last case holds.
\end{proposition}
\begin{proof}
TODO \url{https://math.stackexchange.com/questions/893899/spectrum-of-symmetric-non-selfadjoint-operator-on-hilbert-space}
\end{proof}
...
\begin{proposition}
If $A$ is a positive operator, then $\spec(A) \subseteq [0,\infty[$.
\end{proposition}
\begin{proof}
\url{https://math.stackexchange.com/questions/1014070/positive-operator-has-a-positive-spectrum}
\end{proof}

\begin{proposition}
Let $A$ be a self-adjoint operator. Then
\begin{enumerate}
\item $\inf \sigma(A) = \inf\setbuilder{\inner{x,Ax}}{x\in \dom(A), \; \norm{x} = 1}$;
\item $\sup \sigma(A) = \sup\setbuilder{\inner{x,Ax}}{x\in \dom(A), \; \norm{x} = 1}$.
\end{enumerate}
\end{proposition}
\begin{proof}
