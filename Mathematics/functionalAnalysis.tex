...
\subsubsection{Functions to normed spaces}
...
\end{proposition}
\begin{proof}
$(1) \Leftrightarrow (2)$ By \ref{linearContinuityToNormedSpace}.
...
\end{proposition}
\begin{proof}
(1, 2) That $\xi$ is topological follows from \ref{topologicalInitialFinalConvergence}. Point (2) follows from \ref{pretopologicalInitialConvergence}.
...
By continuity of the inclusion map, we can find an $M \in \vicinity_V(0)$ such that $M\cap U = K$. Then $|f|\leq p_M$ and $f$ can be extended by the Hahn-Banach extension theorem to a functional $f'$ defined in the whole of $V$. Then $f'$ is bounded on $M$ and thus continuous by \ref{linearContinuityToNormedSpace}.
\end{proof}
\begin{corollary}
...
by \ref{principalInherenceAdherenceProperties}.
...
\begin{proposition}
Let $\sSet{X,Y, \pair{\cdot,\cdot}}$ and $\sSet{U,V, \pair{\cdot,\cdot}'}$ be pairings. 
\end{proposition}
...
\begin{definition}
Let $\sSet{\Omega, \mathcal{A}, \mu}$ be a measure space and $f: \Omega\to \R$ a measurable function. The \udef{distribution function} of $f$ is the function
...
Then clearly $A\cap N^c \subseteq B$ (as $|f(\omega)|\leq |g(\omega)|$ and $\alpha < |f(\omega)|$ imply $\alpha < |g(\omega)|$). Also $N$ is measurable (by \ref{measurablesSetsDerivedFromRealMeasurableFunctions}) and a null set (by assumption). Also
...
using \ref{measureSetDifference} and \ref{measureNullSet}. Thus we have
\[ d_f(\alpha) = \mu(A) = \mu\big(A\setminus (N\cap A)\big) = \mu(A\cap N^c) \leq \mu(B) = d_g(\alpha), \]
where the inequality given by \ref{measureProperties}.
...
\end{align*}
TODO by Fubini (for which we need $\sigma$-finite)!! (Why? Tonelli?)
\end{proof}
\begin{corollary}
...
\item $\norm{\cdot}_{p,\infty}: \meas(X, \C)/\overset{a.e.}{=} \to \overline{\R}^+$ is a quasi-norm;
...
\begin{proposition}
Let $T$ be a bounded operator on a Banach space. Then $\spec(T)\neq \emptyset$.
\end{proposition}
\begin{proof}
Assume, towards a contradiction, that $\res(T) = \C$. Then the resolvent norm $\lambda\mapsto \norm{R_T(\lambda)}$ is an entire function by \ref{derivativePseudoresolvent}. By \ref{secondNeumannSeries} we have $\lim_{|\lambda|\to\infty}\norm{R_T(\lambda)} = 0$. By Liouville's theorem \ref{liouvilleTheoremAnalysis}, we have that $\norm{R_T(\lambda)}$ is identically zero.

Now take some unit vector $u\in \dom(T)$. Then
\[ 1 = \norm{u} = \norm{R_T(\lambda)(\lambda\id-T)u} \leq \norm{R_T(\lambda)}\;\norm{(\lambda\id-T)u} = 0. \]
