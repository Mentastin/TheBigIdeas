...
Now consider $\cball(0, \epsilon)\setminus \ball(0, \epsilon)$, which is compact. Then $u^{\imf}\big(\cball(0, \epsilon)\setminus \ball(0, \epsilon)\big) = \big(\cball(0, \epsilon)\setminus \ball(0, \epsilon)\big)\cdot v$ is compact by \ref{compactConstructions} and thus closed by \ref{compactClosedSets} (as $\Span\{v\}$ we assumed Hausdorff). Now $0 \in \Big(\big(\cball(0, \epsilon)\setminus \ball(0, \epsilon)\big)\cdot v\Big)^c$, so there exists a vicinity $U$ of $0$ disjoint from $\big(\cball(0, \epsilon)\setminus \ball(0, \epsilon)\big)\cdot v$.

For each $F$ that converges to $0$ in $\Span\{v\}$, $\neighbourhood_\F(0)\cdot F$ also converges to $0$, so there exists $\delta_F>0$ and $C_F\in F$ such that $\ball(0, \delta_F)\cdot C_F \subseteq U$. Now 

$\bigcup_F C_F \subseteq \ball(0, \epsilon)\cdot v$, so $\ball(0, \epsilon)\cdot v  = u^\imf(\Gamma) \vicinity_{\Span\{v\}}(0)$ and thus $\Gamma \in (u^{-1})^{\imf\imf}\big[\vicinity_{\Span\{v\}}(0)\big]$.
\end{proof}
...
\end{proof}
\begin{corollary}
Let $\sSet{V, \xi}$ and $\sSet{W, \zeta}$ be convergence groups. Let $f: V\to W$ be a linear function and $U\subseteq G$ a subspace such that $N\subseteq \ker f$. Then there exists a unique continuous linear function $f': A/N \to B$ such that
\[ \begin{tikzcd}
