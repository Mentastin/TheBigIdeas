...
Let $\sSet{V, \xi}$ be a convergence vector space, $\sSet{W, \norm{\cdot}}$ a normed space and $f: V\to W$ a function.
\begin{enumerate}
\item If $f$ is continuous, then $f$ is bounded on some $U\in \vicinity_\xi(0)$.
\item If $f$ is positively homogeneous, then the converse also holds.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) Assume $f$ continuous, then $\ball(0,1)\in \neighbourhood_W(0) = \neighbourhood_W(f(0)) \subseteq f^{\imf\imf}[\vicinity_\xi(0)]$ by \ref{continuityVicinityFilter}. So there exists $U\in \vicinity_\xi(0)$ such that $f^{\imf}[U] \subseteq \ball(0,1)$, which means that $f$ is bounded by $1$ on $U$.

(2) Assume $f$ is positively homogeneous and there exists a vicinity $U\in \vicinity_\xi(0)$ such that $f$ is bounded on $U$. Then $f^{\imf}(U)\subseteq \ball_W(0,C)$.
...
\[ f^{\imf}(\frac{\epsilon}{C}U) = \frac{\epsilon}{C}f^{\imf}(U)\subseteq \frac{\epsilon}{C}\ball_W(0,C) = \ball_W(0,\epsilon), \]
means that $\neighbourhood_W(0) \subseteq f^{\imf\imf}[\vicinity_\xi(0)]$ and thus that $f$ is continuous.
\end{proof}
\begin{corollary} \label{majorisedContinuity}
Let $\sSet{V, \xi}$ be a convergence vector space, $\sSet{W, \norm{\cdot}}$ a normed space. Let $f,g: V\to W$ be functions such that $f$ is positively homogeneous and $g$ is continuous. If $f \leq g$, then $f$ is also continuous.
\end{corollary}
\begin{proof}
As $g$ is continuous, it is bounded on some $U\in \vicinity_\xi(0)$. Then $f$ is also bounded on this $U$ and thus continuous by the proposition.
\end{proof}
...
Let $A$ be an algebraically open map. We use \ref{openClosedCriteria} to show $f^\imf[A]$ is also open. Because $f$ is non-zero, there exists a $v\in V$ such that $f(v) \neq 0$. Take some $y\in f^\imf[A]$. Then there exists an $x\in A$ such that $f(x) = y$. Because $A$ is open, $x\in \inh_\mathfrak{x}(A)$ and there exists $x+ \Gamma_v\in \vicinity_\F(0)$ such that $x+\Gamma_v\cdot v \subseteq A$ by \ref{constructionsInAlgebraicConvergence}.
...
\begin{lemma}
Let $V$ be a vector space and $f_0,\ldots, f_n, f$ linear functionals in $(V\to \F)$. Then a linear functional $f: V\to \F$ is a linear combination of $f_0,\ldots, f_n$ \textup{if and only if} .
\end{lemma}
\begin{proof}
...
\item a \udef{norm} is a seminorm $f$ such that $\forall v\in V: f(v) = 0 \implies v = 0$.
\end{itemize}
\end{definition}
...
\begin{lemma} \label{gaugeWellDefined}
...
\item $\frac{v}{p_A(v)+\epsilon} \in A$ for all $v\in V$ and $\epsilon >0$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
(3) We calculate
\begin{align*}
x\in p_{\semibalanced(A)}^\preimf[\ball(0,1)] \iff& p_{\semibalanced(A)}(x) < 1 \\
\implies& x\in \semibalanced(A) \\
\implies& p_{\semibalanced(A)}(x) \leq 1 \\
\iff& x\in p_{\semibalanced(A)}^\preimf[\cball(0,1)].
\end{align*}
Reformulating in terms of sets gives the result.

(4) This follows from (1) and the fact that $p_A(v) < p_A(v) + \epsilon$.
\end{proof}
...
\end{lemma}
\begin{proof}
$(1) \Rightarrow (2)$ This is immediate by \ref{gaugeLemma}.
...
\begin{proposition} \label{gaugeContinuity}
Let $\sSet{V, \xi}$ be a convergence vector space and $A\subseteq V$ an absorbent, semibalanced subset. Then $p_A: \sSet{V,\xi} \to \F$ is continuous \textup{if and only if} $A\in\vicinity_\xi(0)$.
\end{proposition}
\begin{proof}
By \ref{gaugeScaling} we have that $p_A$ is positively homogeneous. Then by \ref{continuityToNormedSpace}, $p_A$ is continuous iff it is bounded on some $B\in\vicinity_\xi(0)$. Now $p_A$ is bounded on $A$ by \ref{gaugeLemma}, so this proves the direction $\Rightarrow$.

Now assume $p_A$ continuous. Then $\ball(0,1) \in \neighbourhood_\F(0) = \neighbourhood_\F(p_A(0))$, so $p_A^\preimf[\ball(0,1)] \in \vicinity_\xi(0)$ by \ref{continuityVicinityFilter}. By \ref{gaugeLemma} (using the fact that $A$ is semibalanced), $p_A^\preimf[\ball(0,1)] \subseteq A$, so $A\in\vicinity_\xi(0)$.
\end{proof}
\begin{corollary} \label{gaugeContinuousAlgebraicConvergence}
Let $V$ be a vector space. Then $p_A: \sSet{V,\mathfrak{a}} \to \F$ is continuous for any absorbent subset $A \subseteq V$.
\end{corollary}
\begin{proof}
By \ref{coreProperties}, we have $0\in\inh_\mathfrak{a}(A)$. Then $A\in \vicinity_\mathfrak{a}$ by \ref{principalAdherenceInherence}.
\end{proof}
\begin{corollary} \label{absoluteFunctionalGauge}
...
\end{corollary}
\begin{proof}
We have that $|f|$ is positively homogeneous, and thus of the form $p_K$ by \ref{gaugeClassification}. The rest follows from the proposition.
\end{proof}
...
\end{proposition}
\begin{proof}
(1) We show both inclusions. First take $x\in \inh_\mathfrak{a}(A)$. Then by \ref{constructionsInAlgebraicConvergence}, there exists $\Gamma\in\neighbourhood_\F(0)$ such that $v+\Gamma\cdot v = (1 + \Gamma)\cdot v \subseteq A$. There exists $\lambda \in 1 + \Gamma$ that is real and strictly less than $1$. Thus $p_A(x) <1$, meaning $x\in p^\preimf_A\big(\ball(0,1)\big)$.

For the other inclusion we use that $p_A$ is continuous by \ref{gaugeContinuousAlgebraicConvergence}. By \ref{adherenceInherenceContinuity} and \ref{gaugeLemma} we have
...
(2) Again we show both inclusions. From \ref{gaugeLemma} we have $A \subseteq p^\preimf_A\big(\cball(0,1)\big)$. Thus, by \ref{adherenceInherenceContinuity},
...
\item if $p_B \leq p_A$, then $\adh_\mathfrak{a}(A) \subseteq \adh_\mathfrak{a}(B)$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
Now let $V$ be a locally convex TVS, meaning there exists a convex base $\mathcal{B}$ of $\neighbourhood(0)$. Each $B\in\mathcal{B}$ is absorbent by \ref{TVSconstruction} and thus also semibalanced by \ref{convexAbsorbentImpliesSemibalanced}. Then $S = \setbuilder{p_B}{B\in \mathcal{B}}$ is a set of continuous seminorms, by \ref{gaugeContinuity}. 
...
\end{corollary}
\begin{proof}
We have that $|f| = p_K$ for some $K\in \vicinity_U(0)$ by \ref{absoluteFunctionalGauge}. By continuity of the inclusion map, we can find an $M \in \vicinity_V(0)$ such that $M\cap U = K$. Then $|f|\leq p_M$ and $f$ can be extended by the Hahn-Banach extension theorem. By \ref{majorisedContinuity} this extension is continuous.
\end{proof}
\begin{corollary}
...
Indeed $A-a \subseteq U+A-a = \inh_\mathfrak{a}(U+A-a) = p_{U+A-a}^\preimf[\ball(0,1)] \subseteq f_{a}^\preimf[\ball(0,1)]$ by \ref{algebraicallyOpen} and \ref{gaugeInherenceAdherence} ($U+A-a$ is semibalanced because $A-a$ is).
...
The initial convergence on $Y$ w.r.t. $\setbuilder{\abspair{x,\cdot}}{x\in X}$ is called the \udef{weak topology} $\sigma(X,Y)$ on $Y$ for the pair $\sSet{X,Y}$\footnote{What we denote $\sigma(X,Y)$ is usually denoted $\sigma(Y,X)$.}.
\end{definition}
This initial convergence is a topology by \ref{topologicalInitialFinalConvergence}.
...
The weak topology is the weakest topology $\tau$ (TODO convergence?) such that $V^* = \sSet{V, \tau}^*$.
\end{corollary}
...
\begin{proposition}
Let $V$ be a locally convex convergence vector space and $B\subseteq V$ a convex subset. Then $\overline{B} = \overline{B}^{\sigma(\dual{V}, V)}$.
\end{proposition}
\begin{proof}
The weak topology is the weakest topology with dual $\dual{V}$. Thus 
\end{proof}
...
The other inclusion is proved by contradiction. Assume, to this end, that $y_0\in B^{\pol\pol} \setminus \overline{\disked(B)}^{\sigma(X,Y)}$. Then $y_0$ is an element of the open set $\overline{\disked(B)}^c$, so we can find a convex open neighbourhood $U$ of $y_0$ in $\overline{\disked(B)}^c$. Next apply Hahn-Banach separation \ref{HahnBanachSeparation} to obtain a continuous functional $f$ such that $f(y_0) \not\in f^\imf\left[\overline{\disked(B)}\right]$. Now $\overline{\disked(B)}$ is absolutely convex by \ref{inherenceAdherenceBalanced} and \ref{inherenceAdherenceConvex}, so $f^\imf\left[\overline{\disked(B)}\right]$ is also absolutely convex by linearity. Then $|f^\imf\left[\overline{\disked(B)}\right]|$ and $|f^\imf[U]|$ are disjoint and $|f^\imf[U]|$ is open, as $0\in |f^\imf\left[\overline{\disked(B)}\right]|$. This also means that there exists $t\in |f^\imf[U]|$ such that $|f(b)| < t < |f(y_0)|$ for all $b\in \overline{\disked(B)}$. Thus $\sup_{b\in \overline{\disked(B)}}|f(b)| < |f(y_0)|$. By rescaling $f$ we can take $\sup_{b\in \overline{\disked(B)}}|f(b)| \leq 1 < |f(y_0)|$.
...
\begin{definition}
Let $X$ be a Banach space. A function $\mathcal{R}:\Lambda \subseteq \C \to \Bounded(X)$ is called a \udef{pseudoresolvent} if, for all $\lambda,\mu\in\Lambda$
\[ \mathcal{R}(\lambda) - \mathcal{R}(\mu) = (\mu-\lambda)\mathcal{R}(\lambda)\mathcal{R}(\mu). \]
This equation is known as the \udef{resolvent identity}.
\end{definition}
