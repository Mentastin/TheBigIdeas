...
\[ \norm{f}_{p} \defeq \left(\int_X |f|^p\diff{\mu}\right)^{1/p} \]
...
\begin{lemma}
Let $0< p<\infty$ and $\sSet{X,\mathcal{A},\mu}$ be a measure space. Let $f,g\in\mathcal{L}^p(X,\mu)$ be functions. Then
\begin{enumerate}
\item $|f+g|^p \leq 2^{p-1}\left(|f|^p + |g|^p\right)$;
\item $\norm{f+g}_p \leq 2^{\frac{1-p}{p}}\left(\norm{f}_p + \norm{g}_p\right)$.
\end{enumerate}
\end{lemma}
Notice that this inequality also works for $p\leq 1$. The sharper Minkowski inequality, \ref{MinkowskisInequality}, only holds for $p>1$.
\begin{proof}
(1) We show this using the convexity of $\R^+\to \R^+: x\mapsto |x|^p$:
\begin{align*}
...
\end{corollary}
\begin{proof}
We need to prove that $\norm{f}_{p}$ is subadditive and absolutely homogeneous. Subadditivity follows straight from $|f+g|\leq |f| + |g|$ and the Minkowski inequality.

Absolute homogeneity is given by \ref{pNormAbsolutelyHomogeneous}.
\end{proof}
