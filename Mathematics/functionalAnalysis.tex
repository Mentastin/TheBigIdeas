...
\begin{proposition}
Let $V$ be a real or complex vector space and $A$ an algebraically open and absolutely convex set. If $U$ is a subspace such that $A\perp U$, then there exists a hyperplane $H \supseteq U$ such that $A\perp H$.
\end{proposition}
\begin{proof}
Take $a\in A$. Then $0\in a-A = \inh_{\mathfrak{a}}(a-A)$, so $a-A$ is absorbing by \ref{coreProperties} and $a-A+U$ is also absorbing. 
