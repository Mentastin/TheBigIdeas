...
\end{lemma}
\begin{lemma}
Let $\omega\in \tdual{X}$ be a bounded linear functional. Then
\begin{align*}
\norm{\omega} &= \sup\{ |\omega(x)|\;|\; \norm{x}=1 \} \\
&= \sup\left\{ \frac{|\omega(x)|}{\norm{x}}\;|\;x\neq 0 \right\} \\
&= \inf\{ c>0\;|\; |\omega(x)|\leq c\norm{x}\forall x\in X \}
\end{align*}
\end{lemma}
\begin{proof}
...
\subsection{The spaces $\mathcal{L}^n(X,\diff{\mu})$}
\subsection{The spaces $L^n(X,\diff{\mu})$}
\begin{theorem}[Riesz-Fisher]
The space $L^n(X,\diff{\mu})$ is complete.
\end{theorem}
...
\item For all closed subspaces $K$ we have $(K^\perp)^\perp = \overline{K}$.
