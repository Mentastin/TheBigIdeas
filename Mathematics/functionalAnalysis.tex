...
\begin{lemma} \label{boundedOnGeneratingSeminormsImpliesvonNeumannBounded}
...
\subsubsection{Weak boundedness}
...
For the other direction, consider the topology on $Y$ as the initial topology w.r.t. the function $f: Y\to \prod_{x\in X}\F$, where $p_x(f(y)) = \pair{x,y}$, as constructed in \ref{initialConvergenceWrtOneFunction}. This function is injective and the inverse is continuous (TODO ref). As the inverse is also linear, it is uniformly continuous by \ref{uniformContinuityGroupHomomorphism}.

Now by \ref{boundednessTVS} (and \ref{uniformContinuityGroupHomomorphism}, \ref{metricBoundedness}) $\abspair{x, B}$ is bounded for all $x\in X$. Then $\overline{\abspair{x,B}}$ is also bounded by \ref{adherenceBoundedSet}.

As $\overline{\abspair{x, B}}\subseteq \R$, we have that it is totally bounded by \ref{realBoundedIffTotallyBounded} and thus precompact by \ref{precompactTotallyBounded}. It is also complete by (TODO ref) and thus compact by \ref{compactPrecompactComplete}.

Now $\prod_{x\in X}\overline{\abspair{x, B}}$ is compact by Tychonoff's theorem \ref{TychonoffsTheorem}, and in particular totally bounded by \ref{precompactTotallyBounded} and \ref{compactPrecompactComplete}. 

Now $f^\imf(B) \subseteq \prod_{x\in X}\overline{\abspair{x, B}}$, so $B\subseteq f^\preimf\Big(\prod_{x\in X}\overline{\abspair{x, B}}\Big) = (f^{-1})^\imf\Big(\prod_{x\in X}\overline{\abspair{x, B}}\Big)$ and thus $B$ is totally bounded by \ref{imageBoundedSet} and \ref{boundedSetsIdeal}.
\end{proof}
