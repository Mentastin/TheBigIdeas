...
\[ f\in \sSet{Y, \sigma(X,Y)}^* \iff \exists \;\text{finite}\; A\subseteq X: \forall y\in Y: \; |f(y)| \leq \max_{x\in A}\abspair{x,y}.  \]
\end{lemma}
\begin{proof}
By point (3) of \ref{initialSeminormConvergence} we have
\[ f\in \sSet{Y, \sigma(X,Y)}^* \iff \exists \;\text{finite}\; A\subseteq X: \exists C >0:  \forall y\in Y: \; |f(y)| \leq C \max_{x\in A}\abspair{x,y}.  \]
Because $C \max_{x\in A}\abspair{x,y} = \max_{x\in A}\abspair{Cx,y} = \max_{x\in C\cdot A}\abspair{x,y}$, we can eliminate the $C$ by replacing $A$ by $C\cdot A$.
\end{proof}
...
\end{proposition}
\begin{proof}
From \ref{functionalContinuityWeakTopology}, it is easy to see that $V^* \subseteq \sSet{V, \sigma(V^*, V)}^*$: take $f\in V^*$ and note that $|f(v)| \leq |f(v)| = \max_{x\in \{f\}}|x(v)| = \max_{x\in \{f\}}\abspair{x,v}$.

Now take $f\in \sSet{V, \sigma(V^*, V)}^*$
\end{proof}
