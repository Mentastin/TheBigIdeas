...
\begin{definition}
A linear operator $T:X\to Y$ between TVSs is \udef{compact} if it maps bounded sets to precompact sets, i.e. 
...
\item there exists a neighbourhood $U\subset X$ of the origin such that $T[U]$ is precompact;
\item there exists a neighbourhood $U \subset X$ of the origin and a compact set $V\subset Y$ such that $T[U] \subset V$;
\item for any bounded sequence $(x_{n})_{n\in \mathbb{N}}$ in $X$, the sequence $(Tx_{n})_{n\in \mathbb{N} }$ contains a converging subsequence. (TODO: only for sequential spaces).
\end{enumerate}
\end{proposition}
...
\subsubsection{Friedrichs extension}
\begin{proposition}
