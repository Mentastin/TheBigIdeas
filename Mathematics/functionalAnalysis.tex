...
\begin{definition}
A linear operator $T:X\to Y$ between TVSs is \udef{compact} if it maps bounded sets to relatively compact sets, i.e. 
\[ \text{$A\subset X$ is bounded} \quad\implies\quad \text{$\overline{T[A]}$ is compact.} \]
\end{definition}

\begin{proposition}
Let $T:X\to Y$ be a linear operator between TVSs. The following are equivalent:
\begin{enumerate}
\item $T$ is a compact operator;
\item the image of the unit ball $B(0,1)$ of $X$ is relatively compact in $Y$;
\item there exists a neighbourhood $U\subset X$ of the origin such that $T[U]$ is relatively compact;
...
\begin{lemma}
Let $X,Y$ be TVSs. The compact operators from $X$ to $Y$ form a vector space. 
\end{lemma}
\begin{proof}
Let $K,K':X\to Y$ be compact operators. Then
...
Let $T:\dom(T)\to Y$ be a bounded operator where $Y$ is a Banach space and $\dom(T)$ lies in a normed space. Then $T$ has a unique extension
\[ \tilde{T}:\overline{\dom(T)}\to Y \]
where $\tilde{T}$ is a bounded operator with $\norm*{\tilde{T}} = \norm{T}$.
\end{proposition}
\begin{proof}
...
Clearly $\norm{\tilde{T}} \geq \norm{T}$. For the converse take $x\in\tilde{X}$ and a Cauchy sequence $\seq{x_i}\subset X$ that converges to $x$ in $\tilde{X}$. Then
\[ \norm{\tilde{T}(x)}_Y = \lim_{i\to\infty}\norm{T(x_i)}_Y \leq \lim_{i\to\infty}\norm{T}\;\norm{x_i}_X = \norm{T}\;\norm{x}_X. \]
\end{proof}
...
\end{proof}
\begin{corollary}[Neumann series]
Let $T$ be a bounded linear operator with $\norm{T}<1$. Then
\[ (\id - T)^{-1} = \sum_{i=1}^\infty T^i. \]
\end{corollary}
\begin{proof}
Let $x\in X$. Then $(\id - T)^{-1}x = y$ iff $x-Tx = y$ iff $x$ is the fixed point of $Tx+y$. So $y = \sum_{i=1}^\infty T^ix$.
\end{proof}
...
\begin{theorem}[Closed graph] \label{theorem:closedGraphTheorem}
Let $X,Y$ be Banach spaces and $T:\dom(T)\subset X\to Y$ a linear operator. Then $T$ is bounded if $T$ has closed graph.
\end{theorem}
\begin{proof}
...
\end{proof}
\begin{corollary}
If $T$ is a linear map $X\to Y$, then $T$ is bounded \textup{if and only if} $T$ has closed graph.
\end{corollary}
...
Maximal orthonormal sets are orthonormal bases. So there is always an o.n. basis.
...
\begin{proposition}
Two Hilbert spaces are (isometrically) isomorphic \textup{if and only if} they have the same Hilbert dimension.
\end{proposition}
\begin{proof}
If we have an isomorphism, this isomorphism maps an orthonormal basis to an orthonormal basis. TODO:converse.
\end{proof}
\begin{corollary}
All separable infinite-dimensional Hilbert spaces are isometrically isomorphic to $\ell^2$.
\end{corollary}
...
\end{corollary}
\begin{proof}
First assume $K$ is closed. Then using $0=(I-P_K)x\;\; \Leftrightarrow \;\; x=P_Kx$, we see
...
A subset $A$ of $\mathcal{H}$ is dense in $\mathcal{H}$ \textup{if and only if} $A^\perp = \{0\}$
\end{corollary}
Notice that one direction of these last two corollaries is true in any inner product space, see proposition \ref{prop:orthogonalComplementClosed} and corollary \ref{corollary:orthogonalComplementClosed}:
\[ A\;\text{dense in}\;\mathcal{H}\implies A^\perp = \{0\} \qquad \text{and}\qquad \overline{K}\subset (K^\perp)^\perp. \]
...
\begin{enumerate}
\item For any subspace $U$, $U^\perp = \{0\}$ implies $U$ is dense in $H$.
\item For all subspaces $K$ we have $(K^\perp)^\perp = \overline{K}$.
\item For all closed subspaces $K$ we can decompose $H = K\oplus K^\perp$.
\end{enumerate}
\end{theorem}
\begin{proof}
We prove the first case. Then the other two imply the first and thus that $H$ is a Hilbert space.
\begin{enumerate}
\item If we assume $H$ is not complete, we can construct a counterexample to 1., i.e. a subspace $U$ such that $U^\perp = \{0\}$ which is not dense in $H$. So assume $H$ is not complete. And let $\mathcal{H}$ be the completion of $H$. There exists a $v\in \mathcal{H}\setminus H$. All orthogonal complements are taken in the completion.
The set
\[ U \defeq H\cap\{v\}^\perp \]
is a proper closed subspace of $H$. So $U$ cannot be dense in $H$. We claim that the orthogonal complement of $U$ in $H$ is $\{0\}$:
\[ U^\perp\cap H = \{0\}. \]
...
\item Assume 2. Let $U$ be a subspace such that $U^\perp = \{0\}$. Then
\[ \overline{U} = (U^\perp)^\perp = \{0\}^\perp = H \]
so $U$ is dense in $H$.
\item Assume 3. Let $U$ be a subspace such that $U^\perp = \{0\}$. Then $H = \overline{U}\oplus \overline{U}^\perp = \overline{U}\oplus\{0\} = \overline{U}$. Thus $U$ is dense in $H$.
\end{enumerate}
\end{proof}
...
\end{theorem}
\begin{proof}
The ``if'' has already been shown. For the ``only if'', take such an operator $T$.
...
\begin{definition}
An operator $T\in\Bounded(H,H')$ is called a \udef{Fredholm operator} if $T$ has a finite-dimensional kernel and cokernel.

The \udef{Fredholm index} of $T$ is defined as
\[ \Index T \defeq \dim\ker T - \dim\coker T.  \]

We denote the space of Fredholm operators from $H$ to $H'$ as $\Fred(H,H')$.
\end{definition}

\begin{example}
\begin{enumerate}
\item If $H=H'$ is finite-dimensional, then all operators are Fredholm with index $0$.
...
\begin{proposition}
Let $H$ be a Hilbert space. Then $\Compact(H)$ is a closed two-sided ideal in $H$.
\end{proposition}
\begin{definition}
Let $H$ be a Hilbert space. The \udef{Calkin algebra} is the quotient $\Bounded(H)/\Compact(H)$.
\end{definition}
TODO: quotient algebra ($[A][B] = [AB]$)

\begin{proposition}
Let $[T]\in\Bounded(H)/\Compact(H)$. Then the following are equivalent:
\begin{enumerate}
\item $[T]$ is invertible in the Calkin algebra;
\item $\exists S\in\Bounded(H):$ both $\vec{1}-TS$ and $\vec{1}-ST$ are compact;
...
\begin{proposition}
Let $S,T\in\Fred(H)$ and $K\in\Compact(H)$. Then
\begin{enumerate}
\item $\Index(T^*) = -\Index(T)$;
\item $\Index(ST) = \Index(S)+\Index(T)$;
\item $\Index(T+K) = \Index(T)$;
\item $\Index(T) = 0$ \textup{if and only if} $T=K+L$ for some invertible $L$.
\end{enumerate}
\end{proposition}
...
\[ \begin{tikzcd}
0 \rar & H \dar{T} \rar & H' \dar{S} \rar & H^{\prime\prime} \dar{R} \rar & 0 \\
0 \rar & H \rar & H' \rar & H^{\prime\prime} \rar & 0
\end{tikzcd} \]
...
\begin{enumerate} 
\item Let $T: H\to H$ and $S:H'\to H'$ be Fredholm, then so is $T\oplus S$ with
\[ \Index(T\oplus S) = \Index(T)+\Index(S). \]
\item Let $T: H\to H'$ and $S:H'\to H^{\prime\prime}$ be Fredholm, then so is $ST$ with
\[ \Index(ST) = \Index(T)+\Index(S). \]
\item Let $K: H\to H$ be compact, then $\id+K$ is Fredholm with
\[ \Index(\id+K) = 0. \]
\end{enumerate}
\end{corollary}
...
\begin{definition}
Let $L$ be an operator on a vector space $V$ over the field $\mathbb{F}$.
\begin{itemize}
\item The \udef{spectrum} of $L$ is the set
\[ \sigma(L) = \{ \lambda \in \mathbb{F} \;|\; L-\lambda \id_V: V\to V \; \text{has no bounded inverse}.  \} \]
By ``having no bounded inverse'', we mean there is no bounded operator $R_\lambda: V\to \dom(L)$ such that
\[ R_\lambda(T-\lambda I) = \id_{\dom(L)} \qquad (T-\lambda \id)R_\lambda = \id_V. \]
\item The \udef{point spectrum} $\sigma_\text{p}(L)$ is the set of all its eigenvalues, i.e. the values of $\lambda$ where $L-\lambda \id_V$ fails to be injective.
\item The \udef{continuous spectrum} $\sigma_\text{c}(L)$ is the set of all values of $\lambda$ such that $L-\lambda \id_V$ is injective, but not surjective, and has an image dense in $V$.
\item The \udef{residual spectrum} $\sigma_\text{r}(L)$ is the set of all values of $\lambda$ such that $L-\lambda \id_V$ is injective, but not surjective, and does not even have an image dense in $V$.
\end{itemize}
\end{definition}
In finite dimensions we know that
\[ \text{$L-\lambda \id_V$ is surjective} \quad\iff\quad \text{$L-\lambda \id_V$ is injective.} \]
So in this case there can only ever be a point spectrum.

\begin{definition}
For $\lambda \notin \sigma_\text{p}(L)$ the \udef{resolvent} $R_\lambda(L): \dom(R_\lambda(L)) \to \dom(L)$ of $L$ is
\[ R_\lambda(L) = (L-\lambda \id_V)^{-1}. \]
Note that $R_\lambda(L)$ is a linear operator on $V$ with $\dom(R_\lambda(L)) \subset V$.
\end{definition}
\begin{proposition}
Let $X$ be a Banach space and $T:X\to X$ a bounded linear operator. Then $\lambda \in \sigma(T)$ \textup{if and only if} $T-\lambda \id_V$ is not bijective.
\end{proposition}
\begin{proof}
If $T$ is bounded, then $T-\lambda \id_V$ is bounded for all $\lambda\in\mathbb{F}$. By corollary \ref{corollary:boundedInverse} to the open mapping theorem, if $T-\lambda \id_V$ is bijective, then it is bounded.
\end{proof}


\begin{lemma}
Let $L$ be an operator on a vector space $V$. If $T-\lambda \id_V$ is not bounded from below, then $\lambda \in \sigma(T)$.
\end{lemma}
\begin{proof}
If $\lambda \notin \sigma(T)$, then $R_\lambda(L)$ is bijective and bounded. By lemma \ref{lemma:boundedBelowBounded}, $T-\lambda \id_V$ is bounded below.
\end{proof}
...
\end{lemma}
\begin{proof}
If $R_\lambda(T)$ is bounded, $T-\lambda \id_V$ then is bounded below by lemma \ref{lemma:boundedBelowBounded} and has closed range by proposition \ref{prop:boundedBelowClosedRange}.
\end{proof}
...
\chapter{Integral operators and transforms}
\begin{definition}
Let $(\Omega, \mathcal{A}, \mu)$ be a measure space. Then an \udef{integral operator} or \udef{integral transform} is a map between function spaces (TODO which?) on $\Omega$ of the form
\[ T: ? \to ?: f \mapsto \int_\Omega K(x,y)f(y) \diff{\mu(y)} \]
where $K\in \Omega\times \Omega$ is the \udef{kernel} or \udef{nucleus} of $T$.
...
\item \udef{Hilbert-Schmidt} if $\int_{\Omega\times \Omega}|K(x,y)|^2\diff{x}\diff{y} < \infty$;
