...
Let $\sSet{V, \xi}$ be a convergence vector space and $\sSet{W, \norm{\cdot}}$ a normed space.
A positively homogeneous function $f: V\to W$ is continuous \textup{if and only if} there exists a neighbourhood $U\in \vicinity_\xi(0)$ such that $f$ is bounded on $U$.
\end{proposition}
\begin{proof}
First assume $f$ continuous, then $\ball(0,1)\in \neighbourhood_W(0) = \neighbourhood_W(f(0)) \subseteq f^{\imf\imf}[\vicinity_\xi(0)]$ by \ref{continuityVicinityFilter}. So there exists $U\in \vicinity_\xi(0)$ such that $f^{\imf}[U] \subseteq \ball(0,1)$, which means that $f$ is bounded by $1$ on $U$.

Now assume there exists a neighbourhood $U\in \vicinity_\xi(0)$ such that $f$ is bounded on $U$. Then $f^{\imf}(U)\subseteq \ball_W(0,C)$.
By \ref{pretopologicalContinuityVicinities} it is enough to prove that $\neighbourhood_W(f(0)) \subseteq f^{\imf\imf}[\vicinity_\xi(0)]$. It is in fact enough to show that a base of $\neighbourhood_W(f(0))$, to wit the set of open balls centered at $0$, is contained in $f^{\imf\imf}[\vicinity_\xi(0)]$. To that end, take some open ball $\ball(0,\epsilon)$. Then $\frac{\epsilon}{C} U\in \frac{\epsilon}{C}\vicinity_\xi(0) = \vicinity_\xi(\frac{\epsilon}{C}0) = \vicinity_\xi(0)$. So
...
\begin{lemma}
Let $V$ be a vector space and $A\subseteq V$ an absorbent subset. Then $p_A = p_{\interval[o]{0,1}\cdot A}$.
\end{lemma}
\begin{proof}
TODO
\end{proof}

TODO replace with $\interval[o]{0,1}\cdot A$!!!!!!!!!!!!
...
\begin{proposition}
Let $\sSet{V, \xi}$ be a convergence vector space and $A\subseteq V$ an absorbent subset. Then $p_A: \sSet{V,\xi} \to \F$ is continuous \textup{if and only if} $A\in\vicinity_\xi(0)$.
\end{proposition}
\begin{proof}
By \ref{gaugeClassification} we have that $p_A$ is positively homogeneous. Then by \ref{continuityToNormedSpace}, $p_A$ is continuous iff it is bounded on some $B\in\vicinity_\xi(0)$. Now $p_A$ is bounded on $A$ by \ref{gaugeLemma}, so this proves the direction $\Rightarrow$.

Now assume $p_A$ continuous. Then $\ball(0,1) \in \neighbourhood_\F(0) = \neighbourhood_\F(p_A(0))$, so $p_A^\preimf[\ball(0,1)] \in \vicinity_\xi(0)$ by \ref{continuityVicinityFilter}. By \ref{gaugeLemma}, $p_A^\preimf[\ball(0,1)] \subseteq A$, so $A\in\vicinity_\xi(0)$.
\end{proof}
...
For the other inclusion, take $x\in p^\preimf_A\big(\cball(0,1)\big)$, which means that $p_A(x) \leq 1$.


First take $x\in \adh_\mathfrak{a}(A)$. Then by \ref{constructionsInAlgebraicConvergence}, there exists $v\in V$ such that for all $\Gamma\in\neighbourhood_\F(0)$, we have $(x+\Gamma\cdot v)\mesh A$. There exists $\lambda \in 1 + \Gamma$ that is real and strictly less than $1$. Thus $p_A(x) <1$, meaning $x\in p^\preimf_A\big(\ball(0,1)\big)$.
\end{proof}
...
\begin{proposition} \label{gaugeMajorisation}
Let $\sSet{V,\xi}$ be a convergence vector space and $f:V\to \F$ a linear functional. Then $f\in V^*$ \textup{if and only if} $|f| \leq p_K$ for some $K\in \vicinity_\xi(0)$.
\end{proposition}
\begin{proof}
If $|f| \leq p_K$ for some $K\in \neighbourhood_\xi(0)$, then $f$ is bounded on $K$ because $p_K^\imf[K] \subseteq \cball(0,1)$ by \ref{gaugeLemma}. Thus $f$ is continuous by \ref{continuityToNormedSpace}.

Now assume $f\in V^*$. By \ref{continuityToNormedSpace} $f$ is bounded by some $C\in \R$ on some neighbourhood $M$, which means that $f^\imf(M) \subseteq \cball(0,C)$. By \ref{gaugeInherenceAdherence}, \ref{inherenceAdherenceInclusion} and \ref{adherenceInherenceContinuity}, we have
\[ f^\imf\Big(p_M^{\preimf}\big(\cball(0,1)\big)\Big) = f^\imf\big(\adh_\mathfrak{a}(M)\big) \subseteq f^\imf\big(\adh_\xi(M)\big) \subseteq \closure_\F\big(f^{\imf}(M)\big) \subseteq \closure_\F\big(\cball(0,C)\big) = \cball(0,C). \]




By \ref{continuity} we have that $f^\imf[\adh_\xi(M)] \subseteq \cball(0,C)$ and by \ref{inherenceAdherenceInclusion} and \ref{gaugeInherenceAdherence}, $f^\imf[p_M^\preimf(\cball(0,1))] = f^\imf[\adh_\mathfrak{a}(M)] \subseteq f^\imf[\adh_\xi(M)]$. This means that $\left|f\left(\frac{x}{p_M(x)}\right)\right| \leq C$ for all $x\in V$. Then $p_M(x)^{-1}|f(x)| \leq C$ and, using \ref{gaugeConstructions},
\[ |f(x)| \leq C p_M(x) = p_{C^{-1}\cdot M}(x). \]
Thus we can take $K = C^{-1}\cdot M$, which is a neighbourhood of the origin by \ref{TVSconstruction}.
\end{proof}
...
\begin{example}
\begin{itemize}
\item Let $\sSet{V, \xi}$ be a convergence vector space. Then $\sSet{V^*, V, \evalMap(\cdot,\cdot)}$ is a pairing. If $\xi$ is a locally convex, topological, Hausdorff convergence, then the pairing is a dual pair by Hahn-Banach separation \ref{HahnBanachSeparation} and the fact that singletons are closed \ref{FrechetCharacterisation}.
\item Let $\sSet{V,\xi}$ be a Hausdorff locally convex TVS. Then $\sSet{\sSet{V,\xi}^*, V, \pair{\cdot,\cdot}}$ is a dual pair by \ref{locallyConvexTVSDualPair}. TODO
\end{itemize}
\end{example}
...
(3) We have
\[ B^\pol = \bigcap_{y\in B}\setbuilder{x\in X}{\abspair{x,y}\leq 1} = \bigcap{y\in B}\abspair{\cdot, y}^{\preimf}[\cball(0,1)], \]
