...
\[ \res(L) \defeq \setbuilder{\lambda\in \C}{\text{$R_L(\lambda)$ exists, has domain $V$ and is bounded}}. \]
...
\[ R_T(\lambda) - R_T(\mu) = (\mu-\lambda)R_T(\lambda)R_T(\mu). \]
\end{proposition}
\begin{proof}
...
\[ R_T(\lambda) = \sum_{n=0}^\infty (\lambda-\lambda_0)^n(-1)^nR_T(\lambda_0)^{n+1} = (1+(\lambda-\lambda_0)R_T(\lambda_0))^{-1}R_T(\lambda_0), \]
with convergence radius $1/\spr(R_T(\lambda_0))$.
\end{proposition}
