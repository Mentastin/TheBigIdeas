...
\begin{proposition}
Let $\mathcal{H}$ be a Hilbert space. The set of projections on $\mathcal{H}$ is a complete lattice.
\end{proposition}
In fact $\Projections(\mathcal{H})$ is still a complete lattice as a subset of $\setbuilder{T\in\SelfAdjoints(\mathcal{H})}{0 \leq T \leq \id}$ and each subset has the same supremum and infimum. TODO: Not as a subset of $\SelfAdjoints(\mathcal{H})$??.
