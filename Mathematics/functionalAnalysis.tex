...
Let $A$ be a symmetric operator on a Hilbert space $H$ such that
\[ \exists z \in \C: \quad \im(A+z\id) = H = \im(A+\overline{z}\id), \]
then $A$ is self-adjoint.
\end{proposition}
\begin{proof}
From \ref{symmetricOperatorAdjointInclusion} and point (5) of \ref{adjointAlgebraicProperties}, we have $A+z\id = (A^* + z\id)\cap(A+z\id) = (A+\overline{z}\id)^*\cap (A+z\id)$.
...
Let $T$ be a \emph{bounded} operator on a Banach space $X$. For $|\lambda|>\norm{T}$ the resolvent $R_T(\lambda)$ is bounded and given by
\[ R_T(\lambda) = (\lambda \id_X - T)^{-1} = \sum_{n=0}^\infty\frac{T^n}{\lambda^{n+1}} \]
...
\begin{itemize}
\item The \udef{point spectrum} or \udef{discrete spectrum} $\pspec(L)$ contains the values of $\lambda$ where $\lambda \id_V - L$ fails to be injective. These values are called the \udef{eigenvalues} of $L$.
...
\end{itemize}
\item The \udef{continuous spectrum} $\cspec(L)$ is the set of all values of $\lambda$ such that $\lambda \id_V - L$ is injective, $\overline{\im(\lambda \id_V - L)} = V$, but $\im(\lambda \id_V - L) \neq V$.
\item The \udef{residual spectrum} $\rspec(L)$ is the set of all values of $\lambda$ such that $\lambda \id_V - L$ is injective, but $\overline{\im(\lambda \id_V - L)} \neq V$.
...
\begin{lemma}
Let $T$ be a bounded operator on a Banach space. Then $\spec(T)\subset [-\norm{T}, \norm{T}]$.
\end{lemma}
\begin{proof}
Follows from \ref{secondNeumannSeries}.
\end{proof}

For the point spectrum a simpler argument leads to $\pspec(T)\subset [-\norm{T}, \norm{T}]$: let $\lambda$ be an eigenvalue with eigenvector $x$. Then
\[ |\lambda|\;\norm{x} = \norm{\lambda x} = \norm{Tx} \leq \norm{T}\;\norm{x}. \]
...
\end{proof}
\begin{corollary}
Let $T$ be a closed linear operator on a Banach space. Then $\spec(T)$ is closed and $\rho(T)$ is open.
\end{corollary}
\begin{proof}
...
\end{proof}
\begin{corollary}
Let $T$ be a bounded linear operator on a Banach space. Then $\spec(T)$ is compact.
\end{corollary}
...
\item $\lambda\in\pspec(T) \iff \lambda^{-1}\in \pspec(T^{-1})$.
\end{enumerate}
\end{proposition}
\begin{proof}
We calculate
