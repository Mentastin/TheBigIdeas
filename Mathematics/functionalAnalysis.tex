...
\end{proposition}
\begin{proof}
$\boxed{\Rightarrow}$ Because in particular $\Lambda = \dom(\mathcal{R}) = \dom(R_T|_\Lambda)$, we need to have that $\Lambda \subseteq \res(T)$. The resolvent $R_T(\lambda): \dom(T) \to X$ is bijective for all $\lambda\in \res(T)\subseteq \Lambda$. It is in particular injective.
...
\end{proof}
\begin{corollary}
Let $\mathcal{R}:\Lambda \subseteq \C \to \Bounded(X)$ be a pseudoresolvent and assume that $\Lambda$ contains an unbounded sequence $\seq{\lambda_n}$.
\begin{enumerate}
\item If $\norm{\lambda_n \mathcal{R}(\lambda_n)} \leq M$ for some $M\geq 0$ and all $n\in\N$, then $\mathcal{R}$ is the resolvent map of an operator.
\item If $\lim_{n\to\infty}\lambda_n\mathcal{R}(\lambda_n)x = x$ for all $x\in X$, then $\mathcal{R}$ is the resolvent map of a densely defined operator.
\end{enumerate}
\end{corollary}
\begin{proof}
TODO
