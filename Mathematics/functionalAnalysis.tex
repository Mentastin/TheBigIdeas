...
We denote the space of Fredholm operators from $H$ to $H'$ as $\Fred(H,H')$. If $H=H'$, we write $\Fred(H)$.
\end{definition}
...
\begin{lemma}
Let $T\in\Bounded(H)$. Then $\dim\coker T = \dim\ker T^*$.
\end{lemma}
\begin{proof}
...
\chapter{Integral and convolution operators}
...
\[ T: L^2(\Omega) \to L^2(\Omega): f \mapsto \int_\Omega K(x,y)f(y) \diff{\mu(y)} \]
where $K\in L^2(\Omega\times \Omega)$ is the \udef{kernel} or \udef{nucleus} of $T$.
...
\end{itemize}
\end{definition}
This definition is well-defined: $T\in (L^2(\Omega) \to L^2(\Omega))$. This follows from the Cauchy-Schwarz inequality:
\begin{align*}
...
\begin{proposition}
Let $T$ be an integral operator with kernel $K(x,y)$, then $T^*$ is the integral operator with kernel $K(y,x)$.
\end{proposition}
\begin{proof}
TODO
\end{proof}
