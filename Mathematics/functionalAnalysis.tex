...
\begin{proposition}
Let $\sSet{V,\xi}$ be a TVS and $p_K:V\to \R^{\geq 0}$ the gauge of some absorbent set $K$. Then $p_K$ is continous \textup{if and only if} $K\in \neighbourhoods_\xi(0)$.
\end{proposition}
\begin{proof}
First assume $p_K$ continuous. Then $p_K^{\preimf}[\ball(0,1)] \subseteq K$ by \ref{gaugeClassification}, so $K$ is a neighbourhood of $0$ by \ref{pretopologicalContinuityVicinities}.

Conversely assume $K$ a neighbourhood of $0$ and take some neighbourhood $\Gamma$ of $0$ in $\R$. Then $\Gamma$ contains a ball $\ball{0,\epsilon}$. By \ref{pretopologicalContinuityVicinities}, it is enough to show that $p_K^{\preimf}[\ball(0,\epsilon)]$ is a neighbourhood. Indeed
