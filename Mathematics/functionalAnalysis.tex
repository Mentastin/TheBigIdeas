...
\begin{proposition}
Let $\sSet{V, \xi}$ be a convergence vector space, $\sSet{W, \norm{\cdot}}$ a normed space and $f: V\to W$ a positively homogeneous function. If $f$ is bounded on some $U\in \vicinity_\xi(0)$, then $f$ is continuous at $0$.
\end{proposition}
\begin{proof}
We have $f(0) = 0$ by \ref{homogeneousFunctionLemma}. Let $f$ be bounded by $C$ on $U$, i.e.\ $f^{\imf}(U)\subseteq \ball_W(0,C)$. 
...
\end{corollary}
\begin{proof}
Assume $f$ continuous, then $\ball\big(0,1\big)\in \neighbourhood_W(0) = \neighbourhood_W\big(f(0)\big) \subseteq f^{\imf\imf}[\vicinity_\xi(0)]$ by \ref{continuityVicinityFilter}. So there exists $U\in \vicinity_\xi(0)$ such that $f^{\imf}[U] \subseteq \ball(0,1)$, which means that $f$ is bounded by $1$ on $U$.

Now assume $f$ is bounded on some $U\in \vicinity_\xi(0)$. Then $f$ is continuous at $0$ by the proposition. Then $f$ is continuous everywhere by \ref{shiftHomeomorphism}.
\end{proof}

\begin{proposition} \label{superSubtractiveContinuityEverywhere}
...
\end{corollary}
\begin{proof}
We have that $|f| = p_K$ for some $K\in \vicinity_U(0)$ by \ref{absoluteFunctionalGauge}. By continuity of the inclusion map, we can find an $M \in \vicinity_V(0)$ such that $M\cap U = K$. Then $|f|\leq p_M$ and $f$ can be extended by the Hahn-Banach extension theorem to a functional $f'$ defined in the whole of $V$. Then $f$ is bounded on $M$ and thus continuous by \ref{continuityToNormedSpace}.
\end{proof}
\begin{corollary}
...
The weak topology is the weakest locally convex convergence $\tau$ such that $V^* = \sSet{V, \tau}^*$.
\end{corollary}
