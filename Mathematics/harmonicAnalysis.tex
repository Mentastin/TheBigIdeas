...
\begin{definition}
Let $G$ be a locally compact group and $f,g\in L^1(G)$. The \udef{convolution} $f*g$ is defined by
\[ (f*g)(x) = \int_Gf(y)g(y^{-1}x)\diff{\mu(y)}. \]
\end{definition}

TODO: Gerald B. Folland: A course in Abstract Harmonic analysis. In section 2.3 (of the second edition), "Interlude: Some Technicalities": dealing with non-$\sigma$-finite spaces.

\begin{lemma}
Let $G$ be a locally compact group and $f,g\in L^1(G)$. Then
\begin{align*}
(f*g)(x) &= \int_Gf(y)g(y^{-1}x)\diff{\mu(y)} \\
...
\end{lemma}
\begin{proof}
TODO (using translation invariance of Haar measure).
\end{proof}



\begin{lemma}
Let $G$ be a locally compact group. Then $f*g = g*f$ for all $f,g\in L^1(G)$ \textup{if and only if} $G$ is Abelian.
\end{lemma}
\begin{proof}
TODO
\end{proof}
...
\begin{lemma}
Let $G$ be a locally compact group and $f,g\in L^1(G)$.
The convolution $f*g$ is defined a.e., is a function in $L^1(G)$ and satisfies
\[ \norm{f*g}_1 \leq \norm{f}_1\norm{g}_1. \]
\end{lemma}
TODO: deal with $\sigma$-finiteness. (Deitmar-Echterhoff ?)
\begin{proof}
We calculate,
\begin{align*}
