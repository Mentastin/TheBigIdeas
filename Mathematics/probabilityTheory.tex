...
\begin{definition}
Let $\Lambda: \R_+\to \R$ be a continuous, increasing function. A stochastic process
\[ N: \R_+\times \sSet{\Omega, \mathcal{A}, P} \to \N \]
is called a \udef{Poisson process} if $N_t - N_s \sim \Poisson(\Lambda(t) - \Lambda(s))$ for all $s\leq t$, independently of $\setbuilder{N_u}{u\leq s}$.
...
Let $\theta: \R_+\to \R$ be a continuous, increasing function and $N_t$ a Poisson process with cumulative rate $\Lambda$. Then $N_{\theta(t)}$ is a Poisson process with cumulative rate $\Lambda\circ \theta$.
\end{lemma}
...
\begin{proposition}
Let $N$ be a Poisson process with cumulative rate $\Lambda$ such that $N_0 = 0$ and $\Lambda(0) = 0$.
\begin{enumerate}
\item For each $t\in \R_+$, we have $N_t \sim \Poisson(\Lambda(t))$.
\item For each $n\in \N$, consider the stopping time
\[ \tau_n: \sSet{\Omega, \mathcal{A}, P} \to \R_+: \omega \mapsto \inf\setbuilder{t}{N_t(\omega) \geq n }. \]
The distribution of $\tau_n$ is given by
...
\item If $\Lambda$ is bijective, then $\Lambda(\tau_n) \sim \Erlang(n, 1)$.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) This follows directly from $N_t = N_t - N_0 \sim \Poisson(\Lambda(t) - \Lambda(0)) = \Poisson(\Lambda(t))$.
...
(3) This follows by the substitution $\Lambda(t) = \lambda t$ and comparison with \ref{ErlangCDF}.

(4) We calculate using \ref{transformationRandomVariable}
\[ F_{\Lambda(\tau_n)} = F_{\tau_n}\circ \Lambda^{-1} = F_{\Erlang(n, 1)}\circ \Lambda\circ \Lambda^{-1} = F_{\Erlang(n, 1)}. \]
\end{proof}
