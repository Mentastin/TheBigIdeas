...
\subsubsection{Other convergences}

\begin{lemma} \label{initialConvergenceWRTlinearOperatorsToNormedSpacesSeminormed}
Let $X$ be a vector space and $\{L_i: X\to Y_i\}_{i\in I}$ a set of linear operators to normed spaces $Y_i$. Then the initial convergence on $X$ w.r.t.\ this set is equal to the seminormed convergence w.r.t.\ the set $\{\norm{\cdot}_{Y_i}\circ L_i: X\to \R^+\}_{i\in I}$ of seminorms.
\end{lemma}
\begin{proof}
Let $\mu$ be the initial convergence on $X$ w.r.t.\ $\{L_i: X\to Y_i\}_{i\in I}$ and $\nu$ the seminormed convergence w.r.t.\ $\{\norm{\cdot}_{Y_i}\circ L_i: X\to \R^+\}_{i\in I}$. Take arbitrary $F\in \powerfilters(X)$ and $x\in X$. Then we have
\begin{align*}
F \overset{\mu}{\longrightarrow} x &\iff \upset L_i^{\imf\imf}(F) \overset{Y}{\longrightarrow} L_i(x) \\
&\iff \norm*{\upset L_i^{\imf\imf}(F) - L_i(x)} \overset{\R^+}{\longrightarrow} 0 \\
&\iff \upset (\norm{\cdot}\circ L_i)^{\imf\imf}(F - x) \overset{\R^+}{\longrightarrow} 0 \\
&\iff F \overset{\nu}{\longrightarrow} x,
\end{align*}
using \ref{initialFinalConvergence} and \ref{initialSeminormConvergence}.
\end{proof}
...
\begin{lemma} \label{normFunctionalAsSup}
Let $X$ be a real normed space and $f\in \dual{X}$. Then $\norm{f} = \sup_{x\in X}f(x)$.
\end{lemma}
\begin{proof}
It is enough to show that $\downset\setbuilder{f(x)}{x\in X} = \downset\setbuilder{\big|f(x)\big|}{x\in X}$. The inclusion $\subseteq$ follows from the inequality $a\leq |a|$ for all $a\in \R$, because then $a\leq f(x)$ implies $a \leq |f(x)|$.

For the other inclusion, suppose $a\in \R$ such that $a\leq |f(x)|$, for some $x\in X$. If $f(x) \geq 0$, then $f(x) = |f(x)|$, so $a\leq f(x)$. If $f(x) \leq 0$, then $f(-x) = |f(x)|$, so $a\leq f(-x)$. 
\end{proof}
...
\begin{proposition} \label{BanachSteinhaus}
...
\subsection{Operators on normed Riesz spaces}

\begin{proposition}
Let $V, W$ be normed Riesz spaces and $T: V\to W$ a linear operator. If $V$ has the weak Riesz-Fischer property and $T$ is order-bounded, then $T$ is norm-bounded.
\end{proposition}
\begin{proof}
See \cite{zaanen_introOperatorTheoryRieszSpaces}, p123.
\end{proof}

\chapter{Differentiation}
Calculus on Normed Vector Spaces - Coleman
...
\section{The Banach algebra of bounded operators}
\begin{proposition}
...
\begin{lemma} \label{homomorphismFromUnitisationOfUnitalAlgebra}
Let $A$ be a unital Banach algebra. Then the function $u: A^\dagger \to A: (a,\lambda) \mapsto a+\lambda \vec{1}$ is a unital algebra homomorphism in $(A^\dagger \to A)$.
\end{lemma}
\begin{proof}
The function is clearly linear. To show that it is multiplicative, take $(a,\lambda), (b,\mu) \in A^\dagger$. Then
\[ u\big((a,\lambda)(b,\mu)\big) = u\big((ab + \lambda b + \mu a, \lambda \mu)\big) = ab + \lambda b + \mu a + \lambda \mu\vec{1} = (a+\lambda\vec{1})(b+\mu\vec{1}) = u(a,\lambda)u(b,\mu). \]
\end{proof}
...
\end{lemma}
By ``$\Psi^\dagger$ extends $\Psi$'', we mean that $\Psi^\dagger\big(\iota(x)\big) = \iota\big(\Psi(x)\big)$ for all $x\in A$.
\begin{proof}
...
If $\Psi$ is multiplicative, we need $\Psi^\dagger$ to be multiplicative. Take $(a,\lambda), (b,\mu)\in A^\dagger$. Then
\begin{align*}
\Psi^\dagger\big((a,\lambda)(b,\mu)\big) &= \Psi^\dagger\big((ab + \lambda b + \mu a, \lambda \mu)\big) \\
&= \big(\Psi(a)\Psi(b) + \lambda \Psi(b) + \mu \Psi(a), \lambda \mu\big) \\
&= \big(\Psi(a), \lambda\big)\big(\Psi(b)+\mu\big) \\
&= \Psi^\dagger(a,\lambda)\Psi^\dagger(b,\mu).
\end{align*}
\end{proof}
...
\begin{lemma} \label{halfUnitalExtensionLinearFunction}
Let $A,B$ be Banach algebras with $B$ unital. Every linear function $\Psi:A\to B$ extends uniquely to a linear function $\Psi^\dagger: A^\dagger \to B$ that maps $\vec{1}_{A^\dagger}$ to $\vec{1}_{B}$. This function is given by:
\[ \Psi^\dagger: A^\dagger \to B: (a,\lambda) \mapsto \Psi(a) + \lambda\vec{1}. \]
In particular, every algebra homomorphism extends uniquely to a unital algebra homomorphism.
\end{lemma}
\begin{proof}
Compose the maps of \ref{unitalExtensionLinearFunction} and \ref{homomorphismFromUnitisationOfUnitalAlgebra}.
\end{proof}
...
TODO: clean up proof
\begin{proof}
The function $\widetilde{(-)}: \hat{A} \to \widehat{A^\dagger}: \varphi \to \varphi^\dagger$ is well-defined by \ref{halfUnitalExtensionLinearFunction}.
...
\subsection{Calculating the spectrum in the bicommutant}
\begin{proposition}
