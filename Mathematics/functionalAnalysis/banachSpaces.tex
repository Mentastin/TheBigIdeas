...
\begin{example}
Power series are not satisfactory, since a holomorphic function may have different power series expansions at different points and an element of a Banach algebras, in some sense, comprises the whole of its spectrum.
...
\end{lemma}
TODO: compare with general theory.
\begin{proof}
...
\begin{align*}
\od{\rho\circ f}{z}\Big|_{z_0} &= \lim_{z\to z_0} \frac{(\rho \circ f)(z) - (\rho \circ f)(z_0)}{z-z_0} \\
&= \lim_{z\to z_0} \rho\Big(\frac{f(z) - f(z_0)}{z-z_0}\Big) \\
&= \rho\Big(\lim_{z\to z_0}\frac{f(z) - f(z_0)}{z-z_0}\Big) \\
&= \rho\Big(\od{f}{z}\Big|_{z_0}\Big).
\end{align*}.
\end{proof}
...
\item $\Phi_x(\id_\C) = x$ and, if $A$ is unital, $\Phi_x(\underline{1}) = \vec{1}$;
\item if $A$ is unital, then $\spec(\Phi_x(f)) = f^\imf\big(\spec(x)\big)$.
\end{enumerate}
\end{theorem}
TODO: check everything works in non-unital case.
...
(3) By \ref{continuousConvergenceCompactOpen}, \ref{spectrumCompact}, (TODO ref: compact open convergence = uniform on compact subsets) and \ref{metricUniformConvergence}, the continuous convergence is metric and thus sequential (TODO ref), so it is enough to check sequential convergence (TODO: is there an easier way to see that it is sequential??).
...
(4) Take $x\in A$ and let $\Gamma$ be a circle centred at $0$ with radius larger than $\norm{x}$. Then, using the Neumann series \ref{NeumannSeries} and (3), we have
\begin{align*}
\Phi_x(\id_{\spec(x)}) &= \frac{1}{2\pi i}\oint_\Gamma zR_x(z)\diff{z} \\
&= \frac{1}{2\pi i}\oint_\Gamma \frac{1}{\vec{1} - x/z}\diff{z} \\
&= \frac{1}{2\pi i}\oint_\Gamma \sum_{n=0}^\infty z^{-n}x^n\diff{z} \\
...
TODO: $\Phi_x(\constant{1}) = \vec{1}$

(5) TODO: \url{https://noncommutativeanalysis.wordpress.com/2014/01/05/advanced-analysis-notes-19-the-holomorphic-functional-calculus-ii-definition-and-basic-properties/}
\end{proof}
