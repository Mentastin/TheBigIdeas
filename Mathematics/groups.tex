...
\subsubsection{Centraliser}
TODO: semigroups. Commutant.
\begin{definition}
Let $G$ be a group and $A\subseteq G$ a subset. The \udef{centraliser} of $A$ is defined as
\[ Z_G(A) \defeq \setbuilder{g\in G}{\forall a\in A:\;\Ad_g(a) = a}. \]
In particular we define the \udef{centre} of $G$ as the centraliser of all of $G$:
\[ Z_G \defeq Z_G(G). \]
\end{definition}
The centraliser of $A$ is the set of elements that commute with all $a\in A$:
\[ Z_G(A) = \setbuilder{g\in G}{\forall a\in A:\;ag = ga}. \]

\begin{proposition}
Let $G$ be a group and $A\subseteq G$ a subset. Then $Z_G(A)$ is a subgroup of $G$.
\end{proposition}
\begin{proof}
TODO
\end{proof}
