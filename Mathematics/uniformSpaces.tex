...
\begin{enumerate}
\item $\vicinity_{\Gamma(\mathcal{U})}(x) = \entourage_\mathcal{U}x$;
...
\begin{align*}
\vicinity_{\Gamma(\mathcal{U})}(x) &= \bigcap \lim^{-1}_{\Gamma(\mathcal{U})}(x) \\
&= \bigcap_{H\in\mathcal{U}} Hx \\
&= \Big(\bigcap_{H\in\mathcal{U}} H\Big)x \\
&= \entourage_{\mathcal{U}}x.
\end{align*}

(2) We first show that $\Gamma(\mathcal{U})$ is pretopological. By (1), it is enough to prove that $\entourage_\mathcal{U}x \to x$.
...
\begin{enumerate}
\item $\adh_{\Gamma(\mathcal{U})}(A) = \bigcap\setbuilder{U^\imf(A)}{U\in \entourage_\mathcal{U}}$;
\item $\adh_{\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})}(M) = \bigcap\setbuilder{V;M;V}{V\in\entourage_\mathcal{U}}$.
\end{enumerate}
\end{proposition}
In particular, $\adh_{\Gamma(\mathcal{U})}(A) \subseteq V^\imf(A)$ for all $V\in \entourage_\mathcal{U}$.
\begin{proof}
(1) By \ref{principalAdherenceInherence}, \ref{vicinityUniformConvergence} and \ref{meshConnection}, we have
\begin{align*}
x\in \adh_{\Gamma(\mathcal{U})}(A) &\iff A\in\vicinity_{\Gamma(\mathcal{U})}(x)^\mesh \\
&\iff \forall U\in \entourage_\mathcal{U}: Ux \mesh A \\
&\iff \forall U\in \entourage_\mathcal{U}: U^\preimf\big(\{x\}\big) \mesh A \\
&\iff \forall U\in \entourage_\mathcal{U}: \{x\} \mesh U^\imf(A) \\
&\iff \forall U\in \entourage_\mathcal{U}: x \in U^\imf(A) \\
&\iff x\in\bigcap_{U\in \entourage_\mathcal{U}} U^\imf(A).
\end{align*}

(2) By \ref{principalAdherenceInherence}, \ref{vicinityUniformConvergence} and \ref{productVicinity}, we have
\begin{align*}
(x,y) \in \adh_{\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})}(M) &\iff \forall U,V\in \entourage_\mathcal{U}: Ux \times Vy \mesh M \\
&\iff \forall U,V\in \entourage_\mathcal{U}:\exists (a,b)\in M: aUx \land bVy \\
&\iff \forall U,V\in \entourage_\mathcal{U}:(x,y) \in U^\transp;M;V \\
&\iff (x,y) \in \bigcap\setbuilder{U^\transp;M;V}{U,V\in\entourage_\mathcal{U}} \\
&\iff (x,y) \in \bigcap\setbuilder{V;M;V}{V\in\entourage_\mathcal{U}}.
\end{align*}
For the last step, we have used the symmetry of $\entourage_\mathcal{U}$, \ref{entourageLemma}, and the fact that it is a filter.
\end{proof}
...
\[ \entourage_A = \setbuilder{V\cap (A\times A)\subseteq A\times A}{V\in \entourage_X}. \]
\end{lemma}
\begin{proof}
...
Take an arbitrary symmetric $A\in \entourage_\mathcal{U}$. We can find a finite set $F\subseteq X$ and $n\in \N$ such that $\bigcup_{x\in F}A^nx \supseteq B$. Now $\entourage_\mathcal{U}x = \neighbourhood_{\Gamma(\mathcal{U})}$ by \ref{adherenceUniformSpace}, so we have (using \ref{principalAdherenceInherence})
\begin{align*}
x\in \adh_{\Gamma(\mathcal{U})}(B) \iff& B\in \neighbourhood(x)^\mesh \\
\iff& B \in (\entourage x)^\mesh \\
...
For this we use \ref{denseIffMeshesWithAllVicinities}. By \ref{vicinityUniformConvergence}, every vicinity in $\hat{X}$ contains a set of the form $V^*(F)$, for some $V\in \entourage_\mathcal{U}$ and $F\in\hat{X}$. Since $F$ is a Cauchy filter, there exists a (non-empty) $B\in F$ such that $B\times B\subseteq V$. Pick any $x\in B$. Since $B\in \pfilter{x}$, we have $(F, \pfilter{x})\in V^*$ and thus $f(x) = \pfilter{x} \in V^*(F)$.
...
\[ V_d(\epsilon) \defeq \setbuilder{(y,z)\in X^2}{d(y,z)< \epsilon} = d^{\preimf}\big(\interval{0,\epsilon}\big). \]
...
TODO: spread the notation $V_d(\epsilon)$.
...
\begin{enumerate}
\item $\cball_d(x,\epsilon) = V_d(\epsilon) x = xV_d(\epsilon)$;
...
\end{lemma}
\begin{proof}
By \ref{entourageSubTopologicalUniformity} both are equal to $\upset \{V_\epsilon \cap (A\times A)\}_{\epsilon > 0}$.
\end{proof}
...
\item $\neighbourhood_d(x) = \upset\setbuilder{\cball_d(x,n^{-1})}{n\in \N} = \upset\setbuilder{\ball_d(x,n^{-1})}{n\in \N}$;
...
\begin{proof}
(1) Since $\neighbourhood_d(x) = \upset\setbuilder{\cball_d(x,n^{-1})}{n\in \N}$, by \ref{metricConvergenceNeighbourhood}, every neighbourhood filter has a countable base.
...
\begin{align*}
x\in \adh(A) &\iff A\in \vicinity(x)^\mesh \\
...
To see that $B\in \neighbourhood(x)$, it is enough to show that $\cball(x, \epsilon / 4) \subseteq B$. First note that $\ceil{(3\epsilon / 4)^{-1}} - (3\epsilon / 4)^{-1} \leq 1$, so
\begin{align*}
...
Indeed, take $x\in X$ and $\ball(x,\epsilon) \in \neighbourhood(x)$. Then there exists $B = \ball_d(y, \ceil{(\epsilon/2)^{-1}}^{-1})\in \mathcal{C}'_{\ceil{(\epsilon/2)^{-1}}}$ such that $x\in B$. Since $x\in B$ and $B$ is open, we have $B\in \neighbourhood(x)$. We also have $B\subseteq \ball(x,\epsilon)$. Indeed, take $z\in B$. Then
