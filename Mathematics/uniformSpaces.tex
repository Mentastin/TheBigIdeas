...
\begin{lemma} \label{principalImageFilterLemma}
Let $X$ be a set, $x\in X$, $F,G\in\powerfilters(X)$ and $H\in \powerfilters(X^2)$. Then
\begin{enumerate}
\item $(F\otimes G)x = \begin{cases}
F & \big(x\in\ker(G)\big) \\
\powerset(X) & (\text{otherwise}).
\end{cases}$;
\item $Hx \otimes \pfilter{x} = H; \big(\pfilter{x}\otimes\pfilter{x}\big)$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
(2) The set $\setbuilder{A;\big\{(x,x)\big\}}{A\in H} = \setbuilder{Ax\times\{x\}}{A\in H}$ is a base of both filters.
\end{proof}
...
\begin{definition}
Let $\sSet{X, \mathcal{U}}$ be a uniform space. Then $\entourages_\mathcal{U} \defeq \bigcap \mathcal{U}$ is called the \udef{entourage filter} of $\mathcal{U}$ and the elements of $\entourages_\mathcal{U}$ are called \udef{entourages}.

We call the uniform space \udef{topological} if $\mathcal{U} = \upset\{\entourages_\mathcal{U}\}$.
\end{definition}
The use of the term ``topological'', rather that ``pretopological'', is justified in \ref{vicinityUniformConvergence}.

\begin{lemma} \label{entourageLemma}
Let $\sSet{X,\mathcal{U}}$ be a uniform space with entourage filter $\entourages$. Then
\begin{enumerate}
\item $\entourages\subseteq \upset\{\id_X\}$;
\item $\entourages^\transp = \entourages$;
\item $\entourages;\entourages \subseteq \entourages$.
\end{enumerate}
If $\sSet{X,\mathcal{U}}$ is a topological uniform space, then
\begin{enumerate}[1'.] \setcounter{enumi}{2}
\item $\entourages;\entourages = \entourages$.
\end{enumerate}
Any filter in $\powerfilters(X\times X)$ satisfying properties 1., 2. and $\entourages \subseteq \entourages;\entourages$ is the entourage filter of a topological uniformity.
\end{lemma}
TODO: can we improve 3??
\begin{proof}
(1) We have
\[ \entourages = \bigcap \mathcal{U} \subseteq \bigcap \setbuilder{\pfilter{x}\otimes \pfilter{x}}{x\in X} = \upset\big\{\setbuilder{(x,x)}{x\in X}\big\} = \upset\{\id_X\}. \]

(2) We have
\[ \entourages^\transp = \bigcap \setbuilder{H^\transp}{H\in\mathcal{U}} = \bigcap \setbuilder{H}{H\in\mathcal{U}} = \entourages, \]
because the transpose is bijective and thus its image function is preserved under intersection.

(3) First take $A;B \in\entourages;\entourages$. We claim $A\subseteq A;B$. Indeed take $(a,b)\in A$. By (1), we have $(b,b)\in B$ and so $(a,b)\in A;B$. Thus $\entourages;\entourages \subseteq \entourages$.

(3') In the topological case, we have that $\entourages\in\mathcal{U}$ and thus $\entourages;\entourages\in \mathcal{U}$, so $\entourages \subseteq \entourages;\entourages$.

To show any such filter is an entourage filter, we check the four requirements
\begin{itemize}
\item From (1), we have for all $x\in X$
\[ \entourages \subseteq \upset\{\Delta\} \subseteq \upset\{(x,x)\} = \pfilter{x}\otimes\pfilter{x}. \]
\item Upwards closure is by construction.
\item If $\entourages \subseteq H$, then $\entourages = \entourages^\transp \subseteq H^\transp$.
\item If $\entourages\subseteq G,H$, then $\entourages \subseteq \entourages;\entourages \subseteq G;H$.
\end{itemize}
\end{proof}
\begin{corollary}
...
\end{proposition}
\begin{proof}
Let $\sSet{X,\mathcal{U}}$ be a topological and factorisable uniform space with entourage filter $\entourages_\mathcal{U}$. Then $\entourages_\mathcal{U} \subseteq p_1^{\imf\imf}[\entourages_\mathcal{U}]\otimes p_2^{\imf\imf}[\entourages_\mathcal{U}]$. Now each $A\in \entourages_\mathcal{U}$ contains $\Delta$, so $p_1^\imf[A] = X = p_2^\imf[A]$. Thus $p_1^{\imf\imf}[\entourages_\mathcal{U}]\otimes p_2^{\imf\imf}[\entourages_\mathcal{U}] = \{X^2\}$.

Now $\entourages_\mathcal{U} = \upset\big\{\{X^2\}\big\}$, which is trivial.
\end{proof}
...
\begin{lemma} \label{uniformConvergencePavement}
Let $\sSet{X,\mathcal{U}}$ be a uniform space and $x\in X$. Then $\lim^{-1}_{\Gamma(\mathcal{U})}(x) = \setbuilder{Hx}{H\in \mathcal{U}}$.
\end{lemma}
In particular, $\setbuilder{Hx}{H\in \mathcal{U}}$ is a pavement of $\Gamma(\mathcal{U})$ at $x$.
\begin{proof}
For the first inclusion, we show that $Hx$ converges to $x$ in $\Gamma(\mathcal{U})$ for all $H\in\mathcal{U}$. Take arbitrary $H\in\mathcal{U}$. Then $Hx \otimes \pfilter{x} = H;\pfilter{x}\otimes\pfilter{x}$ by \ref{principalImageFilterLemma}. By definition, $H;\pfilter{x}\otimes\pfilter{x}\in \mathcal{U}$, so $Hx$ converges to $x$ by definition of $\Gamma(\mathcal{U})$.

Now, for the other inclusion, let $F\in\powerfilters(X)$ be a filter that $\Gamma(\mathcal{U})$-converges to $x$, so $F\otimes\pfilter{x} \in \mathcal{U}$. Then $F = (F\otimes \pfilter{x})x$ by \ref{principalImageFilterLemma}, so $F\in \setbuilder{Hx}{H\in \mathcal{U}}$.
\end{proof}

\begin{proposition} \label{vicinityUniformConvergence}
Let $\sSet{X,\mathcal{U}}$ be a uniform space and $x\in X$. Then
\begin{enumerate}
\item $\vicinities_{\Gamma(\mathcal{U})}(x) = \entourages_\mathcal{U}x$;
\item if $\mathcal{U}$ is topological, then $\Gamma(\mathcal{U})$ is topological.
\end{enumerate}
\end{proposition}
This justifies our use of the term ``topological uniform space'' (rather than ``pretopological uniform space'').
\begin{proof}
(1) We calculate, using \ref{uniformConvergencePavement}, \ref{upsetResiduatedImageGaloisConnection} and \ref{productGaloisConnections},
\begin{align*}
\vicinities_{\Gamma(\mathcal{U})}(x) &= \bigcap \lim^{-1}_{\Gamma(\mathcal{U})}(x) \\
&= \bigcap_{H\in\mathcal{U}} Hx \\
&= \Big(\bigcap_{H\in\mathcal{U}} H\Big)x \\
&= \entourages_{\mathcal{U}}x.
\end{align*}

(2) We first show that $\Gamma(\mathcal{U})$ is pretopological. By (1), it is enough to prove that $\entourages_\mathcal{U}x \to x$.
Because $\entourages_\mathcal{U}$ is a topological entourage filter, we have
\begin{align*}
\entourages_\mathcal{U} &= \entourages_\mathcal{U};\entourages_\mathcal{U} \\
&\subseteq \entourages_\mathcal{U}; \pfilter{x}\otimes \pfilter{x} \\
&= \upset \setbuilder{V; \{(x,x)\}}{V\in \entourages_\mathcal{U}} \\
&= \upset \setbuilder{V x\times\{x\}}{V\in \entourages_\mathcal{U}} \\
&= \upset \setbuilder{V x}{V\in \entourages_\mathcal{U}}\otimes \pfilter{x}.
\end{align*}
Thus $\entourages_\mathcal{U}x \overset{\Gamma(\mathcal{U})}{\longrightarrow} x$.

Finally to show that $\Gamma(\mathcal{U})$ is topological, we use \ref{pretopologicalSpaceTopological}: Take $Vx\in \upset \setbuilder{V x}{V\in \entourages_\mathcal{U}}$. Then because $\entourages_\mathcal{U} = \entourages_\mathcal{U};\entourages_\mathcal{U}$, we can find $U,U'\in \entourages_\mathcal{U}$ such that $V = U;U'$. Consider $U'x$. For all $y\in U'x$, we have that $zUy \implies zU;U'x \iff zVx$, so $Uy \subseteq Vx$. Thus $Vx \in \upset \setbuilder{V y}{V\in \entourages_\mathcal{U}}$.
\end{proof}


\begin{proposition}
Let $\sSet{X,\mathcal{U}}$ be a topological uniform space and $V\in \entourages_\mathcal{U}$.

Then $\interior_{\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})}(V) \in \entourages_\mathcal{U}$.
\end{proposition}
\begin{proof}
By \ref{entourageLemma}, we have $\entourages_\mathcal{U} = \entourages_\mathcal{U};\entourages_\mathcal{U};\entourages_\mathcal{U}$, so can find $U_1,U_2,U_3$ such that $U_1;U_2;U_3 \subseteq V$. Let $U = U_1\cap U_1^\transp \cap U_2 \cap U_2^\transp \cap U_3\cap U_3^\transp$, so $U$ is symmetric and $U;U;U\subseteq V$.
Now
\[ U;U;U = \bigcup_{(x,y)\in U}U;\{(x, y)\};U = \bigcup_{(x,y)\in U}Ux \times Uy, \]
by \ref{relationCompositionResiduated}. For all $x,y\in U$, $Ux$ is a neighbourhood of $x$ in $\Gamma(\mathcal{U})$ and $Uy$ is a neighbourhood of $y$ in $\Gamma(\mathcal{U})$, by \ref{vicinityUniformConvergence}.  Thus $Ux \times Uy$ is a neighbourhood of $(x,y)$ in $\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})$, as this is a vicinity by \ref{productVicinity} and the product convergence is topological by \ref{pretopologicalInitialConvergence}.
...
Let $\sSet{X,\mathcal{U}}$ be a topological uniform space, then $\entourages_\mathcal{U}$ has a base of sets open in $\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})$.
\end{corollary}


\begin{proposition} \label{adherenceUniformSpace}
Let $\sSet{X,\mathcal{U}}$ be a uniform space, $A\subseteq X$ and $M\subseteq X^2$. Then
\begin{enumerate}
\item $\adh_{\Gamma(\mathcal{U})}(A) = \bigcap\setbuilder{U^\imf(A)}{U\in \entourages_\mathcal{U}}$;
\item $\adh_{\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})}(M) = \bigcap\setbuilder{V;M;V}{V\in\entourages_\mathcal{U}}$.
\end{enumerate}
\end{proposition}
In particular, $\adh_{\Gamma(\mathcal{U})}(A) \subseteq V^\imf(A)$ for all $V\in \entourages_\mathcal{U}$.
\begin{proof}
(1) By \ref{principalAdherenceInherence}, \ref{vicinityUniformConvergence} and \ref{meshConnection}, we have
\begin{align*}
x\in \adh_{\Gamma(\mathcal{U})}(A) &\iff A\in\vicinities_{\Gamma(\mathcal{U})}(x)^\mesh \\
&\iff \forall U\in \entourages_\mathcal{U}: Ux \mesh A \\
&\iff \forall U\in \entourages_\mathcal{U}: U^\preimf\big(\{x\}\big) \mesh A \\
&\iff \forall U\in \entourages_\mathcal{U}: \{x\} \mesh U^\imf(A) \\
&\iff \forall U\in \entourages_\mathcal{U}: x \in U^\imf(A) \\
&\iff x\in\bigcap_{U\in \entourages_\mathcal{U}} U^\imf(A).
\end{align*}

(2) By \ref{principalAdherenceInherence}, \ref{vicinityUniformConvergence} and \ref{productVicinity}, we have
\begin{align*}
(x,y) \in \adh_{\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})}(M) &\iff \forall U,V\in \entourages_\mathcal{U}: Ux \times Vy \mesh M \\
&\iff \forall U,V\in \entourages_\mathcal{U}:\exists (a,b)\in M: aUx \land bVy \\
&\iff \forall U,V\in \entourages_\mathcal{U}:(x,y) \in U^\transp;M;V \\
&\iff (x,y) \in \bigcap\setbuilder{U^\transp;M;V}{U,V\in\entourages_\mathcal{U}} \\
&\iff (x,y) \in \bigcap\setbuilder{V;M;V}{V\in\entourages_\mathcal{U}}.
\end{align*}
For the last step, we have used the symmetry of $\entourages_\mathcal{U}$, \ref{entourageLemma}, and the fact that it is a filter.
\end{proof}
\begin{corollary} \label{topologicalEntourageFilterBaseClosedSets}
Let $\sSet{X,\mathcal{U}}$ be a topological uniform space, then $\entourages_\mathcal{U}$ has a base of sets closed in $\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})$.
\end{corollary}
\begin{proof}
Let $U\in \entourages_\mathcal{U}$. Then by \ref{entourageLemma}, we can find a $V\in\entourages_\mathcal{U}$ such that $V;V;V\subseteq U$. By the proposition, $V;V;V$ contains the closure of $V$. Thus each entourage contains a closed set.
\end{proof}
...
\begin{proof}
By the proposition it is enough to prove that $\entourages_\mathcal{U} \subseteq \adh_{\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})}^\imf(\entourages_\mathcal{U})$.

Take arbitrary $A\in \entourages_\mathcal{U}$. By \ref{topologicalEntourageFilterBaseClosedSets} there exists $B\in \entourages_\mathcal{U}$ such that $B\subseteq A$ and $B$ is $\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})$-closed.
Now
\[ \adh_{\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})}(B) = B \subseteq A, \]
so $A\in \adh_{\Gamma(\mathcal{U})\otimes \Gamma(\mathcal{U})}^\imf(\entourages_\mathcal{U})$.
\end{proof}

\begin{proposition}
Let $\sSet{X,\mathcal{U}}$ be a topological uniform space. Then $X$ is Hausdorff \textup{if and only if} $\ker\entourages_{\mathcal{U}} = \graph(\id_X)$.
\end{proposition}
\begin{proof}
TODO!
\end{proof}
...
\begin{enumerate}
\item If $f$ is uniformly continuous, then $\entourages_\mathcal{V} \subseteq \upset (f|f)^{\imf\imf}[\entourages_\mathcal{U}]$;
\item If $\mathcal{V}$ is topological, then opposite implication also holds.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) By uniform continuity we have
\[ \entourages_\mathcal{V} \subseteq \bigcap\setbuilder{\upset (f|f)^{\imf\imf}[H]}{H\in \mathcal{U}} = \upset (f|f)^{\imf\imf}\left[\bigcap \mathcal{U}\right] = \upset (f|f)^{\imf\imf}\left[\entourages_\mathcal{U}\right]. \]
The first equality follows from \ref{imageUpsetsPreservesIntersection}.

(2) Assume $\entourages_\mathcal{V} \subseteq \upset (f|f)^{\imf\imf}[\entourages_\mathcal{U}]$ and take $H\in \mathcal{U}$. Then $\entourages_\mathcal{U}\subseteq H$, so
\[ \entourages_\mathcal{V} \subseteq \upset (f|f)^{\imf\imf}[\entourages_\mathcal{U}] \subseteq \upset (f|f)^{\imf\imf}[H]. \]
...
\[ \entourages_\mathcal{X} = \mathfrak{F}\bigcup_{i\in I} (f_i|f_i)^{\preimf\imf}\big(\entourages_{\mathcal{W}_i}\big) = \mathfrak{F}\setbuilder{(f_i|f_i)^\preimf(U)}{i\in I, U\in \entourages_{\mathcal{W}_i}}. \]
\end{proposition}
\begin{proof}
...
&\iff \forall i\in I: \entourages_{\mathcal{W}_i} \subseteq \upset(f_i|f_i)^{\imf\imf}(H) \\
&\iff \forall i\in I: \upset (f_i|f_i)^{\preimf\imf}\big(\entourages_{\mathcal{W}_i}\big) \subseteq H \\
&\iff \bigcup_{i\in I} \upset (f_i|f_i)^{\preimf\imf}\big(\entourages_{\mathcal{W}_i}\big) \subseteq H.
\end{align*}
Thus the initial uniformity is topological and
\begin{align*}
\entourages_\mathcal{X} &= \mathfrak{F}\bigcup_{i\in I} \upset (f_i|f_i)^{\preimf\imf}\big(\entourages_{\mathcal{W}_i}\big) \\
&= \mathfrak{F}\bigcup_{i\in I}\setbuilder{(f_i|f_i)^\preimf(U)}{U\in \entourages_{\mathcal{W}_i}} \\
&= \mathfrak{F}\setbuilder{(f_i|f_i)^\preimf(U)}{i\in I, U\in \entourages_{\mathcal{W}_i}}.
\end{align*}
\end{proof}
...
\begin{lemma} \label{entourageSubTopologicalUniformity}
Let $\sSet{X, \mathcal{U}}$ be a uniform space and $A\subseteq X$ a subset. If $X$ is topological, then the subspace uniformity on $A$ is topological and
\[ \entourages_A = \setbuilder{V\cap (A\times A)\subseteq A\times A}{V\in \entourages_X}. \]
\end{lemma}
\begin{proof}
This follows straight from \ref{topologicalInitialUniformity}.
\end{proof}
...
\begin{itemize}
\item $f$ is injective;
\item $\upset(f|f)^{\imf\imf}(\entourages_\mathcal{U}) = \entourages_\mathcal{V}|_{\im(f)^2}$.
\end{itemize}
\end{proposition}
\begin{proof}
First assume $f$ is an embedding. Since $f|^{\im(f)}$ is bijective, we automatically have that $f$ is injective.

By \ref{uniformContinuityEntourages}, we have that $\entourages_{\mathcal{V}}\subseteq \upset(f|f)^{\imf\imf}(\entourages_\mathcal{U})$, so $\entourages_{\mathcal{V}}|_{\im(f)^2} \subseteq \upset(f|f)^{\imf\imf}(\entourages_\mathcal{U})$.

Since $(f|^{\im(f)})^{-1}$ is also uniformly continuous, we have
\[ \entourages_\mathcal{U} \subseteq \upset \big((f\big|^{\im(f)})^{-1}|(f|^{\im(f)})^{-1}\big)^{\imf\imf}\big(\entourages_{\mathcal{V}}|_{\im(f)^2}\big) \]
and thus, by \ref{upsetPreimageImageGaloisConnection},
\[ \upset(f|f)^{\imf\imf}(\entourages_\mathcal{U}) = \big(f|^{\im(f)}\big|f|^{\im(f)}\big)^{\imf\imf}(\entourages_{\mathcal{U}}) = \big((f|^{\im(f)})^{-1}|(f|^{\im(f)})^{-1}\big)^{\preimf\imf}(\entourages_{\mathcal{U}}) \subseteq \entourages_{\mathcal{V}}|_{\im(f)^2}. \]
We conclude that $\upset(f|f)^{\imf\imf}(\entourages_\mathcal{U}) = \entourages_\mathcal{V}|_{\im(f)^2}$.
...
\subsection{Extending uniformly continuous functions}

\begin{proposition} \label{extendingFunctionFromDenseSubset}
Let $\sSet{X,\xi}$ be a convergence space, $\sSet{Y,\mathcal{W}}$ a regular uniform space, $A\subseteq X$ and $f: A\to Y$ is a uniformly continuous function, when $A$ is equipped with the uniform structure $(\Theta\circ\Phi)(\xi)|_A$. Suppose $F: X\to \powerfilters(A)$ a function such that
\begin{itemize}
\item $(\upset\incl_A^{\imf\imf}\circ F)(x) \overset{\xi}{\longrightarrow} x$ for all $x\in X$;
\item $\lim G \subseteq \lim (\upset \incl_A^{\imf\imf}\circ F)(G)$ for all $G\in \powerfilters(X)$;
\item $\big(\upset f^{\imf\imf} \circ F\big)(x)$ converges for all $x\in X$.
\end{itemize}
Then $f$ has a continuous extension $\tilde{f}: X\to Y$. If $Y$ is Hausdorff, then the extension is unique.
\end{proposition}
Note that under these assumptions, $A$ is automatically dense in $X$. 

There exist Cauchy filters on $A$ that are not convergent (because they converge to a limit point in $X\setminus A$).

If $Y$ is not Hausdorff, then this result requires choice.

TODO: is $\tilde{f}$ uniformly continuous??? Would we need uniform regularity of $Y$?
\begin{proof}
Define $\tilde{f}$ by setting $\tilde{f}(x) = f(x)$ for all $x\in A$ and
\[ \tilde{f}(x) \in \lim \big(\upset f^{\imf\imf} \circ F\big)(x) \]
for all $x\notin A$. Clearly $\tilde{f}$ extends $f$.

Now we show that $\tilde{f}$ is continuous. Take arbitrary $G\to x \in X$. Then $(\upset\incl_A^{\imf\imf}\circ F)(x) \mathrel{\Phi(\xi)} (\upset\incl_A^{\imf\imf}\circ F)(G)$, which implies
\begin{align*}
(\upset\incl_A^{\imf\imf}\circ F)(x) \otimes (\upset\incl_A^{\imf\imf}\circ F)(G) \in \big(\Theta\circ \Phi\big)(\xi) &\implies \upset\incl_A^{\imf\imf}\big(F(x)\big) \otimes \upset\incl_A^{\imf\imf}\big(F(G)\big) \in \big(\Theta\circ \Phi\big)(\xi) \\
&\implies \upset(\incl_A|\incl_A)^{\imf\imf}\big(F(x) \otimes F(G)\big) \in \big(\Theta\circ \Phi\big)(\xi) \\
&\implies F(x) \otimes F(G) \in \big(\Theta\circ \Phi\big)(\xi)|_A,
\end{align*}
where we have used \ref{imageContour}, \ref{parallelFunctionProductFilter} and \ref{initialFinalUniformity}.

The uniform continuity of $f$ implies (using \ref{preservationUniformStructure} and \ref{imageContour})
\[ (\upset f^{\imf\imf}\circ F)(G) = \upset f^{\imf\imf}\big(F(G)\big) \mathrel{\Xi(\mathcal{W})} (\upset f^{\imf\imf}\circ F)(x) \mathrel{\Xi(\mathcal{W})} \pfilter{\tilde{f}}(x). \]

We check that $\upset f^{\imf\imf} \circ F$ is an $\tilde{f}$-selection. For all $x\notin A$, this is immediate from the definition of $\tilde{f}$. Now take $x\in A$. Then \ref{subspaceConvergence} implies $F(x) \overset{\xi|_A}{\longrightarrow} x$ and thus $\big(\upset f^{\imf\imf} \circ F\big)(x) \overset{\mathcal{W}}{\longrightarrow} f(x) = \tilde{f}(x)$ follows from the continuity of $f$.

Finally, since $Y$ is regular, we have
\[ \tilde{f}(x) \in \lim (\upset f^{\imf\imf} \circ F)(G) \subseteq \lim \upset \tilde{f}^{\imf\imf}(G), \]
from \ref{regularityContourConvergence}.
\end{proof}
\begin{corollary}
Let $\sSet{X,\xi}$ be a diagonal convergence space, $\sSet{Y,\mathcal{W}}$ a regular, complete uniform space, $A\subseteq X$ a dense subset and $f: A\to Y$ is a uniformly continuous function, when $A$ is equipped with the uniform structure $(\Theta\circ\Phi)(\xi)|_A$.
Then $f$ has a continuous extension $\tilde{f}: X\to Y$. If $Y$ is Hausdorff, then the extension is unique.
\end{corollary}
A variant of this result was reported in \cite{cook_continuous_1968}.
...
\item We say $B\subseteq X$ is \udef{bounded} if for all uniform covers $\mathcal{D}$, there exists a \emph{finite} $\mathcal{C} \subseteq \setbuilder{\big(A_0;\ldots;A_{n-1}\big)x}{n\in \N, \seq{A_k}\in \mathcal{D}^n, x\in X}$ such that $B\subseteq \bigcup \mathcal{C}$.
\end{itemize}
\end{definition}
...
\begin{enumerate}
\item if $B$ is totally bounded, then for all $A\in \entourages_\mathcal{U}$ there exists a finite $S\subseteq X$ such that $B\subseteq \bigcup_{x\in S}Ax$;
\item if $B$ is bounded, then for all $A\in \entourages_\mathcal{U}$ there exists $n\in \N$ and a finite $S\subseteq X$ such that $B\subseteq \bigcup_{x\in S}A^nx$.
\end{enumerate}
The converses hold in a topological uniform space.
\end{lemma}
\begin{proof}
We may simply observe that $\{V\}$ is a uniform cover for all $V\in \entourages_\mathcal{U}$. This proves (1) and (2).

Now assume $\mathcal{U}$ topological. Then $\entourages_\mathcal{U} \in \mathcal{U}$, so each uniform cover contains an entourage. 
\end{proof}
...
(Bounded) We can repeat the arguments using $\setbuilder{\big(A_0;\ldots;A_{n-1}\big)x}{n\in \N, \seq{A_k}\in \mathcal{D}^n, x\in X}$ instead of $\setbuilder{Ax}{A\in \mathcal{D}, x\in X}$.
\end{proof}
\begin{corollary}
...
Let $\sSet{X, \mathcal{U}}$ be a symmetric uniform space and $B\subseteq X$ a bounded set. Then $\closure_{\Gamma(\mathcal{U})}(B)$ is also bounded.
\end{proposition}
TODO: can does this hold for adherences in non-topological uniform spaces??
\begin{proof}
We show boundedness using \ref{topologicalBoundednessLemma}.
Take an arbitrary symmetric $A\in \entourages_\mathcal{U}$. We can find a finite set $F\subseteq X$ and $n\in \N$ such that $\bigcup_{x\in F}A^nx \supseteq B$. Now $\entourages_\mathcal{U}x = \neighbourhoods_{\Gamma(\mathcal{U})}$ by \ref{adherenceUniformSpace}, so we have (using \ref{principalAdherenceInherence})
\begin{align*}
x\in \adh_{\Gamma(\mathcal{U})}(B) \iff& B\in \neighbourhoods(x)^\mesh \\
\iff& B \in (\entourages x)^\mesh \\
\implies& \bigcup_{y\in F}A^ny \in (\entourages x)^\mesh \\
\implies& \exists y\in F: A^ny \in (\entourages x)^\mesh \\
\implies& \exists y\in F: A^ny \mesh Ax \\
\implies& \exists y\in F: x\in A^{n+1}y.
\end{align*}
Thus $\bigcup_{x\in F}A^{n+1}x \supseteq \closure_{\Gamma(\mathcal{U})}(B)$.
\end{proof}

\begin{example}
TODO example where adherence is not totally bounded. There should exist metric counterexamples.
\end{example}
...
Now take an arbitrary uniform cover $\mathcal{D}'$ of $\mathcal{V}$. As $(f|f)^{\preimf\imf}(\mathcal{D})$ is a uniform cover of $\mathcal{U}$, by point (1), we can find some finite $\mathcal{C} \subseteq \setbuilder{(f|f)^\preimf(A_0);\ldots;(f|f)^\preimf(A_{n-1}) x}{n\in\N, \seq{A_k}\in \mathcal{D}^{\prime n}, x\in X}$ such that $B\subseteq \bigcup \mathcal{C}$.

Now by \ref{imageFilterComposition} and \ref{principalImageUnderRelationMapping}, we have
\[ (f|f)^\preimf(A_0);\ldots;(f|f)^\preimf(A_{n-1}) x \subseteq (f|f)^\preimf(A_0;\ldots; A_{n-1})x \subseteq f^\preimf\big((A_0;\ldots; A_{n-1})f(x)\big). \]
Set $\mathcal{C}' \defeq \setbuilder{(A_0;\ldots; A_{n-1})f(x)}{(f|f)^\preimf(A_0);\ldots;(f|f)^\preimf(A_{n-1}) x \in \mathcal{C}}$. Thus
\begin{align*}
B &\subseteq \bigcup_{(f|f)^\preimf(A_0);\ldots;(f|f)^\preimf(A_{n-1}) x \in \mathcal{C}}(f|f)^\preimf(A_0);\ldots;(f|f)^\preimf(A_{n-1}) x \\
&\subseteq \bigcup_{(A_0;\ldots; A_{n-1})f(x) \in \mathcal{C}'}f^\preimf\big((A_0;\ldots; A_{n-1})f(x)\big) \\
&= f^\preimf\Big(\bigcup_{(A_0;\ldots; A_{n-1})f(x) \in \mathcal{C}'}(A_0;\ldots; A_{n-1})f(x)\Big),
\end{align*}
and so also
\[ f^\imf(B) \subseteq \bigcup_{(A_0;\ldots; A_{n-1})f(x) \in \mathcal{C}'}(A_0;\ldots; A_{n-1})f(x) = \bigcup \mathcal{C}'. \]
\end{proof}
...
\[ A \in \upset \iota^{\imf\imf}(F) \subseteq \big(\upset \iota^{\imf\imf}(F)\big)^{\mesh} \subseteq \vicinities_{\Gamma(\mathcal{U})}(x)^{\mesh}, \]
...
\begin{proposition} \label{productCompleteSpacesComplete}
Let $\sSet{X,\mathcal{U}}$ and $\sSet{Y,\mathcal{V}}$ be complete uniform spaces. Then $X\times Y$ is also complete.
\end{proposition}
\begin{proof}
Let $F$ be a Cauchy filter in $X\times Y$, which means that $F\otimes F \in \mathcal{U}\otimes \mathcal{V}$. Then, by \ref{parallelFunctionProductFilter},
\[ (\proj_1|\proj_1)^{\imf\imf}(F\otimes F) = \proj_1^{\imf\imf}(F) \otimes \proj_1^{\imf\imf}(F), \]
which is an element of $\mathcal{U}$ by \ref{initialFinalUniformity}. Thus $\proj_1^{\imf\imf}(F)$ is a Cauchy filter in $\mathcal{U}$, which means that it converges to some $x\in X$ by completeness and $\proj_1^{\imf\imf}(F) \otimes \pfilter{x} \in \mathcal{U}$.

Similarly $\proj_2^{\imf\imf}(F)$ is a Cauchy filter in $\mathcal{V}$, which means that it converges to some $y\in Y$ by completeness and $\proj_2^{\imf\imf}(F) \otimes \pfilter{y} \in \mathcal{U}$.
Now
\[ (\proj_1|\proj_1)^{\imf\imf}\big(F\otimes (\pfilter{x}\otimes\pfilter{y})\big) = \proj_1^{\imf\imf}(F) \otimes \pfilter{x} \in \mathcal{U} \]
and
\[ (\proj_2|\proj_2)^{\imf\imf}\big(F\otimes (\pfilter{x}\otimes\pfilter{y})\big) = \proj_2^{\imf\imf}(F) \otimes \pfilter{y} \in \mathcal{V}, \]
so $F\otimes (\pfilter{x}\otimes\pfilter{y}) \in \mathcal{U}\otimes\mathcal{V}$ by \ref{initialFinalUniformity} and $F$ converges to $(x,y)$.
\end{proof}
...
\begin{proof}
(1) Let $\hat{X}$ be the set of Cauchy filters on $X$. For all $V\in\entourages_\mathcal{U}$, define the set
...
\begin{itemize}
\item We first show that $\entourages_{\hat{X}} \defeq \upset\setbuilder{V^*}{V\in \entourages_\mathcal{U}}$ is the entourage filter of a topological uniform space.

Note that $\entourages_{\hat{X}}$ is a filter. Upwards closure is immediate. Now take $V,W\in \entourages_\mathcal{U}$. Then $(V\cap W)^* \subseteq V^*$ and $(V\cap W)^* \subseteq W^*$, so $(V\cap W)^* \subseteq V^*\cap W^*$, which implies that $V^*\cap W^*\in \entourages_{\hat{X}}$.

Now we use \ref{entourageLemma}. For the first point, take arbitrary $V^*\in \entourages_{\hat{X}}$. Take arbitrary $F\in \hat{X}$. Since $F$ is Cauchy, we have $V \in F\otimes F$. Thus there exist $B,B'\in F$ such that $B\times B'\subseteq V$. We may replace $B$ by $B\cap B'$, so $B\times B\subseteq V$. This implies that $(F,F) \in V^*$. Since the choice of $F\in \hat{X}$ was arbitrary, we have $\entourages_{\hat{X}}\subseteq \upset \{\id_{\hat{X}}\}$.

Since $(V^*)^\transp = V^*$, we have $\entourages_{\hat{X}}^\transp = \entourages_{\hat{X}}$ by construction.

Finally take $V^*\in \entourages_{\hat{X}}$. Then we can find $W\in \entourages_\mathcal{U}$ such that $W;W\subseteq V$. I claim that $W^*;W^* \subseteq V^*$. Indeed, take $(F,G), (G,H)\in W^*$. Then we can find $B\in F\cap G$ and $B'\in G\cap H$ such that $B\times B\subseteq W$ and $B'\times B'\subseteq W$. Then $B\times (B\cap B')\subseteq W$ and $B'\times (B\cap B')\subseteq W$, so $(B\cup B')\times (B\cap B')\subseteq W$. Similarly, $(B\cap B')\times (B\cup B')\subseteq W$. This means that $(B\cup B')\times (B\cup B')\subseteq W;W \subseteq V$, by \ref{compositionProductFilters} since $B, B'\in G$ and so $B\mesh B'$. Since $B\cup B'\in F\cap H$, we have $(F,H)\in V^*$. By \ref{entourageLemma}, we conclude that $\entourages_{\hat{X}}$ is the entourage filter of a topological uniformity on $\hat{X}$.
...
\end{align*}
Thus, for all symmetric $V\in \entourages_\mathcal{U}$, we have $(f|f)^{\imf}(V) = V^* \cap \im(f)^2$. Since $\entourages_\mathcal{U}$ has a symmetric base, this implies $\upset(f|f)^{\imf\imf}(\entourages_\mathcal{U}) = \entourages_{\mathcal{V}}|_{\im(f)^2}$. Thus we conclude that $f$ is an embedding, by \ref{embeddingTopologicalUniformity}.

\item Now we show that $\im(f)$ is dense in $\hat{X}$.
For this we use \ref{denseIffMeshesWithAllVicinities}. By \ref{vicinityUniformConvergence}, every vicinity in $\hat{X}$ contains a set of the form $V^*(F)$, for some $V\in \entourages_\mathcal{U}$ and $F\in\hat{X}$. Since $F$ is a Cauchy filter, there exists a (non-empty) $B\in F$ such that $B\times B\subseteq V$. Pick any $x\in B$. Since $B\in \pfilter{x}$, we have $(F, \pfilter{x})\in V^*$ and thus $f(x) = \pfilter{x} \in V^*(F)$.
...
\begin{definition}
Let $\sSet{X,\mathcal{U}}$ be a uniform space. We say $X$ has the \udef{Heine-Borel property} if every bounded set is totally bounded.
\end{definition}

\begin{lemma}
Let $\sSet{X,\mathcal{U}}$ be a complete, symmetric uniform space. Then
\begin{enumerate}
\item if $X$ has the Heine-Borel property, then every closed and bounded set is compact;
\item if $X$ is a topological uniform space, then the converse also holds.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) Suppose $X$ has the Heine-Borel property. Let $A$ be an arbitrary closed and bounded subset of $X$. By the Heine-Borel property, $A$ is totally bounded and thus precompact by \ref{precompactTotallyBounded}. By \ref{closedComplete}, $A$ is complete. Thus $A$ is compact by \ref{compactPrecompactComplete}.

(2) Let $X$ be topological. Suppose every closed and bounded subset of $X$ is compact. Let $A\subseteq X$ be an arbitrary bounded subset. Then $\closure(A)$ is bounded by \ref{adherenceBoundedSet}, so $\closure(A)$ is compact by assumption. Finally $\closure(A)$ is totally bounded by \ref{compactImpliesBounded} and $A$ is totally bounded by \ref{boundedSetsIdeal}.
\end{proof}
...
TODO: Link with Bolzano-Weierstrass. Link with local compactness.
...
\item the \udef{uniformity of uniform convergence} $\mathcal{U}_u$ is given by $\big\{\upset\{\id_X\}\big\}^{\uRelEv^\transp}$.
\end{itemize}
...
\item the \udef{uniformity of uniform convergence on compact subsets} $\mathcal{U}_\text{comp}$ is given by $\setbuilder{\upset\{\id_K\}}{\text{$K\subseteq X$ compact}}^{\uRelEv^\transp}$.
\end{itemize}
\end{definition}
...
\begin{proposition} \label{uniformConvergenceOnCompactsIsCompactOpenConvergence}
...
&\iff \forall \text{compact $K\subseteq X$}:\; \entourages_\mathcal{V} \subseteq \upset\big((\evalMap | \evalMap)\circ t\big)^{\imf\imf}\big((H\otimes\pfilter{f})\otimes \upset\{\id_K\}\big) \\
&\iff \forall \text{compact $K\subseteq X$}: \forall V\in \entourages_{\mathcal{V}}: \exists A\in H: \; \big((\evalMap | \evalMap)\circ t\big)^{\imf}\big((A\times\{f\})\times \{\id_K\}\big) \subseteq V \\
&\iff \forall \text{compact $K\subseteq X$}: \forall V\in \entourages_{\mathcal{V}}: \exists A\in H: \forall k\in K: \; \big((\evalMap | \evalMap)\circ t\big)^{\imf}\big((A\times\{f\})\times \{(k,k)\}\big) \subseteq V \\
&\iff \forall \text{compact $K\subseteq X$}: \forall V\in \entourages_{\mathcal{V}}: \exists A\in H: \forall k\in K: \; \evalMap^\imf(A\times \{k\})\times \{f(k)\} \subseteq V \\
&\iff \forall \text{compact $K\subseteq X$}: \forall V\in \entourages_{\mathcal{V}}: \exists A\in H: \forall k\in K: \; \evalMap^\imf(A\times \{k\}) \subseteq Vf(k),
\end{align*}
and
\begin{multline*}
...
\end{multline*}
First assume $H \overset{\mathcal{U}_\text{comp}}{\longrightarrow} f$. Take arbitrary $K\subseteq X$ compact and $O\subseteq X$ open such that $f^\imf(K)\subseteq O$. Then, for all $k\in K$, $O\in \vicinities\big(f(k)\big)$, so there exists $V_k\in\entourages_{\mathcal{V}}$ such that $V_kf(k)\subseteq O$, by \ref{vicinityUniformConvergence}. By \ref{entourageLemma}, we can find $V'_k\in\entourages_\mathcal{V}$ such that $V'_k;V'_k \subseteq V_k$.

Since $f$ is continuous, $f^\imf(K)$ is compact by \ref{compactConstructions}. Since $f^\imf(K) \subseteq \bigcup_{k\in K}V'_kf(k)$ and $\{V'_kf(k)\}_{k\in K}$ is a convergence cover, there exists a finite $K'\subseteq K$ such that $f^\imf(K)\subseteq \bigcup_{k\in K'}V'_kf(k)$ by \ref{compactFiniteSubcover}. Set $V\defeq \bigcap_{k\in K'}V'_k \in \entourages_{\mathcal{V}}$.
...
Now assume $H\to f$ in the compact-open topology. Take arbitrary compact $K\subseteq X$ and $V\in \entourages_{\mathcal{V}}$. Then there exists $V'\in \entourages_{\mathcal{V}}$ such that $V';V' \subseteq V$ by \ref{entourageLemma}. By \ref{entourageOpenBase}, we can find $V^{\prime\prime} \in \entourages_\mathcal{V}$ such that $V^{\prime\prime} \subseteq V'$ and $V^{\prime\prime}$ is open. By \ref{entourageLemma}, we can find $V^{\prime\prime\prime} \in \entourages_\mathcal{V}$ such that $V^{\prime\prime\prime};V^{\prime\prime\prime} \subseteq V^{\prime\prime}$.
...
For all $k\in K'$, set $K_k \defeq f^\preimf\Big(\closure_{\Gamma(\mathcal{V})}\big(V^{\prime\prime\prime}f(k)\big)\Big)\cap K$. By \ref{adherenceUniformSpace}, we have
...
TODO: we need more than \ref{uniformImpliesContinuousConvergence}, namely that uniform limit of filters of continuous functions are continuous. (I.e. we need to show continuity assumptions of $H$ and $f$).
...
\begin{definition}
Let $\sSet{X,d}$ be a pseudometric space, $\epsilon >0$ and $x\in X$. Then we define
\begin{itemize}
\item the \udef{$\epsilon$-entourage} as the set
\[ \entourage_d(\epsilon) \defeq \setbuilder{(y,z)\in X^2}{d(y,z) \leq \epsilon} = d^{\preimf}\big(\interval{0,\epsilon}\big); \]
...
\begin{enumerate}
\item $\cball_d(x,\epsilon) = \entourage_d(\epsilon) x = x\entourage_d(\epsilon)$;
\item $\sphere_d(x,\epsilon) = \cball_d(x,\epsilon)\setminus \ball_d(x,\epsilon)$.
\end{enumerate}
\end{lemma}

\subsubsection{The induced subspace pseudometric}
\begin{definition}
Let $\sSet{X,d}$ be a pseudometric space and $A\subseteq X$. Then $d|_{A\times A}$ is the \udef{(induced) (subspace) pseudometric} on $A$.
\end{definition}

\begin{lemma}
Let $\sSet{X,d}$ be a pseudometric space and $A\subseteq X$. The subspace pseudometric is a pseudometric. If $d$ is a metric, then the subspace pseudometric is also a metric.
\end{lemma}

\subsubsection{Spheres}
TODO: put somewhere else.

TODO: define $S^n$. Should not be $S_{\R^{n+1}}(0,1)$, rather the $n$-dim spherical parametrisation?

TODO: Riemannian manifold.

TODO: it is nice that $S^n \cong (S^1)^n$.

TODO: What should the metric be? Distance in $\R^{n+1}$ or shortest distance over the sphere? Probably the latter.

\subsection{Uniform structure on a pseudometric space}
\begin{definition}
Let $\sSet{X,d}$ be a pseudometric space. The \udef{pseudometric uniformity} $\mathcal{U}_d$ is the topological uniformity with entourage filter $\entourages_d \defeq \upset \{\entourage_d(\epsilon)\}_{\epsilon > 0}$.
\end{definition}

\begin{proposition}
Let $\sSet{X,d}$ be a pseudometric space. Then the pseudometric uniformity is a topological uniformity.
\end{proposition}
\begin{proof}
We verify the conditions in \ref{entourageLemma}:
\begin{itemize}
\item $d^{\imf}(\id_X) = \{0\}$, so $\id_X \subseteq \entourage_d(\epsilon)$ for all $\epsilon > 0$ and thus $\{\entourage_d(\epsilon)\}_{\epsilon>0}\subseteq\upset\{\id_X\}$;
\item $\{\entourage_d(\epsilon)\}_{\epsilon>0}^\transp = \{\entourage_d(\epsilon)\}_{\epsilon>0}$ by symmetry of the metric;
\item $\entourage_d(\epsilon/2);\entourage_d(\epsilon/2) \subseteq \entourage_d(\epsilon)$ for all $\epsilon>0$ by the triangle inequality, so $\entourage_d(\epsilon) \in \{\entourage_d(\epsilon)\}_{\epsilon>0}; \{\entourage_d(\epsilon)\}_{\epsilon>0}$ and thus $\{\entourage_d(\epsilon)\}_{\epsilon>0} \subseteq \{\entourage_d(\epsilon)\}_{\epsilon>0}; \{\entourage_d(\epsilon)\}_{\epsilon>0}$.
\end{itemize}
\end{proof}

\begin{lemma} \label{topologicalMetricSubuniformity}
Let $\sSet{X,d}$ be a pseudometric space and $A\subseteq X$. Then the subspace uniformity on $A$ is the same as the pseudometric uniformity on $A$ induced by the subspace pseudometric $d|_{A\times A}$.
\end{lemma}
\begin{proof}
By \ref{entourageSubTopologicalUniformity} both are equal to $\upset \{\entourage_d(\epsilon) \cap (A\times A)\}_{\epsilon > 0}$.
\end{proof}
...
\begin{align*}
\entourages_Y \subseteq \upset(f\times f)^{\imf\imf}[\entourages_X] &\iff \forall \entourage_d(\epsilon) \in \entourages_Y: \exists \entourage_d(\delta) \in \entourages_X: \; (f\times f)^{\imf}\big(\entourage_d(\delta)\big) \subseteq \entourage_d(\epsilon) \\
&\iff \forall \epsilon>0: \exists \delta>0: \forall (y,y')\in (f\times f)^{\imf}\big(\entourage_d(\delta)\big): \; (y,y')\in \entourage_d(\epsilon) \\
&\iff \forall \epsilon>0: \exists \delta>0: \forall (x,x')\in \entourage_d(\delta):\; (f(x),f(x'))\in \entourage_d(\epsilon) \\
&\iff \forall \epsilon>0: \exists \delta>0: \forall x,x'\in X: \; d_X(x,x')\leq \delta \implies d_Y(f(x),f(x'))\leq \epsilon.
\end{align*}
\end{proof}

\begin{lemma} \label{metricEquivalentToBoundedMetric}
...
\item $\neighbourhoods_d(x) = \upset\setbuilder{\cball_d(x,\epsilon)}{\epsilon >0} = \upset\setbuilder{\ball_d(x,\epsilon)}{\epsilon >0}$;
\item $\neighbourhoods_d(x) = \upset\setbuilder{\cball_d(x,n^{-1})}{n\in \N} = \upset\setbuilder{\ball_d(x,n^{-1})}{n\in \N}$;
\item open balls are open sets and form a basis of the topology;
\item closed balls are closed sets.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) Immediate by \ref{vicinityUniformConvergence}.

(2) The first equality follows from \ref{vicinityUniformConvergence} and \ref{ballSphereLemma}.
...
(3) For all $\epsilon > 0$, we have $\ball_d(x,\ceil{\epsilon^{-1}}^{-1}) \subseteq \ball_d(x,\epsilon)$ and $\cball_d(x,\ceil{\epsilon^{-1}}^{-1}) \subseteq \ball_d(x,\epsilon)$.

(4) First we show that an open ball $\ball_d(x,\epsilon)$ is an open set, by \ref{openClosedCriteria}. Take $y\in \ball_d(x,\epsilon)$. Then $d(x,y)<\epsilon$, so $\delta \defeq \epsilon - d(x,y) \geq 0$. Now we claim that $\ball_d(y,\delta)\subseteq \ball_d(x,\epsilon)$. Indeed, take $z\in \ball_d(y,\delta)$, so $d(y,z)<\delta$. Then we have
...
(5) We show that a closed ball $\cball_d(x,\epsilon)$ is an open set, by \ref{openClosedCriteria}. Suppose $y\in X$ is such that $\cball_d(x,\epsilon) \in \neighbourhoods_d(y)^\mesh$. Then, by (2), for all $\delta >0$, we have
...
Let $\sSet{X,\xi}$ be a pseudometrisable convergence space. Then $X$ is
\begin{enumerate}
\item $C_1$;
\item Fréchet-Urysohn.
\end{enumerate}
\end{corollary}
In particular, every pseudometrisable uniform space is sequential.
\begin{proof}
(1) Since $\neighbourhoods_d(x) = \upset\setbuilder{\cball_d(x,n^{-1})}{n\in \N}$, by \ref{metricConvergenceNeighbourhood}, every neighbourhood filter has a countable base.

(2) This follows from (1) and \ref{C1ImpliesFrechetUrysohn}.
\end{proof}

\begin{lemma} \label{closureLemmaMetricSpace}
Let $\sSet{X,d}$ be a metric space, $A\subseteq X$ and $x\in X$. Then $x\in \adh(A)$ \textup{if and only if}
\[ \forall \epsilon >0: \exists y\in A: \quad d(x,y) \leq \epsilon. \]
\end{lemma}
\begin{proof}
We have, by \ref{principalAdherenceInherence},
\begin{align*}
x\in \adh(A) &\iff A\in \vicinities(x)^\mesh \\
&\iff \forall \epsilon > 0: \; A\mesh \cball_d(x,\epsilon) \\
&\iff \forall \epsilon > 0: \exists y\in A \; y\in \cball_d(x,\epsilon) \\
&\iff \forall \epsilon > 0: \exists y\in A \; d(x,y) \leq \epsilon.
\end{align*}
\end{proof}

\begin{proposition}
Let $\sSet{X,\xi}$ be a pseudometrisable convergence space. Then the following are equivalent:
\begin{enumerate}
\item $X$ is $C_2$;
\item $X$ is separable;
\item $X$ is Lindelöf.
...
\begin{proof}
Let $d$ be a metric such that $\xi$ is the metric convergence determined by $d$.

For all convergence spaces we have that $C_2$ implies separability and Lindelöf, by \ref{C2properties}.

$(2) \Rightarrow (1)$ Let $D\subseteq X$ be a countable dense subset. The set $\mathcal{B} \defeq \setbuilder{\cball_d(x, n^{-1})}{x\in D, n\in \N}$ is countable. We aim to show that it is a base of the convergence on $X$.

Since the closed balls form a base of the convergence $X$, it is enough to show that, for every closed ball $\cball_d(x,\epsilon)$, there exists $B\in \mathcal{B}$ such that $B\subseteq \cball_d(x,\epsilon)$ and $B \in \neighbourhoods(x)$. WLOG we may fix $\epsilon \leq 1/3$.

Since $x \in X = \adh(D)$, we can find $y\in D$ such that $d(x,y) \leq \epsilon / 4$ by \ref{closureLemmaMetricSpace}. Take $B = \cball_d(y, \ceil{(3\epsilon / 4)^{-1}}^{-1})$. To see that $B\subseteq \cball_d(x,\epsilon)$, take arbitrary $z\in B$. Then
\[ d(x,z) \leq d(x,y)+ d(y,z) \leq \epsilon /4 + \ceil{(3\epsilon / 4)^{-1}}^{-1} \leq \epsilon, \]
so $z\in \cball_d(x,\epsilon)$.

To see that $B\in \neighbourhoods(x)$, it is enough to show that $\cball(x, \epsilon / 4) \subseteq B$. First note that $\ceil{(3\epsilon / 4)^{-1}} - (3\epsilon / 4)^{-1} \leq 1$, so
\begin{align*}
\ceil{(3\epsilon / 4)^{-1}}^{-1} &\geq \big(1 + (3\epsilon / 4)^{-1}\big)^{-1} \\
&= \frac{3\epsilon}{4+3\epsilon} \\
&\geq \frac{3\epsilon}{4+1} \geq \epsilon / 2,
\end{align*}
where we have used the assumption that $\epsilon \leq 1/3$.
Now take $z'\in \cball(x, \epsilon / 4)$. Then
\[ d(y,z') \leq d(y,x) + d(x, z') \leq \epsilon / 4 + \epsilon / 4 = \epsilon / 2 \leq \ceil{(3\epsilon / 4)^{-1}}^{-1}. \]
Thus $z'\in \cball_d(y, \ceil{(3\epsilon / 4)^{-1}}^{-1}) = B$.


$(3) \Rightarrow (1)$ For each $n\in \N$, define the open cover $\mathcal{C}_n \defeq \setbuilder{\ball_d(x, n^{-1})}{x\in X}$. Since $X$ is Lindelöf, each $\mathcal{C}_n$ has a countable subcover $\mathcal{C}'_n$. Take the union $\mathcal{C} \defeq \bigcup_{n\in \N}\mathcal{C}_n$. This is countable and we claim that it is a base of the convergence.

Indeed, take $x\in X$ and $\ball(x,\epsilon) \in \neighbourhoods(x)$. Then there exists $B = \ball_d(y, \ceil{(\epsilon/2)^{-1}}^{-1})\in \mathcal{C}'_{\ceil{(\epsilon/2)^{-1}}}$ such that $x\in B$. Since $x\in B$ and $B$ is open, we have $B\in \neighbourhoods(x)$. We also have $B\subseteq \ball(x,\epsilon)$. Indeed, take $z\in B$. Then
\[ d(x,z) \leq d(x,y) + d(y,z) < \ceil{(\epsilon/2)^{-1}}^{-1} + \ceil{(\epsilon/2)^{-1}}^{-1} \leq \epsilon, \]
so $z\in \ball(x,\epsilon)$.
\end{proof}
...
\end{lemma}
\begin{proof}
Since $Y$ is topological, by \ref{metricConvergenceNeighbourhood}, we have that $f$ is continuous at $x$ iff $\neighbourhoods_{Y}\big(f(x)\big) \subseteq \upset f^{\imf\imf}\big(\neighbourhoods_X(x)\big)$ by \ref{pretopologicalContinuityVicinities}. 
...
\end{lemma}
\begin{proof}
By \ref{uniformContinuityEntourages}, it is enough to prove that $\entourages_\R \subseteq \upset \big(d(a, -)|d(a,-)\big)^{\imf\imf}(\entourages_d)$.

Take arbitrary $\epsilon>0$. Then it is enough to prove thave there exists $A\in \entourages_d$ such that $\big(d(a, -), d(a,-)\big)^{\imf}(A) \subseteq \entourage_d(\epsilon)$. Now there exists $A\in \entourages_d$ such that $d^{\imf}(A)\subseteq \cball(0,\epsilon)$. We claim this is the set we want, indeed
\begin{align*}
d^{\imf}(A)\subseteq \cball(0,\epsilon) \iff& \forall (x,y)\in A: \; d(x,y) \leq \epsilon \\
\implies& \forall (x,y)\in A: \; |d(x,a) - d(a,y)| \leq \epsilon \\
\iff& \forall (x,y)\in A: \; \big(d(-,a)|d(a,-)\big)(x,y) \in \entourage_d(\epsilon) \\
\iff& \big(d(-,a), d(a,-)\big)^\imf(A) = \big(d(a,-)|d(a,-)\big)^\imf(A) \subseteq \entourage_d(\epsilon),
\end{align*}
...
\end{proposition}
\begin{proof}
Since $\cball_\R(0,\epsilon) = \interval{-\epsilon, \epsilon}$ and $\im(d)\subseteq \overline{\R^+}$, we have $d^\preimf\big(\cball_\R(0,\epsilon)\big) = d^\preimf\big(\interval{0,\epsilon}\big) = \entourage_d(\epsilon)$. And thus by $\upset\{\entourage_d(\epsilon)\}_{\epsilon > 0} = \upset d^{\preimf\imf}\big(\neighbourhoods_{\overline{\R^+}}(0)\big)$ by \ref{metricConvergenceNeighbourhood}. Then we calculate, using \ref{upsetPreimageImageGaloisConnection},
\begin{align*}
H \in \mathcal{U}_d &\iff H \supseteq \upset \{\entourage_d(\epsilon)\}_{\epsilon > 0} \\
&\iff H \supseteq \upset d^{\preimf\imf}\big(\neighbourhoods_{\overline{\R^+}}(0)\big) \\
&\iff \upset d^{\imf\imf}(H)\supseteq \neighbourhoods_{\overline{\R^+}}(0) \\
...
For the converse we use \ref{topologicalBoundednessLemma}. Assume $B$ bounded and take some $V\in \entourages_{\R}$. There exists some $\epsilon >0$ such that $\entourage_d(\epsilon) \subseteq V$. As $\entourage_d(\epsilon) \in \entourages_{\R}$, there exists $n\in \N$ and finite $S\subseteq X$ such that $B\subseteq \bigcup_{x\in S}\entourage_d(\epsilon)^nx$.
Now construct
\[ S' \defeq \bigcup_{i=0}^{n-1} (S+\epsilon)\cup(S-\epsilon), \]
which is still finite. We have
\[ \bigcup_{x\in S}\entourage_d(\epsilon)^nx\subseteq \bigcup_{x\in S'}\entourage_d(\epsilon) x \subseteq \bigcup_{x\in S'}V x, \]
so $B\subseteq \bigcup_{x\in S'}\entourage_d(\epsilon) x$, which means the $B$ is totally bounded.
\end{proof}
...
First assume $B$ is bounded. Take $\epsilon>0$, then there exists a finite $S\subseteq X$ such that $B\subseteq \bigcup_{x\in S}\entourage_d(\epsilon) x$. Now let $x_0 = \min S$ and $x_1 = \max S$, which exist because $S$ is finite. Then $B\subseteq \interval{x_0-\epsilon, x_1+\epsilon}$.

Now assume $\diam(B) < \infty$. Then $\inf B > -\infty$ and $\sup B < \infty$, so $B\subseteq \interval{\inf B, \sup B}$. Take $V\in \entourages_\R$, then there exists $\epsilon >0$ such that $\entourage_d(\epsilon) \subseteq V$. We construct
\[ S \defeq \bigcup_{i=0}^{\ceil{\frac{\sup B - \inf B}{2\epsilon}}}\{\inf B + i\cdot \epsilon\}. \]
Then $B\subseteq \interval{\inf B, \sup B} \subseteq \bigcup_{x\in S}\entourage_d(\epsilon) x \subseteq \bigcup_{x\in S}V x$, so $B$ is bounded.
...
\end{proof}
\begin{corollary} \label{realsLocallyCompact}
The real numbers are locally compact.
\end{corollary}
...
\item If $\{U_n\}_{n\in\N} \subseteq \entourages_\mathcal{U}$ for some uniformity $\mathcal{U}$ on $X$, then $\mathcal{U} \subseteq \mathcal{U}_d$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
\[ \neighbourhoods_\R(0) \subseteq \upset \setbuilder{\interval[co]{0, n^{-1}}}{n\in \N} \subseteq \upset d^{\imf\imf}\big(\{U_n\}_{n\in\N}\big) \subseteq d^{\imf\imf}(H). \]
\end{proof}
...
Now suppose $\entourages_\mathcal{U}$ has a countable basis $\seq{B_n}_{n\in \N}$. WLOG we may take $B_0 = X^2$. Then define $\seq{C_n}$ recursively by setting $C_0 = X^2$ and, given $C_{n-1}$, finding some $C'_{n}\in \entourages_\mathcal{U}$ such that $C'_{n};C'_{n};C'_{n} \subseteq C_{n-1}$ and setting $C_n \defeq C'_n \cap B_n$. We have $C_n\subseteq B_n$ for all $n\in \N$ and each $C_n\in \entourages_\mathcal{U}$. Thus $\seq{C_n}$ forms a basis of the intourage filter and thus the entourage filter is generated by a pseudometric by \ref{metrisationLemma}. 
\end{proof}
...
For the other inclusion, it is enough to prove $\entourages_\mathcal{U} \subseteq \entourages_{\bigwedge \mathcal{U}_d}$. Take $V\in \entourages_\mathcal{U}$ and define $\seq{B_n}$ recursively by $B_0 = X^2$, $B_1 = V$ and $B_{n+1}$ is any symmetric set in $\entourages_\mathcal{U}$ such that $B_{n+1};B_{n+1};B_{n+1} \subseteq B_n$. Construct a pseudometric $d'$ as in \ref{metrisationLemma}. Then $d^{\prime\preimf}(\interval[co]{0,1/2})\subseteq V$, so
\[ V \in \entourages_{d'} \subseteq \entourages_{\bigwedge \mathcal{U}_d}. \]
\end{proof}
...
\subsubsection{Continuous pseudometrics}
\begin{definition}
...
\end{proposition}
\begin{proof}
Set $d_f \defeq d_\R\circ(f|f)$. It is enough to prove that $\entourages_{d_f}\subseteq \entourages_\mathcal{U}$. Indeed we have
\[ \entourages_{d_f} = \upset \big(d_\R\circ (f|f)\big)^{\preimf\imf}\big(\neighbourhoods_\R(0)\big) = \upset (f|f)^{\preimf\imf}\Big(d_\R^{\preimf\imf}\big(\neighbourhoods_\R(0)\big)\Big) = \upset (f|f)^{\preimf\imf}(\entourages_\R) \subseteq \entourages_\mathcal{U}, \]
where the last inclusion follows by \ref{upsetPreimageImageGaloisConnection} from $\entourages_\R \subseteq \upset (f|f)^{\imf\imf}(\entourages_\mathcal{U})$, which in turn is the expression of uniform continuity of $f$, by \ref{uniformContinuityEntourages}.
\end{proof}
...
\end{lemma}
In particular $d'$ is a continuous pseudometric if there exists $A\in \R^+$ such that $d'(x,y) \leq Ad(x,y)$ for all $x,y\in X$.
\begin{proof}
We calculate
\begin{align*}
\uniformity_d \subseteq \uniformity_{d'} &\iff \entourages_{d} \in \uniformity_{d'} \\
&\iff \entourages_{d'} \subseteq \entourages_{d} \\
&\iff \forall \epsilon > 0: \; \entourage_{d'}(\epsilon) \in \entourages_d \\
&\iff \forall \epsilon > 0:\exists \delta > 0: \; \entourage_d(\delta) \subseteq \entourage_{d'}(\epsilon) \\
...
$(3) \Rightarrow (1)$ We use \ref{topologicalBoundednessLemma}. Take an arbitrary symmetric $V\in \entourages_\mathcal{U}$ and let $W$ be the transitive closure of $V$, which is an equivalence relation. Let $\seq{A_n}$ be a sequence of distinct $W$-equivalence classes and define
\[ f: X\to \N: x\mapsto \begin{cases}
n & (x\in A_n) \\
0 & (\text{otherwise}).
\end{cases} \]
Now take $(x,y)\in V$. Then $x$ and $y$ must be in the same equivalence class, so $\big(f(x), f(y)\big) = (n,n)$ for some $n\in\N$. Thus $(f|f)^\imf(V)\subseteq \id_\R \subseteq \entourage_d(\epsilon)$ for all $\epsilon>0$. By \ref{uniformContinuityEntourages}, we have that $f$ is uniformly continuous. Now consider the pseudometric $d_f(x,y) = |f(x)-f(y)|$, which is continuous by \ref{continuousPseudometricFunctionLemma}. By assumption, $B$ is of finite radius w.r.t. $d_f$ and thus $B$ meshes with only finitely many $W$-equivalence classes. Let the finite set $F$ consist of one point from the intersection of $B$ with each $W$-equivalence class that meshes with $B$.
...
\end{lemma}
\begin{proof}
We have that $\TailsFilter\seq{x_n}$ is a Cauchy filter \textup{if and only if} $\entourages_d \subseteq \TailsFilter\seq{x_n}\otimes \TailsFilter\seq{x_n}$. This is true iff $\forall \epsilon>0$:
\begin{align*}
\entourage_d(\epsilon) \in \TailsFilter\seq{x_n}\otimes \TailsFilter\seq{x_n} &\iff \exists M,N\in\N: \setbuilder{x_n}{n\geq N}\times\setbuilder{x_n}{n\geq M} \subseteq \entourage_d(\epsilon) \\
&\iff \exists N\in\N: \setbuilder{x_n}{n\geq N}\times\setbuilder{x_n}{n\geq N} \subseteq \entourage_d(\epsilon) \\
...
$(3) \Rightarrow (1)$ Let $F\in \powerfilters(X)$ be a Cauchy filter. Then, for all $n\in\N$, there exists $A_n\in F$ such that $A_n\times A_n \subseteq \entourage_d(n^{-1})$. Now, for each $n\in \N$, choose (by countable choice) $x_n\in \bigcap_{k\leq n}A_k$. We claim that $\seq{x_n}$ is a Cauchy sequence.
Indeed, take $\epsilon >0$. Then for all $m,n\geq \ceil{\epsilon^{-1}}$, we have $x_m,x_n\in A_{\ceil{\epsilon^{-1}}}$, so $(x_m, x_n) \in \entourage_d(\ceil{\epsilon^{-1}}^{-1}) \subseteq \entourage_d(\epsilon)$, which means that $d(x_m, x_n) \leq \epsilon$ and $\seq{x_n}$ is a Cauchy sequence by \ref{metricCauchySequence}.
...
&\iff \{\entourage_d(\epsilon)\}_\epsilon \subseteq (\evalMap, \evalMap)^{\imf\imf}\big((\TailsFilter\seq{f_n} \otimes \pfilter{f})\otimes \{\id_X\}\big)^\ttransp \\
&\iff \forall \epsilon>0: \exists N\in \N: \; (\evalMap, \evalMap)^{\imf}\big((\setbuilder{f_n}{n\geq N} \times \{f\})\times \id_X\big)^\ttransp \subseteq \entourage_d(\epsilon) \\
&\iff \forall \epsilon>0: \exists N\in \N: \; \bigcup_{n\geq N}\bigcup_{x\in X}(\evalMap, \evalMap)^{\imf}\big((\{f_n\} \times \{f\})\times \{(x,x)\}\big)^\ttransp \subseteq \entourage_d(\epsilon) \\
&\iff \forall \epsilon>0: \exists N\in \N: \forall n\geq N: \forall x\in X: \;(\evalMap, \evalMap)^{\imf}\big((\{f_n\} \times \{f\})\times \{(x,x)\}\big)^\ttransp \in \entourage_d(\epsilon) \\
&\iff \forall \epsilon>0: \exists N\in \N: \forall n\geq N: \forall x\in X: \; (\evalMap, \evalMap)^{\imf}\big(\{((f_n, f), (x,x))^\ttransp\}\big) \in \entourage_d(\epsilon) \\
&\iff \forall \epsilon>0: \exists N\in \N: \forall n\geq N: \forall x\in X: \; (\evalMap, \evalMap)\big((f_n,x),(f,x)\big) \in \entourage_d(\epsilon) \\
&\iff \forall \epsilon>0: \exists N\in \N: \forall n\geq N: \forall x\in X: \; (f_n(x), f(x)) \in \entourage_d(\epsilon) \\
...
\begin{proposition} \label{metricEquicontinuity}
...
\[ \forall \epsilon >0: \exists U \in \vicinities(x): \forall y\in U: \forall f\in H: \quad d\big(f(y), f(x)\big) < \epsilon. \] 
\end{proposition}
\begin{proof}
By \ref{equicontinuityUnionLemma}, the equicontinuity of $H$ is equivalent to $\upset k^{\imf\imf}(\{H\}\otimes \vicinities(x)) \in \uniformity_d$. We calculate
\begin{align*}
\upset k^{\imf\imf}(\{H\}\otimes \vicinities(x)) \in \uniformity_d &\iff \{\entourage_d(\epsilon)\}_{\epsilon >0} \subseteq \upset k^{\imf\imf}(\{H\}\otimes \vicinities(x)) \\
&\iff \forall \epsilon >0: \exists U\in \vicinities(x): k^{\imf}(H\times U) \subseteq \entourage_d(\epsilon) \\
&\iff \forall \epsilon >0: \exists U\in \vicinities(x): \forall y\in U: \forall f\in H: k(f, y) \in \entourage_d(\epsilon) \\
&\iff \forall \epsilon >0: \exists U\in \vicinities(x): \forall y\in U: \forall f\in H: \big(f(y), f(x)\big) \in \entourage_d(\epsilon) \\
&\iff \forall \epsilon >0: \exists U\in \vicinities(x): \forall y\in U: \forall f\in H: d\big(f(y), f(x)\big) \leq \epsilon.
\end{align*}
\end{proof}
...
\begin{lemma} \label{uniformCauchyPointwiseConvergentUniformConvergent}
...
\begin{proposition} \label{uniformLimitTheorem}
...
\end{proof}
\begin{corollary} \label{swapLimitsUniformConvergenceMetricSpaces}
...
\begin{definition}
Let $X$ be a set, $\sSet{Y,d}$ a metric space and $f: X\to Y$ a function. Then we call $f$ \udef{bounded} if $\im(f)$ is bounded.
\end{definition}
...
(2) By \ref{upsetResiduatedImageGaloisConnection} and \ref{principalImageFilterLemma}, we have
\begin{align*}
...
Let $X$ be a set.
\begin{enumerate}
\item Let $\entourages$ be a topological entourage filter. Then
\[ \setbuilder{C}{\exists V\in\entourages: C_V\subseteq \downset C} = \setbuilder{C_V}{V\in\entourages}_{<^*} \]
...
\end{proposition}
\begin{proof}
(1) We first verify the equality. Take $V\in \entourages$. Then there exists $W\in \entourages$ such that $W;W;W\subseteq V$.
