...
\end{enumerate}
\end{proposition}
In particular, all $f_i$s are uniformly continuous w.r.t. the initial uniformity.
\begin{proof}
...
\begin{corollary}[Characteristic property of initial and final uniformity] \label{characteristicPropertyInitialFinalUniformity}
...
\item If $B$ is bounded, then $f^\imf(B)$ is also bounded.
\end{enumerate}
\end{proposition}
\begin{proof}
...
(2) Now take an arbitrary uniform cover $\mathcal{D}'$ of $\mathcal{V}$. As $(f,f)^{\preimf\imf}(\mathcal{D})$ is a uniform cover of $\mathcal{U}$, by point (1), we can find some finite $\mathcal{C} \subseteq \setbuilder{(f,f)^\preimf(A)^nx}{A\in \mathcal{D}', n\in\N, x\in X}$ such that $B\subseteq \bigcup \mathcal{C}$.
...
\item if $f$ is uniformly continuous, then $d_\R\circ (f,f): X^2\to \R^+: (x,y)\mapsto |f(x)- f(f)|$ is a continuous pseudometric.
\end{enumerate}
\end{proposition}
\begin{proof}
...
For example, consider $d_\R$ and $d' \defeq d_\R \wedge \underline{1}$. Then both metrics define the same uniformity, but $\sup d^{\prime\imf}(A) \leq 1$ for all subsets $A\subseteq \R$. 
\end{example}
