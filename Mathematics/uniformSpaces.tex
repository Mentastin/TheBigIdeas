...
\[ \mathcal{U}_d = \setbuilder{H\in \powerfilters(X^2)}{\upset d^{\imf\imf}(H) \overset{\R}{\longrightarrow} 0}.  \]
\end{proposition}
\begin{proof}
Since $\cball_\R(0,\epsilon) = \interval{-\epsilon, \epsilon}$ and $\im(d)\subseteq \R^+$, we have $d^\preimf\big(\cball_\R(0,\epsilon)\big) = d^\preimf\big(\interval{0,\epsilon}\big) = V_\epsilon$. And thus by $\upset\{V_\epsilon\}_{\epsilon > 0} = \upset d^{\preimf\imf}\big(\neighbourhood_\R(0)\big)$ by \ref{metricConvergenceNeighbourhood}. Then we calculate, using \ref{filterPreimageImageGaloisConnection},
\begin{align*}
...
&\iff \upset d^{\imf\imf}(H)\overset{\R}{\longrightarrow} 0.
\end{align*}
\end{proof}
...
\[ F \overset{d}{\longrightarrow} x \qquad\iff\qquad \upset d^{\imf\imf}(F\otimes \pfilter{x}) \overset{\R}{\longrightarrow} 0 \]
for all $F\in \powerfilters(X)$ and $x\in X$.
\end{corollary}
\begin{proof}
We have that $F \overset{d}{\longrightarrow} x$ iff $F\otimes \pfilter{x}\in \mathcal{U}_d$ iff $\upset d^{\imf\imf}(F\otimes \pfilter{x}) \overset{\R}{\longrightarrow} 0$.
\end{proof}
