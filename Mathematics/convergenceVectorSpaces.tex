...
By \ref{dualSystemBijection} we have $\sSet{X, \sigma^*(X,Y)} \cong \dual{\sSet{Y, \sigma(X,Y)}} \subseteq \Lin(Y, \F)_p$. Let $\pol'$ be the relation defined on the pairing $\sSet{\dual{\sSet{Y, \sigma(X,Y)}},Y, \evalMap}$ and $\pol^{\prime\prime}$ the relation defined on the pairing $\sSet{\Lin(Y, \F),Y, \evalMap}$. We show that $A^{\pol'} = A^{\pol^{\prime\prime}}$. Clearly $A^{\pol'} = A^{\pol^{\prime\prime}} \cap \dual{\sSet{Y, \sigma(X,Y)}}$, so $A^{\pol'} \subseteq A^{\pol^{\prime\prime}}$. For the other inclusion, take $f\in A^{\pol^{\prime\prime}}$, which is bounded on a neighbourhood of the origin (i.e. $A$) and thus is is continuous by \ref{boundedOnVicinityImpliesContinuous}, which means that $f\in A^{\pol^{\prime\prime}}$.
...
\end{corollary}
TODO: polars taken w.r.t. which pairings?
...
\end{proposition}
\begin{proof}
Let $\pol$ be the polar relation in the pairing $\sSet{X,Y,\pair{\cdot,\cdot}}$ and $\pol'$ the polar relation in the pairing $\sSet{\dual{\sSet{X, \tau(X,Y)}}, X, \evalMap}$.
\end{proof}
