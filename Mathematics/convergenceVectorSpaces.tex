...
Now assume $\Span\{v\}$ Hausdorff. We need to show that $u^{-1}$ is continuous. It is enough to show continuity at $0$. We use \ref{pretopologicalContinuityVicinities}, so take $\Gamma \in \vicinities_\F(0)$ and we need to show that $\Gamma \in (u^{-1})^{\imf\imf}\big[\vicinities_{\Span\{v\}}(0)\big]$. WLOG we may take $\Gamma = \ball(0, \epsilon)$.
...
For each $F$ that converges to $0$ in $\Span\{v\}$, $\neighbourhoods_\F(0)\cdot F$ also converges to $0$, so there exists $\delta_F>0$ and $C_F\in F$ such that $\ball(0, \delta_F)\cdot C_F \subseteq U$. If $\delta_F > 1$, then $C_F \subseteq \ball(0, \delta_F)\cdot C_F \subseteq \ball(0,\epsilon)\cdot v$, so $\ball(0,\epsilon)\cdot v\in F$.
...
As $\ball(0,\epsilon)\cdot v \in F$ for all $F$ that converge to $0$, we have $\ball(0,\epsilon)\cdot v\in \vicinities_{\Span\{v\}}$, so $\ball(0,\epsilon) = \Gamma \in (u^{-1})^{\imf\imf}\big[\vicinities_{\Span\{v\}}(0)\big]$, which is what we had to prove.
\end{proof}
...
\item if $F\in \mathcal{F}$, then $\neighbourhoods_\F(0)\cdot F \in \mathcal{F}$;
\item if $v\in V$, then $\neighbourhoods_\F(0)\cdot v \in \mathcal{F}$;
...
\item The convergence on the scalar field is pretopological, so $\neighbourhoods_\F(0)\to 0$. By \ref{convergenceFiniteProductFilter}, $\neighbourhoods_\F(0)\otimes F \to (0,0)$ and by continuity of the scalar multiplication $\neighbourhoods_\F(0)\cdot F \to 0$.
...
Let $G \to (\lambda, v)$. Then $G_1 = p_1^{\imf\imf}(G) \to \lambda$ and $G_2 = p_2^{\imf\imf}(G) \to v$, so $G_1 \supseteq \neighbourhoods_\F(\lambda)$. We have
\begin{align*}
\cdot^{\imf\imf}[G] - \lambda\cdot v &\supseteq \cdot^{\imf\imf}[G_1\otimes G_2] - \lambda\cdot v = G_1\cdot G_2 - \lambda\cdot v \\
&\supseteq \neighbourhoods_\F(\lambda) \cdot G_2 - \lambda\cdot v \\
&= (\neighbourhoods_\F(0) + \lambda)\cdot((G_2 - v) + v) - \lambda\cdot v \\
&\supseteq \lambda\cdot (G_2 - v) + \neighbourhoods_\F(0)\cdot(G_2-v) + \lambda\cdot v + \neighbourhoods_\F(0)\cdot v - \lambda\cdot v \\
&= \lambda\cdot (G_2 - v) + \neighbourhoods_\F(0)\cdot(G_2-v) + \neighbourhoods_\F(0)\cdot v \in \mathcal{F}.
\end{align*}
So $\cdot^{\imf\imf}[G] \to \lambda\cdot v$, making the scalar multiplication continuous. Note the last inclusion is not an equality because we go from one instance of $G_2$ and $\neighbourhoods_\F(0)$ to two!
\end{proof}
\begin{corollary} \label{vicinityFilterAtOrigin}
Let $\sSet{V, \xi}$ be a convergence vector space. Then
\begin{enumerate}
\item for all $A\in \vicinities_\xi(0)$ and $\lambda\in \F\setminus\{0\}$: $\lambda A\in \vicinities_\xi(0)$;
\item each $A \in \vicinities_\xi(0)$ is absorbent;
\item if $\xi$ is topological, then $\neighbourhoods_\xi(0)$ has a balanced base;
\item if $\xi$ is topological, then $\neighbourhoods_\xi(0)$ has a closed, balanced base.
\end{enumerate}
\end{corollary}
If $\sSet{V,\xi}$ is equable, then $\vicinities_\xi(0)$ has a balanced base, see \ref{equableConvergenceBalancedBase}.

This is not true for all convergence vector spaces. What is true for all convergence vector spaces is that each $A \in \vicinities_\xi(0)$ contains a balanced subset. This is immediate since $\{0\}$ is balanced and $\{0\} \subseteq A$ because $A$ is absorbent.
\begin{proof}
(1) Take arbitrary $\lambda\in\F\setminus\{0\}$. Then $v\mapsto \lambda v$ is a homeomorphism by \ref{continuityLemmaVectorConvergence}, so $\lambda A \in \vicinities_\xi(\lambda 0) = \vicinities_\xi(0)$, by \ref{homeomorphismPreservation}.

(2) For absorbence, take $A\in \vicinities_\xi(0)$ and $v\in V$. As $\neighbourhoods_\F(0)\cdot v \to 0$, we must have $A\in \upset\neighbourhoods_\F(0)\cdot v$, so there exists $\Gamma \in \neighbourhoods_\F(0)$ such that $\Gamma\cdot v \subseteq A$. Now we can find a $r>0$ such that $\ball(0,r)\subseteq \Gamma$, so for all $|c|\geq r^{-1}$ we have $v\in cA$.

(3) By point (3) of of the proposition, $\neighbourhoods_\F(0)\cdot \vicinities_\xi(0)$ converges to $0$ and thus $\vicinities_\xi(0) \subseteq \neighbourhoods_\F(0)\cdot\vicinities_\xi(0)$. Take $A\in \vicinities_\xi(0)$. Then there exists a $\Gamma\in\neighbourhoods_\F(0)$ and $B\in \vicinities_\xi(0)$ such that $\Gamma\cdot B \subseteq \vicinities_\xi(0)$. We can find some ball $\ball(0,\epsilon) \subseteq \Gamma$, so $\ball(0,1)\cdot \epsilon B\subseteq \epsilon B \subseteq A$. Thus $\epsilon B$ is balanced and a neighbourhood by point(1). So every $A\in \vicinities_\xi(0)$ contains a balanced set in $\vicinities_\xi(0)$.

(4) Take arbitrary $A\in \neighbourhoods_\xi(0)$. By regularity, \ref{topologicalGroupsRegular}, $A$ contains a closed neighbourhood $B$. By (3), $B$ contains a balanced neighbourhood $C$. Now consider $\closure_\xi(C)$, which is closed and a subset of $B$ (as $\closure(C)\subseteq \closure(B) = B$). It is now enough to note that the closure of a balanced set is balanced: if $x$ is the limit of a filter $F$ in $B$, then $r\cdot x$ is the limit of $r\cdot F$ for all $|r|\leq 1$. Now $r\cdot F$ is a filter in $B$ by balance.
\end{proof}
...
Let $\sSet{V,\xi}$ be a topological convergence vector space. Then $\neighbourhoods_\xi(0)$ is equable and thus $\sSet{V,\xi}$ is equable.
\end{lemma}
\begin{proof}
By \ref{vicinityFilterAtOrigin}, $\neighbourhoods_\xi(0)$ has a balanced base and $U\in \neighbourhoods_\xi(0)$ iff $\lambda U\in \neighbourhoods_\xi(0)$ for all $\lambda\in \F\setminus\{0\}$. Thus we have
\[ \neighbourhoods_\xi(0) = \upset\{\cball(0,1)\}\cdot \neighbourhoods_\xi(0) = \upset\{\cball(0,\epsilon)\}\cdot \neighbourhoods_\xi(0) = \neighbourhoods_\F(0)\cdot \neighbourhoods_\xi(0). \]
This shows that $\neighbourhoods_\xi(0)$ is equable. Now each filter that converges to $0$ contains $\neighbourhoods_\xi(0)$, which is equable, so $\sSet{V, \xi}$ is equable.
\end{proof}

\begin{proposition} \label{equableConvergenceBalancedBase}
Let $\sSet{V,\xi}$ be an equable convergence vector space. Then $\vicinities_\xi(0)$ has a balanced base.
\end{proposition}
\begin{proof}
We have
\[ \vicinities_\xi(0) = \bigcap_{F\in \lim^{-1}(0)} F = \bigcap_{F\in \lim^{-1}(0)} \neighbourhoods_\F(0)\cdot F. \]
Now take some $A\in \vicinities_\xi(0)$. As each $\neighbourhoods_\F(0)\cdot F$ has a balanced base and $A\in\neighbourhoods_\F(0)\cdot F$, we can find a balanced $B_F \in \neighbourhoods_\F(0)\cdot F$ that is contained in $A$. Now $B = \bigcup_{F\in\lim^{-1}(0)}B_F$ is balanced by \ref{balancedLemma} and a subset of $A$. Since $B\in \neighbourhoods_\F(0)\cdot F$ for all $F$ by upwards closure, we have $B\in \vicinities_\xi(0)$.
\end{proof}
...
\item Take $F\in \pfilter{N}$. We need to show that $\upset(\neighbourhoods_\F(0)\cdot F) \supseteq N$, which means that for all $A\in N$ there exists a $\Gamma\in \neighbourhoods_\F(0)$ and $B\in F$ such that $\Gamma \cdot B\subseteq A$. We can take $B = \balancedCore(A) \in N \subseteq F$ and $\Gamma = B(0,1)$.
\item Take $v\in V$. We need to show that all $A\in N$ contain $\Gamma\cdot v$ for some $\Gamma \in \neighbourhoods_\F(0)$. Because $A$ is absorbent, there exists an $r>0$ such that $v\in cA$ for all $|c|\geq r$. Conversely $c^{-1}v \in A$ for all $|c^{-1}| \leq r^{-1}$. So $\ball(0,r^{-1})\cdot v \subseteq A$ and $B(0,r^{-1}) \in \neighbourhoods_\F(0)$.
...
Now assume $\xi$ is a topological vector space convergence and $N = \neighbourhoods_\xi(0)$. The first 3 points follow from \ref{vicinityFilterAtOrigin}. The fourth from \ref{vicinityFactorisation}.
\end{proof}
...
\end{enumerate}
then $\mathfrak{F}(\mathcal{B}) = \neighbourhoods_\xi(0)$ for some topological convergence on $V$.
\end{corollary}
\begin{proof}
...
Take $|\lambda|\leq 1$ and $v\in \adh_\xi(A)$, then we need to show that $\lambda v \in \adh_\xi(A)$. We have $A\in \vicinities_\xi(v)^{\mesh}$ and $A\subseteq \lambda^{-1}A$ (TODO: why, is this correct??). So for all $B\in \vicinities_\xi(v)$:
\[ A\mesh B \quad\implies\quad \lambda^{-1}A\mesh B \quad\implies\quad A\mesh \lambda B. \]
Thus $A\in \vicinities_\xi(\lambda v)^{\mesh}$, which is what we needed to show by \ref{principalAdherenceInherence}.
\end{proof}
...
(4) Assume $K$ is salient. Suppose, towards a contradiction, that $0\in \inh(K)$. Then $K\in \vicinities(0)$ and so $K$ is absorbent by \ref{vicinityFilterAtOrigin}.
...
\begin{proof}
Take $v\in \inh_\xi(C)$ and $w\in \adh_\xi(C)$. Then, by \ref{principalAdherenceInherence}, $C\in \vicinities_\xi(v)$. Then $(C- v)\in \vicinities_\xi(0)$, so $-\frac{r}{1-r}(C-v)\in \vicinities_\xi(0)$ and $w - \frac{r}{1-r}(C-v) \in \vicinities_\xi(w)$. Again by \ref{principalAdherenceInherence}, we have $C \mesh \big(w - \frac{r}{1-r}(C-v)\big)$, so we can take $u \in C \cap \big(w - \frac{r}{1-r}(C-v)\big)$. Since $u\in w - \frac{r}{1-r}(C-v)$, we can write $u = w - \frac{r}{1-r}(u'-v)$ for some $u'\in C$. Rearranging gives $rv + (1-r)w = ru'+ (1-r)u \in C$.
\end{proof}
...
Now take $v\in \inh_\xi\big(\adh_\xi(C)\big)\subseteq \adh_\xi(C)$, which implies that $\adh_\xi(C)\in \vicinities_\xi(x)$ and thus $\adh_\xi(C)-x\in \vicinities_\xi(0)$, which is absorbent by \ref{vicinityFilterAtOrigin}. Now take $w\in \inh_\xi(C)$. By arbsorbence, there exists $0<\epsilon \leq 1$ such that $\epsilon(v-w)\in \adh_\xi(C)-v$. Thus $v+ \epsilon(v-w)\in \adh_\xi(C)$. Also
...
\end{lemma}
\begin{proof}
This is immediate because $\neighbourhoods_\F(0)\cdot w \overset{\xi}{\longrightarrow} 0$ and $F-v\overset{\xi}{\longrightarrow}0$ implies $F\overset{\xi}{\longrightarrow} v$.
\end{proof}
...
\end{lemma}
\begin{proof}
If $V$ is 1D, then $V= \Span\{v\}$ for some/any $v\in V$ and $\neighbourhoods_F(0)\cdot w = \neighbourhoods_F(0)\cdot v$ for all $w\in v$. Thus $\sSet{V, \mathfrak{a}}$ is homeomorphic with $\F$ and $(2),(3),(4),(5),(6)$ follow.

Now $(2)\Rightarrow (3) \Rightarrow (4) \Rightarrow (5)$ is immediate by \ref{depthImplications}, so we prove $(5)\Rightarrow (1)$. Take arbitrary $v,w\in V$. Then $\neighbourhoods_\F(0)\cdot v \cap \neighbourhoods_\F(0)\cdot w$ converges to $0$ by finite depth, so there exists $u\in V$ such that $\neighbourhoods_\F(0)\cdot u \subseteq \neighbourhoods_\F(0)\cdot v\cap \neighbourhoods_\F(0)\cdot w$. Thus for any $\epsilon > 0$, there exist $\epsilon_1, \epsilon_2 >0$ such that
...
\item $\vicinities_\mathfrak{a}(v) = v+\vicinities_\mathfrak{a}(0)$;
\item $\begin{aligned}[t]
\vicinities_\mathfrak{a}(0) &= \bigcap_{v\in V} \upset \neighbourhoods_\F(0)\cdot v \\
&= \setbuilder{B\in \powerset(V)}{\forall v\in V: \exists \Gamma_v\in \neighbourhoods_\F(0):\; \Gamma_v\cdot v\subseteq B} \\
&= \setbuilder{\bigcup_{v\in V} \Gamma_v\cdot v}{\forall v\in V:\; \Gamma_v \in \neighbourhoods_\F(0)};
\end{aligned}$
\item $\inh_\mathfrak{a}(A) = \setbuilder{x\in V}{\forall v\in V:\exists \Gamma_v \in \neighbourhoods_\F(0):\; x + \Gamma_v\cdot v \subseteq A}$;
\item $\adh_\mathfrak{a}(A) = \setbuilder{x\in V}{\exists v\in V: \forall \Gamma\in\neighbourhoods_\F(0):\; (x+\Gamma\cdot v)\mesh A}$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1, 2, 3) are immediate from the definition and \ref{homeomorphismPreservation}.

(4) We have $\inh_\mathfrak{a}(A) = \setbuilder{x}{A\in \vicinities_\mathfrak{a}(x)} = \setbuilder{x}{A-x\in \vicinities_\mathfrak{a}(0)}$. From (1) we get 
\begin{align*}
\inh_\mathfrak{a}(A) &= \setbuilder{x}{\forall v\in V: \exists \Gamma_v\in \neighbourhoods_\F(0): \Gamma_v\cdot v \subseteq A-x} \\
...
\end{lemma}
\begin{proof}
We need to show that $\adh_\mathfrak{a}(U)\subseteq U$. Take $x\in \adh_\mathfrak{a}(U)$. Then take a $v\in V$ such that $\forall \Gamma\in\neighbourhoods_\F(0):\; (x+\Gamma\cdot v)\mesh U$.

Pick some $\Gamma\in\neighbourhoods_\F(0)$. Then $x+\lambda v\in U$ for some $\lambda\in \Gamma$. Then take $\ball(0,|\lambda|/2)\in \neighbourhoods_\F(0)$, so $x+\mu v\in U$ for some $\mu\in \ball(0,|\lambda|/2)$. In particular $\lambda \neq \mu$. If either $\lambda =0$ or $\mu = 0$, then $x\in U$ and we are done. Suppose $\lambda\neq 0 \neq \mu$. Then
...
\end{lemma}
\begin{proof}
We use \ref{pretopologicalContinuityVicinities} to show continuity. To that end take $K\in \neighbourhoods_\F(0)$. Then there exists $\epsilon >0$ such that $\ball(0,\epsilon)\subseteq K$ and so
...
\item $f$ is \udef{even} if $\forall x\in V: f(x) = f(-x)$;
...
\begin{lemma} \label{evenSubadditivePositivity}
Let $f:V\to \R$ be an even quasi-subadditive functional. Then $\im(f)\subseteq \R^+$.
\end{lemma}
...
Let $V$ be a vector space and $f: V\to \R$ an even subadditive function. Then $\forall v,w\in V$:
\begin{enumerate}
\item $|f(v)-f(w)|\leq f(v-w)$;
\item $|f(v)-f(w)|\leq f(v+w)$.
\end{enumerate}
\end{proposition}
\begin{proof}
We calculate $f(v) = f(v-w+w) \leq f(v-w) + f(w)$, so $f(v)-f(w)\leq f(v-w)$. By swapping $v\leftrightarrow w$ we also get $-f(v)+f(w)\leq f(w-v) = f(v-w)$, using the fact that $f$ is even, and thus the first inequality is established.

For the second inequality, set $w\to -w$ and use the fact that $f$ is even, so $f(-w) = f(w)$.
\end{proof}
...
\item $f$ is bounded on some vicinity $U\in\vicinities_\xi(v)$ for some point $v\in C$;
\item for all $w\in C$ there exists a vicinity $U'\in\vicinities_\xi(w)$ such that $f^\imf(U')$ has an upper bound.
\end{enumerate}
...
$(2) \Rightarrow (3)$ 
We have $\vicinities_\R\big(f(v)\big) \subseteq \upset f^{\imf\imf}\big(\vicinities_{\xi|_C}(v)\big)$ by \ref{continuityVicinityFilter}, so there exists $U\in \vicinities_{\xi|_C}(v)$ such that $f^\imf(U) \subseteq \interval{f(v)-1, f(v)+1}$. By \ref{subspaceVicinityFilter}, we may take $U\in \vicinities_\xi(v)$.

$(3) \Rightarrow (4)$ Pick arbitrary $w\in C$. The function $g: \lambda \mapsto w + \lambda(w-v)$ is continuous. By \ref{openClosedCriteria}, we can find an $A\in \vicinities_\xi(w)$ such that $A\subseteq C$. Then $g^\preimf(A)$ is a vicinity of $0$ by \ref{continuityVicinityFilter}. In particular, there exists $\epsilon \in g^\preimf(A)$ for some $\epsilon >0$, so $u = g(\epsilon) = w + \epsilon(w-v) \in A\subseteq C$. Rearranging gives $w = \frac{1}{1+\epsilon}u + \frac{\epsilon}{1+\epsilon}v$.

We may take $U\in\vicinities_\xi(v)$ such that $f^\imf(U)$ is bounded. Let $M$ be an upper bound of $f^\imf(U)$. We claim that $f$ is upper bounded on
...
$(4) \Rightarrow (1)$ Take arbitrary $w$. By \ref{pretopologicalContinuityVicinities}, it is enough to show $\vicinities_\R\big(f(w)\big) \subseteq \upset f^{\imf\imf}\big(\vicinities_\xi(w)\big)$.
...
\item $f$ is bounded on some $U\in\vicinities_\xi(0)$.
\end{enumerate}
\end{proposition}
\begin{proof}
The implication $(1) \Rightarrow (2)$ is immediate. 
...
\section{Extended positive real functionals}
\subsection{Extended positive reals}
\begin{definition}
We consider $\overline{\R^+}$ to be equipped with the order convergence. We define
\begin{itemize}
\item $a + \infty = \infty$ for all $a\in \overline{\R^+}$;
\item $a\cdot \infty = \infty$ for all $a\in \overline{\R^+} \setminus \{0\}$ and $0\cdot \infty = 0$.
\end{itemize}
For all $a,b\in\overline{\R^+}$, the expression $b-a$ is defined if and only if $b\geq a$. We set $\infty - \infty = 0$.
\end{definition}
Note that the multiplication in $\overline{\R^+}$ is no longer continuous.

\subsection{Seminorms}
\subsubsection{Definitions}
\begin{definition}
Let $V$ be a real or complex vector space. Let $f: V\to \overline{\R^+}$ be an extended real functional. The definitions of (quasi-)subadditive, even and point-separating also make sense in this context.
...
\end{itemize}
We also call $f$ \udef{finite} if $\infty \notin \im(f)$.
\end{definition}
Every subadditive and absolutely homogeneous has positive image, by \ref{evenSubadditivePositivity}, so this definition of seminorm is a strict generalisation of ``subadditive and absolutely homogeneous real-valued function''.

Note that `finite' does not mean `bounded', which is stronger.


\begin{lemma} \label{kernelSeminormVectorSpace}
Let $V$ be a vector space and $p:V\to \overline{\R^+}$ a seminorm. Then $\ker(p)$ is a subspace of $V$.
\end{lemma}
Note this does not follow from \ref{kernelSubspace} because seminorms are not linear.
\begin{proof}
We verify the subspace criterion \ref{subspaceCriterion}. First $0\in\ker(p)$ because $p(0) = p(0\cdot 0) = |0|p(0) = 0$.
...
Take $v,w\in f^{\preimf}(\R)$. As $\im(f) \subseteq \overline{\R^+}$ by \ref{evenSubadditivePositivity}, we have $0\leq f(v+w)$. Also $f(v+w)\leq K\big(f(v)+f(w)\big)\in \R$. Thus $f(v+w)\in\R$.

Take $\lambda\in\F$. Then $f(\lambda v) = |\lambda|f(v)\in \R$.
\end{proof}

\begin{proposition} \label{uniformContinuitySeminorms}
Let $\sSet{V,\xi}$ be a convergence vector space and $p: V\to \overline{\R^+}$ a seminorm. Then the following are equivalent:
\begin{enumerate}
...
$(2) \Rightarrow (3)$ We have that $p\circ \Delta$ is a pseudometric. We first show that $p\circ \Delta$ is a continuous pseudometric by taking $H\in \uniformity_V$. By definition of the uniformity, we have $\Delta^{\imf\imf}(H)\overset{\xi}{\longrightarrow} 0$, so $(p\circ \Delta)^{\imf\imf}(H) \overset{\overline{\R^+}}{\longrightarrow} 0$ by continuity of $p$ at $0$. Thus $H\in \uniformity_{p\circ \Delta}$ and $p\circ \Delta$ is continuous.
...
\end{proposition}
We do not allow $f$ to reach $\infty$.

Note that $f: V\to \R^+$ is open as a function with codomain $\R^+$, but not as a functiona with codomain $\R$. Indeed $f^\imf(V) = \interval[co]{0,\infty}$, which is open in $\R^+$ but not in $\R$.

We have a similar result for linear functions, see \ref{linearFunctionalOpen}.
\begin{proof}
It is enough to show $f$ is open when $V$ is equipped with the algebraic convergence, since every open set is algebraically open.
...
First suppose $f(x) \neq 0$. Because $A$ is open, $x\in \inh_\mathfrak{a}(A)$ and there exists $\Gamma\in \neighbourhoods_\F(0)$ such that $x+\Gamma\cdot x \subseteq A$ by \ref{constructionsInAlgebraicConvergence}. There exists $\epsilon > 0$ such that $\ball_\F(0,\epsilon) \subseteq f(x)\cdot \Gamma$ and $\epsilon \leq f(x)$. Now we calculate
\begin{align*}
...
so $tf(v) = f(x+tv)$
Because $A$ is algebraically open, $x\in \inh_\mathfrak{a}(A)$ and there exists $\Gamma\in \neighbourhoods_\F(0)$ such that $x+\Gamma\cdot v \subseteq A$ by \ref{constructionsInAlgebraicConvergence}. Now there exists $\epsilon > 0$ such that $\interval[co]{0,\epsilon} \subseteq \Gamma$, so
\[ \interval[co]{0,\epsilon f(v)} = \interval[co]{0,\epsilon}\cdot f(v) = f^\imf\big(x + \interval[co]{0,\epsilon}\cdot v\big) \subseteq f^\imf(A). \]
\end{proof}

\begin{example}
We cannot simply let $f$ reach $\infty$ in \ref{seminormOpenMap}. Consider, for example, the seminorm
\[ f: R^2 \to \overline{\R^+}: (x,y) \mapsto \begin{cases}
0 & (y = 0) \\
\infty & (\text{otherwise})
\end{cases} \]
This is a seminorm, but not an open map, since $\im(f) = \{0,\infty\}$.
\end{example}

\subsection{Gauges}
\begin{definition}
Let $V$ be a vector space and $A\subseteq V$ a subset. The function
...
Let $V$ be a vector space and $A\subseteq V$ a subset. For all $v\in V$ and $t\geq 0$:
...
We calculate
\begin{align*}
p_A(tv) &= \inf\setbuilder{\lambda\in \R^{+}}{tv\in \lambda A} \\
&= \inf\setbuilder{\lambda\in \R^{+}}{v\in t^{-1}\lambda A} = p_{t^{-1}A}(v) \\
&= \inf\setbuilder{t\lambda\in \R^{+}}{v\in \lambda A} \\
&= t\inf\setbuilder{\lambda\in \R^{+}}{v\in \lambda A} = tf(v).
\end{align*}
\end{proof}

\begin{lemma} \label{semibalancedClosureGauge}
Let $V$ be a vector space and $A\subseteq V$ a subset. Then $p_A = p_{\semibalanced(A)}$.
\end{lemma}
\begin{proof}
We calculate
\begin{align*}
\setbuilder{\lambda\in\R^+}{v\in \lambda \semibalanced(A)} &= \setbuilder{\lambda\in\R^+}{v\in \lambda\cdot\interval{0,1}\cdot A} \\
&= \setbuilder{\lambda\in\R^+}{\exists k\in \interval{0,1}:\; v\in \lambda k\cdot A} \\
&= \setbuilder{\lambda\in\R^+}{\exists \lambda'\leq \lambda:\; v\in \lambda' \cdot A} \\
&= \upset \setbuilder{\lambda'\in\R^+}{v\in \lambda' \cdot A}.
\end{align*}
Thus the infima of both $\setbuilder{\lambda\in\R^+}{v\in \lambda \semibalanced(A)}$ and $\setbuilder{\lambda'\in\R^+}{v\in \lambda' \cdot A}$ are the same.
\end{proof}

\begin{lemma} \label{gaugeLemma}
Let $V$ be a vector space, $A\subseteq V$ a subset and $\lambda\in \R^{> 0}$. Then
\begin{enumerate}
...
Let $V$ be a vector space, $f: V\to \R^{+}$ a positively homogeneous function and $A \subseteq V$ a semibalanced subset.
Then the following are equivalent:
\begin{enumerate}
\item $f = p_A$;
\item $f^{\preimf}\interval[co]{0,1} \subseteq A \subseteq f^{\preimf}\interval{0,1}$.
\end{enumerate}
\end{lemma}
\begin{proof}
$(1) \Rightarrow (2)$ We calculate, using \ref{gaugeLemma},
\begin{align*}
x\in p_{A}^\preimf\interval[co]{0,1} \iff& p_{A}(x) < 1 \\
\implies& x\in A \\
\implies& p_{A}(x) \leq 1 \\
\iff& x\in p_{A}^\preimf\interval{0,1}.
\end{align*}
...
&\leq \inf\setbuilder{\lambda\in \R^{\geq 0}}{v\in \lambda f^{\preimf}\interval[co]{0,1}} \\
&= \inf\setbuilder{\lambda\in \R^{\geq 0}}{v\in f^{\preimf}\interval[co]{0,\lambda}} \\
&= \inf\setbuilder{\lambda\in \R^{\geq 0}}{f(v) < \lambda} = f(v)
\end{aligned} \quad\text{and}\quad \begin{aligned}
p_A(v) &= \inf\setbuilder{\lambda\in \R^{\geq 0}}{v\in \lambda A} \\
&\geq \inf\setbuilder{\lambda\in \R^{\geq 0}}{v\in \lambda f^{\preimf}\interval{0,1}} \\
&= \inf\setbuilder{\lambda\in \R^{\geq 0}}{v\in f^{\preimf}\interval{0,\lambda}} \\
...
Let $V$ be a vector space and $f: V\to \overline{\R^{+}}$ a function.
Then the following are equivalent:
\begin{enumerate}
\item $f$ is positively homogenous;
\item $f = p_A$ for some semibalanced subset $A$.
\end{enumerate}
\end{proposition}
\begin{proof}
Assume $f$ positively homogeneous. Then we can set $A = f^{\preimf}\interval[co]{0,1}$ in \ref{gaugeClassificationLemma} because $f^{\preimf}\interval[co]{0,1}$ is semibalanced. Indeed
\[ \interval{0,1}\cdot f^{\preimf}\interval[co]{0,1} = f^{\preimf}\big(\interval{0,1}\cdot\interval[co]{0,1}\big) = f^{\preimf}\interval[co]{0,1}. \]

The converse is given by \ref{gaugeScaling}.
\end{proof}

\begin{lemma} \label{gaugeZeroLemma}
Let $V$ be a vector space, $A\subseteq V$ a subset and $a\in A$. If there exists a subspace $U\subseteq A$ such that $a\in U$, then $p_A(a) = 0$.
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{corollary} \label{gaugeAbsolutelyConvexSet}
The gauge of an absolutely convex subset is a seminorm. If the subset contains only the trivial subspace, then the gauge is a norm.
\end{corollary}
Note that the seminorm may be infinite! TODO: allow that in the definition of seminorm.

\begin{proposition} \label{continuityConvexGauge}
Let $\sSet{V, \xi}$ be a convergence vector space and $A\subseteq V$ a convex subset. Then $p_A: \sSet{V,\xi} \to \F$ is continuous \textup{if and only if} $A\in\vicinities_\xi(0)$.
\end{proposition}
\begin{proof}
First assume $A\in\vicinities_\xi(0)$. By \ref{gaugeProperties}, we have that $p_A$ is sublinear. By \ref{gaugeClassificationLemma}, we have that $A \subseteq p_A^\preimf[\cball(0,1)]$ and thus that $p_A$ is bounded on $A$. Then $p_A$ is continuous by \ref{sublinearContinuity}.

Now assume $p_A$ continuous. Then $\ball(0,1) \in \neighbourhoods_\F(0) = \neighbourhoods_\F(p_A(0))$, so $p_A^\preimf[\ball(0,1)] \in \vicinities_\xi(0)$ by \ref{continuityVicinityFilter}. By \ref{gaugeClassificationLemma} (using the fact that $A$ is semibalanced, \ref{convexSemibalanced}), $p_A^\preimf[\ball(0,1)] = p_A^\preimf\interval[co]{0,1} \subseteq A$, so $A\in\vicinities_\xi(0)$.
\end{proof}
\begin{corollary} \label{gaugeContinuousAlgebraicConvergence}
Let $V$ be a vector space. Then $p_A: \sSet{V,\mathfrak{a}} \to \F$ is continuous for any convex subset $A \subseteq V$.
\end{corollary}
\begin{proof}
By \ref{coreProperties}, we have $0\in\inh_\mathfrak{a}(A)$. Then $A\in \vicinities_\mathfrak{a}(0)$ by \ref{principalAdherenceInherence}.
\end{proof}

\begin{lemma} \label{absoluteFunctionalGauge}
Let $\sSet{V, \xi}$ be a convergence vector space and $f: V\to \F$ a linear functional. Then $f$ is continuous \textup{if and only if} $|f| = p_K$ for some $K\in\vicinities_\xi(0)$.
\end{lemma}
\begin{proof}
We have that $|f|$ is positively homogeneous. Set $K \defeq |f|^{\preimf}[\ball(0,1)] = |f|^{\preimf}(\interval[co]{0,1})$. Then $|f| = p_{K}$, by \ref{gaugeClassificationLemma}. 

First assume $f$ is continuous. Then $p_K^\preimf\big[\ball(0,1)\big] = K$ is in $\vicinities_\xi(0)$ by \ref{continuityVicinityFilter}.

Now assume $K\in\vicinities_\xi(0)$. As $K \subseteq p_K^\preimf\interval{0,1}$, by \ref{gaugeClassificationLemma}, we have that $|f|$ is bounded on $K$. It is also sublinear and thus continuous by \ref{sublinearContinuity}.
\end{proof}


\begin{proposition} \label{gaugeInherenceAdherence}
Let $V$ be a vector space and $A\subseteq V$ a semibalanced subset. Then
\begin{enumerate}
\item $\inh_\mathfrak{a}(A) \subseteq p_A^{\preimf}\interval[co]{0,1}$;
\item $p_A^{\preimf}\interval{0,1} \subseteq \adh_\mathfrak{a}(A)$.
\end{enumerate}
The inclusions are equalities if $A$ is convex.
\end{proposition}
\begin{proof}
(1) Take $x\in \inh_\mathfrak{a}(A)$. Then by \ref{constructionsInAlgebraicConvergence}, there exists $\Gamma\in\neighbourhoods_\F(0)$ such that $v+\Gamma\cdot v = (1 + \Gamma)\cdot v \subseteq A$. There exists $\lambda \in 1 + \Gamma$ that is real and strictly less than $1$. Thus $p_A(x) <1$, meaning $x\in p^\preimf_A\interval[co]{0,1}$.
...
\[ p_A^{\preimf}\interval[co]{0,1} = p_A^{\preimf}\Big[\interior_{\overline{\R^+}}\big(\interval[co]{0,1}\big)] \subseteq \inh_\mathfrak{a}\big(p_A^{\preimf}\interval[co]{0,1}\big) \subseteq \inh_\mathfrak{a}(A). \]

(2) Take $x\in p^\preimf_A\big(\interval{0,1}\big)$, which means that $p_A(x) \leq 1$, so $p_A(x) < 1+\epsilon$ for all $\epsilon > 0$. By \ref{gaugeLemma}, $(1+\epsilon)^{-1}x = x - \epsilon(1+\epsilon)^{-1}x \in A$. Because $\frac{\epsilon}{1+\epsilon} < \epsilon$, we have $x - \epsilon(1+\epsilon)^{-1}x \in x + \ball(0,\epsilon)\cdot x$ and so $x + \ball(0,\epsilon)\cdot x \mesh A$.

Now for all $\Gamma\in\neighbourhoods_\F(0)$, we have $\ball(0,\epsilon) \subseteq \Gamma$ for some $\epsilon >0$. Thus $x+ \Gamma\cdot x \mesh A$ and $x\in\adh_\mathfrak{a}(A)$ by \ref{constructionsInAlgebraicConvergence}.

(Convex) For the other inclusion we again use that $p_A$ is continuous by \ref{gaugeContinuousAlgebraicConvergence}. From \ref{gaugeClassificationLemma} we have $A \subseteq p^\preimf_A\interval{0,1}$. Thus, by \ref{adherenceInherenceContinuity},
\[ \adh_\mathfrak{a}(A) \subseteq \adh_\mathfrak{a}\big(p^\preimf_A\interval{0,1}\big) \subseteq p_A^\preimf\Big(\closure_{\overline{\R^+}}\big(\interval{0,1}\big)\Big) = p^\preimf_A\interval{0,1}. \]
\end{proof}
...
Because $f$ is non-zero, there exists a $v\in V$ such that $f(v) \neq 0$. Because $A$ is open, $x\in \inh_\mathfrak{a}(A)$ and there exists $\Gamma\in \neighbourhoods_\F(0)$ such that $x+\Gamma\cdot v \subseteq A$ by \ref{constructionsInAlgebraicConvergence}.
...
\item there exists a vicinity $U\in \vicinities_\xi(0)$ such that $f$ is bounded on $U$;
...
\begin{lemma} \label{localConvexityPropertyOfRealCVSs}
If $\sSet{V,\xi}$ is a complex convergence vector space. Then $\sSet{V,\xi}$ is locally convex \textup{if and only if} $\sSet{V_\R,\xi}$ is locally convex.
\end{lemma}
\begin{proof}
From \ref{convexityPropertyOfRealVectorSpaces}
\end{proof}
...
\end{lemma}
\begin{proof}
Take $v\in V$ and $A\in \neighbourhoods(v)$. Since $\xi$ is locally convex, $A$ contains a convex neighbourhood $B$ of $v$. Then $\interior(B)$ is convex by \ref{inherenceAdherenceConvex} and it is a neighbourhood of $v$ since $\xi$ is locally open.
\end{proof}
...
\item $\neighbourhoods_\xi(0) = \upset\disked^{\imf}\big(\neighbourhoods_\xi(0)\big)$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
$(3) \Leftrightarrow (4)$ Because $\disked$ preserves intersections (as it is a closure operator), we have that $\upset\disked^{\imf}\big(\neighbourhoods_\xi(0)\big)$ is a filter.

We automatically have $\neighbourhoods_\xi(0) \subseteq \upset\disked^{\imf}\big(\neighbourhoods_\xi(0)\big)$. If $\neighbourhoods_\xi(0)$ is based in the absolutely convex sets, then $\upset\disked^{\imf}\big(\neighbourhoods_\xi(0)\big)$ contains a base of $\upset\disked^{\imf}\big(\neighbourhoods_\xi(0)\big)$ and so the other inclusion holds.

If $\neighbourhoods_\xi(0) = \upset\disked^{\imf}\big(\neighbourhoods_\xi(0)\big)$, then $\neighbourhoods_\xi(0)$ is based in the absolutely convex sets, because clearly $\upset\disked^{\imf}\big(\neighbourhoods_\xi(0)\big)$ is.
\end{proof}

\begin{proposition} \label{LCTVSconstruction}
Let $V$ be a vector space and $N\in\powerfilters(V)$. Then $N = \neighbourhoods_\xi(0)$ for some locally convex topological convergence on $V$ \textup{if and only if}
\begin{enumerate}
...
\subsubsection{One seminorm}

\begin{lemma}
Let $V$ be a vector space and $p:V\to \overline{\R^+}$ a seminorm. Then $p$ is a group seminorm.
\end{lemma}
We also have that $p$ is cyclically permutable, since $V$ is an Abelian group.

\begin{proposition} \label{seminormConvergenceScalarMultiplicationContinuous}
Let $V$ be a vector space and $p:V\to \overline{\R^+}$ a seminorm. The group seminorm convergence on $V$ makes the scalar multiplication function
\[ \lambda \cdot -: V\to V: v\mapsto \lambda\cdot v \]
continuous for all $\lambda \in \F$.
\end{proposition}
Note that this is strictly weaker than saying that the scalar multiplication $\F\times V \to V: (\lambda, v) \mapsto \lambda \cdot v$ is continuous.
\begin{proof}
Take $F\in \powerfilters(V)$ and $v\in V$ such that $F\overset{p}{\longrightarrow} v$. Then
\[ \upset p^{\imf\imf}(\lambda\cdot F - \lambda \pfilter{v}) = \upset |\lambda|\cdot p^{\imf\imf}(F - \pfilter{v}) \to 0, \]
so $\lambda\cdot F \overset{p}{\longrightarrow} \lambda \pfilter{v}$, by \ref{metricConvergence}.
\end{proof}

\begin{proposition} \label{seminormConvergenceVectorSpaceConvergence}
Let $V$ be a vector space and $p:V\to \R^+$ a seminorm. The group seminorm convergence on $V$ makes $V$ a convergence vector space.
\end{proposition}
Note that now $p$ is not allowed to take the value $\infty$.
\begin{proof}
It is enough to show that the scalar multiplication $\F\times V \to V: (\lambda, v) \mapsto \lambda \cdot v$ is continuous.
As the convergence in $\F$ is topological, it is enough to consider the convergence of $\neighbourhoods_\F(\lambda)\cdot F$ for some $\lambda \in \F$ and $F\to v\in V$. 

For all $s \in \F$ and $x\in V$, we have
\begin{align*}
0\leq p(s\cdot x - \lambda v) &= p(s\cdot x - s\cdot v + s\cdot v - \lambda v) \\
&\leq p(s\cdot x - s\cdot v) + p(s\cdot v - \lambda v) \\
&= |s|\cdot p(x - v) + |s-\lambda|p(v),
\end{align*}
so
\[ \constant{0} \leq \Big((s,x) \mapsto p(s\cdot x - \lambda v)\Big) \leq \Big((s,x) \mapsto |s|\cdot p(x - v) + |s-\lambda|p(v)\Big) \]
pointwise. We have $\big|\neighbourhoods_\F(\lambda)\big|\cdot p^{\imf\imf}(F-\pfilter{v}) \to 0$ and $\big(\big|\neighbourhoods_\F(\lambda)\big| - \lambda\big)\pfilter{p}(v) \to 0$, by \ref{metricConvergence} and continuity of the multiplication in $\R$ (this is where the argument in $\overline{\R^+}$ breaks down). Since the seminorm convergence is a group convergence, we have $\big|\neighbourhoods_\F(\lambda)\big|\cdot p^{\imf\imf}(F-\pfilter{v}) + \big(\big|\neighbourhoods_\F(\lambda)\big| - \lambda\big)\pfilter{p}(v) \to 0$.

By the squeeze theorem for filters, \ref{squeezeTheoremFilters}, we have that $\upset p^{\imf\imf}\big(\neighbourhoods_\F(\lambda)\cdot F - \lambda \pfilter{v}\big) \to 0$ and thus $\neighbourhoods_\F(\lambda)\cdot F \overset{p}{\longrightarrow} \lambda \cdot v$.
\end{proof}
\begin{corollary} \label{seminormConvergenceInitialVectorSpaceConvergence}
Let $V$ be a vector space and $p:V\to \R^+$ a seminorm. The seminorm convergence is equal to the initial vector space convergence w.r.t. $\{p\}$.
\end{corollary}
\begin{proof}
By \ref{groupSeminormConvergence}, the seminorm convergence is equal to the initial group convergence w.r.t. $\{p\}$. Since this group convergence is also a vector space convergence, it is the initial vector space convergence.
\end{proof}
...
\end{example}
In fact the seminorm convergence is the initial convergence (among all convergences, not just vector convergences) iff $S = \{\underline{0}\}$.



\subsubsection{Sets of finite seminorms}
\begin{definition}
Let $V$ be a vector space and $S$ a set of finite seminorms on $V$. 
...
Let $V$ be a vector space, $S$ a set of finite seminorms on $V$, $F\in\powerfilters{V}$ and $v\in V$.
\begin{enumerate}
...
\begin{align*}
\neighbourhoods_\xi(0) &= \mathfrak{F}\setbuilder{p^\preimf[\ball(0,\epsilon)]}{p\in S, \epsilon > 0} \\
&= \mathfrak{F}\setbuilder{p^\preimf[\cball(0,\epsilon)]}{p\in S, \epsilon > 0}.
\end{align*}
\end{enumerate}
\end{proposition}
Note that $\neighbourhoods_\xi(0)$ is the same as the neighbourhood filter at $0$ in the initial (not necessarily vector space) convergence w.r.t. $S$.
\begin{proof}
...
Point (2) then follows from (TODO ref?) and \ref{metricConvergence}.
...
Let $V$ be a vector space and $S$ a set of finite seminorms on $V$. Let $\xi$ be the initial convergence on $V$ w.r.t. $S$. Then $f\in \sSet{V, \xi}^*$ \textup{if and only if}
\begin{itemize}
...
\[ f\in \sSet{V, \xi}^* \iff \exists D>0: \exists U\in \neighbourhoods_\xi(0):\; f^\imf[U] \subseteq \cball(0,D). \]
Now $U\in \neighbourhoods_\xi(0)$ iff there exists a finite $A = \{p_n^\preimf[\ball(0,\epsilon_n)]\}_{n=0}^N \subseteq \setbuilder{p^\preimf[\ball(0,\epsilon)]}{p\in S, \epsilon > 0}$ such that $\bigcap A\subseteq U$. So WLOG we may take $U$ of this form.
Now 
\begin{align*}
...
Let $V$ be a vector space. A topological convergence on $V$ is locally convex \textup{if and only if} it is the seminorm convergence w.r.t. some set $S$ of finite seminorms on $V$.
\end{proposition}
\begin{proof}
...
Now let $V$ be a locally convex TVS, so there exists an absolutely convex base $\mathcal{B}$ of $\neighbourhoods(0)$ by \ref{locallyConvexNeighbourhoodsLemma}.

Since $\{\inh(B)\}_{B\in\mathcal{B}}$ is a basis of $\neighbourhoods(0)$ and $\inh(B) \subseteq \inh_\mathfrak{a}(B)$ by \ref{principalInherenceAdherenceProperties}, we may replace $\mathcal{B}$ by $\{\inh_\mathfrak{a}(B)\}_{B\in\mathcal{B}}$.
...
In order to show that the convergence on $V$ is the seminorm convergence w.r.t. $S$, we verify the form of $\neighbourhoods(0)$ given in \ref{initialSeminormConvergence}.

Since $\epsilon B\in \neighbourhoods(0)$ for each $B\in\mathcal{B}$ and all $\epsilon > 0$, we have that $\setbuilder{\epsilon B}{\epsilon > 0, B\in \mathcal{B}}$ is a basis of $\neighbourhoods(0)$.
Then
\begin{align*}
\neighbourhoods(0) &= \mathfrak{F}\setbuilder{\epsilon B}{\epsilon > 0, B\in \mathcal{B}} \\
...
\begin{enumerate}
\item $\neighbourhoods_{\lconvMod(\xi)}(0) = \upset \bigcup_{p\in S} p^{\preimf\imf}\big(\neighbourhoods_\R(0)\big) = \upset \setbuilder{p^{\preimf}\big(\ball_\R(0, \epsilon)\big)}{p\in S, \epsilon > 0}$;
\item $\neighbourhoods_{\lconvMod(\xi)}(0) = \upset\disked^{\imf}\big(\vicinities_\xi(0)\big)$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
We may simply take $\epsilon = \min\{\epsilon_1, \epsilon_2\}$ and $q = p_1+p_2$, which is continuous. Indeed, $x\in (p_1+p_2)^\preimf\big(\ball_\R(0,\epsilon)\big)$ is equivalent to $p_1(x) + p_2(x) < \epsilon$, which, by positivity of the seminorm \ref{evenSubadditivePositivity}, implies $p_1(x)\leq \epsilon \leq \epsilon_1$ and $p_2(x)\leq \epsilon \leq \epsilon_2$.

(2) By \ref{locallyConvexNeighbourhoodsLemma}, $\neighbourhoods_{\lconvMod(\xi)}(0)$ has a base of absolutely convex sets. As $\vicinities_\xi(0)\subseteq\neighbourhoods_{\lconvMod(\xi)}(0)$, we have $\upset\disked^{\imf}\big(\vicinities_\xi(0)\big) \subseteq \neighbourhoods_{\lconvMod(\xi)}(0)$. The show the converse, we just need to prove that the pretopological convergence with vicinity filter $\upset\disked^{\imf}\big(\vicinities_\xi(0)\big)$ at $0$ is a locally convex topological vector space convergence. We prove this by verifying the three points of \ref{LCTVSconstruction}.

First, take $A\in \vicinities_\xi(0)$. Then for all $\lambda\in\F\setminus\{0\}$, $\lambda A \in \vicinities_\xi(0)$ by \ref{vicinityFilterAtOrigin}. As $\disked(\lambda A) = \lambda\disked(A)$, by \ref{balancedHullHomogeneous}, \ref{convexHullHomogeneous} and \ref{diskedIsCoBal}, we have $\lambda \disked(A)\in \upset\disked^{\imf}\big(\vicinities_\xi(0)\big)$.

Secondly, each $A\in \vicinities_\xi(0)$ is absorbent by \ref{vicinityFilterAtOrigin}. As $A\subseteq \disked(A)$, $\disked(A)$ is absorbent by \ref{absorbingSetProperties} and so each element of $\upset\disked^{\imf}\big(\vicinities_\xi(0)\big)$ is absorbent.
...
\[ \upset \evalMap^{\imf\imf}\big(\{H\}\otimes \neighbourhoods_{\lconvMod(\xi)}(0)\big) \overset{\zeta}{\longrightarrow} 0 \]
...
&= \upset\evalMap^{\imf\imf}\Big(\{H\}\otimes \bigcap_{F\overset{\xi}{\longrightarrow} 0} F\Big) = \upset\evalMap^{\imf\imf}\big(\{H\}\otimes \vicinities_\xi(0)\big)
\end{align*}
...
\[ \upset\disked^\imf\Big(\evalMap^{\imf\imf}\big(\{H\}\otimes \vicinities_\xi(0)\big)\Big) \subseteq \upset \evalMap^{\imf\imf}\Big(\{H\}\otimes \disked^{\imf}\big(\vicinities_\xi(0)\big)\Big) = \upset \evalMap^{\imf\imf}\big(\{H\}\otimes \neighbourhoods_{\lconvMod(\xi)}(0)\big), \]
...
\end{corollary}
\begin{proof}
We have that $|f| = p_K$ for some $K\in \vicinities_U(0)$ by \ref{absoluteFunctionalGauge}. 
By continuity of the inclusion map, we can find an $M \in \vicinities_V(0)$ such that $M\cap U = K$. Then $|f|\leq p_M$ and $f$ can be extended by the Hahn-Banach extension theorem to a functional $f'$ defined in the whole of $V$. Then $f'$ is bounded on $M$ and thus continuous by \ref{boundedOnVicinityImpliesContinuous}.
\end{proof}
...
Then $f^\imf[U]$ is open by \ref{linearFunctionalOpen} and thus in $\neighbourhoods_\F\big(f(v)\big)$. It is however disjoint from $f^\imf[B]$, which means that $f(v) \notin \overline{f^\imf[B]}$.
\end{proof}
\begin{corollary}
...
\begin{enumerate}
\item If $A$ is von Neumann bounded, then $A$ is absorbed by all vicinities of the origin (i.e.\ by all elements of $\vicinities_{\xi}(0)$).
...
\item $\forall U\in \neighbourhoods_{\xi}(0): \exists r > 0: \forall c \geq r: \; A \subseteq cU$;
\item $\{\interval{0,\epsilon}\}_{\epsilon >0}\cdot \{A\} \overset{\xi}{\longrightarrow} 0$.
\end{enumerate}
\end{enumerate}
\end{lemma}
\begin{proof}
We just need to show that ``$A$ is absorbed by all vicinities of the origin'' is equivalent to $\vicinities_\xi(0) \subseteq \neighbourhoods_\F(0) \cdot \{A\}$. Indeed $A$ is absorbed by $V\in \vicinities_\xi(0)$ iff there exists $\epsilon >0$ such that $\ball(0,\epsilon)\cdot A \subseteq V$ (see \ref{absorbingSetLemma}). Then (1) follows from the fact that $\vicinities_\xi(0)$ is smaller than all convergent filters.

(2) The implication $(a) \Rightarrow (b)$ is given by point (1).

$(b)\Rightarrow (a)$ As above, $(b)$ implies $\neighbourhoods_\xi(0) \subseteq \neighbourhoods_\F(0) \cdot \{A\}$. By topologicity this implies that $\neighbourhoods_\F(0) \cdot \{A\}$ converges and thus that $A$ is von Neumann bounded.
...
$(d) \Rightarrow (c)$ Pick a balanced base of $\neighbourhoods_{\xi}(0)$ (which is possible due to \ref{vicinityFilterAtOrigin}). Then the result follows immediately from the observation that $cU = |c|U$ (\ref{balancedLemma}) for all balanced sets.

$(d) \Leftrightarrow (e)$ We have that $\forall c \geq r: A\subseteq cU$ is equivalent to $\interval{0,r^{-1}}\cdot A \subseteq U$. Thus $(d)$ is equivalent to $\neighbourhoods_\xi(0)\subseteq \{\interval{0,\epsilon}\}_{\epsilon >0}\cdot \{A\}$, which is equivalent to $(e)$ because $\xi$ is topological.
\end{proof}
...
\end{lemma}
\begin{proof}
(1) We have $\neighbourhoods_\F(0) \cdot^{\imf\imf} \upset\{A\} \subseteq \neighbourhoods_\F(0) \cdot^{\imf\imf} \upset\{C\}$.

(2) It is enough to show that $\{v\}$ is von Neumann bounded for all $v\in V$. We have that $\neighbourhoods_\F(0) \cdot^{\imf\imf} \pfilter{v}$ converges for all $v\in V$ by continuity of scalar multiplication and the fact that $\pfilter{v}$ converges. 

(3) We have, by \ref{balancedClosures},
\[ \neighbourhoods_\F(0) \cdot \upset\{A\} = \upset \{\cball(0,\epsilon) \cdot A\}_{\epsilon >0} = \upset \{\cball(0,\epsilon)\cdot \cball(0,1) \cdot A\}_{\epsilon >0} = \upset \{\cball(0,\epsilon)\cdot \balanced(A)\}_{\epsilon >0} = \neighbourhoods_\F(0) \cdot \upset\{\balanced(A)\} \]

(4,5) Immediate by continuity of scalar multiplication and addition.

(6) We have $\neighbourhoods_\F(0)\cdot^{\imf\imf}\upset\{A\cup B\} = \neighbourhoods_\F(0)\cdot^{\imf\imf}\upset\{A\} \cap \neighbourhoods_\F(0)\cdot^{\imf\imf}\upset\{B\}$.

(7) By \ref{vicinityFilterAtOrigin}, we may take a closed and balanced base of $\vicinities_{\xi}(0)$.  
...
\end{proposition}
\begin{proof}
(1) As $A$ is von Neumann bounded set, we have $\neighbourhoods_\F(0) \cdot \upset\{A\} \overset{\xi}{\longrightarrow} 0$. By continuity and linearity of $f$, we have
\[ \neighbourhoods_\F(0) \cdot \upset\big\{f^{\imf}(A)\big\} = \upset f^{\imf\imf}\big(\neighbourhoods_\F(0)\cdot \{A\}\big) \overset{\zeta}{\longrightarrow} 0, \]
so $f^\imf(A)$ is von Neumann bounded.

(2) We have $\neighbourhoods_\F(0) \cdot \upset\{A\} \subseteq \{\interval{0,\epsilon}\}_{\epsilon >0} \cdot \upset\{A\} \overset{\xi}{\longrightarrow} 0$. By positive homogeneity and continuity at $0$, we have
...
\[ \neighbourhoods_\xi(0) = \neighbourhoods_\F(0) \cdot\upset\{U\} = \upset\{\epsilon U\}_{\epsilon >0}. \]
\end{lemma}
TODO does this imply topological for equable CVSs?
\begin{proof}
As $U$ is von Neumann bounded, we have $\neighbourhoods_\xi(0) \subseteq \neighbourhoods_\F(0) \cdot \upset\{U\}$. The converse inclusion is given by the fact that $U\in \neighbourhoods_\xi(0)$, so $\upset\{U\}\subseteq \neighbourhoods_\xi(0)$ and $\neighbourhoods_\F(0) \cdot \upset\{U\} \subseteq \neighbourhoods_\F(0) \cdot\neighbourhoods_\xi(0) = \neighbourhoods_\xi(0)$ by \ref{TVSEquable}.
...
\[ \neighbourhoods_\xi(0) = \neighbourhoods_\F(0) \cdot\upset\{U'\} = \upset \{\cball(0,\epsilon)\cdot U'\}_{\epsilon > 0} = \upset \{\epsilon\cball(0,1)\cdot U'\}_{\epsilon > 0} = \upset \{\epsilon U'\}_{\epsilon > 0}. \]
Then we have
\[ \neighbourhoods_\xi(0) = \neighbourhoods_\F(0) \cdot\upset\{U\} \subseteq \upset \{\epsilon U\}_{\epsilon > 0} \subseteq \upset \{\epsilon U'\}_{\epsilon > 0} = \neighbourhoods_\xi(0). \]
\end{proof}
...
\end{corollary}
\begin{proof}
(1) Clearly $\neighbourhoods_\xi(0)$ has a countable base, so the neighbourhood filter at every point has a countabel base by \ref{shiftHomeomorphism}, which means that $\xi$ is second countable. It is also regular by \ref{topologicalGroupsRegular} and this pseudometrisable by Urysohn's metrisation theorem \ref{UrysohnMetrisationTheorem}.
...
\begin{enumerate}
\item if there exists $U\in \vicinities_\xi(0)$ such that $f^\imf(U)$ is bounded, then $f$ is continuous at the origin;
\item if $f$ is linear, then this implies the continuity of $f$ everywhere.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) By \ref{pretopologicalContinuityVicinities}, it is enough to verify $\neighbourhoods_\zeta(0) \subseteq \upset f^{\imf\imf}\big(\vicinities_\xi(0)\big)$. Indeed, by \ref{boundedSetVicinityBase}, we have
\[ \neighbourhoods_\zeta(0) = \upset \{\epsilon f^\imf(U)\}_{\epsilon > 0} = \upset f^{\imf\imf}\big(\{\epsilon U\}_{\epsilon > 0}\big) \subseteq \upset f^{\imf\imf}\big(\vicinities_\xi(0)\big), \]
...
\begin{enumerate}
\item $f$ is continuous at $0$ \textup{if and only if} $f$ is bounded on some $U\in \vicinities_\xi(0)$;
...
For the converse, assume $f$ continuous at $0$, then $\ball\big(0,1\big)\in \neighbourhoods_W(0) = \neighbourhoods_W\big(f(0)\big) \subseteq f^{\imf\imf}[\vicinities_\xi(0)]$ by \ref{continuityVicinityFilter}. So there exists $U\in \vicinities_\xi(0)$ such that $f^{\imf}[U] \subseteq \ball(0,1)$, which means that $f$ is bounded by $1$ on $U$.
...
\end{proposition}
\begin{proof}
(1) We have, by \ref{initialFinalConvergence}, that $\neighbourhoods_\F(0)\cdot\upset \{B\}$ converges to $0$ in the initial convergence if and only if $f_i^{\imf\imf}\big(\neighbourhoods_\F(0)\cdot\upset \{B\}\big) = \neighbourhoods_\F(0)\cdot\upset \big\{f_i^{\imf}(B)\big\} \overset{\xi_i}{\longrightarrow} 0$. I.e\ $f_i^\imf(B)$ is bounded for each $i\in I$.
...
Now $\Delta^\preimf(U')\in \entourages_\xi$ by \ref{entourageConvergenceGroup}. So by total boundedness (and \ref{topologicalBoundednessLemma}), there exists a finite set $S\subseteq V$ such that $B\subseteq \bigcup_{v\in S}\Delta^\preimf(U')v$. By \ref{deltaPreimageLemma}, we have
...
Now assume $B$ is von Neumann bounded. We show boundedness using \ref{topologicalBoundednessLemma}, so take $A\in \entourages_\xi$. Then there exists $U\in \neighbourhoods_\xi(0)$ such that $\Delta^\preimf(U)\subseteq A$.
...
\begin{proof}
Let $K\subseteq V$ be a compact set. We need to show that $\neighbourhoods_\F(0)\cdot K$ converges to $0$. Since $\xi$ is pseudotopological, it is enough to show that any larger ultrafilter converges to $0$. Take such an ultrafilter $G\supseteq \neighbourhoods_\F(0)\cdot K$. Now $\cball(0,\epsilon)\cdot K = \cdot^{\imf}\big(\cball(0,1)\times K\big)$ is compact for all $\epsilon > 0$ by Tychonoff's theorem \ref{TychonoffsTheorem} and \ref{compactConstructions}.
...
\[ \upset \evalMap^{\imf\imf}(\{H\}\otimes F) \supseteq \upset \evalMap^{\imf\imf}(\{H\}\otimes G) = \upset \evalMap^{\imf\imf}(\{H\}\otimes \neighbourhoods_\F(0)\cdot G) = \upset \evalMap^{\imf\imf}(\neighbourhoods_\F(0)\cdot \{H\}\otimes G). \]
Since $\neighbourhoods_\F(0)\cdot \{H\}$ continuously converges to $0$ (by the vN boundedness of $H$), we have $\upset \evalMap^{\imf\imf}(\neighbourhoods_\F(0)\cdot \{H\}\otimes G)\to 0$.
...
\begin{proof}
If $B$ is $\xi$-von Neumann bounded, then $\neighbourhoods_\F(0)\cdot\{B\} \subseteq \neighbourhoods_\F(0)\cdot\{B\}$, so
\[ \neighbourhoods_\F(0)\cdot\{B\} \overset{\mackeyMod(\xi)}{\longrightarrow} 0, \]
which means that $B$ is $\mackeyMod(\xi)$-von Neumann bounded.

Now assume $B$ is $\mackeyMod(\xi)$-von Neumann bounded. Then $\neighbourhoods_\F(0)\cdot\{B'\} \subseteq \neighbourhoods_\F(0)\cdot\{B\}$ for some $\xi$-von Neumann bounded $B'\subseteq V$. As $\neighbourhoods_\F(0)\cdot\{B'\}$ $\xi$-converges to $0$, $\neighbourhoods_\F(0)\cdot\{B\}$ must do so as well, which means that $B$ is $\xi$-von Neumann bounded.
\end{proof}
...
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ Let $F\overset{\mackeyMod(\xi)}{\longrightarrow} 0$. Then there exists $B\in \vNborn{\xi}$ such that $\neighbourhoods_\F(0)\cdot \{B\} \subseteq F$. We have 
\[ \upset f^{\imf\imf}(F) \supseteq \upset f^{\imf\imf}\big(\neighbourhoods_\F(0)\cdot \{B\}\big) = \neighbourhoods_\F(0)\cdot \{f^\imf(B)\}. \]
...
$(3) \Rightarrow (1)$ Take arbitrary $B\in \vNborn{\xi}$, so $\neighbourhoods_\F(0)\cdot \{B\}\overset{\mackeyMod(\xi)}{\longrightarrow} 0$. Then
\[ \neighbourhoods_\F(0)\cdot \{f^\imf(B)\} = \upset f^{\imf\imf}\big(\neighbourhoods_\F(0)\cdot \{B\}\big) \overset{\zeta}{\longrightarrow} 0, \]
...
\[ \neighbourhoods_{\sigma(X,Y)}(0) = \mathfrak{F}\setbuilder{y\in Y}{\exists x\in X: \abspair{x,y} \leq 1}. \]
\end{enumerate}
\end{lemma}
\begin{proof}
...
\[ \neighbourhoods(0) \defeq \mathfrak{F}\setbuilder{\lambda A^\pol}{\lambda\in \F, A\in \mathcal{A}} \]
...
\[ \neighbourhoods_{\sigma^*(X,Y)}(0) = \mathfrak{F}\setbuilder{\{y\}^\pol}{y\in Y} = \upset\setbuilder{F^\pol}{\text{$F\subseteq Y$ finite}} \]
\end{proposition}
\begin{proof}
...
\end{theorem}
\begin{proof}
$(1) \Rightarrow (2)$ By \ref{lconvModPolarTopology}, $\zeta$ is the polar topology determined by $\setbuilder{U^\pol}{U\in \neighbourhoods_\zeta(0)}$, which is a set of absolutely convex $\sigma(\dual{V}, V)$-compact (= $\sigma(\dual{\sSet{V, \xi}}, V)$-compact = $\sigma(\dual{\sSet{V, \zeta}}, V)$-compact) subsets of $\dual{V}$, by \ref{polarPropertiesLemma} and \ref{alaogluTheorem}.
...
\begin{align*}
\upset\evalMap^{\imf\imf}\big(\{H\}\otimes \neighbourhoods_\xi(0)\big) \overset{\F}{\longrightarrow} 0 &\iff \forall \epsilon >0: \exists U\in \neighbourhoods_\xi(0): \; \evalMap^\imf(H\times U) \subseteq \cball(0,\epsilon) \\
&\iff \forall \epsilon >0: \exists U\in \neighbourhoods_\xi(0): \forall f\in H: \forall x\in U: \; \abspair{f,x}\leq \epsilon \\
&\iff \forall \epsilon >0: \exists U\in \neighbourhoods_\xi(0): \forall f\in H: \forall x\in U: \; \abspair{f,\epsilon^{-1}x}\leq 1 \\
&\iff \forall \epsilon >0: \exists U\in \neighbourhoods_\xi(0): \;H\subseteq (\epsilon^{-1}U)^\pol = \epsilon U^\pol.
\end{align*}
In particular, setting $\epsilon =1$, there exists $U\in \neighbourhoods_\xi(0)$ such that $H\subseteq U^\pol$. 

Conversely, if $H\subseteq U^\pol$, then for all $\epsilon > 0$, we have $H \subseteq \epsilon (\epsilon U)^\pol$ and $\epsilon U\in \neighbourhoods_\xi(0)$ by \ref{vicinityFilterAtOrigin}.
\end{proof}
\begin{corollary}
...
\end{corollary}
\begin{proof}
The equicontinuity of $A$ is equivalent to being a subset of $U^\pol$ for some $U\in \neighbourhoods_{\sigma(X,Y)}(0)$ by the proposition. Now $U^\pol$ is compact by \ref{alaogluTheorem}, and so $\overline{A}^{\sigma(X,Y)}$ is compact by \ref{compactClosedSets}.
\end{proof}
...
\begin{proof}
By \ref{vicinityFilterAtOrigin}, we $\neighbourhoods_{\lconvMod(\xi)}(0)$ has a base of closed balanced sets. These sets are also weakly closed by \ref{weakClosureConvexSubsets}. For each $U$ in this base, we have $U = U^{\pol\pol}$ by the bipolar theorem \ref{bipolarTheorem} (applied to the pairing $\sSet{V,\dual{V}, \evalMap}$).
...
Since $\neighbourhoods_\F(0)\cdot v\to 0$, we have, by equicontinuity \ref{equicontinuityGroupHomomorphisms}, that
\[ \neighbourhoods_\F(0)\cdot \{\evalMap_x^\imf(H)\} = \neighbourhoods_\F(0)\cdot \evalMap^{\imf\imf}(\{H\}\otimes \pfilter{x}) = \evalMap^{\imf\imf}\big(\{H\}\otimes \neighbourhoods_\F(0)\cdot \pfilter{x}\big) \to 0, \]
...
\begin{definition}
Let $\sSet{V,\xi}$ be a convergence vector space. It is called \udef{reflexive} if $\evalMap_-$ is a homeomorphism.
\end{definition}
...
\[ \neighbourhoods_{\dual{V}_c}(0) = \upset\setbuilder{K^\pol}{\text{$K\subseteq V$ compact}}. \]
\end{proposition}
\begin{proof}
