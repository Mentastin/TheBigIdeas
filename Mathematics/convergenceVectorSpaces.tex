...
\item $\{v\}$ is von Neumann bounded for all $v\in V$;
\end{enumerate}
...
\end{lemma}
\begin{proof}
(1,2) Immediate by continuity of scalar multiplication and addition.
...
(6,7) By \ref{vicinityFilterAtOrigin}, we may take a closed and balanced base of $\vicinity_{\xi}(0)$.  
...
\subsection{Subsets of function spaces}

\begin{proposition}
Let $\sSet{V,\xi}$ be an equable convergence vector space and $H\subseteq \contLin(V)$. Then $H$ is von Neumann bounded \textup{if and only if} it is compactoid.
\end{proposition}
\begin{proof}
TODO
\end{proof}
...
\end{proposition}
\begin{proof}
By \refl{compactFiniteSubcover} the convergence cover of von Neumann bounded subsets has a finite subset that covers $K$. This finite union of von Neumann bounded sets is von Neumann bounded by \ref{vonNeumannBoundedSetLemma} and thus the subset $K$ is von Neumann bounded, also by \ref{vonNeumannBoundedSetLemma}.
\end{proof}

\section{Bornologies}
\begin{definition}
Let $V$ be a vector space. A set $\mathcal{B}\subseteq \powerset(V)$ of subsets of $V$ is called a \udef{bornology} if
\begin{itemize}
\item it is an ideal;
\item it covers $V$.
\end{itemize}
It is called a \udef{vector bornology} if, in addition,
\begin{itemize}
...
\end{itemize}
If is called a \udef{convex vector bornology} if, in addition,
\begin{itemize}
\item $\convex(A) \in \mathcal{B}$ for all $A\in \mathcal{B}$.
\end{itemize}
\end{definition}
TODO move to section on convergence: bounded and totally bounded sets form bornology in general uniform space.

\subsection{Convergence derived from bornology}

\subsubsection{The Mackey modification}
...
\chapter{Operators on topological vector spaces}
