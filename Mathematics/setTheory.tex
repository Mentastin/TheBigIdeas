...
If $A$ is a set, there is a set of all equivalence classes. This is called the \udef{quotient set} of $A$ by $\sim$
...
\begin{definition}
Let $A$ be a class and $f: A\times A \to A$ a partial binary function. We call $f$ \udef{associative} if $\forall x,y,z\in A$ we have
\begin{itemize}
\item $f(f(x,y),z) = f(x,f(y,z))$ if $f(x,y)$ and $f(y,z)$ are defined;
\item if $f(x,y)$ is undefined, then $f(x,f(y,z))$ is also undefined;
\item if $f(y,z)$ is undefined, then $f(f(x,y), z)$ is also undefined.
\end{itemize}
\[ \begin{cases}
f(f(x,y),z) = f(x,f(y,z)) & \text{if $f(x,y)$ and $f(y,z)$ are defined} \\
\text{$f(f(x,y),z)$ and $f(x,f(y,z))$ are undefined} & \text{otherwise.}
\end{cases} \]
\end{definition}
...
\subsubsection{Green's relations and principal ideals}
\begin{definition}
Let $A$ be a class, $f: A\times A \to A$ a binary function and $x, y\in A$. TODO
\end{definition}
