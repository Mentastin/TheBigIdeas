...
\begin{enumerate} \setcounter{enumi}{2}
\item $f(A, x) \subseteq f(A,y)$ \textup{if and only if} $\exists b\in A: \; f(e, x) = f(b, y)$;
\item $f(x, A) \subseteq f(y, A)$ \textup{if and only if} $\exists b\in A: \; f(x, e) = f(y, b)$.
\end{enumerate}
\end{lemma}
...
And thus
\begin{enumerate} \setcounter{enumi}{2}
\item $a\greensL b$ \textup{if and only if} $\exists x,y\in \widetilde{A}: (f(x, a) = b) \land (a = f(y, b))$;
\item $a\greensR b$ \textup{if and only if} $\exists x,y\in \widetilde{A}: (f(a, x) = b) \land (a = f(b, y))$.
\end{enumerate}
\end{lemma}
