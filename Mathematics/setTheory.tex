...
\item $(f^\preimf,f^\imf)$ is a Galois connection between $\sSet{\powerset(A), \subseteq}$ and $\sSet{\powerset(B), \subseteq}$;
\item $(f^\imf,f^\preimf)$ and $(f^\preimf,f^\imf)$ are Galois connections between $\sSet{\powerset(A), \id_{\powerset(A)}}$ and $\sSet{\powerset(B), \id_{\powerset(B)}}$.
\end{enumerate}
\end{proposition}
\begin{proof}
We have
\[ f^\imf(X) \subseteq Y \iff \forall x\in X: \exists y\in Y:\; f(x) = y \iff X \subseteq f^{\preimf}(Y) \]
and
\[ f^\imf(X) \supseteq Y \iff \forall y\in Y: \exists x\in X:\; f(x) = y \iff X \supseteq f^{\preimf}(Y). \]
\end{proof}
