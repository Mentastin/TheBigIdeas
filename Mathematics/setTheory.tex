...
\item $\chi_{A^c} = \underline{1}-\chi_A$; and thus $\chi_A + \chi_{A^c} = 1$;
...
\end{itemize}
An \udef{ordered set} is a pair $\seq{P,\Yleft}$ such that $P$ is a set and $\Yleft$ is an order on $P$. We call this ordered set
\begin{itemize}
...
If we say $\seq{P,\Yleft}$ is an ordered set without any other qualifiers, we only assume $\Yleft$ is transitive.

\begin{example}
\begin{itemize}
\item Every equivalence relation is a preorder. Conversely, symmetric preorders are equivalence relations.
\item Let $A$ be a set and $\id_A$ the identity relation on $A$. Then $\seq{A,\id_A}$ is a poset. Such posets are called \udef{discrete posets}.
...
\subsection{Subsets of ordered sets}
\subsubsection{Up- and down-sets}
\begin{definition}
Let $\seq{P, \Yleft}$ be an ordered set and $Q\subseteq P$. We call
\begin{itemize}
\item $Q$ an \udef{up-set} or \udef{order filter} if $\forall x\in Q: \forall y\in P: x\Yleft y \implies y\in Q$;
\item $Q$ a \udef{down-set} or \udef{order ideal} if $\forall x\in Q: \forall y\in P: y\Yleft x \implies y\in Q$.
\end{itemize}
We define
\[ \upset Q \defeq \setbuilder{y\in P}{\exists x\in Q: x\Yleft y} \qquad \text{and} \qquad \downset Q \defeq \setbuilder{y\in P}{\exists x\in Q: y\Yleft x} \]
and, for $x\in P$,
\begin{itemize}
...
\begin{lemma}
Let $\seq{P,\Yleft}$ be an ordered set and $x\in P$. Then
\begin{enumerate}
...
Let $\seq{P,\precsim}$ be a preordered set and $Q\subseteq P$. Then
\begin{enumerate}
...
\begin{lemma}
Let $\seq{P,\Yleft}$ be an ordered set and $Q,R\subseteq P$. Then
\begin{enumerate}
...
\begin{lemma}
Let $\seq{P, \Yleft}$ be an ordered set and $Q\subseteq P$. Then
\begin{enumerate}
...
\begin{lemma}
If $\seq{P, \precsim}$ is a poset and $S\subseteq P$, then $\max(S), \min(S), \sup(S), \inf(S)$ are either singletons or empty.
\end{lemma}
\begin{proof}
...
If $\seq{P, \precsim}$ is a poset and $S\subseteq P$, then
\begin{enumerate}
...
\begin{definition}
An ordered set $(D,\Yleft)$ is a \udef{directed set} if any pair of elements $a,b\in D$ has an upper bound. We call the relation $\Yleft$ a \udef{direction}.
\end{definition}
\begin{proposition}
...
\subsection{Functions on ordered sets}
\begin{definition}
...
\end{proof}
TODO: reverse image.
\begin{corollary} \label{corollary:orderPreservingReversingBounds}
Let $(P, \precsim_P)$ and $(Q, \precsim_Q)$ be prosets, $f: P\to Q$ a function and $S\subseteq P$ a subset.
\begin{enumerate}
...
\begin{itemize}
\item \udef{expansive} if $\forall x\in P: x\precsim f(x)$;
\item \udef{contractive} if $\forall x\in P: f(x)\precsim x$.
\end{itemize}
\end{definition}
...
\subsection{Ordered sets of subsets}
\subsubsection{The ordered set of downsets}
\begin{definition}
