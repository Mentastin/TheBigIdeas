...
\item has an algebraic closure operator $\powerset(A^2)\to \ReflRel(A): R\mapsto R\cup \id_A$;
...
\begin{enumerate}
\item is a complete $\wedge$-subsemilattice of $\powerset(A^2)$;
\item has an algebraic closure operator $\powerset(A^2)\to \ReflRel(A): R\mapsto R\cup R^\transp$;
\item is an algebraic lattice.
\end{enumerate}
\end{proposition}
Note that the proof of (1) works if $I=\emptyset$. 
...
Conversely, assume $R \subseteq \incl_{\ReflRel(A)}(R')$. Then we have $R^\transp \subseteq R'$, so $R\cup R^\transp \subseteq R'$.
...
Let $A$ be a set. Then
\begin{enumerate}
\item $\EquivRel(A)$ is a complete $\wedge$-subsemilattice of $\powerset(A\times A)$;
\item $\EquivRel(A)$ is an algebraic lattice.
\end{enumerate}
\end{proposition}
\begin{proof}
...
\subsection{Constructing functions from expressions}
\begin{definition}
