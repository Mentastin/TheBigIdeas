...
\item The set $^RB$ is the \udef{active domain}, \udef{domain of definition}, \udef{preimage} or \udef{prerange} of the relation, also denoted $\preim(R)$.
\end{enumerate}
...
\item $Ry \defeq {^R\{x\}}$.
\end{itemize}
We call such images and preimages \udef{principle}.
\end{definition}
...
\begin{itemize}
\item \udef{left-total}, \udef{serial} or simply \udef{total} if $A^R = B$;
...
Let $\sSet{X, R}$ and $\sSet{Y, S}$ be relational structures and $f: X\to Y$ a relation-preserving function. Then
\begin{enumerate}
...
\begin{itemize}
\item \udef{contractive} if $\forall a,b\in A: aRb \implies f(a)Rb$;
\item \udef{expansive} if $\forall a,b\in A: aRb \implies aRf(b)$.
\end{itemize}
\end{definition}
...
\begin{proposition}
Let $S$ be a semigroup of relation-preserving functions and $f: \sSet{A, R} \to \sSet{B,S}$, $g: \sSet{A, R} \to \sSet{C,T}$ relation-preserving functions. Then
\[ f\mathcal{R}g \;\implies\; f;S;f^\transp = g;T;g^\transp. \]
If $S$ and $T$ are reflexive and anti-symmetric, then the opposite implication also holds.
\end{proposition}
\begin{proof}
Assume $f\mathcal{R}g$, then there exist $x,y\in S$ such that $f = g;x$ and $g = f;y$. Clearly $\ker f = \ker g$. Now $x$ and $y$ are relation preserving: $T \subseteq x;S;x^\transp$ and $S \subseteq y;T;y^\transp$. So
...
\begin{definition}
Let $a: \sSet{A,R} \to \sSet{B,S}$ and $a': \sSet{B,S} \to \sSet{A,R}$ be relation-preserving generalised inverses. We say $(a,a')$ is a \udef{Galois connection} if $aa'R \subseteq R$ and $a'aS^\transp \subseteq S^\transp$.
\end{definition}

\begin{proposition}
Let $a: \sSet{A,R} \to \sSet{B,S}$ and $a': \sSet{B,S} \to \sSet{A,R}$ be relation-preserving generalised inverses. If $(a,a')$ is a monotone Galois connection, then $aS = Ra^{\prime \transp}$.
\end{proposition}
\begin{proof}
...
\begin{definition}
Let $R,S$ be homogeneous relations (not necessarily defined on the same set). We say $R$ and $S$ are \udef{Galois connected} if there exist functions $f,g$ such that
\[ \forall x,y: \; f(x)Ry \iff xSg(y). \]

We call $(f,g)$ a \udef{Galois connection}.
\end{definition}

Clearly this is the same as saying $f;R = S;g^\transp$.
