...
\begin{definition}
Let $A, B, C$ be classes and $f: A\times B \to C$ a binary function. We define a new function
\[ \swap(f): B\times A \to C: (b,a) \mapsto f(a,b). \] 
\end{definition}

If we restrict ourselves to the class of binary functions between sets, then $\swap$ can be considered a function.

\begin{lemma}
Let $A, B, C$ be classes and $f: A\times B \to C$ a binary function. Then $\swap(\swap(f)) = f$.
\end{lemma}
...
Let $A$ be a class and $f: A\times A \to A$ a homogeneous binary function. Then $\swap(f): A\times A \to A$ is also a homogeneous binary function.

Often a property of $f$ can be translated to a different property of $\swap(f)$. In this case we say the properties are \udef{dual} to each other.

\begin{proposition}
If a certain logical dependence holds between certain properties of binary homogenous functions, for all binary homgenous functions, then the same logical dependence holds between the duals of these properties.
\end{proposition}
\begin{proof}
If the logical dependence holds for all $f$, it in particular holds for all $\swap(f)$.
\end{proof}
...
We have $\widetilde{f}(\widetilde{f}(x,y),z) = \widetilde{f}(x,\widetilde{f}(y,z))$ if $f(f(x,y),z)$ and $f(x,f(y,z))$ are defined.

If $f(f(x,y),z)$ is not defined, we claim $\widetilde{f}(\widetilde{f}(x,y),z) = u$. Indeed,
\begin{itemize}
\item if $f(x,y)$ is defined, then $f(\widetilde{f}(x,y),z) = f(f(x,y),z)$ is undefined and thus equal to $u$;
\item if $f(x,y)$ is undefined, then $\widetilde{f}(\widetilde{f}(x,y),z) = \widetilde{f}(u,z) = f(u,z) = u$.
\end{itemize}
...
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y\in A$. Then
\begin{itemize}
\item the \udef{left principal ideal} generated by $x$ is $f(A, x)$;
\item the \udef{right principal ideal} generated by $x$ is $f(x,A)$.
\end{itemize}
The \udef{Green's relations} of $f$ are relations on $A$ defined by
\begin{itemize}
\item $x\mathcal{L}y \;\defequiv\; f(A,x) = f(A, y) \;\lor\; x=y$;
\item $x\mathcal{R}y \;\defequiv\; f(x, A) = f(y, A) \;\lor\; x=y$;
\item $\mathcal{H} \;\defeq\; \mathcal{L}\cap\mathcal{R}$;
\item $\mathcal{D} \;\defeq\; \mathcal{L}\cup\mathcal{R}$.
\end{itemize}
\end{definition}
...
\item $f(y,x) \mathcal{L} x$ and $x \mathcal{R} f(y,x)$.
\end{enumerate}
\end{lemma}
