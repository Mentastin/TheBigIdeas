...
Often we will write $R \subseteq S$ as a shorthand for $\graph(R)\subseteq \graph(S)$ if $\dom(R) = \dom(S)$ and $\codom(R) = \codom(S)$. Similarly we may write $R\cup S$, $R\cap S$ etc.
...
\begin{enumerate}
\item $\graph(\id_{A\cup B}) = \graph(\id_A)\cup \graph(\id_B)$;
\item $\graph(\id_{A\cap B}) = \graph(\id_A)\cap \graph(\id_B)$;
\item $\graph(E_{A\cup C, B\cup D}) = \graph(E_{A,B})\cup \graph(E_{C,D})$;
\item $\graph(E_{A\cap C, B\cap D}) = \graph(E_{A,B})\cap \graph(E_{C,D})$;
\item $\graph(U_{A\cap C, B\cap D}) = \graph(U_{A,B})\cap \graph(U_{C,D})$.
\end{enumerate}
\end{lemma}
Note that writing $\graph$ is important here because the types (i.e. domain and codomain) do not match, even though the graph does.
\begin{proof}
(1,2) $(x,x)\in \graph(\id_{A\cup B})$ iff $x\in A\lor x\in B$ iff $(x,x)\in \graph(\id_A) \lor (x,x)\in\graph(\id_B)$ iff $(x,x)\in \graph(\id_A)\cup \graph(\id_B)$. For the second part replace $\lor$ with $\land$.

(3,4) Trivial because all sets are $\emptyset$.

(5) $(x,y)\in \graph(U_{A\cap C, B\cap D})$ iff $(x\in A\cap C) \land (y\in B\cap D)$ iff $(x\in A)\land (y\in B)\land(x\in C)\land(y\in D)$ iff $\big((x,y)\in \graph(U_{A, B})\big)\land\big((x,y)\in \graph(U_{A, B})\big)$ iff $(x,y)\in \graph(U_{A,B})\cap \graph(U_{C,D})$.
\end{proof}
...
\begin{lemma}
Let $R$ be a homogeneous relation on $A$. Then $R$ is reflexive \textup{if and only if} $\id_A \subseteq R$.
\end{lemma}
...
\item $x(R;S)y \iff xR \mesh Sy$ and $x(\overline{R;S})y \iff xR\perp Sy$;
\item $\overline{R;S} = \overline{R};S\cup R;\overline{S}$.
\end{enumerate}
\end{lemma}
\begin{proof}
TODO
...
\begin{enumerate}
\item $\graph(\id_{\preim(R)}) \subseteq R;R^\transp \subseteq \graph(U_{\preim(R)})$;
\item $\graph(\id_{\im(R)}) \subseteq R^\transp;R \subseteq \graph(U_{\im(R)})$.
\end{enumerate}
\end{lemma}
