...
\section{Relations}
\begin{definition}
Let $A,B$ be sets and $R$ any subset of the Cartesian product $A\times B$. A \udef{binary relation} $R$ is a structured set $(R,(A,B))$. The set $R$ is the \udef{graph} of the binary relation.
We write
\[ xRy \defequiv  (x,y)\in R. \]
Analogously we can define \udef{ternary relations} on $A,B,C$ as subsets of $A\times B\times C$. A similar definition can be made for $n$-ary relations.
\end{definition}
...
\end{itemize}
A binary relation is \udef{homogeneous} or an \udef{endorelation} if it has the same domain and codomain. It is \udef{heterogeneous} is the domain and codomain are different.
\end{definition}

\begin{definition}
A \udef{relational structure} is a structured set $(A, R)$ where $R$ is an endorelation on $A$.
\end{definition}
...
\begin{itemize}
\item the \udef{empty relation} $E=\emptyset$;
\item the \udef{universal relation} $U_{A,B}=A\times B$;
\item the \udef{identity relation} $\id_A=\{(x,y)\in A\times A\;|\; x=y \}$.
\end{itemize}
\end{definition}
The identity relation is also known as the \udef{diagonal relation}.
...
is the \udef{converse} of $R$. It is also known as the \udef{inverse}, \udef{transpose}, \udef{reciprocal}, \udef{opposite} or \udef{dual} of $R$.
\end{definition}
...
\[ \overline{R} = \{(x,y)\;|\; \neg xRy\} \]
is the \udef{complementary relation} of $R$.
\end{definition}
\begin{lemma}
...
\begin{definition}
Let $R\subset A\times B$ and $S\subset B\times C$ be relations. Then the \udef{composition} of $R$ and $S$ is a new relation $R;S \subset A\times C$:
...
Let $R\subset A\times B$ be a binary relation.
\begin{itemize}
\item Let $S\subseteq A$. Then the \udef{left-restriction} of $R$ to $S$ is
\[ \left.R\right|_S = \{ (x,y)\in R \;|\; x\in S \}. \]
\item Let $S\subseteq B$. Then the \udef{right-restriction} of $R$ to $S$ is
...
\begin{itemize}
\item The \udef{reflexive closure}
\begin{align*}
...
\item The \udef{symmetric closure}
\begin{align*}
...
\[ x:\N\to A. \]
We usually write $x_i$ instead of $x(i)$ and $(x_i)_{i\in \N}$ instead of $x$. We may also write $\seq{x_i}_i$ or $\seq{x_i}$ if the index is clear.
\end{definition}
This notation will also be used for functions from sets other than $\N$, if these functions feel a bit like sequences. In these cases the domain, typically denoted $I$ or $J$, is called an \udef{index set}.
...
\begin{definition}
Let $(x_i)_{i\in \N}$ be a sequence. Let $f:\N\to I\subset \N$ be an order-preserving bijection to a subset $I$ of $\N$. Then we call $(x_{f(i)})_{i\in \N}$ a \udef{subsequence} of the sequence.

A \udef{tail} of the sequence $(x_i)_{i\in \N}$ is a subsequence where $f$ is of the form
\[ f: \N\to [n,\infty[: i\mapsto i+n \]
for some $n\in \N$
\end{definition}

\subsection{Finite sequences}
\begin{definition}
A \udef{finite sequence} or \udef{word} or \udef{string} in a set $A$ is a function
\[ x:[0,n[ \to A \]
for some $n\in \N$. This $n$ is the \udef{length} of the sequence, denoted $\len(x)$. We write
\begin{align*}
A^{(n)} &\defeq ([0,n[\to A) \\
