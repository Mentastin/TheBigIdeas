...
\end{itemize}
The classes $X^R$ and $^RY$ are also called \udef{polars}.\footnote{Birkhoff states (TODO ref) that the term ``polar'' was chosen due to the link with the polars of conic sections.}
...
\subsubsection{Inverses and cancellation}
\begin{definition}
...
\begin{lemma} \label{leftRightInverse}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function with identity $e$ and $x$ in $A$. If $x$ has both a left-inverse $l$ and a right-inverse $r$, then $l = r$.
\end{lemma}
\begin{proof}
...
\begin{lemma}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x,y$ in $A$.
\begin{enumerate}
...
\begin{lemma}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x,y$ in $A$. Then
\begin{enumerate}
...
\subsubsection{Left and right relation}
\begin{definition}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y\in A$. Then
\begin{itemize}
...
\end{lemma}
\begin{proof}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y, z\in A$ such that $xLy$ and $yLz$. Then there exist $a,b\in A$ such that $f(a,x) = y$ and $f(b,y) = z$. Then
\[ z = f(b,y) = f(b,f(a,x)) = f(f(b,a), x), \]
so $xLz$. The statement for $R$ is dual.
\end{proof}

\begin{lemma}
Let $A$ be a class and $f: A\times A \to A$ an associative binary function. Then
\begin{enumerate}
...
Let $A$ be a class and $f: A\times A \to A$ an associative binary function. Then $f$ has an identity and every $x\in A$ is invertible \textup{if and only if}
...
\begin{definition}
Let $A$ be a class and $f: A\times A \to A$ an associative binary function.
...
\begin{lemma}
Let $A$ be a class and $f: A\times A \to A$ an associative binary function. Then the relations $\greensL$ and $\greensR$ are equivalence relations.
\end{lemma}

\begin{lemma}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y\in A$. Then the following are equivalent:
\begin{enumerate}
\item $x \greensL y$;
...
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y, z\in A$.
\begin{enumerate}
...
\begin{proof}
Assume $x(\greensL;\greensR)z$, meaning $\exists y: x\greensL y$ and $y\greensR z$. Then there exist $a,b,c,d\in \widetilde{A}$ such that
\[ \begin{tikzcd}
x \ar[r, maps to, shift left, "\lambda_a"] & y \ar[l, maps to, shift left, "\lambda_b"] \ar[r, maps to, shift left, "\rho_c"] & z \ar[l, maps to, shift left, "\rho_d"]
\end{tikzcd}. \]
Using \ref{lambdaRhoCommute}, we can rearrange such that we also get the mappings along the left and bottom sides of
\[ \begin{tikzcd}[sep=large]
x \ar[r, maps to, shift left, "\lambda_a"] \ar[d, maps to, shift left, "\rho_c"] & y \ar[l, maps to, shift left, "\lambda_b"] \ar[d, maps to, shift left, "\rho_c"] \\
y' \ar[u, maps to, shift left, "\rho_d"] \ar[r, maps to, shift left, "\lambda_a"] & z \ar[u, maps to, shift left, "\rho_d"] \ar[l, maps to, shift left, "\lambda_b"]
\end{tikzcd} \]
for some $y' \in A$. Thus $x(\greensR;\greensL)y$. The other inclusion is similar.
\end{proof}

From \ref{commutingEquivalenceRelations} we also know that the $\greensL,\greensR$-egg-box diagram decomposes into blocks, which are $\greensD$-equivalence classes. The cells are $\greensH$-equivalence classes.
...
\end{proof}
\begin{corollary}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y\in A$ such that $x\greensD y$. Then $|[x]_\greensH| = |[y]_\greensH|$.
\end{corollary}
\begin{proof}
If $x(\greensL ;\greensR)y$, then there exists a $z\in A$ and $a,b\in\widetilde{A}$ such that $z = f(a,x)$ and $y = f(z, b) = f(f(a,x), b)$. Then $\lambda_a\circ \rho_b$ is the required bijection $[x]_\greensH \to [z]_\greensH \to [y]_\greensH$.
\end{proof}
...
\subsubsection{Regular elements and generalised inverses}
\begin{definition}
Let $A$ be a class, $f: A\times A \to A$ a binary function and $x, y\in A$. We call
\begin{itemize}
...
\begin{lemma}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x\in A$ a regular element with $x = f(f(x, y), x)$. Then
\begin{enumerate}
\item $f(x,y)$ and $f(y,x)$ are idempotent;
\item $f(y,x) \greensL x$ and $x \greensR f(y,x)$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) $f(f(x,y), f(x,y)) = f(f(f(x,y), x), y) = f(x, y)$ and $f(f(y,x), f(y,x)) = f(y, f(x, f(y,x))) = f(y,x)$.
...
\begin{proposition} 
Let $A$ be a class and $f: A\times A \to A$ a binary associative function. If $a\in A$ is regular, then every element in $[a]_{\greensD}$ is regular.
\end{proposition}
...
\begin{lemma}
Let $A$ be a class, $f: A\times A \to A$ a binary function and $x\in A$. Then $x$ is regular \textup{if and only if} it has a generalised inverse.
\end{lemma}
\begin{proof}
Clearly every element with a generalised inverse is regular. Conversely, assume $x$ regular with $x = f(f(x, a), x)$. Then $y = f(f(a, x), a)$ is a generalised inverse of $x$.
\end{proof}
Note that we do not have that $x = f(f(x, y), x)$ implies $y = f(f(y, x), y)$.
