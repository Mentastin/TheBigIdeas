...
\begin{lemma} \label{maximalMinimalConditions}
Let $\sSet{A, R}$ be a relational structure, $X\subseteq A$ a subclass and $x\in X$. Then
\begin{enumerate}
\item if $xR\cap X = \emptyset$, then $x$ is maximal;
\item if $Rx\cap X = \emptyset$, then $x$ is minimal.
\end{enumerate}
\end{lemma}
\begin{proof}
...
\begin{definition}
Let $\sSet{A,f}$ be an associative class. We call this class
\begin{itemize}
\item a \udef{left semi-category} if for all $x\in A$, there exist a left-identities $e, e'\in A$ such that $f(e, x)$ and $f(x,e)$ are defined.
\item a \udef{category} if for all $x\in A$, there exist identities $e,e'\in A$ such that both $f(e,x)$ and $f(x, e')$ are defined.
\end{itemize}
\end{definition}
...
Let $\sSet{A,f}$ be a category. Then
\begin{enumerate}
\item there exists a function $\dom: A\to A: x\mapsto \dom(x)$, where $\dom(x)$ is the unique identity such that $f\big(x,\dom(x)\big)$ exists;
\item there exists a function $\codom: A\to A: x\mapsto \codom(x)$, where $\codom(x)$ is the unique identity such that $f\big(\codom(x), x\big)$ exists;
\item for all $x,y\in A$: $f(x,y)$ is defined \textup{if and only if} $\dom(x) = \codom(y)$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) and (2) are immediate. (We do not need choice due to the uniqueness, \ref{uniquenessIdentity})

(3) $\Rightarrow$ We have
\[ f(x,y) = f\Big(f\big(x, \dom(x)\big), f\big(\codom(y), y\big)\Big) = f\Big(f\Big(x, f\big(\dom(x), \codom(y)\big)\Big), y\Big), \]
so $f\big(\dom(x), \codom(y)\big)$ is defined. Thus $\dom(x) = \codom(y)$ by \ref{uniquenessIdentity}.

$\Leftarrow$ If $\dom(x) = e = \codom(y)$, then $f(x,e)$ and $f(e,y)$ is defined, so $f(x,y) = f\big(f(x,e), y\big)$ is defined by the definition of category.
\end{proof}


\begin{lemma}
Let $\sSet{A,f}$ be a category and $x,y\in A$. Then
\begin{enumerate}
\item $\dom\big(\dom(x)\big) = \dom(x)$;
\item $\codom\big(\codom(x)\big) = \codom(x)$;
\item $\dom\big(f(x,y)\big) = \dom(y)$;
\item $\codom\big(f(x,y)\big) = \codom(x)$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) We have that $f\Big(\dom\big(\dom(x)\big), \dom(x)\Big)$ is defined by definition. As $\dom\big(\dom(x)\big)$ and $\dom(x)$ are both identities, we have $\dom\big(\dom(x)\big) = \dom(x)$ from \ref{uniquenessIdentity}.

(2) Dual to (1).

(3) By definition, we have that
\[ f\Big(f(x,y), \dom\big(f(x,y)\big)\Big) = f\Big(x,f\Big(y, \dom\big(f(x,y)\big)\Big)\Big) \]
is defined, so $f\Big(y, \dom\big(f(x,y)\big)\Big)$ is defined. As $\dom\big(f(x,y)\big)$ is an identity, it is equal to $\dom(y)$ by \ref{categoryIdentities}.
\end{proof}
