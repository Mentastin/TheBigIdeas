...
\begin{itemize}
\item If $R,S$ have the same domain, we define the \udef{left residual} as $R\diagdown S \defeq \overline{R^\transp; \overline{S}}$.
\item If $R,S$ have the same codomain, we define the \udef{right residual} as $R\diagup S \defeq \overline{\overline{R}; S^\transp}$.
\end{itemize}
\end{definition}

\begin{lemma} \label{residualsAsUnion}
Let $R,S$ be relations. Then
\begin{enumerate}
\item $R\diagdown S = \overline{R^\transp; \overline{S}} = R^\transp; S \;\cup\; \overline{R}^\transp; \overline{S}$;
\item $R\diagup S = \overline{\overline{R}; S^\transp} = R;S^\transp \;\cup\; \overline{R}; \overline{S}^\transp$.
\end{enumerate}
\end{lemma}

\begin{proposition}[Schröder rule]
...
\end{proof}
We can also give a proof by a direct calculation of $T\diagup S$.
\begin{proof}
It is enough to prove that $R;S \subseteq T$ implies $R \subseteq T\diagup S$. Everything else follows by taking complements and converses as before.

So assume $R;S \subseteq T$. We calculate, using \ref{residualsAsUnion}
\[ T\diagup S = T;S^\transp \;\cup\; \overline{T}; \overline{S}^\transp \supseteq (R;S);S^\transp \;\cup\; \overline{T}; \overline{S}^\transp \]
\end{proof}

\begin{corollary}[Dedekind formula]
Let $R,S,T$ be compatible relations. Then
\[ (R;S)\cap T \subseteq (R\cap (T; S^\transp));(S\cap (R^\transp;T)). \]
\end{corollary}
We can recover the formula $(R;S)^\transp = S^\transp;R^\transp$ as a special case of the Dedkind formula
