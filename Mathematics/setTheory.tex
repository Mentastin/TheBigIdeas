...
\end{itemize}
If $x'\in [x]_\greensH$, then $\lambda_a(x')\greensL \lambda_a(x) = y$ by \ref{lambdaRhoPreservation}, so $\lambda_a(x')\in [y]_\greensH$.
...
\begin{lemma}
Let $A$ be a class, $f: A\times A \to A$ a binary function and $x\in A$ a regular element with $x = f(f(x, y), x)$. Then
\begin{enumerate}
...
\subsubsection{Generalised inverses and idempotents}
\begin{lemma}
