...
\begin{proof}
$(1)\Leftrightarrow (2)$ Assume $R_1;R_2 = R_2;R_1$. We verify
\begin{itemize}
...
$(1)\Leftrightarrow (3, 4)$ On reflection, one may understand that (3) and (4) are equivalent.

First assume (1) and take any three occupied cells that lie on the corners of a rectangle. Take $a,b$ from the diagonally opposing cells. Then $a(R_1;R_2)b$, because there exists a $c$ in the third cell. So also $a(R_2;R_1)b$, meaning there exists an element in the fourth sell, so (4) holds.
...
\subsection{Green's relations and principal ideals}
\begin{definition}
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y\in A$. Then
\begin{itemize}
\item the \udef{left principal ideal} generated by $x$ is $f(A, x)\cup \{x\}$;
\item the \udef{right principal ideal} generated by $x$ is $f(x,A)\cup \{x\}$.
\end{itemize}
The \udef{Green's relations} of $f$ are relations on $A$ defined by
\begin{itemize}
\item $x\greensL y \;\defequiv\; f(A,x) = f(A, y)$;
\item $x\greensR y \;\defequiv\; f(x, A) = f(y, A)$;
...
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y\in A$. Then
\begin{enumerate}
\item $x\greensL y$ \textup{if and only if} $f(\widetilde{A},x) = f(\widetilde{A}, y)$;
\item $x\greensR y$ \textup{if and only if} $f(x, \widetilde{A}) = f(y, \widetilde{A})$.
\end{enumerate}
\end{lemma}
...
Let $A$ be a class, $f: A\times A \to A$ an associative binary function and $x, y\in A$. Then
\begin{enumerate}
\item $a\mathcal{L}b$ \textup{if and only if} $\exists x,y\in \widetilde{A}: (xa = b) \land (yb = a)$;
\item $a\mathcal{R}b$ \textup{if and only if} $\exists x,y\in \widetilde{A}: (ax = b) \land (by = a)$.
\end{enumerate}
\end{lemma}
\begin{proof}
We prove (1), (2) is analogous.

($\Rightarrow$) $a\in f(\widetilde{A}, a) = f(\widetilde{A}, b)$, so there exists $y\in \widetilde{A}$ such that $a = f(y, b)$. The other equation is similar.

($\Leftarrow$) Because $\widetilde{A}x \subseteq \widetilde{A}$, we have $\widetilde{A}a \subseteq \widetilde{A}xa = \widetilde{A}b$. Similarly $\widetilde{A}b \subseteq \widetilde{A}a$.
\end{proof}
...
\begin{proof}
Assume $a(\mathcal{L};\mathcal{R})b$, meaning $\exists c: a\mathcal{L}c$ and $c\mathcal{R}b$. Then there exist $x,y,u,v\in \widetilde{A}$ such that
