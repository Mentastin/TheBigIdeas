...
\end{cases}. \]
Given a syntax tree $\syntaxTree(t)$ of a term $t\in T(\Omega, V)$, we call the elements of $\syntaxTree(t)$ the \udef{positions} of the term $t$.
\end{definition}
...
TODO: function that maps linear order to sequence
...
\[ \forall \omega\in\Omega: \forall x\in (A,B)^{\alpha(\omega)}: \quad \omega_{A\times B}^\imf(R^{\alpha(\omega)}) \subseteq R. \]
