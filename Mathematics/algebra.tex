...
\item a node in $\syntaxTree(t)$ is terminal \textup{if and only if} it end with some element of $\Omega \uplus V$.
\end{enumerate}
\end{lemma}
\begin{proof}
TODO
...
\begin{enumerate}
\item $\#\big(\positions(t)\big) = \len(t)$;
\item Every terminal node in $\positions(t)$ is a proper substring of exactly one (terminal) node in $\syntaxTree(t)$.
\item Let $\positions(t)\subseteq \N^*$ be ordered by depth-first ordering. There exists a similarity between $\big(0:\len(t)\big)$ and the terminal nodes of $\positions(t)$.
\item There exists a unique strictly monotone function
...
\end{lemma}
\begin{proof}
$\boxed{\Rightarrow}$ Consider $\leafAt_t\big(\len(u)\big)$
\end{proof}
...
\[ \sigma(u) = \begin{cases}
\seq{} & (u = \seq{}) \\
\sigma(u_0)\concat \sigma\big(u[1:]\big) & \big(u_0\in \preim(\hat{\sigma})\big) \\
...
\subsubsection{$\Omega$-identities}
\begin{definition}
...
We call a set $E\subseteq T(\Omega, V)\times T(\Omega, V)$ \udef{closed under $\Omega$-operations} if for all operator symbols $\omega\in\Omega$ and $t,s\in \big(T(\Omega, V)\big)^{\alpha(\omega)}$ such that $(s_i, t_i)\in E$ for all $i\in 0:\alpha(\omega)$, we have
...
\begin{definition}
Let $(\Omega, \alpha)$ be a signature, $V$ a set of variables and $E$ a set of $\Omega$-identities. We define the \udef{reduction relation} $\to_E$ on $T(\Omega, V)$ by
\[ s\to_E t \;\defequiv\; \exists u,v\in (\Omega\uplus V)^*: \exists (r_1, r_2)\in E: \exists \sigma\in \varSubs(\Omega, V):\; \begin{cases}
