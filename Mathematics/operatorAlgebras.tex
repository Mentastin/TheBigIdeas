...
\begin{lemma} \label{lemma:multiplicationContinuous}
Let $A$ be a Banach algebra. The multiplication map $\cdot: A\times A \to A: (x,y)\mapsto xy$ is continuous.
\end{lemma}
\begin{proof}
Because $A\times A$ is a metric space, we can combine \ref{prop:continuousLimitInterchange} and \ref{corollary:convergenceFiniteProductTopology} to conclude that the multiplication map is continuous iff $x_ny_n \to xy$ whenever $x_n \to x$ and $y_n \to y$.
...
\end{align*}
\end{proof}
As a consequence multiplication by a fixed factor, $x\mapsto cx$ or $x\mapsto xc$ for some $c$, is also continuous, by \ref{corollary:productInclusionsContinuous}. This is also immediate from the boundedness of multiplication $\norm{xy}\leq\norm{x}\norm{y}$ and \ref{theorem:boundedLinearMaps}.
...
\begin{proposition} \label{prop:smallestBanachAlgebra}
...
\begin{proposition} \label{prop:normsOfUnitisation}
...
\end{proof}
\begin{corollary} \label{corollary:projectionOnACommutes}
...
\begin{lemma} \label{lemma:DaggerMorphismProperties}
...
\end{lemma}
\begin{proof}
The third point follows from the second and \ref{prop:injectivityKernelTriviality}.
\end{proof}
...
\begin{proposition}[Neumann series] \label{prop:NeumannSeries}
...
\end{proposition}
\begin{proof}
Since $\norm{x^n}\leq \norm{x}^n$ for all $n\geq 1$ and $\sum \norm{x}^n$ is a convergent geometric series, the series $\sum x^n$ is convergent by \ref{prop:absoluteUnconditionalConvergenceBanach}.
\end{proof}
\begin{corollary} \label{corollary:openSetInvertibles}
Let $x\in\GL(A)$, then $B(x,\norm{x^{-1}}^{-1})\subset \GL(A)$. The invertible elements $\GL(A)$ form an open subset of $A$.
\end{corollary}
\begin{proof}
The second assertion follows from the first by \ref{prop:interior}. Let $y\in B(x,\norm{x^{-1}}^{-1})$. Then
\[ \norm{\vec{1} - x^{-1}y} \leq \norm{x^{-1}}\cdot\norm{x-y} < \norm{x^{-1}}\cdot\norm{x^{-1}}^{-1} = 1  \]
and, by the proposition, $x^{-1}y$ is invertible. Similarly $yx^{-1}$ is invertible and thus $y$ is invertible by \ref{lemma:productInvertibility}.
\end{proof}
\begin{corollary} \label{corollary:inverseMapContinuous}
The map $^{-1}: \GL(A)\to\GL(A): x\mapsto x^{-1}$ is continuous.
\end{corollary}
\begin{proof}
Take a convergent sequence $(x_n)\subset\GL(A)$ with limit $x$. We wish to prove $(x_n^{-1})$ converges to $x^{-1}$, because then the map is continuous by \ref{prop:continuousLimitInterchange}. We can choose an $n_0$ such that $\forall n\geq n_0: x_n \in B(x,\norm{x^{-1}}^{-1})$. From now on we consider only the tails $(x_n)_{n=n_0}^\infty$ and $(x_n^{-1})_{n=n_0}^\infty$, which have the same limits by TODO ref. Then
\[ \norm{x^{-1}}\cdot\norm{x-x_n} < \norm{x^{-1}}\cdot\norm{x^{-1}}^{-1} = 1. \]
Also
\[ \norm{\vec{1} - x^{-1}x_n} = \norm{x^{-1}(x-x_n)} \leq \norm{x^{-1}}\cdot\norm{x-x_n} < 1. \]
...
it is absolutely convergent and thus convergent, by \ref{prop:absoluteUnconditionalConvergenceBanach}.
\end{proof}
...
is invertible in $\widetilde{A_\C}$, by \ref{lemma:productInvertibility} and thus also in $A$, by TODO ref.
\end{proof}
...
\begin{proposition} \label{corollary:spectrumCompact}
...
By \ref{prop:NeumannSeries}, $\vec{1} - x/\lambda$ is invertible and thus so is $x-\lambda$.

It is then enough to show that $\sigma(x)$ is closed. By \ref{corollary:openSetInvertibles}, $\GL(A)$ is open and the set of non-invertibles $A\setminus \GL(A)$ is closed. Consider $f: \C \to A: \lambda \mapsto x-\lambda$. Then $\sigma(x) = f^{-1}[A\setminus \GL(A)]$ is the preimage of a closed set under a continuous map, and hence is closed.
\end{proof}
...
\end{proof}
\begin{corollary}[Gelfand-Mazur] \label{corollary:GelfandMazur}
...
\begin{lemma} \label{lemma:spectrumOfImage}
...
\begin{proposition} \label{prop:charactersUnital} \label{prop:charactersContinuous}
...
Next $\norm{\varphi}\leq 1$ follows from $\norm{\tilde{\varphi}} \leq 1$ by \ref{lemma:DaggerMorphismProperties}. To this end suppose that for some $x\in \tilde{A}$, $|\tilde{\varphi}(x)|>\norm{x}$. Then $x-\tilde{\varphi}(x)$ is invertible by corollary \ref{corollary:spectrumCompact}. Thus
...
which is a contradiction. Then $\norm{\tilde{\varphi}} \leq 1$ and thus $\norm{\varphi} \leq 1$. By \ref{theorem:boundedLinearMaps}, $\varphi$ is continuous.
...
All elements of the Gelfand spectrum are unital and continuous, by \ref{prop:charactersUnital} and \ref{prop:charactersContinuous}.

If $A$ is a real algebra, then
\[ \hat{A} = \setbuilder{\varphi|_A}{\varphi\in \widehat{A_\C}}. \]

\begin{proposition} \label{prop:commutativeSameSpectrum}
...
\begin{proposition} \label{prop:BanachAlgebraIdeals}
...
\end{proposition}
\begin{proof}
If $\mathcal{J}$ is not closed, then $\overline{\mathcal{J}} = A$ by maximality. So by proposition \ref{prop:closure} every open set must intersect $\mathcal{J}$, in particular the set $\GL(A)$, open by \ref{corollary:openSetInvertibles}. Because $\mathcal{J}$ is an ideal containing an invertible element, $\mathcal{J}=A$ by TODO ref. A contradiction.

If $A$ is commutative, $A/\mathcal{J}$ is a field by TODO ref. It is also a unital Banach algebra (TODO), so $A/\mathcal{J} \cong \C$ by the Gelfand-Mazur theorem, \ref{corollary:GelfandMazur}.
\end{proof}

\begin{proposition} \label{prop:characterMaximalIdealsComplex}
...
\end{proposition}
\begin{proof}
First we verify that for each character $\varphi$ the kernel is a maximal ideal. Indeed applying \ref{prop:splittingMap} to $\varphi$ we get an isomorphism $A/\ker\varphi \cong \im\varphi = \C$, meaning $\ker(\varphi)$ has codimension 1 and thus is a maximal proper subspace. By \ref{prop:kernelIsIdeal}, $\ker(\varphi)$ is an ideal.
...
By \ref{prop:BanachAlgebraIdeals}, $A/\mathcal{J}\cong \C$, so the quotient map $A\to A/\mathcal{J}\cong \C$ can be seen as a character with kernel $\mathcal{J}$.
\end{proof}

TODO characterMaximalIdealsReal!


\begin{proposition} \label{prop:spectrumFromSpectrum}
...
Suppose $\lambda \in \sigma(x)$. Because $x-\lambda$ is non-invertible, the ideal generated by it is proper, by \ref{lemma:nonInvertibleGeneratedIdeals}, and $x-\lambda$ lies in a maximal ideal, by \ref{prop:everyProperIdealInMaximalIdeal}. By \ref{prop:characterMaximalIdealsComplex}, this means $x-\lambda \in \ker\varphi$ for some $\varphi\in\hat{A}$ and then $\lambda = \varphi(x)$.
\end{proof}
...
\end{definition}
\begin{lemma} \label{lemma:GelfandTransformHomomorphism}
...
We prove in turn:
\begin{enumerate}
\item We have equipped $\hat{A}$ with the weak-$*$ topology. Then proposition \ref{prop:weak*continuousFunctional} says each $\hat{x}$ is continuous.
...
where we have used that $\norm{\varphi} = 1$ by \ref{prop:charactersUnital}, $\sigma(x) = \setbuilder{|\varphi(x)|}{\varphi\in\hat{A}}$ by \ref{prop:spectrumFromSpectrum} and the inequality is from \ref{corollary:spectrumCompact}.
\end{enumerate}
\end{proof}
...
\end{lemma}
\begin{lemma} \label{lemma:elementaryStarLemma}
...
\begin{proposition} \label{prop:smallestBanach*Algebra}
Let $A$ be a Banach-$*$-algebra and $S\subset A$ a subset. Then
\[ \mathcal{B}^*(S) \defeq \mathcal{B}(S\cup S^*) \]
is the smallest Banach-$*$-subalgebra in $A$ that contains $S$, where $\mathcal{B}$ is defined as in \ref{prop:smallestBanachAlgebra} and $S^* = \setbuilder{s^*\in A}{s\in S}$.
\end{proposition}
...
\begin{lemma} \label{lemma:orthogonalProjection}
...
\begin{lemma} \label{lemma:realImaginaryParts}
...
\begin{lemma} \label{lemma:C*identityEquivalent}
The $C^*$-identity is equivalent to
\[\forall x\in A: \quad \norm{x^*x} \geq \norm{x}^2.\]
\end{lemma}
\begin{proof}
...
\begin{lemma} \label{lemma:consequencesC*}
...
\begin{proposition} \label{prop:normNormalElement}
...
\end{proof}
\begin{corollary} \label{corollary:atMostOneNorm}
...
\end{proof}
It is important to not the spectral radius is a purely algebraic property and is independent of the norm.
...
\end{proposition}
\begin{proof}
Because $x^*x$ and $\Psi(x^*x)$ are normal, we calculate using \ref{prop:normNormalElement}
\[ \norm{x}^2 = \norm{x^*x} = r(x^*x) \geq r(\Psi(x^*x)) = \norm{\Psi(x^*x)} = \norm{\Psi(x)^*\Psi(x)} = \norm{\Psi(x)}^2, \]
where the inequality follows from an application of lemma \ref{lemma:spectrumOfImage} to $x^*x$. Hence $\norm{\Psi}\leq 1$. Equality follows from $\Psi(\vec{1}) = 1$.
\end{proof}
\begin{corollary}
...
\begin{lemma} \label{lemma:injectiveLifts}
...
\section{Unitisation of $C^*$-algebras}
...
\end{proposition}
\begin{proof}
There is at most one such norm, by \ref{corollary:atMostOneNorm}. Because the operator norm is a suitable norm for $A^\dagger$ by \ref{prop:normsOfUnitisation}, we just need to verify the $C^*$-identity:
\begin{align*}
...
\begin{lemma} \label{lemma:normSelfAdjoint}
...
\end{proposition}
\begin{proof}
We may assume $\varphi \neq 0$ and $\varphi(\vec{1}) = 1$. Using \ref{lemma:normSelfAdjoint}, we calculate
\[ |\varphi(x)+it|^2 = |\varphi(x+it)|^2 \leq \norm{x+it}^2 \leq \norm{x}^2 + t^2. \]
By \ref{lemma:boundedThenReal} this means $\varphi(x)\in\R$.
\end{proof}
TODO: alternate proof in Fillmore using exponential map.
\begin{corollary} \label{corollary:selfAdjointSpectrumReal}
Let $x\in A$ be self-adjoint. Then $\sigma(x) \subseteq \R$.
\end{corollary}
\begin{proof}
By \ref{prop:commutativeSameSpectrum}, and the fact that $x$ is self-adjoint (TODO ref) we may assume $A$ commutative. Let $\lambda \in \sigma(x)$. By \ref{prop:spectrumFromSpectrum} there is a $\varphi\in\hat{A}$ such that $\lambda = \varphi(x)$. By \ref{prop:charactersUnital}, $\norm{\varphi} = \varphi(\vec{1}) = 1$. Then by the proposition $\lambda = \varphi(x) \in \R$.
\end{proof}
\begin{corollary}
...
\end{corollary}
\begin{proof}
By \ref{lemma:realImaginaryParts} we can write $x= x_1+ix_2$. Then $\varphi(x^*) = \varphi(x_1-ix_2) = \varphi(x_1) - i \varphi(x_2)$. By the proposition $\varphi(x_1), \varphi(x_2)\in \R$.
\end{proof}
\begin{corollary} \label{corollary:characters*Preserving}
...
Conversely, suppose $x$ not invertible in $B$. Then either $x^*x$ or $xx^*$ is not invertible in $B$ by \ref{lemma:elementaryStarLemma} and \ref{lemma:productInvertibility}. Let $y$ be one of the two that is not invertible. Because $y$ is self-adjoint, $\sigma_B(y)\subset \R$, by \ref{corollary:selfAdjointSpectrumReal}. Thus $(y_n) = (y+\frac{i}{n})$ is a sequence of invertibles converging to $y$. By \ref{corollary:openSetInvertibles}, $\norm{y_n - y}\geq \norm{y_n^{-1}}^{-1}$ must hold for all $n$, otherwise $y$ would be invertible. Thus $\norm{y_n^{-1}}^{-1}$ must converge to zero and $\norm{y_n^{-1}}$ must diverge (TODO ref). Since the inversion map is continuous on $\GL(A)$, by \ref{corollary:inverseMapContinuous}, it follows that $y$ cannot be invertible in $A$. Since $x$ is invertible if and only if $x^*$ is invertible, by \ref{lemma:elementaryStarLemma}, $y$ being non-invertible implies $x$ is not invertible, by \ref{lemma:productInvertibility}.
\end{proof}
\begin{corollary} \label{corollary:spectrumIndependentOfSurroundingAlgebra}
...
\begin{proposition} \label{prop:generatedAlgebraSpectrumHomeomorphism}
...
and characters are continuous homomorphisms, \ref{prop:charactersContinuous}, we see that $\varphi = \psi$.

Finally we need to show the map is open. Because $\sigma(x)$ is compact, this follows from \ref{prop:compactToHausdorffHomeomorphism}.
\end{proof}

\section{Commutative $C^*$-algebras}
\subsection{The Gelfand-Naimark theorem}
TODO: non-unital case!
\begin{theorem}[Gelfand-Naimark] \label{theorem:GelfandNaimarkCommutative}
...
\begin{proof}
We already know the Gelfand transform is a homomorphism by \ref{lemma:GelfandTransformHomomorphism}.
...
where the third equality is due to \ref{corollary:characters*Preserving}.

For isometry, notice that every element is normal due to commutativity. By \ref{prop:normNormalElement} and \ref{lemma:GelfandTransformHomomorphism}, we have
\[ \norm{x} = r(x) = \norm{\hat{x}}. \]

Then we just need to show the Gelfand transform is bijective (TODO ref). Injectivity follows from isometry, \ref{lemma:isometryInjective}. For surjectivity we want to apply the Stone-Weierstrass theorem \ref{theorem:StoneWeierstrass}. To show the image of $A$ separates points, take $\varphi \neq \psi$ in $\hat{A}$. Then $\varphi(x) \neq \psi(x)$ for some $x\in A$, meaning $\hat{x}(\varphi) \neq \hat{x}(\psi)$. 

By the Stone-Weierstrass theorem \ref{theorem:StoneWeierstrass} the image of $A$ is dense in $\mathcal{\hat{A}}$. By \label{lemma:isometryClosed} the image of an isometry from a complete space is closed. Thus the image of $A$ is all of $\mathcal{\hat{A}}$.
\end{proof}
\begin{corollary}
...
We can assume $A,B$ unital by passing to $\Psi^\dagger$ using \ref{lemma:DaggerMorphismProperties}.

We can also assume $A, B$ are commutative by restricting $\Psi$ to $\Psi': C^*(a,\vec{1}) \to C^*(\Psi(a),\vec{1})$.

By the Gelfand-Naimark theorem \ref{theorem:GelfandNaimarkCommutative} we can suppose we have is a unital injection $\Psi: \mathcal{C}(X)\to \mathcal{C}(Y)$ for some compact Hausdorff spaces $X,Y$. There is then (TODO ref) some continuous surjection $\alpha: Y\to X$ such that $\forall f\in\mathcal{C}(X): \Psi(f) = f\circ \alpha$. Then $\norm{\Psi(f)} = \norm{f}$ because both functions have the same range.
\end{proof}

TODO non-unital Gelfand-Naimark!

\section{Continuous / bounded (?) functional calculus}

TODO: relocate.
\begin{lemma} \label{lemma:WeierstrassApproximation}
...
such that $I_{\sigma(x)}(x) = x$ and $\vec{1}_{\sigma(x)}(x) = \vec{1}_A$. Moreover
\[ \im\Phi_x = C^*(x,\vec{1}). \]
\end{theorem}
\begin{proof}
Unicity from \ref{lemma:WeierstrassApproximation} TODO.

For existence, let $B = C^*(x,\vec{1})$, which is commutative because $x$ is normal. Then $\hat{x}: \hat{B}\to \sigma(x): \varphi\mapsto \varphi(x)$ is a homeomorphism by \ref{prop:generatedAlgebraSpectrumHomeomorphism} and $\hat{x}^t: \mathcal{C}(\sigma(x))\to \mathcal{C}(\hat{B})$ is an isometric isomorphism (TODO!). The we have the isometric $*$-homomorphism
...
where the inverse Gelfand transform $\wedge^{-1}$ exists and is isometric by the Gelfand-Naimark theorem \ref{theorem:GelfandNaimarkCommutative}, because $B$ is commutative.

We verify
\[ I_{\sigma(x)}(x) = \Phi_x(I_{\sigma(x)}) = (\iota\circ\wedge^{-1}\circ\hat{x}^t)(I_{\sigma(x)}) = (\iota\circ\wedge^{-1})(I_{\sigma(x)}\circ\hat{x}) = (\iota\circ\wedge^{-1})(\hat{x}) = \iota(x) = x \]
...
using the fact that the Gelfand transform of $\vec{1}$ is $\varphi\mapsto \varphi(\vec{1})$, which is $\vec{1}_{\hat{B}}$ by \ref{prop:charactersUnital}.
\end{proof}
For polynomials in $z,\overline{z}$, this functional calculus works as expected, because it is a $*$-homomorphism.

In fact we can apply functional calculus to any continuous defined on a superset of the spectrum: restricting to the spectrum still yields a continuous function by \ref{prop:continuousConstructions}.
...
\begin{proposition} \label{prop:propertiesContinuousFunctionalCalculus}
...
\hspace{1em}
\begin{enumerate}
\item The claim is well-defined because $\sigma(\Psi(x))\subseteq \sigma(x)$, by \ref{lemma:spectrumOfImage}. It is easy to check both sides are unital $*$-homomorphisms from $\mathcal{C}(\sigma(x))$ to $B$, sending the identity function to $\Psi(x)$. The claim then follows from uniqueness of the functional calculus.
...
using \ref{prop:spectrumFromSpectrum} and the previous point.
...
\begin{proposition} \label{prop:continuityContinuousFunctionalCalculus}
...
\begin{corollary} \label{corollary:propertiesFromSpectrum}
...
\begin{enumerate}
\item By \ref{corollary:selfAdjointSpectrumReal} we have that self-adjoint implies real spectrum. The converse follows from the lemma applied to $z = \overline{z}$.
\item Assume $x$ unitary. By the $C^*$-identity $\norm{x} = \sqrt{\norm{x^*x}} = 
\sqrt{\norm{\vec{1}}} = 1$, by \ref{lemma:consequencesC*}. By \ref{prop:normNormalElement}, $r(x) = \norm{x} = 1$. Also $\norm{x^{-1}}^{-1} = 1$. So $\sigma(x)\subseteq \mathbb{T}$ by \ref{corollary:openSetInvertibles}. The converse follows from the lemma applied to $1 = z\overline{z} = \overline{z}z$.
...
\end{proposition}
\begin{proof}
Let $x\in A$. By \ref{lemma:realImaginaryParts} we can write $x=x_1+ix_2$ for some self-adjoint $x_1,x_2$.
TODO
\end{proof}
...
\end{definition}
By \ref{corollary:propertiesFromSpectrum} every projection is positive and every positive element is in fact self-adjoint.

By \ref{lemma:normalSpectralRadiusEqualsNorm} we have for all positive $a\in A$:
...
In this case there is a unique $b$ that is positive.
\end{proposition}
\begin{proof}
...
Converse by spectral mapping \ref{prop:propertiesContinuousFunctionalCalculus}.
\end{proof}

\begin{lemma} \label{lemma:positivityDistanceToNorm}
...
$(1.\Rightarrow 2.)$ Assume $a$ positive. Then $\sigma(a) \subseteq [0,\norm{a}]$. By spectral mapping, \ref{prop:propertiesContinuousFunctionalCalculus}, we have $\sigma(\frac{1}{2}\vec{1} - a / \norm{a}) \subseteq [-1/2, 1/2]$ and thus $\norm{\frac{1}{2}\vec{1} - a / \norm{a}} \leq \frac{1}{2}$, by \ref{prop:normNormalElement}.
...
\end{proposition}
\begin{proof}
(1) By spectral mapping, \ref{prop:propertiesContinuousFunctionalCalculus}.

(2) Take $a,b\in A$ and set $p= \frac{\norm{a}+\norm{b}}{\norm{a+b}} \geq 1 \geq 1/2$. Then
\[ \norm{p\vec{1} - \frac{a+b}{\norm{a+b}}} \leq \frac{1}{\norm{a+b}}\left(\norm{\norm{a}-a} + \norm{\norm{b}-b}\right) \leq \frac{\norm{a}+\norm{b}}{\norm{a+b}} = p \]
using the triangle inequality and point 3. of \ref{lemma:positivityDistanceToNorm} with $r=1$.

(3) Convexity follows from additive closure by \ref{lemma:convexityAdditiveClosure}.
\end{proof}


\begin{lemma} \label{lemma:positiveNegativeParts}
Let $A$ be a unital $C^*$-algebra, and $a\in A$ a self-adjoint element. Then there exists a unique decomposition $a=a_+−a_−$ where $a_+,a_−\in A^+$ and $a_+a_− = 0$.
\end{lemma}
\begin{proof}
TODO
\end{proof}
Thus every element of $A$ is a linear combination of four positive elements, by \ref{lemma:realImaginaryParts}.

\begin{proposition}\label{prop:existenceSquareRoot}
Let $A$ be a $C^*$-algebra and $a\in A$. Then $a$ is positive \textup{if and only if} $a = b^*b$ for some $b\in A$.
\end{proposition}
TODO: link with $\sqrt{a}$?
\begin{corollary}
Let $A$ be a concrete $C^*$-algebra of bounded operators on some Hilbert space $\mathcal{H}$ and $a\in A$. Then $a$ is positive \textup{if and only if} for all $x\in \mathcal{H}: \inner{x,ax}\geq 0$.
\end{corollary}
...
using the fact that the spectrum of $a$ is real, \ref{corollary:selfAdjointSpectrumReal}.
\end{proof}
\begin{corollary}
...
\begin{lemma} \label{lemma:propertiesAbsoluteValue}
...
The second point follows from spectral mapping \ref{prop:propertiesContinuousFunctionalCalculus} using the fact that $z\mapsto \overline{z}z$ has positive image in $\C$.

For the third point, $a\mapsto a^*a$ is continuous by \ref{lemma:multiplicationContinuous} and \ref{lemma:consequencesC*}. Then $a\mapsto |a|$ is continuous by \ref{prop:continuityContinuousFunctionalCalculus}.
\end{proof}
...
\end{proposition}
\begin{proof}
If $a$ is invertible, then so are $a^*$ and $|a|$ (TODO ref). Put $u(a) = a|a|^{-1}$. Clearly $a = u(a)|a|$ and $u(a)$ is unitary because it is invertible and
\[ u(a)^*u(a) = |a|^{-1}a^*a|a|^{-1} = |a|^{-1}|a|^2|a|^{-1} = \vec{1}. \]

The continuity of u: $a\mapsto a|a|^{-1}$ follows from the continuity of multiplication, \ref{lemma:multiplicationContinuous}, the continuity of the absolute value, \ref{lemma:propertiesAbsoluteValue} and the continuity of the inverse, \ref{corollary:inverseMapContinuous}.
\end{proof}
...
\end{definition}
By \ref{prop:existenceSquareRoot}, this is equivalent to the condition that $f(x^*x)\geq 0$ for all $x\in A$.
...
\end{proposition}
\begin{corollary}
This form obeys the Cauchy-Schwarz inequality, \ref{theorem:CauchySchwarz}:
...
\end{proposition}
\begin{proof}
By \ref{lemma:realImaginaryParts} and \ref{lemma:positiveNegativeParts} we can write $a\in A$ as
...
\begin{lemma} \label{lemma:nonDegeneracyAlgebraRepresentation}
...
This implies $\overline{\pi(A)\mathcal{H}} = \mathcal{H}_0^\perp$ by \ref{corollary:doubleComplementClosure}. The claim then follows from \ref{prop:OrthogonalComplementProperties}.

The proof then rests on the equality
\[ \forall x,y\in \mathcal{H}, a\in A: \quad \inner{x,\pi(a)y} = \inner{\pi(a^*)x,y}. \]
An $x\in\mathcal{H}$ is an element of $(\pi(A)\mathcal{H})^\perp$ iff the left side is zero for all $y\in\mathcal{H},a\in A$. An $x\in\mathcal{H}$ is an element of $\mathcal{H}_0$ iff $\pi(a^*)x = 0$ for all $a^*\in A$. This is equivalent to saying the right side is zero for all $y\in\mathcal{H},a\in A$ by the non-degeneracy of the inner product \ref{lemma:nonDegeneracyInnerProduct}.
\end{proof}
...
where $\omega(y^*x), \omega(x^*y)$ are zero by corollary \ref{corollary:preInnerProductCSBZero} to the Cauchy-Schwarz inequality.
\end{proof}
...
\end{lemma}
\begin{proof}
We verify this inner product is well-defined: take $x,x'\in [x]$ and $y,y'\in[y]$. Then $(x'-x) \in N_\omega$ and $(y'-y) \in N_\omega$. Again using corollary \ref{corollary:preInnerProductCSBZero} to the Cauchy-Schwarz inequality, we see
...
\begin{lemma} \label{lemma:C*idealSquared}
...
\subset I \subset I^2$.
\end{proof}
\begin{lemma} \label{lemma:productC*ideals}
Let $A$ be a $C^*$-algebra and $I,J\subset A$ ideals. Then $IJ = I\cap J$.
\end{lemma}
\begin{proof}
We calculate $I\cap J = (I\cap J)^2 \subset IJ \subset I\cap J$ using \ref{lemma:C*idealSquared}.
\end{proof}
...
We first show $I(\pi[A])$ contains $\pi[A]$ as an essential ideal. That it contains $A$ as an ideal is obvious. Non-degeneracy of the representation means that the only $x\in\mathcal{H}$ that is mapped to $0$ by all $\pi[A]$ is $0$, by \ref{lemma:nonDegeneracyAlgebraRepresentation}. 
...
\item The relation $(\mathcal{X}\;|\;\mathcal{R}) = (\{x,y\}\;|\;\{\norm{\vec{1}-x^*x-y^*y}\leq 0\})$ is bounded: writing $x$ for $\pi(x)$, the spectrum of $x^*x = 1-y^*y$ is positive and bounded by $1$ according to the spectral mapping theorem \ref{prop:propertiesContinuousFunctionalCalculus}. Now $\norm{\pi(x)} = \sqrt{r(\pi(x)^*\pi(x))} \leq 1$ by \ref{prop:normNormalElement} and so
