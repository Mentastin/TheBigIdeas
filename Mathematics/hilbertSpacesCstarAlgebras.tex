...
(2) We need to show that $\dom(T^*T)$ is dense in $\dom(T)$ w.r.t. the graph norm. Consider the inner product $\inner{}_{\norm{\cdot}_T}$ associated to the graph norm (TODO: yields norm equivalent to graph norm). Suppose $h\in \dom(T)$ is in the orthogonal complement of $\dom(T^*T)$ w.r.t.\ this inner product. Then, for all $x\in \dom(T^*T)$, we have
\[ 0 = \inner{x,h}_{\norm{\cdot}_T} = \inner{x,h} + \inner{Tx,Th} = \inner{x,h} + \inner{T^*Tx,h} = \inner{(\id + T^*T)x,h}. \]
Thus $h\in \im(\id+T^*T)^\perp$ and thus $h=0$ by the calculations in (1). So the orthogonal complement of $\dom(T^*T)$ w.r.t.\ the graph inner product is $\{0\}$, which shows density.
\end{proof}
...
\section{Functionals, morphisms and spectrum}
...
Then we just need to show the Gelfand transform is bijective (TODO ref). Injectivity follows from isometry, \ref{isometryLemma}. For surjectivity we want to apply the Stone-Weierstrass theorem \ref{StoneWeierstrass}. To show the image of $A$ separates points, take $\varphi \neq \psi$ in $\hat{A}$. Then $\varphi(x) \neq \psi(x)$ for some $x\in A$, meaning $\hat{x}(\varphi) \neq \hat{x}(\psi)$. 

By the Stone-Weierstrass theorem \ref{StoneWeierstrass} the image of $A$ is dense in $\mathcal{\hat{A}}$. By \ref{isometryLemma} the image of an isometry from a complete space is closed. Thus the image of $A$ is all of $\mathcal{\hat{A}}$.
\end{proof}
\begin{corollary}
...
\end{lemma}
\begin{proof}
By \ref{normNormalElement}, we have $\norm{a} = \spr(a)$, so, by spectral mapping \ref{propertiesContinuousFunctionalCalculus},
\begin{align*}
...
\end{lemma}
\begin{proof}
(1) Since the function $x\mapsto x-x^2$ is positive on $\interval{0,1}$ and $\spec(a) \subseteq \interval{0,1}$, we have that $a - a^2$ is positive by spectral mapping \ref{propertiesContinuousFunctionalCalculus}.
...
(2) This follows from spectral mapping \ref{propertiesContinuousFunctionalCalculus} using the fact that $z\mapsto \overline{z}z$ has positive image in $\C$.
