...
\begin{lemma}
An inner product over a complex vector space $V$ is antilinear in the first component.
\end{lemma}
...
\begin{enumerate}
\item A \udef{sesquilinear form} is a function $V\times V\to \C$ that is linear in the second component and antilinear in the first.
...
Now we want extend this form of $\omega|_U$ to the whole of $\mathcal{H}$. This works exactly if $v\in(\ker\omega)^\perp$. So we need $U=(\ker\omega)^\perp$ which is true if and only if $\mathcal{H} = \ker\omega\oplus U = \ker\omega\oplus (\ker\omega)^\perp$, which only works in general if $\ker\omega$ is closed and $\mathcal{H}$ is a Hilbert space. Now $\ker\omega$ is closed because $\omega$ it is continuous, by \ref{kernelClosed}.

With this idea we give a full proof:
\begin{proof}
If $\ker\omega = \mathcal{H}$, we can take $v_\omega = 0$.

Assume $\ker\omega\neq \mathcal{H}$, then $(\ker\omega)^\perp\neq \{0\}$ by \ref{denseZeroComplement}, because $\ker\omega$ is closed (\ref{kernelClosed}). So we can take a non-zero $u\in (\ker\omega)^\perp$. We can choose it such that $\omega(u) = 1$, by rescaling. Now let $h\in\mathcal{H}$. We can write $h = (h - \omega(h)u)+\omega(h)u\in\ker\omega\oplus (\ker\omega)^\perp$, because $\omega(h - \omega(h)u) = 0$. So
...
\begin{enumerate}
\item the map $C_\mathcal{H}:\mathcal{H}\to \dual{\mathcal{H}}: v\mapsto \inner{v,\cdot}$ is a bijective antilinear isometry;
...
\begin{lemma} \label{WOTforInnerProductSpace}
...
Remember that finite-rank operators are bounded by definition (this is not automatic, cfr. \ref{closedKernelLinearFunctionToFiniteDimSpaceContinuous}).
...
The dilation $\begin{pmatrix}
T & D_{T^*} \\ D_T & -T^*
\end{pmatrix}$ is called the \udef{elementary rotation} or \udef{Julia operator}.
...
\[ [\DFT_N]_{j,k} = \frac{1}{\sqrt{N}}\omega_N^{-jk}, \]
where $\omega_N$ is the $N^\text{th}$ root of unity, $e^{\frac{2\pi i}{N}}$.
\end{definition}
The indices $j,k$ start at zero!

By \ref{DFTlemma}, we have $[\DFT_N^{-1}]_{j,k} = \frac{1}{\sqrt{N}}\omega_N^{jk}$. For any vector $\vec{x}\in \C^N$, we write
\[ [\vec{x}]_k = \big[\DFT_N^{-1}\DFT_N\vec{x}\big]_k = \sum_{l=0}^{N-1}\frac{\omega_N^{kl}}{\sqrt{N}}[\DFT_N\vec{x}]_l = \sum_{l=0}^{N-1}[\DFT_N\vec{x}]_l\frac{e^{2\pi i\frac{l}{N}k}}{\sqrt{N}} \]
So we have written $\vec{x}$ as a combination of `waves' with different frequencies. The $l^\text{th}$ component of $\DFT_N\vec{x}$ is the amplitude of the wave with frequency $\frac{l}{N}$.

The discrete Fourier transform is sometimes defined as
\[ [\DFT_N]_{j,k} = \omega_N^{-jk} \quad\text{or}\quad [\DFT_N]_{j,k} = \frac{1}{N}\omega_N^{-jk}. \]
In these cases it is not unitary.

The discrete Fourier transform is sometimes defined as
\[ [\DFT_N]_{j,k} = \frac{1}{\sqrt{N}}\omega_N^{jk}, \]
which swaps the roles of $\DFT_N$ and $\DFT_N^{-1}$. This can be interpreted as inverting the frequency axis (i.e.\ the $l^\text{th}$ component of $\DFT_N\vec{x}$ is now the amplitude of the wave with frequency $-\frac{l}{N}$.)

\begin{lemma} \mbox{} \label{DFTlemma}
\begin{enumerate}
...
(2) We calculate
\[ [\DFT_N\cdot \DFT_N]_{j,l} = \frac{1}{N}\sum_{k=0}^{N-1}\omega_N^{-jk}\overline{\omega_N}^{-kl} = \frac{1}{N}\sum_{k=0}^{N-1}\omega_N^{k(l-j)} = \delta_{j,l}. \]
TODO ref sum of roots of unity is zero.
\end{proof}

\begin{proposition}[Danielson-Lanczos lemma]
Let $M,N\in \N$ and $\seq{x_k}\in \C^{MN}$. Then
\[ [\DFT_{MN}\seq{x_k}]_{l} = \sum_{k_0 = 0}^{M-1}\frac{\omega_{MN}^{-lk_0}}{\sqrt{M}}\big[\DFT_N\seq{x_{k_1\cdot M+k_0}}_{k_1}\big]_{l}. \]
\end{proposition}
Often the name ``Danielson-Lanczos lemma'' is used to mean the special case of $M=2$. In this case the proposition simplifies to
\[ [\DFT_{2N}\seq{x_k}]_{l} = \frac{1}{\sqrt{2}}\big[\DFT_N\seq{x_{2k_1}}_{k_1}\big]_{l} + \frac{\omega_{2N}^{-l}}{\sqrt{2}}\big[\DFT_N\seq{x_{2k_1+1}}_{k_1}\big]_{l}, \]
which is a sum of the DFT of the even components with the DFT of the odd components times a factor, often called a \udef{twiddle factor}. This procedure is also known as ``decimation in time''.
\begin{proof}
Using \ref{sumAsDecimation}, we can calculate
\begin{align*}
[\DFT_{MN}\seq{x_k}]_{l} &= \sum_{k=0}^{MN - 1}\frac{\omega_{MN}^{-kl}}{\sqrt{MN}}x_k \\
&= \sum_{k_0=0}^{M - 1}\sum_{k_1=0}^{N-1}\frac{\omega_{MN}^{-(k_1M + k_0)l}}{\sqrt{MN}}x_{k_1\cdot M+k_0} \\
&= \sum_{k_0=0}^{M - 1}\sum_{k_1=0}^{N-1}\frac{\omega_{MN}^{-k_0l}}{\sqrt{M}}\frac{\omega_{MN}^{-k_1M l}}{\sqrt{N}}x_{k_1\cdot M+k_0} \\
&= \sum_{k_0=0}^{M - 1}\frac{\omega_{MN}^{-k_0l}}{\sqrt{M}}\sum_{k_1=0}^{N-1}\frac{\omega_{N}^{-k_1l}}{\sqrt{N}}x_{k_1\cdot M+k_0} \\
&= \sum_{k_0=0}^{M - 1}\frac{\omega_{MN}^{-lk_0}}{\sqrt{M}} \big[\DFT_N\seq{x_{k_1\cdot M+k_0}}_{k_1}\big]_{l}.
\end{align*}
\end{proof}
By swapping the order of summation, we also have
\[ [\DFT_{MN}\seq{x_k}]_{l} = \sum_{k_1=0}^{N-1}\sum_{k_0=0}^{M - 1}\frac{\omega_{MN}^{-(k_1M + k_0)l}}{\sqrt{MN}}x_{k_1\cdot M+k_0} = \sum_{k_1=0}^{N-1}\frac{\omega_{N}^{-k_1l}}{\sqrt{MN}}\sum_{k_0=0}^{M - 1}\omega_{M}^{\frac{-k_0l}{N}}x_{k_1\cdot M+k_0}. \]
This is known as ``decimation in frequency''. In the case $M=2$, we get
\[ [\DFT_{2N}\seq{x_k}]_{l} = \sum_{k_1=0}^{N-1}\frac{\omega_{N}^{-k_1l}}{\sqrt{2N}}\Big(x_{2k_1} + e^{\frac{-l\pi i}{N}}x_{2k_1+1}\Big). \]
...
\begin{proposition}
The $B^*$-identity is equivalent to the \udef{$C^*$-identity}
\[\forall x\in A: \quad \norm{x^*x} = \norm{x^*}\cdot\norm{x}.\]
\end{proposition}
\begin{proof}
...
\end{proposition}
TODO: version of this proposition for non-unital $A$, using approximate unit.
\begin{proof}
We may assume $\varphi \neq 0$ and $\varphi(\vec{1}) = 1 = \norm{\varphi}$, by rescaling. Using \ref{normSelfAdjoint}, we calculate
...
\end{proof}
\begin{corollary} \label{linearFunctionalStarPreserving}
...
\end{corollary}
TODO non-unital version with approximate unit.
\begin{proof}
...
\begin{proposition} \label{charactersStarPreserving}
Let $A$ be a $C^*$-algebra and $\varphi$ a character on $A$. Then $\varphi$ is $*$-preserving.
\end{proposition}
\begin{proof}
We extend $\varphi$ to the character $\varphi^\dagger$ on $A^\dagger$ using \ref{halfUnitalExtensionLinearFunction}. Now $\varphi^\dagger$ is $*$-preserving by \ref{linearFunctionalStarPreserving} and \ref{charactersUnital}. By restricting to $A$, we have that $\varphi$ is $*$-preserving.
\end{proof}

\subsubsection{Projections}

\begin{lemma} \label{projectionsNorm}
Let $A$ be a $C^*$-algebra and $p\in \Projections(A)$. Then $\norm{p} \in \{0,1\}$.
\end{lemma}
\begin{proof}
We have $\norm{p}^2 = \norm{p^*p} = \norm{p^2} = \norm{p}$ from the $B^*$-identity. The only solutions of this equation are $\norm{p} = 0$ and $\norm{p} = 1$.
\end{proof}

\subsubsection{Partial isometries}
Partial isometry is the same as saying the adjoint is the Moore-Penrose pseudoinverse (i.e. $x^* = x^\dagger$).

\begin{lemma} \label{partialIsometriesNorm}
Let $A$ be a $C^*$-algebra and $v\in A$ a partial isometry. Then $\norm{v} \in \{0,1\}$.
\end{lemma}
\begin{proof}
We have $\norm{v}^2 = \norm{v^*v} \in \{0,1\}$ from the $B^*$-identity and \ref{projectionsNorm}.
\end{proof}

\begin{lemma} \label{partialIsometries}
Let $A$ be a $C^*$-algebra and $v\in A$.
If either $v^*v$ or $vv^*$ is idempotent, then
\begin{enumerate}
\item $v=vv^*v$ and $v^* = v^*vv^*$;
\item the other is also idempotent;
\item $v$ is a partial isometry.
\end{enumerate}
Thus either condition is sufficient for $v$ to be a partial isometry.
\end{lemma}
\begin{proof}
First assume $v^*v$ is idempotent. Put $z = (\vec{1}- vv^*)v$. Then
\[ z^*z = v^*(\vec{1}-vv^*)(\vec{1}-vv^*)v = v^*v - v^*vv^*v -v^*vv^*v + v^*vv^*vv^*vv^*v = v^*v - v^*v - v^*v + v^*v = 0. \]
Now by the $C^*$-identity $\norm{z} = \sqrt{\norm{z^*z}} = 0$, so $0=z= v - vv^*v$. Multiplying this on the right by $v^*$ gives $vv^* = (vv^*)^2$.

If we assume $vv^*$ is idempotent, we put $z = (\vec{1}- v^*v)v^*$ instead and again $z^*z = 0$.

In this case both $v^*v$ and $vv^*$ are projections, since they are automatically self-adjoint.
\end{proof}
...
\subsubsection{Unitaries}
\begin{lemma} \label{unitariesNorm}
Let $A$ be a $C^*$-algebra and $u\in \Unitaries(A)$. Then $\norm{u} = 1$.
\end{lemma}
\begin{proof}
We have $\norm{u}^2 = \norm{u^*u} = \norm{\vec{1}} = 1$ from the $B^*$-identity and \ref{consequencesC*}.
\end{proof}
...
\subsection{The Gelfand-Neumark theorem}
\begin{theorem}[Unital Gelfand-Neumark] \label{GelfandNeumarkCommutative}
...
where the third equality is due to \ref{charactersStarPreserving}.
...
\end{corollary}
\begin{proof}
The first part follows from the Gelfand-Neumark theorem and the fact that $\hat{A}$ is compact, \ref{characterSpaceLocallyCompact}.
...
\begin{theorem}[Non-unital Gelfand-Neumark] \label{nonUnitalGelfandNeumarkCommutative}
Let $A$ be a commutative $C^*$-algebra. Then the Gelfand transform
\[ \evalMap_{-}|_{\hat{A}}: A\to\cont_0(\hat{A}): x\mapsto \evalMap_{x} \]
is an isometric $*$-isomorphism.
\end{theorem}
\begin{proof}
By \ref{GelfandNeumarkCommutative}, the function $\evalMap_{-}|_{\hat{A^\dagger}}: A^\dagger\to\cont(\hat{A^\dagger}): x\mapsto \evalMap_{x}$ is an isometric $*$-isomorphism. Let $h: \hat{A}^\dagger \to \widehat{A^\dagger}$ be the homeomorphism from \ref{compactificationOfCharacterSpaceIsCharacterSpaceOfUnitisation}, so $h^\star$ is an algebra $*$-isomorphism. Let $\Phi: \cont_0(\hat{A}^\dagger) \to \cont(\hat{A}^\dagger)$ be the $*$-isomorphism from \ref{unitisationOnePointCompactificationIsomorphism}. Then
...
TODO non-unital Gelfand-Neumark!
...
where the inverse Gelfand transform $\big(\evalMap_{-}|_{\hat{B}}\big)^{-1}$ exists and is an isometric isomorphism by the Gelfand-Neumark theorem \ref{GelfandNeumarkCommutative}, since $B$ is commutative.
...
\begin{lemma} \label{productPositiveCommutingElementsPositive}
Let $A$ be a $C^*$-algebra and $a,b\in A^+$. If $a$ and $b$ commute, then $ab\in A^+$.
\end{lemma}
It is not true that $a^2\leq b^2$ or that $ab$ is positive in general!
\begin{proof}
Immediate from \ref{spectrumAlgebraicOperationsOnCommutingElements}.
\end{proof}
\begin{proof}[Alternate proof]
We calculate have $ab = \sqrt{a}b\sqrt{a} = \big(\sqrt{b}\sqrt{a}\big)^*\sqrt{b}\sqrt{a} \geq 0$, using \ref{existenceSquareRoot}.
\end{proof}
...
\begin{definition}
Let $A$ be a $C^*$-algebra. An \udef{increasing approximate unit} on $A$ is an approximate unit that is a monotonically increasing net of positive elements.
\end{definition}
Note the positivity requirement!

\begin{lemma} \label{increasingApproximateUnitLemma}
Let $t\in \R^+$ and $n\in \N\setminus\{0\}$. Then
\begin{enumerate}
\item $0 \leq t - \frac{t^2}{t+n^{-1}} \leq \frac{1}{n}$;
\item $t - \frac{t^2}{t+n^{-1}} = 1 - (1 + nt)^{-1}$. 
\end{enumerate}
\end{lemma}

\begin{proposition} \label{approximateUnitInIdeal}
Let $A$ be a unital $C^*$-algebra and $J\subseteq A$ a (not necessarily
closed) left-ideal. Then there exists a net $\seq{e_\lambda}_{\lambda \in \Lambda}$ in $J$ such that
\begin{itemize}
\item $\seq{e_\lambda}$ is increasing;
\item $e_\lambda \geq 0$ for all $\lambda \in \Lambda$;
\item $\norm{e_\lambda} \leq 1$ for all $\lambda \in \Lambda$;
\item $xe_\lambda \to x$ for all $x\in J$.
\end{itemize}
\end{proposition}
Note that it is not an approximate unit of $J$ because we do not have $e_\lambda x \to x$.
\begin{proof}
Let $\Lambda$ be the set of finite non-empty subsets of $J$, which is a directed set. For all $\lambda\in \Lambda$, we define $v_\lambda \defeq \sum_{x \in \lambda}x^*x$, which is an element of $J$ because $J$ is a left-ideal and $x\in J$ for all $x\in \lambda$. Set $n\defeq |\lambda|$. Since $v_\lambda$ is positive, we have that $n^{-1}\vec{1}+v_\lambda$ is invertible by spectral mapping \ref{spectralMappingCFC}. Thus we can define
\[ e_\lambda \defeq (n^{-1}\vec{1}+v_\lambda)^{-1}v_\lambda \in J, \]
which is an element of $J$, since $J$ is a left ideal. We have
\[ \norm{e_\lambda} = \norm{(n^{-1}\vec{1}+v_\lambda)^{-1}v_\lambda} \leq 1, \]
by spectral mapping \ref{spectralMappingCFC} and \ref{normNormalElement}.

Now take arbitrary $x\in J$ and $\lambda \in \Lambda$ such that $x\in \lambda$. We compute
\begin{align*}
n^{-1} &\geq \norm*{v_\lambda - v_\lambda^2(n^{-1}\vec{1} + v_\lambda)^{-1}} \\
&= \norm*{v_\lambda(\vec{1} - e_\lambda)} \\
&\geq \norm{(\vec{1}-e_\lambda)}\norm*{v_\lambda(\vec{1} - e_\lambda)} \\
&\geq \norm*{(\vec{1} - e_\lambda)v_\lambda(\vec{1} - e_\lambda)} \\
&\geq \norm*{(\vec{1} - e_\lambda)x^*x(\vec{1} - e_\lambda)} \\
&= \norm*{x - xe_\lambda}^2,
\end{align*}
using \ref{increasingApproximateUnitLemma}, \ref{spectralMappingCFC}, \ref{normNormalElement}, \ref{inequalityPositiveElementsLemma}, \ref{CstarOrderLemma} and the $B^*$-identity. This implies $xe_\lambda \to x$.

Finally, we show that $\seq{e_\lambda}$ is increasing. From \ref{increasingApproximateUnitLemma}, we have $e_\lambda = \vec{1} - (1+nv_\lambda)^{-1}$. Suppose $\mu\in \Lambda$ and $\lambda \subseteq \mu$. Set $m \defeq |\mu|$. Then $n\leq m$ and $v_\lambda \leq v_\mu$, so $nv_\lambda \leq mv_\mu$. We have $(\vec{1}+mv_\mu)^{-1} \leq (\vec{1}+nv_\lambda)^{-1}$ from \ref{inequalityPositiveElementsLemma} and thus
\[ e_\lambda = \vec{1} - (1+nv_\lambda)^{-1} \leq \vec{1} - (1+mv_\mu)^{-1} = e_\mu. \]
\end{proof}
\begin{corollary} \label{CstarAlgebraHasIncreasingApproximateUnit}
Every $C^*$-algebra has an increasing approximate unit.
\end{corollary}
\begin{proof}
Let $A$ be a $C^*$-algebra. If $A$ is unital, the constant net $\constant{\vec{1}}$ is an increasing approximate unit.

If $A$ is non-unital, we have that $A\subseteq A^\dagger$ as an ideal. The proposition yields a net $\seq{e_\lambda}_{\lambda\in\Lambda}$. All we need to prove is that $e_\lambda a \to a$ for all $a\in A$. Since $a^*\in A$, we have $a^*e_\lambda \to a^*$. Taking the adjoint gives $e_\lambda a \to a$.
\end{proof}
\begin{corollary} \label{closedTwoSidedIdealCstarAlgebra}
Let $A$ be a $C^*$-algebra and $J\subseteq A$ a closed two-sided ideal. Then
\begin{enumerate}
\item $J = J^*$;
\item $J$ is a $C^*$-subalgebra.
\end{enumerate}
\end{corollary}
\begin{proof}
(1) Take arbitrary $x\in J$. Then $xe_\lambda \to x$, so $e_\lambda x^*\to x$. Since $J$ is a two-sided ideal, we have $e_\lambda x^*\in J$ for all $\lambda\in\Lambda$. Since $J$ is closed, we have $x^*\in J$.

(2) We have that $J$ is a subalgebra by definition of ideal. From (1), we have that $J$ is a $*$-subalgebra. Finally, $J$ is complete because it is closed, using \ref{closedComplete}.
\end{proof}
\begin{corollary}\label{closedTwoSidedIdealHasIncreasingApproximateUnit}
Every closed two-sided ideal in a $C^*$-algebra has an increasing approximate unit.
\end{corollary}
\begin{proof}
Immediate from \ref{closedTwoSidedIdealCstarAlgebra} and \ref{CstarAlgebraHasIncreasingApproximateUnit}.
\end{proof}
...
\end{proposition}
Every $C^*$-algebra has an increasing approximate unit, by \ref{CstarAlgebraHasIncreasingApproximateUnit}.
\begin{proof}
...
\begin{lemma}
Let $A$ be a $C^*$-algebra, $f: A\to \C$ a positive bounded linear functional on $A$ and $a,x\in A$. Then
\begin{enumerate}
\item $f(a) = \overline{f(a^*)}$;
\item $|f(a)|^2 \leq \norm{f}f(a^*a) = \norm{f}\inner{a,a}_f$.
\end{enumerate}
\end{lemma}
TODO: compare with \ref{linearFunctionalStarPreserving} (also non-unital version).
\begin{proof}
...
Let $\seq{e_\lambda}_{\lambda\in \Lambda}$ be an increasing approximate unit, which exists due to \ref{CstarAlgebraHasIncreasingApproximateUnit}. Since $f$ is bounded \ref{positiveLinearFunctionalBounded}, we have
...
\begin{definition}
A \udef{state} on a $C^*$-algebra $A$ is a positive linear functional of norm $1$. The set $\states(A)$ of all states on $A$ is called the \udef{state space} of $A$. The extreme points of $\states(A)$ are called \udef{pure states}.
\end{definition}
...
\begin{proposition} \label{setOfStatesConvex}
Let $A$ be a $C^*$ algebra. Then $\states(A)$ is convex.
\end{proposition}
\begin{proof}
Take $\omega_1,\omega_2\in \states(A)$ and $r\in \interval{0,1}$. Take $a\in A^+$. Then $\omega_1(a), \omega_2(a)\in \R^+$, so $r\omega_1(a) + (1-r)\omega_2(a) \in \R^+$. This implies that $r\omega_1 + (1-r)\omega_2$ is a positive linear functional. We just need to check that the norm is $1$. Let $\seq{e_\lambda}$ be an increasing approximate unit. Then, from \ref{positiveFunctionalNormValueAtUnit}, we have
\begin{align*}
\norm{r\omega_1 + (1-r)\omega_2} &= \lim_\lambda \big(r\omega_1 + (1-r)\omega_2\big)(e_\lambda) \\
&= r\lim_\lambda \omega_1(e_\lambda) + (1-r)\lim_\lambda \omega_2(e_\lambda) \\
&= r\norm{\omega_1} + (1-r)\norm{\omega_2} = r + (1-r) = 1.
\end{align*}
Thus $r\omega_1 + (1-r)\omega_2 \in \states(A)$ and $\states(A)$ is convex.
\end{proof}

\begin{lemma}
Let $B\subseteq A$ be unital $C^*$-algebras. A state on $B$ extends to a state on $A$.
\end{lemma}
\begin{proof}
...
Let $A$ be a $C^*$-algebra and $a\in \Normals(A)\setminus\{0\}$. Then there exists a state $\omega\in \states(A)$ such that $|\omega(a)| = \norm{a}$.
\end{lemma}
\begin{proof}
TODO see Murphy, theorem 5.1.11.
\end{proof}

\begin{proposition} \label{propertiesPositiveFunctionals}
...
\end{enumerate}
\end{proposition}
TODO: obtain from general theory of positive operators.
\begin{proof}
By \ref{CartesianDecomposition} we can write $a\in A$ as
...
\begin{lemma} \label{pureStateEquivalents}
Let $A$ be a $C^*$-algebra and $\omega\in \states(A)$. Then the following are equivalent:
\begin{enumerate}
\item $\omega$ is pure;
\item for all linear functionals $\rho \in \interval{\constant{0}, \omega}$, we have $\rho = t\omega$ for some $t\in \interval{0,1}$.
\end{enumerate}
\end{lemma}
\begin{proof}
$(1) \Rightarrow (2)$ Suppose $\rho \in \interval{\constant{0}, \omega}$ is a linear functional. Then $\rho$ and $\omega-\rho$ are positive. Let $\seq{e_\lambda}$ be an increasing approximate unit, which exists by \ref{CstarAlgebraHasIncreasingApproximateUnit}. Then we calculate, using \ref{positiveFunctionalNormValueAtUnit},
\begin{align*}
\omega &= \rho + (\omega - \rho) \\
&= \norm{\rho}\frac{\rho}{\norm{\rho}} + \norm{\omega - \rho}\frac{\omega - \rho}{\norm{\omega - \rho}} \\
&= \norm{\rho}\frac{\rho}{\norm{\rho}} + \lim_{\lambda}(\omega - \rho)(e_\lambda)\frac{\omega - \rho}{\norm{\omega - \rho}} \\
&= \norm{\rho}\frac{\rho}{\norm{\rho}} + \big(\lim_{\lambda}\omega(e_\lambda) - \lim_{\lambda}\rho(e_\lambda)\big)\frac{\omega - \rho}{\norm{\omega - \rho}} \\
&= \norm{\rho}\frac{\rho}{\norm{\rho}} + \big(1 - \norm{\rho}\big)\frac{\omega - \rho}{\norm{\omega - \rho}}.
\end{align*}
Since $\omega$ is an extreme point, we have $\omega = \frac{\rho}{\norm{\rho}}$. Finally we need to observe that $\norm{\rho} \leq 1$. This follows from $\norm{\rho} = \lim_\lambda\rho(e_\lambda) \leq \lim_\lambda\omega(e_\lambda) = \norm{\omega} = 1$.

$(2) \Rightarrow (1)$ Suppose $\omega = r\omega_1 + (1-r)\omega_2$, for some $\omega_1, \omega_2\in\states(A)$ and $r\in \interval[o]{0,1}$. Then $r\omega_1 \leq \omega$, since $(1-r)\omega_2 \geq 0$. By assumption $r\omega_1 = t\omega$, for some $t\in \interval{0,1}$. Taking the norm gives $t = r$, so $\omega_1 = \omega$ and thus $\omega_2 = \omega$. We conclude that $\omega$ is an extreme point.
\end{proof}

\begin{proposition}
Let $A$ be a $C^*$-algebra. Then
\begin{enumerate}
\item every character on $A$ is a pure state;
\item if $A$ is commutative, then every pure state is a character.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) Let $\varphi$ be a character on $A$. Take $a\in A^+$. Then we can write $a = b^*b$, by \ref{existenceSquareRoot}. So, using \ref{charactersStarPreserving}, we have
\[ \varphi(a) = \varphi(b^*b) = \varphi(b^*)\varphi(b) = \overline{\varphi(b)}\varphi(b) = |\varphi(b)|^2 \]
and thus $\varphi(a) \geq 0$. We also have $\norm{\varphi} = 1$ from \ref{charactersUnital} and \ref{CstarAlgebraHasIncreasingApproximateUnit}. Thus $\varphi$ is a state.

We now show that $\varphi$ is pure. Pick a linear functional $\rho \in \interval{0, \varphi}$. Take $a\in \ker(\varphi)$. Then, by \ref{propertiesPositiveFunctionals} and the calculation above,
\[ |\rho(a)|^2 \leq \norm{\rho}\rho(a^*a) \leq \norm{\rho}\varphi(a^*a) = \norm{\rho}|\varphi(a)|^2 = 0. \]
Thus $\ker(\varphi) \subseteq \ker(\rho)$. Using \ref{kernelInclusionDecomposition}, this implies that there exists $t\in \C$ such that $\rho = t\varphi$. If $\rho = 0$, we may take $t = 0$. If not, there exists $a\in A^+$ such that $\rho(a)\neq 0$. Then $0 < \rho(a) \leq \varphi(a)$, so $t = \frac{\rho(a)}{\varphi(a)} \in \interval{0,1}$.

We conclude that $\varphi$ is pure using \ref{pureStateEquivalents}.

(2) Now assume $A$ commutative. Let $\varphi$ be a pure state. Pick arbitrary $a\in A^+\setminus\{0\}$. For all $b\in A^+$, we have $ab\in A^+$ from \ref{productPositiveCommutingElementsPositive} and thus $\varphi(ab) \geq 0$. This means that the functional $x\mapsto \varphi(ax)$ is positive.

Again, take arbitrary $b\in A^+$. Then $\varphi(ab) = |\varphi(ab)| = \big|\varphi\big(\sqrt{b}a\sqrt{b}\big)\big| \leq \varphi(b)\norm{a}$ from \ref{propertiesPositiveFunctionals}. This implies that $x \mapsto \varphi\big(\frac{a}{\norm{a}}x\big)$ is in $\interval{0,\varphi}$. Thus, from \ref{pureStateEquivalents}, there exists $t\in \interval{0,1}$ such that for all $x\in A$, we have $\varphi(ax) = t\varphi(x)$.

Let $\seq{e_\lambda}$ be an increasing positive unit (which exists, \ref{CstarAlgebraHasIncreasingApproximateUnit}). Then
\[ t = \lim_{\lambda}t\varphi(e_\lambda) = \lim_{\lambda}\varphi(ae_\lambda) = \varphi\big(\lim_{\lambda}ae_\lambda\big) = \varphi(a), \]
using \ref{positiveFunctionalNormValueAtUnit}. Thus $\varphi(ax) = \varphi(a)\varphi(x)$ for all $x\in A$ and $a\in A^+$. Since the positive elements span $A$, see \ref{CartesianDecomposition}, we have that $\varphi$ is multiplicative.
\end{proof}
...
\begin{lemma} \label{CstarIdealInIdealIsIdeal}
Let $A$ be a $C^*$-algebra, $J\subseteq A$ a closed two-sided ideal and $K\subseteq J$ a closed two-sided ideal. Then $K\subseteq A$ is a closed two-sided ideal.
\end{lemma}
\begin{proof}
We have that $K$ is closed in $A$ from \ref{subspaceAdherence}, using the fact that $J$ is closed.

We now just need to show that $K$ is an ideal in $A$. We have that $J$ contains an increasing approximate unit $\seq{e_\lambda}$, by \ref{closedTwoSidedIdealHasIncreasingApproximateUnit}. Take $k\in K$ and $a\in A$. Then $ka = \lim_\lambda ke_\lambda a$. Since $e_\lambda a \in J$, we have $ke_\lambda a \in K$ and $ka\in K$ because $K$ is closed. The argument for $ak$ is similar.
\end{proof}

\begin{lemma} \label{starCongruenceIffStarClosed}
Let $A$ be a $*$-algebra and $\mathfrak{q}$ a translation invariant binary relation on $A$. Then $\mathfrak{q}$ is a $\{*\}$-congruence \textup{if and only if} $\widetilde{\mathfrak{q}}^* = \widetilde{\mathfrak{q}}$.
\end{lemma}
\begin{proof}
First suppose $\mathfrak{q}$ is a $\{*\}$-congruence and take $x-y\in \widetilde{\mathfrak{q}}$. Then $(x-y)^* = x^* - y^* \in\widetilde{\mathfrak{q}}$. This implies $\widetilde{\mathfrak{q}}^* \subseteq \widetilde{\mathfrak{q}} = \widetilde{\mathfrak{q}}^{**} \subseteq \widetilde{\mathfrak{q}}^*$.

Now suppose $\widetilde{\mathfrak{q}}^* = \widetilde{\mathfrak{q}}$ and $x \mathrel{\mathfrak{q}}y$. Then $x^*-y^* = (x-y)^* \in \widetilde{\mathfrak{q}}^* = \widetilde{\mathfrak{q}}$, so $x^* \mathrel{\mathfrak{q}}y^*$.
\end{proof}

\begin{lemma} \label{quotientNormApproximateUnitCstarAlgebra}
Let $A$ be a $C^*$-algebra, $J\subseteq A$ a closed two-sided ideal, $a\in A$ and $\seq{e_\lambda}_{\lambda\in\Lambda}$ and increasing approximate unit in $J$. Then $\norm{a+J}_{A/J} = \lim_\lambda \norm{a-ae_\lambda}$.
\end{lemma}
There exists a due to \ref{closedTwoSidedIdealHasIncreasingApproximateUnit}.
\begin{proof}
We have
\[ \norm{a+J}_{A/J} = \inf_{x\in J}\norm{a+x} \leq \liminf_\lambda\norm{a-ae_\lambda}, \]
since $ae_\lambda \in J$ for all $\lambda\in\Lambda$.

Now take arbitrary $x\in J$ and $\epsilon >0$. There exists $\lambda\in\Lambda$ such that $\norm{x-xe_\lambda} \leq \epsilon$ and we can calculate in $A^\dagger$:
\begin{align*}
\norm{a-ae_\lambda} &= \norm{(a+x)(\vec{1}-e_\lambda) - (x-xe_\lambda)} \\
&\leq \norm{(a+x)(\vec{1}-e_\lambda)} + \norm{x-xe_\lambda} \\
&\leq \norm{(a+x)(\vec{1}-e_\lambda)} + \epsilon \\
&\leq \norm{a+x} + \epsilon,
\end{align*}
where we have used $\norm{\vec{1}-e_\lambda} \leq 1$, which follows from \ref{selfAdjointUnitSphereFromOrder} (using $e_\lambda \geq 0$ and $\norm{e_\lambda} \leq 1$). Thus $\limsup_\lambda \norm{a-ae_\lambda} \leq \norm{a+x} + \epsilon$ for all $x\in J$ and $\epsilon > 0$, which implies $\limsup_\lambda \norm{a-ae_\lambda} \leq \inf_{x\in J}\norm{a+x} = \norm{a+J}_{A/J}$. Now
\[ \norm{a+J}_{A/J} \leq \liminf_\lambda\norm{a-ae_\lambda} \leq \limsup_\lambda\norm{a-ae_\lambda} \leq \liminf_\lambda\norm{a-ae_\lambda}, \]
which implies that $\lim_\lambda\norm{a-ae_\lambda}$ exists and is equal to $\norm{a+J}_{A/J}$.
\end{proof}

\begin{proposition}
Let $A$ be a $C^*$-algebra and $J\subseteq A$ a closed two-sided ideal. Then the quotient $A/J$ is a $C^*$-subalgebra.
\end{proposition}
\begin{proof}
From \ref{quotientBanachAlgebra}, we know that $A/J$ is a Banach algebra. Since $J$ is $*$-closed, by \ref{closedTwoSidedIdealCstarAlgebra}, we have that $A/J$ is a $*$-Banach quotient algebra using \ref{starCongruenceIffStarClosed}.

Finally, we need to verify that the $C^*$-identity holds. Take arbitrary $x\in J$ and calculate (in the unitisation $A^\dagger$) using \ref{quotientNormApproximateUnitCstarAlgebra}:
\begin{align*}
\norm{a+J}_{A/J}^2 &= \lim_\lambda \norm{a-ae_\lambda}^2 \\
&= \lim_\lambda \norm{a(\vec{1}-e_\lambda)}^2 \\
&= \lim_\lambda \norm{(\vec{1}-e_\lambda)a^*a(\vec{1}-e_\lambda)} \\
&= \lim_\lambda \norm{(\vec{1}-e_\lambda)(a^*a+ x)(\vec{1}-e_\lambda) - (\vec{1}-e_\lambda)x(\vec{1}-e_\lambda)} \\
&\leq \lim_\lambda \norm{(\vec{1}-e_\lambda)(a^*a+ x)(\vec{1}-e_\lambda)} - \lim_\lambda \norm{(\vec{1}-e_\lambda)x(\vec{1}-e_\lambda)} \\
&\leq \lim_\lambda \norm{(\vec{1}-e_\lambda)(a^*a+ x)(\vec{1}-e_\lambda)} \\
&\leq \norm{a^*a+ x}.
\end{align*}
Since $x\in J$ was taken arbitrarily, we have $\norm{a+J}_{A/J}^2 \leq \inf_{x\in J}\norm{a^*a+ x} = \norm{a^*a+J}_{A/J}$ and we conclude using \ref{C*identityEquivalent}.
\end{proof}

\subsection{Moore-Penrose pseudoinverse}
\begin{definition}
Let $A$ be a $C^*$ algebra and $x\in A$. Let $x^\dagger$ be an element of $A$ such that
\begin{itemize}
\item $xx^\dagger x = x$;
\item $x^\dagger xx^\dagger  = x^\dagger $;
\item $xx^\dagger $ is self-adjoint;
\item $x^\dagger x$ is self-adjoint.
\end{itemize}
Then $x^\dagger $ is called a \udef{Moore-Penrose pseudoinverse} of $x$.
\end{definition}
We will show that an element has at most one Moore-Penrose pseudoinverse, so we are justified in speaking of \emph{the} Moore-Penrose pseudoinverse of $x$, which we denote $x^\dagger$.

If $x$ has a generalised inverse $x^\dagger $, then $xx^\dagger $ and $x^\dagger x$ are automatically idempotent, so the requirement that they be Hermitian is equivalent to requiring they be orthogonal projectors.

\begin{lemma} \label{supportRangeProjectionsElementWithMPPseudoinverse}
Let $A$ be a $C^*$ algebra and $x\in A$ be an element that has a Moore-Penrose pseudoinverse. Then
\begin{enumerate}
\item $xx^\dagger$ and $x^\dagger x$ are projections;
\item $\norm{xx^\dagger}, \norm{x^\dagger x} \in \{0,1\}$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) Since $xx^\dagger$ and $x^\dagger x$ are self-adjoint, we just need to observe that they are idempotent. Indeed, $(xx^\dagger)^2 = x(x^\dagger x x^\dagger) = xx^\dagger$ and similarly for $x^\dagger x$.

(2) Immediate from \ref{projectionsNorm}.
\end{proof}

\begin{lemma} \label{MoorePenrosePseudoinverseSwapLemma}
Let $A$ be a $C^*$ algebra, $x\in A$ and $x^\dagger $ be a Moore-Penrose pseudoinverse of $x$. Then
\begin{enumerate}
\item $xx^\dagger  = (x^\dagger )^*x^*$;
\item $x^\dagger x = x^*(x^\dagger )^*$;
\item $x = xx^*x^{\dagger *} = x^{\dagger *}x^*x$;
\item $x^\dagger  = x^\dagger x^{\dagger *}x^* = x^*x^{\dagger *}x^\dagger $;
\item $x^* = x^*xx^\dagger  = x^\dagger xx^*$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1,2) Immediate from $xx^\dagger  = (xx^\dagger )^* = (x^\dagger )^*x^*$ and similarly for (2).

(3, 4) We have $x = xx^\dagger x$, which is equal to $xx^*x^{+*}$ and $x^{+*}x^*x$ by (1) and (2). For (3) we start with $x^\dagger  = x^\dagger xx^\dagger $.
...
\end{proof}
\begin{corollary}
Let $A$ be a $C^*$ algebra and $x\in A$ such that $x$ has a Moore-Penrose pseudoinverse. Then $(x^\dagger )^\dagger  = x$.
\end{corollary}
\begin{proof}
Since the definition is symmetric w.r.t.\ $x\leftrightarrow x^\dagger $, we have that $x^\dagger $ is a Moore-Penrose pseudoinverse of $x$. By the proposition it is the only one.
\end{proof}

\begin{lemma} \label{MPPseudoinverseAdjoint}
Let $A$ be a $C^*$ algebra and $x\in A$. Then $x^*$ has a Moore-Penrose pseudoinverse \textup{if and only if} $x$ has one. In this case $(x^*)^\dagger  = (x^\dagger )^*$.
\end{lemma}
\begin{proof}
Assume $x$ has a Moore-Penrose pseudoinverse $x^\dagger $. We show that $(x^\dagger )^*$ satisfies the requirements to be a Moore-Penrose pseudoinverse of $x^*$. We have $(x^*)^\dagger  = (x^\dagger )^*$ by uniqueness of the Moore-Penrose pseudoinverse \ref{MPPseudoinverseUnique}.

Taking the adjoint of $x = xx^\dagger x$ gives $x^* = x^*(x^\dagger )^*x^*$ and the adjoint of $x^\dagger  = x^\dagger xx^\dagger $ gives $(x^\dagger )^* = (x^\dagger )^*x^*(x^\dagger )^*$.

Next, we have
\begin{align*}
\big(x^*(x^\dagger )^*\big)^* &= x^\dagger x = (x^\dagger x)^* = x^*(x^\dagger )^* \\
\big((x^\dagger )^*x^*\big)^* &= xx^\dagger  = (xx^\dagger )^* = (x^\dagger )^*x^*.
\end{align*}
...
The Moore-Penrose pseudoinverse of a self-adjoint element is self-adjoint, if it exists.
\end{corollary}
\begin{corollary} \label{selfAdjointCommutesWithMPPseudoinverse}
Let $A$ be a $C^*$ algebra and $a\in A$ a self-adjoint element that has a Moore-Penrose pseudoinverse. Then $a^\dagger a = aa^\dagger$.
\end{corollary}
...
\begin{proposition} \label{MoorePenrosePseudoinverseEquivalents}
...
y^{-1} & (y \neq 0) \\ 0 & (y = 0).
\end{cases}$ is continuous when restricted to $\spec(x^*x)$. It also maps $0$ to $0$, so we can define $(x^*x)^\dagger  \defeq f(x^*x)$ using \ref{nonUnitalContinuousFunctionalCalculus}.
...
$(8) \Rightarrow (1)$ Suppose $x^*x$ has a Moore-Penrose pseudoinverse $(x^*x)^\dagger $. We set $x^\dagger  \defeq (x^*x)^\dagger x^*$ and claim this is a Moore-Penrose pseudoinverse. Indeed, to show $x = xx^\dagger x$, we calculate, using the fact that $x^*x$ is Hermitian and \ref{MPPseudoinverseAdjoint},
\begin{align*}
(x - xx^\dagger x)^*(x - xx^\dagger x) &= \big(x - x(x^*x)^\dagger x^*x\big)^*\big(x - x(x^*x)^\dagger x^*x\big) \\
&= x^*x - x^*x(x^*x)^\dagger x^*x - x^*x(x^*x)^{\dagger *}x^*x + x^*x(x^*x)^{\dagger *}x^*x(x^*x)^\dagger x^*x \\
&= x^*x - x^*x - (x^*x)^* + x^*x(x^*x)^{\dagger *}x^*x \\
&= x^*x - x^*x - x^*x + (x^*x)^{*} = 0.
\end{align*}
Thus, by the $B^*$-identity,
\[ \norm{x - xx^\dagger x} = \sqrt{\norm{(x - xx^\dagger x)^*(x - xx^\dagger x)}} = 0, \]
so $x = xx^\dagger x$.

We also calculate
\[ x^\dagger xx^\dagger  = (x^*x)^\dagger x^*x(x^*x)^\dagger x^* = (x^*x)^\dagger x^* = x^\dagger . \]

Finally, we note that since $(x^*x)^\dagger x^*x$ is Hermitian, we have that $x^\dagger x = \big((x^*x)^\dagger x^*\big)x = (x^*x)^\dagger x^*x$ is also Hermitian. Since $(x^*x)^\dagger$ is Hermitian, by \ref{MPPseudoinverseHermitianElement}, we have
\[ xx^\dagger  = x(x^*x)^\dagger x^* = \big(x(x^*x)^\dagger x^*\big)^*, \]
so $xx^\dagger $ is Hermitian.

$(1-8) \Leftrightarrow (9)$ We have that $|x|$ is simply polar if and only if $\big||x|\big|$ is simply polar.
\end{proof}
...
\begin{definition}
Let $A$ be a $C^*$-algebra and $a\in A$. A \udef{polar decomposition} of $a$ is an element $v\in A$ such that 
\begin{enumerate}
\item $a = v|a|$;
\item $v$ is a partial isometry;
\item $ap = 0$ implies $vp = 0$ for all projections $p\in\Projections(A)$.
\end{enumerate}
We say $A$ has the \udef{polar decomposition property} if each $a\in A$ has a \emph{unique} pair polar decomposition.
\end{definition}
...
\begin{lemma} \label{polarDecompositionFromPseudoinverseLemma}
Let $A$ be a $C^*$-algebra and $a\in A$ an element that has a Moore-Penrose pseudoinverse. Then $a = a|a|^\dagger |a|$.
\end{lemma}
\begin{proof}
We calculate in the unitisation $\tilde{A}$, using \ref{firstFactorInNormToAbsValue},
\begin{align*}
\norm{a - a|a|^\dagger |a|} &= \norm{a(\vec{1} - |a|^\dagger |a|)} \\
&= \norm{|a|(\vec{1} - |a|^\dagger |a|)} \\
&= \norm{|a| - |a||a|^\dagger |a|} = 0.
\end{align*}
\end{proof}
\begin{proof}[Alternate proof]
We calculate, using \ref{MoorePenrosePseudoinverseSwapLemma},
\begin{align*}
a|a|^\dagger |a| &= a^{\dagger *}a^*a|a|^\dagger |a| \\
&= a^{+*}|a|^2|a|^\dagger |a| \\
&= a^{+*}|a|^2 \\
&= a^{+*}a^*a \\
&= a.
...
\begin{proposition}[Polar decomposition for elements with pseudoinverse] \label{polarDecompositionFromPseudoinverse}
Let $A$ be a $C^*$-algebra and $a\in A$ an element that has a Moore-Penrose pseudoinverse. Then
\begin{enumerate}
\item $a$ has a polar decomposition $v$ that is given by $v = a|a|^\dagger $;
\item $|a| = v^*a$;
\item if $A$ is unital, then the polar decomposition is unique.
\end{enumerate}
\end{proposition}
TODO: is the polar decomposition also unique in the non-unital case??????

TODO: Is the map from $a$ to $v$ continuous??
\begin{proof}
(1) The existence of $v$ is given by \ref{MoorePenrosePseudoinverseEquivalents}.

We first observe that $a = v|a| = a|a|^\dagger |a|$ follows from \ref{polarDecompositionFromPseudoinverseLemma}.

Next, to show that $v$ is a partial isometry, we calculate using \ref{MPPseudoinverseAdjoint} and \ref{MoorePenrosePseudoinverseSwapLemma},
\begin{align*}
v^*v &= \big(|a|^\dagger \big)^*a^*a|a|^\dagger  \\
&= |a|^\dagger |a|^2|a|^\dagger  = |a|^\dagger |a|,
\end{align*}
which is clearly idempotent, and conclude using \ref{partialIsometries}.

Let $p\in\Projections(A)$ be such that $ap = 0$. Then we calculate, using \ref{MoorePenrosePseudoinverseSwapLemma},
\begin{align*}
vp &= a|a|^\dagger p \\
&= a^{\dagger *}a^*a|a|^\dagger p \\
&= a^{\dagger *}|a|^2|a|^\dagger p \\
&= a^{\dagger *}|a|^\dagger |a|^2p \\
&= a^{\dagger *}|a|^\dagger a^*(ap) = 0.
\end{align*}

(2) We calculate
\[ v^*a = |a|^\dagger a^*a = |a|^\dagger |a|^2 = |a|, \]
from \ref{selfAdjointCommutesWithMPPseudoinverse}.

(3) We have that $p \defeq (\vec{1} - |a||a|^\dagger )$ is a projector and $ap = a - a|a||a|^\dagger  = a - a|a|^\dagger |a| = 0$ from \ref{MoorePenrosePseudoinverseSwapLemma} and \ref{polarDecompositionFromPseudoinverseLemma}. Thus we also have
\[ 0 = vp = v - v|a||a|^\dagger  = v - a|a|^\dagger , \]
so $v = a|a|^\dagger $.
\end{proof}


\subsection{Singular value decomposition}
Suppose $a$ has a pseudoinverse $a^\dagger $, can we construct an SVD if $aa^\dagger $ and $a^\dagger a$ are Murray-von Neumann equivalent??
...
\begin{proposition} \label{positivityMatrixC*algebra}
...
\subsubsection{Positivity of $2\times 2$ matrices}
For more, see \cite{Moslehian_2019}.


\begin{proposition} \label{2by2PositiveFromPseudoinverse}
Let $A$ be a unital $C^*$-algebra and $a,b,c\in A$. Suppose $a$ is positive and has Moore-Penrose pseudoinverse. Then the following are equivalent:
\begin{enumerate}
\item $\begin{pmatrix}
a & c \\
c^* & b
\end{pmatrix}$ is positive;
\item $c^*a^\dagger c \leq b$ and $c = aa^\dagger c$.
\end{enumerate}
If this holds and $b$ has a Moore-Penrose pseudoinverse, then $c = aa^\dagger c b^\dagger b$.
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ From \ref{positivityMatrixC*algebra}, we have
\begin{align*}
0 &\leq \begin{pmatrix}
c^*a^\dagger & -\vec{1}
\end{pmatrix} \begin{pmatrix}
...
a^\dagger c \\ - \vec{1}
\end{pmatrix} \\
&= c^*a^\dagger a a^\dagger c - c^*a^\dagger c - c^*a^\dagger c + b \\
&= - c^*a^\dagger c + b.
\end{align*}
Now set $x \defeq (\vec{1} - a^\dagger a)$, so $ax = 0$. From \ref{positivityMatrixC*algebra}, we have
\begin{align*}
0 &\leq \begin{pmatrix}
x^* & -\frac{x^*c}{\norm{b}}
\end{pmatrix} \begin{pmatrix}
...
\end{pmatrix} \\
&= x^*ax  - \frac{x^*cc^*x}{\norm{b}} - \frac{x^*cc^*x}{\norm{b}} + \frac{x^*cbc^*x}{\norm{b}^2} \\
&\leq x^*ax  - \frac{x^*cc^*x}{\norm{b}} - \frac{x^*cc^*x}{\norm{b}} + \frac{x^*cc^*x}{\norm{b}} \\
&= - \frac{x^*cc^*x}{\norm{b}} \leq 0,
\end{align*}
where we have used \ref{C*PositivityVectorOrder}, \ref{normInequalityFromSAInequality} and \ref{CstarOrderLemma}. From the antisymmetry of the order (\ref{C*PositivityVectorOrder}), we have that $0\leq - \frac{x^*cc^*x}{\norm{b}} \leq 0$ implies $0 = \norm{x^*cc^*x} = \norm{c^*x}^2$ and thus $0 = c^*x = c^*(1-a^\dagger a)$. So $c^*a^\dagger a = c^*$ and we conclude by taking the adjoint.
...
c^*a^\dagger & \vec{1}
\end{pmatrix}\begin{pmatrix}
a & 0 \\ 0 & b - c^*a^\dagger c
\end{pmatrix}\begin{pmatrix}
a^\dagger a & a^\dagger c \\
0 & \vec{1}
\end{pmatrix}, \]
which is positive by \ref{spectrumDiagonalMatrix} and \ref{CstarOrderLemma}.
...
0 & \vec{1} \\ \vec{1} & 0
\end{pmatrix} \]
is positive from \ref{CstarOrderLemma}. The proof above then gives $c^* = bb^\dagger c^*$ and taking the adjoint yields $c = cb^\dagger b = aa^\dagger c b^\dagger b$.
\end{proof}
\begin{corollary}
...
\begin{proposition} \label{2by2PositiveIffContraction}
Let $A$ be a unital $C^*$-algebra and $a,b,c\in A$. Suppose $a,b$ are positive and have Moore-Penrose pseudoinverses. Then the following are equivalent:
\begin{enumerate}
\item $\begin{pmatrix}
a & c \\
c^* & b
...
\end{enumerate}
\end{proposition}
We can drop requirement that $a,b$ have pseudoinverses if $A$ is the algebra of bounded operators on a Hilbert space by considering $\begin{pmatrix}
a & c \\
c^* & b
\end{pmatrix} + \epsilon \begin{pmatrix}
\vec{1} & 0 \\ 0 & \vec{1}
\end{pmatrix}$. See Schur parameters, Constantinescu. Does this work in other $C^*$-algebras with the polar decomposition property?
\begin{proof}
$(1) \Rightarrow (2)$ From \ref{2by2PositiveFromPseudoinverse}, a natural choice is $x \defeq \sqrt{a}^\dagger c\sqrt{b}^\dagger$. We just need to check $\norm{\sqrt{a}^\dagger c\sqrt{b}^\dagger} \leq 1$. We calculate
\[ \norm{\sqrt{a}^\dagger c\sqrt{b}^\dagger}^2 = \norm{\sqrt{b}^\dagger c^* a^\dagger c \sqrt{b}^\dagger} \leq \norm{\sqrt{b}^\dagger b \sqrt{b}^\dagger} = \norm{b^\dagger b} \leq 1, \]
where we have used \ref{2by2PositiveFromPseudoinverse}, \ref{CstarOrderLemma}, \ref{normInequalityFromSAInequality}, \ref{selfAdjointCommutesWithMPPseudoinverse} and \ref{supportRangeProjectionsElementWithMPPseudoinverse}.
...
\begin{proposition} \label{inequalityBetweenAbsoluteValueSquared}
Let $A$ be a unital $C^*$-algebra and $a,b\in A$. Suppose $b$ has a Moore-Penrose pseudoinverse. Then the following are equivalent:
\begin{enumerate}
\item $a^*a \leq b^*b$;
\item there exists $x\in A$ such that $\norm{x} \leq 1$ and $a = xb$.
\end{enumerate}
\end{proposition}
\begin{proof}
From \ref{2by2PositiveFromPseudoinverse} we have $a^*a \leq b^*b \iff \begin{pmatrix}
\vec{1} & a \\ a^* & b^*b
\end{pmatrix} \geq 0$, which, by \ref{2by2PositiveIffContraction}, is in turn equivalent to the existence of some $x\in A$ such that $\norm{x} \leq 1$ and $a = x_0|b|$. Here we have used that $b^*b$ has a Moore-Penrose pseudoinverse, see \ref{MoorePenrosePseudoinverseEquivalents}. We need to show that this is equivalent to (2).

$(1) \Rightarrow (2)$ Let $b = v|b|$ be the polar decomposition of $b$ according to \ref{polarDecompositionFromPseudoinverse}. Then $a = x_0v^*b$ and $\norm{x_0v^*} \leq \norm{x_0}\norm{v} \leq 1$ by \ref{partialIsometriesNorm}.

$(2) \Rightarrow (1)$ We have $a = xb = xv|b|$ and $\norm{xv} \leq \norm{x}\norm{v} \leq 1$ by \ref{partialIsometriesNorm}.
\end{proof}

\subsubsection{Column contractions}

\begin{proposition}
Let $A$ be a unital $C^*$-algebra such that every element has a Moore-Penrose pseudoinverse and $\seq{x_k}_{k=0}^{n-1} \in A^n$. Then the following are equivalent:
\begin{enumerate}
\item $\norm{\seq{x_k}} \leq 1$;
\item There exists $\seq{\Gamma_k}_{k=0}^{n-1} \in A^n$ such that
\begin{itemize}
\item $\norm{\Gamma_k} \leq 1$ for all $k\in [0:n-1]$;
\item $x_0 = \Gamma_0$;
\item $\begin{aligned}[t]x_k &= \Gamma_kD_{\Gamma_{k-1}}D_{\Gamma_{k-2}}\ldots D_{\Gamma_0} \\ &= \Gamma_k\prod_{j=1}^{k}D_{\Gamma_{k-j}}\end{aligned}$ for all $k\in [1:n-1]$.
\end{itemize}
\end{enumerate}
\end{proposition}
TODO: The existence of all Moore-Penrose pseudoinverses is a way too strong condition! Holds in all von Neumann algebras?
\begin{proof}
$(1) \Rightarrow (2)$ The proof goes by induction on $n$. The base case $n=1$ is immediate.

Assume the implication holds for $n-1$ and suppose $\norm{\seq{x_k}_{k=0}^{n-1}} \leq 1$. Then, using $0\leq x_{n-1}^*x_{n-1}$, \ref{C*PositivityVectorOrder} and \ref{inequalityPositiveElementsLemma}, we have
\[ \norm{\seq{x_k}_{k=0}^{n-2}} = \norm*{\sum_{k=0}^{n-2}x_k^*x_k} \leq \norm*{\sum_{k=0}^{n-2}x_k^*x_k + x_{n-1}^*x_{n-1}} = \norm{\seq{x_k}_{k=0}^{n-1}} \leq 1, \]
so $x_k = D_{\Gamma_0^*}D_{\Gamma_1^*}\ldots D_{\Gamma_{k-1}^*}\Gamma_k$ for all $k\in [1:n-2]$. We just need to show that this also holds for $k=n-1$.

From \ref{normInequalityFromSAInequality} and \ref{C*PositivityVectorOrder}, we have
\[ \sum_{k=0}^{n-1}x_k^*x_k \leq \norm*{\sum_{k=0}^{n-1}x_k^*x_k}\vec{1} \leq \vec{1}. \]
Thus
\begin{align*}
x_{n-1}^*x_{n-1} &\leq \vec{1} - \sum_{k=0}^{n-2}x_k^*x_k \\
&= \vec{1} - \Gamma_0^*\Gamma_0 - \sum_{k=1}^{n-2}\Big(\prod_{j=0}^{k-1}D_{\Gamma_j}\Big)\Gamma_k^*\Gamma_k\Big(\prod_{j=1}^k D_{\Gamma_{k-j}}\Big) \\
&= \vec{1} - \Gamma_0^*\Gamma_0 - \sum_{k=1}^{n-2}D_{\Gamma_0}\Big(\prod_{j=1}^{k-1}D_{\Gamma_j}\Big)\Gamma_k^*\Gamma_k\Big(\prod_{j=1}^{k-1} D_{\Gamma_{k-j}}\Big)D_{\Gamma_0} \\
&= D_{\Gamma_0}D_{\Gamma_0} - \sum_{k=1}^{n-2}D_{\Gamma_0}\Big(\prod_{j=1}^{k-1}D_{\Gamma_j}\Big)\Gamma_k^*\Gamma_k\Big(\prod_{j=1}^{k-1} D_{\Gamma_{k-j}}\Big)D_{\Gamma_0} \\
&= D_{\Gamma_0}\bigg(\vec{1} - \sum_{k=1}^{n-2}\Big(\prod_{j=1}^{k-1}D_{\Gamma_j}\Big)\Gamma_k^*\Gamma_k\Big(\prod_{j=1}^{k-1} D_{\Gamma_{k-j}}\Big)\bigg)D_{\Gamma_0} \\
&= D_{\Gamma_0}\bigg(\vec{1} - \Gamma_1^*\Gamma_1 - \sum_{k=2}^{n-2}\Big(\prod_{j=1}^{k-1}D_{\Gamma_j}\Big)\Gamma_k^*\Gamma_k\Big(\prod_{j=1}^{k-1} D_{\Gamma_{k-j}}\Big)\bigg)D_{\Gamma_0} \\
&= \ldots \\
&= \Big(\prod_{j=0}^{n-2}D_{\Gamma_j}\Big)\Big(\prod_{j=1}^{n-1} D_{\Gamma_{n-1-j}}\Big)
\end{align*}
Now \ref{inequalityBetweenAbsoluteValueSquared} gives the existence of a contraction $\Gamma_{n-1}$ such that $x_{n-1} = \Gamma_{n-1}\prod_{j=1}^{n-1} D_{\Gamma_{n-1-j}}$.
\end{proof}
TODO: make the calculation $(\ldots)$ formal (by induction or something).
...
\chapter{Concrete $C^*$-algebras}
\begin{definition}
A \udef{concrete $C^*$-algebra} is a $C^*$-subalgebra of $\Bounded(\mathcal{H})$, for some Hilbert space $\mathcal{H}$.

We say a concrete $C^*$-algebra $A$ is
\begin{enumerate}
\item \udef{non-degenerate} if $\overline{A\mathcal{H}} = \mathcal{H}$;
\item \udef{cyclic} w.r.t. a unit vector $\xi\in\mathcal{H}$ if $\overline{A\xi} = \mathcal{H}$.
\end{enumerate}
\end{definition}
A concrete $C^*$-algebra is a norm-closed $*$-subalgebra of $\Bounded(\mathcal{H})$.

\begin{proposition} \label{boundedIncreasingNetConvergesPointwise}
Let $\mathcal{H}$ be a Hilbert space and $\seq{T_i}_{i\in I}$ a norm-bounded increasing net of bounded self-adjoint operators. Then $\seq{T_i}$ converges pointwise to a bounded self-adjoint operator on $\mathcal{H}$.
\end{proposition}
TODO: connect to abstract theory??
\begin{proof}
From \ref{boundedSetNormedSpace}, we can find $M\in\R^+$ such that $\norm{T_i} \leq M$ for all $i\in I$.

TODO: OpAlg-ExS-4
\end{proof}

TODO: From here we implicitly make use of the fact that any matrix algebra of concrete $C^*$-algebras is isomorphic to a concrete $C^*$-algebra of operators on a direct sum of a Hilbert space. TODO: make explicit!

\begin{proposition}
Let $\mathcal{H}$ be a Hilbert space and $\varphi: \Bounded(\mathcal{H})\to \C$ a linear functional. Then the following are equivalent:
\begin{enumerate}
\item there exist vectors $\xi_1,\ldots, \xi_n, \eta_1,\ldots, \eta_n \in \mathcal{H}$ such that
\[ \varphi(T) = \sum_{k=1}^n \inner{\eta_k, T\xi_k} \]
for all $T\in \Bounded(\mathcal{H})$;
\item $\varphi$ is WOT-continuous;
\item $\varphi$ is SOT-continuous.
\end{enumerate}
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ Follows from \ref{WOTforInnerProductSpace} and the continuity of the addition.

$(2) \Rightarrow (3)$ Immediate, since the SOT is stronger than the WOT.

$(3) \Rightarrow (1)$ The SOT is the seminormed convergence w.r.t. the seminorms $\setbuilder{\norm{\cdot}\circ \evalMap_\xi}{\xi\in \mathcal{H}}$, see \ref{initialConvergenceWRTlinearOperatorsToNormedSpacesSeminormed}.

From \ref{dualSeminormedConvergence}, there exists $\xi_1, \ldots, \xi_n$ and $C>0$ such that
\[ |\varphi(T)| \leq C\max_{k\in [1:n]}\norm{T\xi_k} \leq C \norm{\seq{T\xi_k}_{k\in[1:n]}}_{\mathcal{H}^{\oplus n}} = C \norm*{\begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix}T}_{\mathcal{H}^{\oplus n}} \]
for all $T\in \Bounded(\mathcal{H})$. This implies $\ker \varphi \supseteq \ker \begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix}$, so \ref{kernelInclusionDecomposition} gives a composition $\varphi = \varphi'\circ \begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix}$, where $\varphi': \im \begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix} \to \C$ is a linear functional. For all $\begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix}T \in \im \begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix}$, the inequality yields
\[ \Big|\varphi'\Big(\begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix}T\Big)\Big| = |\varphi(T)| \leq C\norm*{\begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix}T}, \]
so $\varphi'$ is bounded. By the Riesz representation theorem, \ref{rieszRepresentation}, we have $\varphi' = \inner{\seq{\eta_k}_{k\in[1:n]}, -}$ for some $\seq{\eta_k}_{k\in[1:n]} \in \mathcal{H}^{\oplus n}$. Putting every thing together yields, for arbitrary $T\in \Bounded(\mathcal{H})$,
\[ \varphi(T) = \varphi'\circ \begin{pmatrix}
\evalMap_{\xi_1} \\ \hdots \\ \evalMap_{\xi_n}
\end{pmatrix} T = \inner{\big. \seq{\eta_k}_{k\in[1:n]}, \seq{T\xi_k}_{k\in[1:n]}} = \sum_{k=1}^n\inner{\eta_k, T\xi_k}. \]
\end{proof}
\begin{corollary}
Let $\mathcal{H}$ be a Hilbert space and $S\subseteq \Bounded(\mathcal{H})$ a convex subset. Then $\overline{S}^\text{SOT} = \overline{S}^\text{WOT}$.
\end{corollary}
\begin{proof}
This follows from \ref{weakClosureConvexSubsets}.
\end{proof}
...
\begin{proposition} \label{unitInSOTclosure}
Let $\mathcal{H}$ be a Hilbert space and $A\subseteq \Bounded(\mathcal{H})$ a concrete $C^*$-algebra. Then
\begin{enumerate}
\item there exists $e\in \overline{A}^{\text{SOT}}$ such that $ea = a = ae$ for all $a\in A$;
\item $e = \id_{\mathcal{H}}$ \textup{if and only if} $A$ is non-degenerate.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) Let $\seq{e_\lambda}$ be an increasing approximate unit in $A$, which exists by \ref{CstarAlgebraHasIncreasingApproximateUnit}. By \ref{boundedIncreasingNetConvergesPointwise}, $\seq{e_\lambda}$ has a SOT-limit $e$. For any $a\in A$, we have
\[ ea = \big(\lim_{\lambda}e_\lambda\big)a = \lim_{\lambda}e_\lambda a = a = \lim_{\lambda}a e_\lambda = a\big(\lim_{\lambda}e_\lambda\big) = ae, \]
where all limits are in the SOT. We have used that SOT-convergence is weaker than norm-convergence and that the multiplication is separately SOT-continuous, \ref{multiplicationSeparatelySOTWOTcontinuous}.

(2) First assume $e = \id_{\mathcal{H}}$, i.e.\ $\id_{\mathcal{H}} \in \overline{A}^{\text{SOT}}$. Then there exists a net $\seq{e_\lambda}_{\lambda\in\Lambda}$ in $A$ that SOT-converges to $e$, so $e_\lambda\xi \to \xi$ for all $\xi\in \mathcal{H}$. Since $\seq{e_\lambda\xi}_{\lambda\in\Lambda}$ is a net in $A\mathcal{H}$, we have $\xi \in \overline{A\mathcal{H}}$. Thus $\overline{A\mathcal{H}} = \mathcal{H}$.

For the other direction, we have $\overline{\{e\}\mathcal{H}} = \overline{A \mathcal{H}} = \mathcal{H}$, so for all $x\in \mathcal{H}$ there exists a sequence $\seq{x_n}$ in $\mathcal{H}$ such that $x = \lim_{n\to \infty}ex_n$ (using the fact that metric spaces are sequential). Now
\[ ex = e\big(\lim_{n\to\infty}ex_n\big) = \lim_{n\to\infty}e^2x_n = \lim_{n\to\infty}ex_n = x. \]
Thus $e = \id_{\mathcal{H}}$.
\end{proof}
\begin{corollary}
Let $\mathcal{H}$ be a Hilbert space and $A\subseteq \Bounded(\mathcal{H})$ a von Neumann algebra. Then
\begin{enumerate}
\item $A$ is unital;
\item $\id_{\mathcal{H}} \in A$ \textup{if and only if} $A$ is non-degenerate.
\end{enumerate}
\end{corollary}
...
\begin{enumerate}
\item $S^{\comm}$ is a Banach algebra;
\item $S^{\comm}$ is a $C^*$-algebra if $S = S^*$;
\item $S^{\comm}\subseteq \Bounded(\mathcal{H})$ is WOT-closed.
\end{enumerate}
\end{proposition}

\begin{lemma} \label{vonNeumannBicommutantLemma}
Let $A$ be a non-degenerate concrete $C^*$-algebra, $\xi \in \mathcal{H}$ and $T \in A^{\comm\comm}$. Then $T\xi \in \overline{A\{\xi\}}$.
\end{lemma}
\begin{proof}
First we show that $\xi \in \overline{A\{\xi\}}$. 
Consider the $e$ of \ref{unitInSOTclosure}. Then there exists a net $\seq{e_\lambda}_{\lambda\in\Lambda}$ in $A$ that SOT-converges to $e$, so $e_\lambda\xi \to \xi$. Since $\seq{e_\lambda\xi}$ is a net in $A\{\xi\}$, we have $\xi\in \overline{A\{\xi\}}$.

Let $P$ be the orthogonal projection onto $\overline{A\{\xi\}}$. Take $a\in A$. Then $a$ maps $\overline{A\{\xi\}}$ into itself, so $aP = PaP$. If $a$ is self-adjoint, then $aP = PaP = Pa$. If $a$ is not self-adjoint, this still holds by \ref{realImaginaryParts}. In particular $P \in A^{\comm}$, so $T \in A^{\comm\comm}\subseteq \{P\}^{\comm}$.

Finally we have, since $\xi\in \overline{A\{\xi\}}$,
\[ T\xi = TP\xi = PT\xi, \]
so $T\xi\in \overline{A\{\xi\}}$.
\end{proof}

\begin{proposition}[von Neumann's bicommutant theorem]
Let $\mathcal{H}$ be a Hilbert space and $A\subseteq \Bounded(\mathcal{H})$ a $*$-subalgebra such that $\overline{A\mathcal{H}} = \mathcal{H}$. Then $A^{\comm\comm} = \overline{A}^\text{SOT} = \overline{A}^\text{WOT}$.
\end{proposition}
\begin{proof}
We have $\overline{A}^\text{SOT} \subseteq \overline{A}^\text{WOT} \subseteq A^{\comm\comm}$, from \ref{principalInherenceAdherenceProperties}, the fact that $A\subseteq A^{\comm\comm}$ and \ref{commutantBanachAlgebra}.

We finally need to show that $A$ is SOT-dense in $A^{\comm\comm}$. We do this using \ref{denseIffMeshesWithAllVicinities}. Take arbitrary $T\in A^{\comm\comm}$ and $U\in \vicinities_{\text{SOT}}(T)$. From \ref{pretopologicalInitialConvergence} and \ref{vicinitiesPretopologicalSubspaceConvergence}, there exist $\xi_1, \ldots, \xi_n\in \mathcal{H}$ and $\epsilon_1, \ldots, \epsilon_n \in \R^+\setminus\{0\}$ such that $\bigcap_{k=1}^n\evalMap_{\xi_k}^{\preimf}\big(\cball(T\xi_k, \epsilon_k)\big) \cap A^{\comm\comm} \subseteq U$. Setting $\epsilon \defeq \min\{\epsilon_1, \ldots, \epsilon_n\}$, we have
\[ \setbuilder{a\in A^{\comm\comm}}{\forall k\in[1:n]:\; \norm{(T-a)\xi_k}\leq \epsilon} = \bigcap_{k=1}^n\evalMap_{\xi_k}^{\preimf}\big(\cball(T\xi_k, \epsilon)\big)\cap A^{\comm\comm} \subseteq U. \]
From \ref{commutantsMatrixAlgebrasLemma}, we have $T\cdot\mathbb{1}_n \in A^{\comm\comm}\cdot^\imf\{\mathbb{1}_n\} =  (A\cdot^\imf \{\mathbb{1}_n\})^{\comm\comm}$ and so, from \ref{vonNeumannBicommutantLemma}, there exists $a\cdot\mathbb{1}_n \in A\cdot^\imf \{\mathbb{1}_n\}$ such that
\[ \norm*{\big(T\cdot\mathbb{1}_n\big)\begin{pmatrix}
\xi_1 \\ \hdots \\ \xi_n
\end{pmatrix} - \big(a\cdot\mathbb{1}_n\big)\begin{pmatrix}
\xi_1 \\ \hdots \\ \xi_n
\end{pmatrix}} \leq \epsilon. \]
This implies that $a\in U$, so $A\mesh U$.
\end{proof}
\begin{corollary}
Let $A\subseteq \Bounded(\mathcal{H})$ be a concrete $C^*$-algebra with $\id_{\mathcal{H}} \in A$. Then $A$ is a von Neumann algebra \textup{if and only if} $A = A^{\comm\comm}$.
\end{corollary}

\subsection{Kaplansky's density theorem}
\begin{proposition}
Let $\mathcal{H}$ be a Hilbert space and $f: \R\to \R$ a continuous function. Then
\begin{enumerate}
\item if $\lim_{t\to\pm\infty}f(t) = 0$, then $f: \SelfAdjoints(\mathcal{H}) \to \SelfAdjoints(\mathcal{H})$ is SOT-SOT-continuous;
\item if $\lim_{t\to\pm\infty}\frac{f(t)}{t} = 0$, then $f: \SelfAdjoints(\mathcal{H}) \to \SelfAdjoints(\mathcal{H})$ is SOT-SOT-continuous.
\end{enumerate}
\end{proposition}
The function $f: \SelfAdjoints(\mathcal{H}) \to \SelfAdjoints(\mathcal{H})$ is defined by continuous functional calculus.
\begin{proof}
(1) Let $A_0$ be the set of continuous real functions $f$ such that $f: \SelfAdjoints(\mathcal{H}) \to \SelfAdjoints(\mathcal{H})$ is SOT-SOT-continuous.

TODO Stone-Weierstrass: see Conway. \cite{conwayFA}

\end{proof}

\begin{theorem}[Kaplansky's density theorem]
Let $A\subseteq \Bounded(\mathcal{H})$ be a concrete $C^*$-algebra.
\end{theorem}
...
\item \udef{non-degenerate} if $\im(\pi)$ is non-degenerate;
\item \udef{cyclic} w.r.t.\ a unit vector $\xi\in\mathcal{H}$ if $\im(\pi)$ is cyclic w.r.t.\ $\xi$.
\end{enumerate}
\end{definition}
...
\item all non-zero elements of the commutant $D^{\comm}$ are invertible; 
\item the commutant $D^{\comm}$ consists of multiples of the identity operator;
\item the commutant $D^{\comm}$ contains no projectors other than the identity;
...
Let $A$ be a $C^*$-algebra and $J$ an left-ideal in $A$. Let $\rho: J\to \Bounded(\mathcal{H})$ be a nondegenerate representation of $J$. For all $b_1,\ldots, b_n\in J$, $h_1,\ldots, h_n\in\mathcal{H}$ and $a\in A$, we have
...
\begin{proposition}
Let $A$ be a $C^*$-algebra and $J$ a left-ideal in $A$. Let $\rho: J\to \Bounded(\mathcal{H})$ be a nondegenerate representation of $J$. Then there is a unique representation $\overline{\rho}: A\to \Bounded(\mathcal{H})$ of $A$ that extends $\rho$. If $\rho$ and $\sigma$ are equivalent representations of $J$, then $\overline{\rho}$ and $\overline{\sigma}$ are equivalent representations of $A$.
\end{proposition}
\begin{proof}
...
\begin{theorem}[Gelfand-Neumark-Segal construction] \label{GNSconstruction}
...
Let $\seq{e_\lambda}$ be an increasing approximate unit in $A$, which exists by \ref{CstarAlgebraHasIncreasingApproximateUnit}. Then $\seq{[e_\lambda]}$ is a Cauchy net in $A/N_\omega$ by \ref{GNSlemma}, which means it has a limit in $H_\omega.$ We call this limit $\xi_\omega$.
...
\begin{theorem}[Gelfand-Neumark theorem]
...
between positive functionals $\omega_T$ over $A$ majorised by $\omega$ and positive operators $T$ in the commutant $\pi_\omega^{\comm}$ with $\norm{T}\leq 1$.
\end{lemma}
...
\end{proof}
\begin{lemma}
The algebra $C_c(G)$ has an involutive antilinear anti-automorphism
