...
is a unital $*$-isomorphism. If $X$ is ..., then $\Psi$ is isometric.
\end{lemma}
TODO: when isometric?? Regularity???
...
Finaly we prove that $\Psi$ is isometric. For all $f\in \cont(X)$, we have
\begin{align*}
...
We have that $|f(x)|$ attains a maximum at $x_1 \in X$ by
If $x_1 \neq x_0$, then we have 
...
where $\iota: A\to \widetilde{A}$ is the canonical embedding and $\Phi_{\iota(x)}$ is the continuous functional calculus of $\iota(x)\in A^\dagger$. In particular
\begin{enumerate}
...
For uniqueness, suppose $\Phi''_x$ is some other such $*$-homomorphism. Then the unitisation $(\Phi''_x)^\dagger$ is a unital $*$-homomorphism that maps $\id_{\spec(x)}$ to $x$, by \ref{unitalExtensionLinearFunction}. 
...
It is equal to $\Phi_{\iota(x)}$ by \ref{continuousFunctionalCalculus}. This implies
\[ \Phi''_x = \proj_1 \circ (\Phi''_x)^\dagger = \proj_1 \circ (\Phi''_x)^\dagger \]
...
note that the unitisation $(\Phi'_x)^\dagger = \cont\big(\spec(x)\big)\to A^\dagger$ is a continuous functional calculus for $A^\dagger$ and thus unique by \ref{continuousFunctionalCalculus}. This means that the restriction is also unique.

TODO: rework, using \ref{unitisationNonUnitalFunctionalC*algebra}
\end{proof}
...
\end{lemma}
\begin{proof}
If each $x_k$ is invertible, then we can calculate
\[ \diag\big(\seq{x_k}_{k=0}^{n-1}\big)\cdot \diag\big(\seq{x_k^{-1}}_{k=0}^{n-1}\big) = \diag(\seq{\vec{1}}_{k=0}^{n-1}). \]
Now suppose $\diag\big(\seq{x_k}_{k=0}^{n-1}\big)$ is invertible with inverse $Y$. Then
\[ \delta_{i,j}\vec{1} = \big[Y\cdot \diag\big(\seq{x_k}_{k=0}^{n-1}\big)\big]_{i,j} = [Y]_{i,j}x_j \]
and
\[ \delta_{i,j}\vec{1} = \big[\diag\big(\seq{x_k}_{k=0}^{n-1}\big)\cdot Y\big]_{i,j} = x_i[Y]_{i,j}. \]
This implies, for all $j\in [0:n-1]$, that $[Y]_{j,j}$ is the inverse of $x_j$, so $x_j$ is invertible.
\end{proof}
...
We have
\begin{align*}
\lambda \in \spec\Big(\diag\big(\seq{x_k}_{k=0}^{n-1}\big)\Big) &\iff \text{$\lambda\vec{1} - \diag\big(\seq{x_k}_{k=0}^{n-1}\big)$ is not invertible} \\
&\iff \text{$\diag\big(\seq{\lambda\vec{1} - x_k}_{k=0}^{n-1}\big)$ is not invertible} \\
&\iff \exists k\in [0:n-1]: \text{$\lambda\vec{1} - x_k$ is not invertible} \\
&\iff \exists k\in [0:n-1]: \lambda \in \spec(x_k) \\
&\iff \lambda \in \bigcup_{k=0}^{n-1}\spec(x_k).
\end{align*}
\end{proof}
...
\item $\sum_{i,j=0}^{n-1}x^*_i[\mathfrak{a}]_{i,j}x_j \geq 0$ for all $\seq{x_k} \in A^n$ such that $\big(\seq{x_k},\seq{x_k}\big) = \vec{1}$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
\begin{definition}
Let $A$ be a $C^*$-algebra. A function $K: \Z\times \Z\to A$ is called a \udef{positive definite kernel} if $[A(i,j)]_{i,j\in [-n:n]}$ is positive for all $n\in \N$.
\end{definition}

\begin{proposition}[Kolmogorov decomposition]
Let $A$ be a $C^*$-algebra and $K: \Z\times \Z\to A$ a positive kernel. Then there exists a function $V: \Z\to A$ such that $A(i,j) = V^*(i)V(j)$ for all $i,j\in \Z$.
\end{proposition}
\begin{proof}
