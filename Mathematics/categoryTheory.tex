...
\begin{itemize}
\item a \udef{left semi-category} if for all $x\in \cat{C}$, there exist a left-identities $e, e'\in \cat{C}$ such that $ex$ and $xe'$ are defined.
...
\begin{definition}
Let $F,G,H: A\to B$ be functors and $\alpha: F\Rightarrow G$, $\beta: G\Rightarrow H$ natural transformations. The \udef{vertical composition} of $\alpha$ and $\beta$ is the natural transformation $\beta\circ\alpha: F\Rightarrow H$ defined by $(\beta\circ\alpha)_f = \beta_f\alpha_f$ for all $f\in A$.
\end{definition}
Note that $\beta\circ\alpha: F\Rightarrow H$ is (TODO: NOT!!) a natural transformation from $F$ to $H$: $\forall f,g\in A$ we have
\[ H(f)(\beta\circ\alpha)_g = H(f)\beta_g\alpha_g = \beta_fG(g)\alpha_g \] 
...
We also define
\begin{itemize}
\item a \udef{limit} of the diagram $F: J\to A$ is a $[\setbuilder{\rho_a\circ -}{a\in A}]^!$-maximum in the set of cones over $F$.
\item a \udef{colimit} of the diagram $F: J\to A$ is a $[\setbuilder{\lambda_a\circ -}{a\in A}]^!$-maximum in the set of cocones over $F$.
\end{itemize}
\end{definition}
...
Thus $g$ determines a coleg of $\begin{tikzcd}
A \arrow[r, shift left, "f"] \arrow[r, shift right, swap, "{0_{A,B}}"] & B
\end{tikzcd}$. We need to show that it is a terminal coleg. Assume there exists some other coleg $g': B\to C'$:
\[ \begin{tikzcd}
A \arrow[r, shift left, "f"] \arrow[r, shift right, swap, "{0_{A,B}}"] & B \arrow[r, "g"] \arrow[dr, swap, "{g'}"]  & C \\
&& C'
\end{tikzcd} \]
\end{proof}

\begin{proposition}
A monic an epic morphism has a zero kernel and cokernel.

An identity morphism is the kernel and cokernel of a zero morphism.
\end{proposition}

\begin{proposition}
Let $\cat{C}$ be an abelian category. Consider the relations $\ker$ and $\coker$. Then
\begin{enumerate}
\item $\ker = \ker\circ\coker\circ\ker$;
\item $\coker = \coker\circ\ker\circ\coker$.
\end{enumerate}
\end{proposition}
TODO: make more precise!! Plus direction relational notation.
\begin{proof}
Let $f:B\to C$ be a morphism in $\cat{C}$. 

First take $g\in \ker(f): A\to B$. It is enough to show that for any morphisms $h,h'$ such $h\in \ker(f)$ and $h'\in\coker(h)$, we have $g\in \ker(h')$. Diagrammatically, we have
\[ \begin{tikzcd}[row sep=small, column sep=huge]
A \arrow[dr, "{g}"] & {} & C \\
{} & B \arrow[ur, "{f}"] \arrow[dr, swap, "{h'}"] & {} \\
A' \arrow[ur, swap, "{h}"] & {} & C'
\end{tikzcd} \]

We construct $\coker(\ker f): B \to C'$ and $\ker\big(\coker(\ker f)\big): A'\to B$. Diagrammatically, this gives us
\[ \begin{tikzcd}[row sep=small, column sep=huge]
A \arrow[dr, "{\ker f}"] & {} & C \\
{} & B \arrow[ur, "{f}"] \arrow[dr, "{\coker(\ker f)}"] & {} \\
A' \arrow[ur, "{\ker(\coker(\ker f))}"] & {} & C'
\end{tikzcd} \]
Now $f\ker(f) = 0_{B,C}\ker(f) = 0_{A,C} = f0_{A,B}$, so $f$ is a leg compatible with $\begin{tikzcd}
A \arrow[r, shift left, "{\ker(f)}"] \arrow[r, swap, shift right, "0"] & B
\end{tikzcd}$. It can be extended to a cocone. An thus there exists a unique $g: C'\to C$. That makes the diagram
\[ \begin{tikzcd}[row sep=small, column sep=huge]
A \arrow[dr, "{\ker f}"] & {} & C  \\
{} & B \arrow[ur, "{f}"] \arrow[dr, swap, "{\coker(\ker f)}"] & {} \\
A' \arrow[ur, "{\ker(\coker(\ker f))}"] & {} & C' \arrow[uu, "g"]
\end{tikzcd} \qquad\text{commute.} \]
We also have that
\[ \coker(\ker f)\ker(f) = \coker(\ker f)0_{A,B} = 0_{A, C'} = 0_{B,C'}\ker(f), \]
so $\ker(f)$ is a leg compatible with $\begin{tikzcd}
B \arrow[r, shift left, "{\coker(\ker f)}"] \arrow[r, swap, shift right, "0"] & C'
\end{tikzcd}$, and
\begin{align*}
f\ker\big(\coker(\ker f)\big) &= g\coker(\ker f)\ker\big(\coker(\ker f)\big) \\
&= g0_{B,C'}\ker\big(\coker(\ker f)\big) \\
&= 0_{B,C}\ker\big(\coker(\ker f)\big),
\end{align*}
so $\ker\big(\coker(\ker f)\big)$ is a leg compatible with $\begin{tikzcd}
B \arrow[r, shift left, "f"] \arrow[r, swap, shift right, "0"] & C
\end{tikzcd}$.

Thus there exist unique $h,h'$ such that the diagram
\[ \begin{tikzcd}[row sep=small, column sep=huge]
A \arrow[dr, "{\ker f}"] \arrow[dd, shift left, "h"] & {} \\
{} & B \\
A' \arrow[ur, swap, "{\ker(\coker(\ker f))}"] \arrow[uu, shift left, "{h'}"] & {}
\end{tikzcd} \qquad\text{commutes.} \]
Thus we have $ker(f)hh' = \ker(f)\id_A$. As $\ker(f)$ is monic, we have $hh'=\id_A$. Similarly $h'h=\id_{A'}$, so $A$ and $A'$ are isomorphic.

Thus $\ker$ and $\coker$ are generalised inverses up to isomorphism. By the definition of an abelian category, we have that $\im\ker$ is exactly the class of monomorphisms. Similarly, $\im\coker$ is the class of epimorphisms. 
\end{proof}

\begin{proof}
Let $f:B\to C$ be an epimorphism in $\cat{C}$. Then $\ker(f): A\to B$ is a monomorphism by \ref{equaliserMonic}.
