...
\end{tikzcd}$. We need to show that it is a terminal coleg.
...
A' \arrow[ur, swap, "{f'}"] & {} & C'
\end{tikzcd} \]
Because $g$ is a a coleg of $\begin{tikzcd}
A \arrow[r, shift left, "f"] \arrow[r, shift right, swap, "{0_{A,B}}"] & B
...
A' \arrow[ur, swap, "{f'}"] & {} & C'
\end{tikzcd} \]
This allows us to note that $g'$ is a coleg of $\begin{tikzcd}
A' \arrow[r, shift left, "f'"] \arrow[r, shift right, swap, "{0_{A',B}}"] & B
...
\end{tikzcd} \]
commutes if we remove $h$. By \ref{leftRightRelatedIsomorphic} the entire diagram commutes and $h,h'$ are inverses of each other. As they determine isomorphisms in the coleg categories, we have that $g$ also determines a terminal coleg (by \ref{initialTerminalIsomorphisms}). So $g\in \coker(f)$.
...
\begin{proposition}
Abelian categories are complete.
\end{proposition}
