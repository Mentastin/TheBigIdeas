...
\[ \forall f,g\in A: \qquad G(f)\alpha_g  = \alpha_f F(g), \]
...
\begin{definition}
Let $\sSet{J,\cdot}$ be an associative set and $\sSet{A,\cdot}$ an associative class. Let $F: J\to A$ be a functor. Then
\begin{itemize}
\item a \udef{cone} over $F$ is a natural transformation from a constant functor to $F$;
\item a \udef{cocone} under $F$ is a natural transformation from $F$ to a constant functor.
\end{itemize}
TODO: limit = unique whiskering!
\end{definition}
...
\begin{proposition}
Let $\cat{C}$ be an abelian category. The functions $\ker: \mor(\cat{C}) \to \mor(\cat{C})$ and $\coker: \mor(\cat{C}) \to \mor(\cat{C})$ are bijective. Also $\ker^{-1} = \coker$.
\end{proposition}
\begin{proof}
