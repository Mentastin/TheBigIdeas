...
\begin{definition}
Let $\cat{C}$ be a category with terminal object $1$. Then a morphism is called a \udef{constant} in an object $A$ if it is of type $1 \to A$.

We write constants underligned, e.g.\ $\underline{c}: 1\to A$.

Let $A,B\in\ob(\cat{C})$ be objects. A morphism $f: A\to B$ is called \udef{constant} if $f$ factors through the terminal object: $f: A \to 1 \to B$. We underline $f$ to show it is constant: $\underline{f}: A\to B$.
\end{definition}
...
\[ \begin{tikzcd}
U \arrow[r] \arrow[d, hook, "j"] \arrow[dr, phantom, "\ulcorner", very near start] & 1 \arrow[d, "\underline{T}"] \\
A \arrow[r, "\chi"] & \{T,F\}
\end{tikzcd} \]
\end{lemma}
...
\begin{proof}
\[ \begin{tikzcd}
V \arrow[dr, phantom, "\ulcorner", very near start] \rar \arrow[d, hook] & U \arrow[r] \arrow[d, hook, "j"] \arrow[dr, phantom, "\ulcorner", very near start] & 1 \arrow[d, "\underline{T}"] \\
...
\begin{definition}
Let $f: A\to B$ be a morphism in a category $\cat{C}$. Then the \udef{graph} of $f$ is the morphism $(\id_A, f): A\to A\times B$.
\end{definition}
