...
\begin{enumerate}
\item a class, denoted $\mor(\cat{C})$, of arrows $f,g,h,\ldots$
\item a subclass, denoted $\id(\cat{C})\subseteq \mor(\cat{C})$, of identity arrows;
\item a partial binary function $\circ: \mor(\cat{C}) \times \mor(\cat{C}) \not\to \mor(\cat{C})$;
\end{enumerate}
such that
\begin{itemize}
\item for all $I\in \id(\cat{C})$
\begin{align*}
If &= f & &\text{whenever $If$ is defined} \\
fI &= f & &\text{whenever $fI$ is defined;}
\end{align*}
\item for each arrow there exist identity arrows $I,I'$ such that $If$ and $fI'$ exist;
\item $\circ$ is associative.
\end{itemize}
\end{lemma}
\begin{proof}
Clearly a category with objects yields a category in this sense, by just forgetting about the objects.

Conversely, given such an ``arrows only'' category $\cat{C}$, we may identify $\ob(\cat{C}) = \id{\cat{C}}$. For any $f\in \mor(\cat{C})$, there is in fact a unique identity arrow $I$ such that $f = If$. Indeed, assume there were two such arrows, $I_1$ and $I_2$, then $f = I_1f = I_1I_2f$, so $I_1I_2$ is defined and by definition of identity arrow, we must have $I_1 = I_1I_2 = I_2$. Similarly there is a unique identity arrow such that $fI'$ exists. We set $\dom(f) = I$ and $\codom(f) = I'$. The rest is easy to verify.
\end{proof}
...
\begin{itemize}
\item $\id_{\ob(\cat{C})} = \id_{\ob(\cat{C^{op}})}$;
\item $\id_{\mor(\cat{C})} = \id_{\mor(\cat{C^{op}})}$
\end{itemize}
form a contravariant functor $\operatorname{OP}: \cat{C} \to \cat{C^{op}}$.
