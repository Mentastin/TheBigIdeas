...
\begin{itemize}
\item \udef{terminating} if $T_R$ is terminating;
...
\begin{definition}
Let $R,S$ be two relations on a class $X$. We say $R$ and $S$
\begin{itemize}
\item \udef{commute} if $\forall x,y_1,y_2\in X:\; \big(xR^*y_1\big)\land\big(xS^*y_2\big)\implies \exists z\in X: \big(y_1S^*z\big)\land\big(y_2R^*z\big)$;
\item \udef{strongly commute} if $\forall x,y_1,y_2\in X:\; \big(xRy_1\big)\land\big(xSy_2\big)\implies \exists z\in X: \big(y_1S^?z\big)\land\big(y_2R^*z\big)$;
\item have the \udef{commuting diamond property} if $\forall x,y_1,y_2\in X:\; \big(xRy_1\big)\land\big(xSy_2\big)\implies \exists z\in X: \big(y_1Sz\big)\land\big(y_2Rz\big)$.
\end{itemize}
\end{definition}
...
\begin{lemma}
The property of strong confluence is symmetric in $R$ and $S$.
\end{lemma}
The property of confluence and the commuting diamond property are also symmetric in $R$ and $S$, but this is immediate from the definition.
\begin{proof}
TODO ???
\end{proof}
