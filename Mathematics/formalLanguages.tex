...
\end{proposition}
\begin{proof}
We have that $T_{\im(f)}$ is a subtree of $T_S$ by.

Assume $X$ is not terminating. Then $g$
\end{proof}

\begin{lemma}
A finitely branching rewriting system terminiates \textup{if and only if} there exists a monotone embedding in $\sSet{\N,>}$.
\end{lemma}
\begin{proof}
$\boxed{\Rightarrow}$
