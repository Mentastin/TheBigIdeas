\chapter{Observations for designing the formal system}
\section{Desiderata}
\subsection{Examples}
Potential examples refer to a label here. \label{typeExample}

\begin{itemize}
\item Sets need to be context-aware, i.e. know whether they are considered as a subset or not in the given context. (See also the juggling of the $\incl$ function in this context).
\end{itemize}
...
\begin{proposition} \label{subalgebrasCompleteWedgeSubsemilattice}
...
\section{Relational universal algebra}

\subsection{Closure under a relation}
...
Let $f:A\to B$ be a homomorphism of $\Omega$-algebras. Then there exists a unique isomorphism $f': A/\ker f \to \im f$ such that
\[ \begin{tikzcd}
A \arrow[r, "{[\cdot]_{\ker f}}"] \arrow[dr, swap, "f|^{\im f }"] & A/\ker f \arrow[d, dashed, "{f'}"] \\
& \im f
\end{tikzcd} \qquad\text{commutes.} \]
In particular, we have $A/\ker f \cong \im f$.
\end{theorem}
\begin{proof}
...
\end{proof}
\begin{corollary} \label{kernelInclusionDecomposition}
Let $f:A\to B$ and $g: A\to C$ be homomorphisms between $\Omega$-algebras. If $\ker f \supseteq \ker g$, then there exists $f': \im g \to B$ such that
\[ \begin{tikzcd}
A \arrow[r, "g"] \arrow[dr, swap, "f"] & \im g \arrow[d, dashed, "{f'}"] \\
& B
\end{tikzcd} \qquad\text{commutes.} \]
\end{corollary}
In other words, $f = f'\circ g$.
\begin{proof}
Since $\ker f \supseteq \ker g$, we can use the factor theorem \ref{factorTheorem} to decompose $f = f_0\circ [\cdot]_{\ker g}$. We now use the theorem to decompose $g|^{\im g} = g'\circ [\cdot]_{\ker g}$, where $g'$ is an isomerphism. Now set $f' \defeq f_0 \circ (g')^{-1}$, so we have
\begin{align*}
f &= f_0\circ [\cdot]_{\ker g} \\
&= \big(f_0\circ (g')^{-1}\big) \circ \big(g'\circ [\cdot]_{\ker g}\big) \\
&= f' \circ g|^{\im g} \\
&= f' \circ g.
\end{align*}
\end{proof}
...
\begin{lemma} \label{polarSubalgebra}
Let $A$ be a set and $B$ an $\Omega$-algebra, $X\subseteq A$ and $R$ a relation on $(A,B)$. Suppose $aR$ is a subalgebra of $B$ for all $a\in A$, then $X^R$ is a subalgebra of $B$.
\end{lemma}
\begin{proof}
Immediate from \ref{polarIntersectionPrincipalPolars} and \ref{subalgebrasCompleteWedgeSubsemilattice}.
\end{proof}
...
\begin{lemma} \label{GreensRelationsLemma}
...
\end{proof}
\begin{corollary} \label{greensDCompositionOfGreensLGreensR}
...
\item by \ref{greensDCompositionOfGreensLGreensR}, $a\mathcal{D}b$ \textup{if and only if} there is an element in the intersection of the row of $a$ and the column of $b$ or vice versa.
\end{itemize}
...
\begin{theorem}[Green's theorem] \label{GreensTheoremSemigroups}
...
\subsubsection{Regularity of elements and $\mathcal{D}$-classes}
\begin{definition}
Let $S$ be a semigroup. An element $a\in S$ is called \udef{regular} if there exists $x\in S$ such that $axa = a$.
\end{definition}

\begin{lemma}
Let $S$ be a semigroup, $a\in S$ a regular element and $b\in S$. Then
\begin{enumerate}
\item if $a\mathcal{L}b$, then $b$ is regular;
\item if $a\mathcal{R}b$, then $b$ is regular.
\end{enumerate}
\end{lemma}
\begin{proof}
Since $a$ is regular, there exists $x\in S$ such that $a=axa$.

(1) By \ref{GreensRelationsLemma}, there exist $y,z\in \widetilde{S}$ such that $ya = b$ and $zb = a$. Then $b= ya = yaxa = bxa = bxzb$, so $b$ is regular.
...
\begin{lemma} \label{principalIdealsRegularElements}
Let $S$ be a semigroup and $a\in S$ a regular element. Then $a \in aS \cap Sa$.
\end{lemma}
This is equivalent to $aS = a\widetilde{S}$ and $Sa = \widetilde{S}a$.
\begin{proof}
Suppose $a = axa$. Then the result follows since clearly $axa \in aS \cap Sa$.
\end{proof}
...
\begin{lemma} \label{generalisedInverseIffRegular}
...
\subsubsection{Simply polar elements}
\begin{definition}
Let $S$ be a semigroup and $a\in S$. Then $a$ is called \udef{simply polar} if $a$ has a generalised inverse $a'$ such that $a$ and $a'$ commute.
\end{definition}
Terminology by Robin Harte?? (Lead: On generalized inverses in $C^*$-algebras by Harte and Mbekhta).

\begin{lemma} \label{squareSimplyPolar}
Let $S$ be a semigroup and $a\in S$. If $a$ is simply polar, then $a^2$ is simply polar.
\end{lemma}
\begin{proof}
Assume $a$ is simply polar and let $a'$ be a commuting generalised inverse. Then we claim $(a')^2$ is a commuting generalised inverse of $a^2$. Clearly $(a')^2$ commutes with $a^2$. Also
\begin{align*}
a^2 (a')^2 a^2 &= aa'aa'aa = aa'aa = aa = a^2 \\
(a')^2a^2(a')^2 &= a'aa'aa'a' = a'aa'a' = a'a' = (a')^2.
\end{align*}
\end{proof}

\begin{proposition} \label{simplyPolarLemma}
Let $S$ be a semigroup and $a\in S$. Then the following are equivalent:
\begin{enumerate}
\item $a$ is simply polar;
\item there exists $a'\in S$ such that $a = aa'a$ and $a'$ commutes with $a$;
\item $a\mathcal{H}a^2$;
\item $[a]_\mathcal{H}$ is a subgroup of $S$.
\end{enumerate}
\end{proposition}
Note that $(2) \Rightarrow (1)$ is similar to \ref{generalisedInverseIffRegular}, but does not quite follow immediately from it. We still need to check that the generalised inverse we have constructed commutes with $a$. It is however straightforward to see that $a'aa'$ commutes with $a$.
\begin{proof}
$(1) \Rightarrow (2)$ Immediate.

$(2) \Rightarrow (3)$ Let $a'$ be as in (2). Then $a = aa'a = a'a^2$, and $a^2 = aa$, so $a\mathcal{L}a^2$. Similarly, $a\mathcal{R}a^2$ and thus $a\mathcal{H}a^2$.

$(3) \Rightarrow (4)$ Since $a^2 \in [a]_\mathcal{H}$ and $a^2 \in [a]_\mathcal{H}^2$, we have that $[a]_\mathcal{H}$ and $[a]_\mathcal{H}^2$ are not disjoint. By Green's theorem, \ref{GreensTheoremSemigroups}, we have that $[a]_\mathcal{H}$ is a subgroup of $S$.

$(4) \Rightarrow (1)$ The element $a$ has an inverse $a'$ in $[a]_\mathcal{H}$, since it is a group. Now $a$ commutes with $a'$, since elements commute with their inverses in groups. Let $e$ be the identity in $[a]_\mathcal{H}$. We calculate $a = ae = aa'a$ and $a' = a'e = a'aa'$, to show that $a$ and $a'$ are generalised inverses in $S$.
\end{proof}
...
\end{itemize}
We let $(A\to B)_{R,S}$ denote the set of order-preserving functions and $(A\not\to B)_{R,S}$ the set of order-preserving partial functions.
\end{definition}
...
\subsection{Bounds and relation-preserving functions}

\begin{lemma} \label{orderPreservingFunctionBoundsLemma}
Let $f: \sSet{X,R} \to \sSet{Y,S}$ be a relation preserving function and $A\subseteq X$. Then $f^\imf\big(A^R\big) \subseteq \big(f^\imf(A)\big)^S$.
\end{lemma}
\begin{proof}
Take $f(b) \in f^\imf\big(A^R\big)$, such that $b\in A^R$. Now take arbitrary $f(a) \in f^\imf(A)$. Since $aRb$, we have $f(a)Rf(b)$. Since $f(a)$ was chosen arbitrarily, we have $f(b)\in \big(f^\imf(A)\big)^\leq$.
\end{proof}
...
\begin{enumerate}
\item If $f$ is a residuated map, then
\begin{enumerate}
\item $f^\imf\big(X^R\big)^{S^\transp} = \big(f^\imf(X)^{S}\big)^{S^\transp}$;
\item $f[\sup(X)] \subseteq \sup(f[X])$.
\end{enumerate}
\item If $g$ is a residual map, then
\begin{enumerate}
\item $g^\imf\big(X^{R^\transp}\big)^{S} = \big(g^\imf(X)^{S^\transp}\big)^{S}$;
\item $g[\inf(X)] \subseteq \inf(g[X])$.
\end{enumerate}
\end{enumerate}
\end{proposition}
\begin{proof}
(1a) Since $f^\imf(X^R) \subseteq f^\imf(X)^S$, we have $f^\imf\big(X^R\big)^{S^\transp} \supseteq \big(f^\imf(X)^{S}\big)^{S^\transp}$. We now only need to prove the other inclusion.

Let $f': B\to A$ be a residual of $f$. From above we see that it is enough to have $f[X^R]^{S^\transp} \subseteq (f[X]^S)^{S^\transp}$.
...
(1b) Follows immediately from the calculation above.
...
\end{proof}
\begin{corollary} \label{polarIntersectionPrincipalPolars}
Let $R$ be a relation on $(A,B)$ and $X\subseteq A$. Then $A^R = \bigcap_{a\in A}\{a\}^R$.
\end{corollary}
...
Let $P$ be a poset. Then
\begin{enumerate}
\item if $\{A_i\}_{i\in I}\subseteq \mathcal{J}$, then $\bigcup_{i\in I}A_i \in \mathcal{J}$ \textup{if and only if} $\big\{\bigvee A_i\big\}_{i\in I} \in \mathcal{J}$ and
...
\end{enumerate}
\end{proposition}
Here $\mathcal{J}$ is the set of sets that have a join and $\mathcal{M}$ is the set of sets that have a meet.
\begin{proof}
(1) The direction $\Rightarrow$ is an application of \ref{residuationPreservesSupInf} to \ref{upsetDownsetConnections}. For the converse, assume $\bigvee_{i\in I}\Big(\bigvee A_i\Big)$ exists. It is an upper bound of $\bigcup_{i\in I}A_i$: take $x\in \bigcup_{i\in I}A_i$, then there exists $i\in I$ such that $x\in A_i$, so $x \leq \bigvee A_i$ and thus $x\leq \bigvee A_i \leq \bigvee_{i\in I}\Big(\bigvee A_i\Big)$.

Now assume $u$ is some other upper bound of $\bigcup_{i\in I} A_i$. Since $u$ is an upper bound of $A_i$ for all $i\in I$, we have $\bigvee A_i \leq u$. Thus $u$ is also an upper bound of $\setbuilder{\bigvee A_i}{i\in I}$, which means that $\bigvee_{i\in I}\Big(\bigvee A_i\Big) \leq u$.
