...
\begin{definition}
Let $A,B$ be $\Omega$-algebras. A homomorphism $f:A\to B$ is called an \udef{embedding} if $f|^{\imf(f)}$ is an isomorphism.
\end{definition}
...
\begin{proof}
\begin{enumerate}
\item $(a) \Rightarrow (b)$ By \ref{residuationPreservesSupInf}.

$(b) \Rightarrow (c)$ Assume $f$ preserves joins. By \ref{upsetDownsetConnections} we need to show that $\downset \bigvee f^{\preimf}(\downset y) = f^{\preimf}(\downset y)$ for all $y\in Q$. In fact it is enough to prove $\downset \bigvee f^{\preimf}(\downset y) \subseteq f^{\preimf}(\downset y)$. We calculate
\begin{align*}
...
$(c) \Rightarrow (a)$ Take arbitrary $x\in P$ and $y\in Q$.
