...
\begin{lemma}
Let $V,W$ be vector spaces over a field $\F$ and $A,C\in\Hom(V)$ and $B,D\in\Hom(W)$. Then
\begin{enumerate}
...
We denote the set of all linear maps from $V$ to $W$ as $\Hom_\mathbb{F}(V,W)$, or $\Hom(V,W)$. The set of endomorphisms on $V$ is denoted $\Hom(V) \defeq \End(V) = \Hom(V,V)$.
\end{definition}
...
\begin{definition}
Let $L \in \Hom(V,W)$. The \udef{kernel} or \udef{null space} of $L$ is the set of vectors that $L$ maps to zero:
\[ \ker(L) = \{ v\in V \;|\; L(v) = 0 \}. \]
\end{definition}
\begin{proposition}
The kernel of $L\in \Hom(V,W)$ is a subspace of $V$.
\end{proposition}
\begin{definition}
The dimension of the kernel of a linear map is its \udef{nullity}.
\end{definition}
\begin{proposition} \label{prop:injectivityKernelTriviality}
Let $L\in\Hom(V,W)$. Then $L$ is injective if and only if $\ker(L) = 0$.
\end{proposition}
...
\begin{definition}
Let $L \in \Hom(V,W)$. The \udef{image} or \udef{range} of $L$ is the set of vectors that are of the form $L(v)$ for some $v\in V$:
\[ \im(L) = \{ L(v) \;|\; v\in V \}. \]
\end{definition}
\begin{proposition}
The range of $L\in \Hom(V,W)$ is a subspace of $W$.
\end{proposition}
\begin{definition}
...
\begin{definition}
Suppose $K,L \in \Hom_{\mathbb{F}}(V,W)$ and $\lambda \in \mathbb{F}$.
\begin{itemize}
...
\item With addition and scalar multiplication defined as above, $\Hom_\mathbb{F}(V,W)$ is a vector space.
\end{itemize}
\end{proposition}

\begin{definition}
Let $K\in \Hom_\mathbb{F}(U,V)$ and $L\in \Hom_\mathbb{F}(V,W)$. The \udef{product} $LK$ is defined as the composition
...
\item[\textbf{Identity}] Let $L\in \Hom(V,W)$. The identity maps $I_V:V\to V$ and $I_W:W\to W$ are linear and have the property that
\[ LI_V = I_W L = L. \]
\item[\textbf{Distributive properties}]
$ (S_1+S_2)T = S_1T + S_2T \qquad \text{and} \qquad S(T_1 + T_2) = ST_1 + ST_2 $
whenever $T,T_1, T_2 \in \Hom(U,V)$ and $S,S_1, S_2\in \Hom(V,W)$.
\end{itemize}
These properties mean that for any vector space $V$, $\Hom(V)$ forms a unital algebra.
\end{proposition}
...
Let $L\in\Hom(V,W)$ be an isomorphism. Let $\beta$ be a basis of $V$, then $L[\beta]$ is a basis of $W$.
\end{proposition}

\begin{proposition} \label{prop:invertibleFiniteDim}
Suppose $V$ is a finite-dimensional vector space and $L\in \Hom(V)$ is a linear map on $V$, then
...
Let $T\in \Hom(V,W)$. Then $T$ induces a linear map
