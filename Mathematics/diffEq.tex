...
Let $A$ be a densely defined operator on a Hilbert space $H$. Then $iA$ generates a unitary group $T: \R \to \Bounded(H)$ \textup{if and only if} $A$ is self-adjoint.
\end{theorem}
\begin{proof}
TODO
\end{proof}
...
R^+\to \Bounded(X)$ and there exists a constant $C>0$ such that
\[ \norm{R_A(r+is)} \leq \frac{1}{\tan\delta|s|} \qquad \forall r>0, \forall s\in\R\setminus\{0\}; \]
...
\end{proposition}
\begin{proof}
We first prove the proposition for contraction semigroups ($w=0$ and $M = 1$). Take $\lambda>0$. Then
\[ \lambda\id_X - A-B = (\id_X - BR_A(\lambda))(\lambda\id_X - A), \]
which is bijective iff $\id_X - BR_A(\lambda)$ is bijective, in which case $R_{A+B}(\lambda) = R_A(\lambda)\big(\id_X - BR_A(\lambda)\big)^{-1}$. If $\norm{B}< \lambda$, then $BR_A(\lambda)$ is a contraction and thus we can form the Neumann series \ref{operatorNeumannSeries}, which allows the calculation
\[ \norm{R_{A+B}(\lambda)} \leq \norm{R_A(\lambda)}\sum_{n=0}^\infty\big(\norm{B}\,\norm{R_A(\lambda)}\big)^n = \frac{\norm{R_A(\lambda)}}{1-\norm{B}\,\norm{R_A(\lambda)}} = \frac{1}{\lambda - \norm{B}}. \]
By \ref{quasiContractiveHilleYosida} $A+B$ then generates a semigroup $S(t)$ satisfying $\norm{S(t)} \leq e^{\norm{B}t}$ for all $t\in\R^+$.

TODO rescaling
\end{proof}
\begin{corollary}[Variation of parameters]
...
\subsection{Perturbations of contractive semigroups}
\subsection{Perturbations of analytic semigroups}
