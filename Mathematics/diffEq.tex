...
\begin{definition}
Let $X$ be a Banach space. We call a function $T: \R^+ \to \Lin(X)$ a \udef{semigroup of linear operators} (or just \udef{semigroup}) if
\begin{itemize}
\item $T(0) = \id_X$;
...
\item We call $T$ \udef{bounded} if $T(t)$ is bounded for all $t\in \R^+$.
\item A bounded semigroup is called \udef{uniformly continuous} if it is continuous when $\Bounded(X)$ is equipped with the norm topology.
\end{itemize}
\end{definition}
