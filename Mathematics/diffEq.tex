...
\begin{lemma}
Let $A$ be a densely defined operator on a Banach space $X$. Suppose
\begin{itemize}
...
\end{itemize}
Then $\lim_{r\to \infty} r R_A(r)x = x$ for all $x\in X$.
\end{lemma}
\begin{proof}
...
Now we need to extend this result to all of $X$. Take $x\in X$, then we can find a sequence $\seq{x_n}$ in $\dom(A)$ that converges to $x$, because $A$ is densely defined. By assumption $r \to r R_A(r)$ is uniformly bounded and thus uniformly continuous (TODO ref), meaning we can use the Moore-Osgood theorem (TODO ref):
\begin{align*}
...
If $A$ is closed in addition to the other requirements, then $\lim_{r\to \infty} r AR_A(r)x = Ax$ for all $x\in \dom(A)$.
\end{corollary}
\begin{proof}
...
\subsubsection{Generation theorems for dissipative operators}

\begin{theorem}[Lumer-Phillips]
Let $A$ be an operator on a Banach space $X$. The following are equivalent:
\begin{enumerate}
\item $A$ generates a strongly continuous contraction semigroup;
\item $A$ is closed, densely defined, dissipative and $\im(\lambda\id-A) = X$ for some (and hence all) $\lambda > 0$.
\end{enumerate}
\end{theorem}
\begin{proof}
$(1) \Rightarrow (2)$ The Hille-Yosida theorem \ref{HilleYosidaContraction} gives that $A$ is closed and densely defined. The surjectivity of $\lambda\id-A$ for all $\lambda > 0$ is immediately implied by $\R^{>0}\subseteq \res(A)$. We just need to verify that $A$ is dissipative. This follows from $\norm{\lambda R_T(\lambda)}\leq 1$ by \ref{dissipativeResolventBound}.

$(2) \Rightarrow (1)$ Again we use the Hille-Yosida theorem \ref{HilleYosidaContraction}. We have that $\norm{\lambda R_T(\lambda)}\leq 1$ for all $\lambda>0$ by \ref{dissipativeResolventBound}. Also by \ref{dissipativeResolventBound}, the equation $\im(\lambda\id-A) = X$ implies $\lambda\in\res(A)$. So $]0,+\infty[\,\subseteq \res(A)$ by \ref{spectrumDissipativeOperator}.
\end{proof}
\begin{corollary}
...
\end{corollary}
Note that $A$ is closable by \ref{dissipativeOperatorClosable}.
\begin{proof}
It is enough to prove that $\id - \overline{A}$ is surjective, which is equivalent to proving $\im(\id - A)$ is dense by \ref{dissipativeOperatorClosable}.
...
\end{proof}
\begin{corollary}
Let $A$ be a dissipative operator on a Banach space $X$. If $\lambda\id - A$ is surjective for some $\lambda > 0$, then $A|^{\dom(A)}: \dom(A)\to \dom(A)$ generates a contraction semigroup on $\overline{\dom(A)}$.
\end{corollary}
\begin{proof}
$\im(\lambda\id-A)$ is closed, so $A|^{\dom(A)}$ closed.

$\im(\lambda\id-A)$ still surjective, so dense in $\overline{\dom(A)}$

we just need densely defined
\end{proof}
