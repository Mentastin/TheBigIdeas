...
\begin{definition}
Let $X$ be a Banach space over a field $\F$ and $I\subseteq \F$ a submonoid of $\sSet{\F, +, 0}$. We call a function $T: I \to \Bounded(X)$ a \udef{semigroup of linear operators} (or just \udef{semigroup}) if
\begin{itemize}
\item $T(0) = \id_X$;
\item $T(s+t) = T(s)T(t)$ for all $s,t \in I$.
\end{itemize}
If $I$ is a subgroup of $\sSet{\F, +, 0}$, then $T$ is called a \udef{group of linear operators}.

A semigroup is called
\begin{itemize}
\item \udef{strongly continuous} or \udef{$C_0$} if it is continuous when $\Bounded(X)$ is equipped with the strong operator topology;
\item \udef{uniformly continuous} when $\Bounded(X)$ is equipped with the norm topology.
\end{itemize}
\end{definition}
...
\end{definition}
A semigroup of linear operators is just a monoid homomorphism from $\sSet{I,+,0}$ to $\Bounded(X)$.
...
\begin{proposition}
Let $X$ be a Banach space and $A\in \Bounded(X)$. Then $T: \R^+ \to \Bounded(X): t\mapsto e^{tA}$ is a uniformly continuous and differentiable semigroup that satisfies $\od{T(t)}{t} = AT(t)$.
\end{proposition}
\begin{proof}
...
\begin{proposition}
Let $X$ be a Banach space and $T: \R^+ \to \Bounded(X)$ a uniformly continuous and differentiable \emph{function} such that 
\[ \begin{cases}
\od{T(t)}{t} = AT(t) \\
T(0) = \id_X,
\end{cases} \]
for some $A\in \Bounded(X)$. Then $T(t) = e^{tA}$.
\end{proposition}
\begin{proof}
...
\begin{proposition}
Let $X$ be a Banach space and $T: \R^+ \to \Bounded(X)$ a uniformly continuous semigroup. Then $T$ is differentiable and $\od{T(t)}{t} = AT(t)$ for some $A\in\Bounded(X)$.
\end{proposition}
\begin{proof}

\end{proof}
\begin{corollary}
Let $X$ be a Banach space and $T: \R^+ \to \Bounded(X)$ a uniformly continuous semigroup. Then $T(t) = e^{tA}$ for some $A\in\Bounded(X)$.
\end{corollary}
\begin{proof}
...
\begin{definition}
A sequence of functions $\seq{\phi_n}_{n\in\N}$ is a \udef{gauge sequence} or \udef{asymptotic scale} for $x\to x_0$ if, for all $n\in \N$,
\[ \phi_{n+1}(x) = o\big(\phi_n(x)\big) \qquad\text{as $x\to x_0$.} \]
\end{definition}
...
\subsubsection{The scale $(x-x_0)^n$}
\begin{lemma}
...
\[ \int_{x}^N \Big(f(t) - a_0 - \frac{a_1}{t}\Big) \diff{t} = \sum_{n=2}^N \frac{a_n}{n-1}x^{-n + 1} + O(x^{-N}). \]
\end{proposition}
\begin{proof}
...
Let $\sum_{i=0}^n a_i(t)u^{(i)}(t) = 0$ be a linear homogenous $n^\text{th}$-order differential equation with $a_n = \constant{1}$. Then $u(t)$ is a solution of this differential equation \textup{if and only if} it is the $0^{\text{t}}$ component of a solution to the following matrix differential equation
...
\begin{lemma}
Let $A, \Psi: \interval{a,b}\to \F^{n\times n}: t\mapsto A(t)$ be matrix-valued functions.
...
\begin{definition}
Let $A, \Psi: \interval{a,b}\to \F^{n\times n}: t\mapsto A(t)$ be matrix-valued functions. If
\begin{itemize}
...
Let $A, \Psi: \interval{a,b}\to \F^{n\times n}: t\mapsto A(t)$ be matrix-valued function and $\Psi(t)$ a fundamental matrix. Then
\begin{enumerate}
