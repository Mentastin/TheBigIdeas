...
\end{lemma}
\begin{proof}
(1) Consider the initial slice $A \defeq \seq{n_k}|_{k:k+n_k+1}$. This is an injective function, so $A \geq_c \interval[co]{k, k+n_k+1}$ by \ref{injectivityCardinality}, $\#(A) = n_k +1$ by \ref{sizeOfIntervalsInN} and thus there exists an element of $A$ that is larger than $n_k$ by \ref{sizeEstimationByIntervalInclusion}.
