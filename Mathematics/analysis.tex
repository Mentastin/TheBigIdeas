...
\end{itemize}
If $M = \overline{\R^+}$, then we call the content \udef{positive}.
\end{definition}
...
\[ \mu(A\uplus B) = \mu(A) + \mu(B) \]
for all disjoint $A,B\in \mathcal{S}$.
\end{proof}
...
\end{proof}
For positive contents this is requiement is $\mu(A) < \infty$.

\begin{proposition} \label{contentOfInclusion}
Let $\mathcal{S}$ be a ring on a set $X$ and $\mu$ a positive content on $\mathcal{S}$. Take $A,B\in \mathcal{S}$ such that $A\subseteq B$. Then
\begin{enumerate}
\item $\mu(A) \leq \mu(B)$;
\item if $\mu(A) < \infty$, then $\mu(B\setminus A) = \mu(B)-\mu(A)$.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) We calculate $\mu(B) = \mu\big((B\setminus A) \uplus A\big) = \mu(B\setminus A) + \mu(A) \geq \mu(A)$.

(2) We rearrange the above equality.
\end{proof}

\begin{proposition} \label{contentSubadditive}
Let $\mathcal{S}$ be a ring on a set $X$, $\mu$ a positive content on $\mathcal{S}$ and $A,B\in \mathcal{S}$. Then $\mu(A\cup B) \leq \mu(A) + \mu(B)$.
\end{proposition}
\begin{proof}
We have
\[\mu(A\cup B) = \mu\big((A\setminus B)\uplus B\big) = \mu(A\setminus B) + \mu(B) \leq \mu(A) + \mu(B), \]
using \ref{contentOfInclusion} (because $A\setminus B \subseteq A$).
\end{proof}
...
\end{enumerate}
\end{proposition}
TODO cfr. \ref{contentSubadditive}.
\begin{proof}
(1) TODO: given by \ref{contentOfInclusion}
...
\end{itemize}
We define the function $\lambda$ on the elementary sets by $\lambda(I) = |b-a|$ and $\lambda(\R) = \infty$.
...
\begin{lemma}
The set of elementary subsets of the real line is a semi-algebra.
\end{lemma}
\begin{proof}
...
Using \ref{algebraFromSemiAlgebra}, we can extend the family of elementary subsets to an algebra. We extend the function $\lambda$ by defining
\[ \lambda(I\uplus J) = \lambda(I) + \lambda(J). \]
We will often use the term ``elementary set'' to refer to the whole algebra.

This is exactly the construction described in \ref{contentsOnSemiRingToRing}.

\subsubsection{The Peano-Jordan content}
\begin{definition}
A subset $A\subseteq \R$ is called \udef{Jordan measurable} if
\[ \sup\setbuilder{\lambda(B)}{\text{$B\subseteq A$ and $B$ is elementary}} = \inf\setbuilder{\lambda(C)}{\text{$A\subseteq C$ and $C$ is elementary}}. \]
We define the \udef{Jordan content} of $A$ to be equal to this quantity.
\end{definition}

\begin{proposition}
A subset $A\subseteq \R$ is Jordan measurable \textup{if and only if} for all $\epsilon >0$, there exists a pair of sets elementary $B_\epsilon, C_\epsilon \subseteq \R$ such that
\begin{itemize}
\item $B_\epsilon \subseteq A\subseteq C_\epsilon$;
\item $\lambda(C_\epsilon \setminus B_\epsilon) \leq \epsilon$.
\end{itemize}
The Jordan content of a Jordan measurable set $A$ is equal to $\lim_{\epsilon\to 0}\lambda(B_\epsilon) = \lim_{\epsilon\to 0}\lambda(C_\epsilon)$.
\end{proposition}
\begin{proof}
First assume $A$ is Jordan measurable with Jordan content $x$. Then
\end{proof}


\begin{itemize}
\item Notice that $\lambda(C_\epsilon \setminus B_\epsilon)$ is well-defined because $\lambda$ is defined on the whole \emph{algebra} of elementary sets.
\item WLOG we may take $B_\epsilon$ monotonically decreasing in $\epsilon$ and $C_\epsilon$ monotonically increasing in $\epsilon$. Indeed we may replace these families by
\begin{align*}
B'_\epsilon &= \bigcup_{k}
\end{align*}
\item For the Jordan content to be well-defined, we need $\lim_{\epsilon\to 0}\lambda(B_\epsilon) = \lim_{\epsilon\to 0}\lambda(C_\epsilon)$. If $\lambda(B_\epsilon)$ is finite for all $\epsilon >0$, then we have $\lambda(C_\epsilon \setminus B_\epsilon) = \lambda(C_\epsilon) - \lambda(B_\epsilon)$ by \ref{contentOfInclusion}. The result then follows by continuity of the subtraction.

If $\lambda(B_\epsilon)$ is not finite
\end{itemize}
...
\begin{lemma}
The set of Jordan measurable sets is an algebra, which is admissible. The Jordan content is also admissible.
\end{lemma}
\begin{proof}
Admissibility is clear. We just need to show that the set of Jordan measurable sets forms an algebra.

We prove this from the fact that elementary sets form an algebra, using \ref{setAlgebraCriteria}.

Clearly $\R\in\mathcal{A}$.

Let $A$ be Jordan measurable. Then there exist families $B_\epsilon^A$ and $C_\epsilon^{A}$ of elementary sets such that $B_\epsilon^A \subseteq A \subseteq C_\epsilon^{A}$. Then $(B_\epsilon^A)^c$ and $(C_\epsilon^{A})^c$ are also families of elementary sets and $(B_\epsilon^A)^c \subseteq A^c \subseteq (C_\epsilon^{A})^c$.
\end{proof}
