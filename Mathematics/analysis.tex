...
\begin{definition}
Let $\sSet{X,d,\mu}$ be a metric measure space. A \udef{Vitali covering} of $E\subseteq X$ is a set $\mathcal{C}\subseteq \powerset(X)$ such that 
\[ \forall x\in E: \forall \delta >0: \exists C\in \mathcal{C}: \; (x\in C) \land \big(\diam(C) \leq \delta\big). \]
\end{definition}

\begin{theorem}[Vitali's covering theorem]
Let $\sSet{X,d,\mu}$ be a doubling metric measure space, $A\subseteq X$ a subset and $\mathcal{F}$ a collection of closed balls whose centers are in $A$ such that
\[ \forall a\in A:\quad \inf\setbuilder{r}{\cball(a,r)\in \mathcal{F}} = 0. \]
THen there exists a countable disjoint subfamily $\mathcal{G}\subseteq \mathcal{F}$ such that
\[ \mu\Big(A\setminus \bigcup \mathcal{G}\Big) = 0. \]
\end{theorem}
Notice that in particular $\mathcal{F}$ must cover $A$.
\begin{proof}
TODO
\end{proof}
...
Let $\sSet{X,d,\mu}$ be a doubling metric measure space and $f: X\to \R^+$ a locally integrable function. Then
...
\begin{proposition}
Let $\sSet{X,d}$ be a metric space equipped with a Borel regular doubling measure $\mu$ with doubling function $D$, $f: X\to \R^+$ some function, $\epsilon > 0$ and $\alpha > 0$. Then
\[ \mu\{\HLmax(f) > \alpha\} \leq \frac{D(3+\epsilon)}{\alpha}\int_{\{\HLmax(f) > \alpha\}}f\diff{\mu}. \]
\end{proposition}
\begin{proof}
