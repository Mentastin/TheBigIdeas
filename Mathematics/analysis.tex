...
\begin{definition}
Let $V$ be a real vector space. A function $f: \R \to V$ is called a \udef{step function} if
\begin{itemize}
...
\end{itemize}
We call $\seq{x_k}_{k=0}^n$ the \udef{string of jumps} of $f$. We denote the set of step functions in $(\R\to V)$ by $\SF(\R,V)$ or $\SF(V)$.
\end{definition}
Note that we allow $f$ to take any value at the jumps.
...
is called the \udef{Riemann sum} of $f$.

We call the function
\[ I: \{\text{step functions to $V$}\} \to V: f \mapsto \sum f \Delta{x} \]
the \udef{Riemann summation} function.
\end{definition}
...
is called the \udef{Stieljes sum} of $f$ with respect to $g$.

We call the function
\[ I_g: \{\text{step functions to $V$}\} \to V: f \mapsto \sum f \Delta{g} \]
the \udef{Stieltjes summation} function.
\end{definition}
