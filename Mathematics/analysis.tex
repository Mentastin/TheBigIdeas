...
\subsection{Carathéodory's construction of measures}
...
Let $G$ be a locally compact group. There exists a non-zero measure $\mu$ on the Borel $\sigma$-algebra $\mathcal{B}$ such that
\[ \forall x\in G: \forall A\in \mathcal{B}:\quad \mu(x\cdot A) = \mu(A). \]
It is uniquely determined up to scalar multiples.
\end{theorem}
This measure is called the \udef{Haar measure} of $G$.

\begin{lemma}
Let $\mu$ be the Haar measure on a locally compact group $G$.
\begin{enumerate}
\item Let $A\subseteq G$ be an open set, then $\mu(A) =0 \iff A = \emptyset$.
