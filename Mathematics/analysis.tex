...
Let $\sSet{X, \xi}$ be a locally compact Hausdorff space. For any continuous linear functional $\psi$ on $\cont_0(X)$, there is a unique complex-valued regular Borel measure $\mu$ on $X$ such that
\[ \psi(f) = \int_X f \diff{\mu}. \]
\end{theorem}
\begin{proof}
TODO. See Garling, ``A `short' proof of the Riesz representation theorem'' and also ``Another `short' proof of the Riesz representation theorem''.
\end{proof}
...
\begin{lemma} \label{expInequality}
...
TODO: \url{https://www.johndcook.com/blog/modes_of_convergence/}
...
\begin{itemize}
\item We call $C \geq 0$ an \udef{essential bound} of some $f\in \meas(X,\C)$ if $|f| \leq C$ almost everywhere.
...
\begin{lemma} \label{essentialSupremumEssentialBound}
...
\begin{definition}
The \udef{integral ordering} $\leq_I$ on $\mathcal{L}^1(\R^+,\R)$ is defined by
...
\end{cases} \]
The \udef{weak integral ordering} $\leq_{wI}$ on $\mathcal{L}^1(\R^+,\R)$ is defined by
...
\begin{definition}
Let $\sSet{\Omega, \mathcal{A}, \mu}$ be a measure space and $f: \Omega\to \R^+$ a positive measurable function. The \udef{decreasing rearrangement} of $f$ is the function
...
\begin{theorem}[Hardy-Littlewood inequality]
\[ \int_{X}|fg|\diff{\mu} \leq \int_{0}^{\infty}\decRearrange{|f|}(t)\decRearrange{|g|}(t)\diff{t}. \]
\end{theorem}

\subsubsection{Majorisation}
\begin{definition}
Let $\sSet{\Omega, \mathcal{A}, \mu}$ be a measure space. We define the \udef{majorisation relation} $\prec$ on $\mathcal{L}^1(\Omega, \mu)$ by
\[ f \prec g \quad\defequiv\quad \decRearrange{|f|} \leq_I \decRearrange{|g|} \]
and the \udef{weak majorisation relation} $\prec_w$ on $\mathcal{L}^1(\Omega, \mu)$ by
...
is called the \udef{typewriter sequence}. It marches over and over the unit interval, ever decreasing in width.
...
\subsubsection{General measure spaces}
\begin{proposition}
...
\subsubsection{Dominated sequences}
