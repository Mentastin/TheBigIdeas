...
\subsection{Pre-measures and measures}
\begin{definition}
...
\item \udef{$\sigma$-finite} if there exists $\seq{A_n}\subseteq \mathcal{A}$ such that $\Omega = \bigcup_{n\in\N}A_n$ and $\mu(A_n)<\infty$ for all $n\in\N$.
\end{itemize}
\end{definition}
...
\begin{theorem}[Carathéodory-Fréchet-Hahn-Kolmogorov-Hopf extension theorem]
...
\end{lemma}
\begin{proof}
For each $\epsilon > 0$, we can find 
\end{proof}
