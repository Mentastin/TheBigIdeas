...
This implies that $f-g$ is a constant function. In order to fix this constant, note that $g(a) = 0$, so $f(x) = f(a) + g(x)$ and, in particular, $f(b) = f(a) + \int_a^bf'(t)\diff{t}$.
\end{proof}
...
\[ d_f: \R\to \R^+: \alpha \mapsto \mu\setbuilder[\big]{\omega}{|f(\omega)|>\alpha}.  \]
\end{definition}
...
\subsubsection{Distribution inequalities}
