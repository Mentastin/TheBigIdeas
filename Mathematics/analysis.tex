...
\begin{itemize}
\item The \udef{segment set} of $J$ is the set
\[ \seg(J) \defeq \setbuilder{(x,y)\in J\times J}{x \lessdot y}. \]
\item We call $\min_{(x,y) \in \seg(J)}y-x$ the \udef{width} of $J$.
\item A \udef{free tagging function} for $J$ is a function $t: \seg(J) \to \R$.
\item We call $t$ a \udef{tagging function} or a \udef{confined tagging function} if $t\big((x,y)\big) \in \interval{x,y}$.
\item We call $t$ a \udef{strict tagging function} or a \udef{strictly confined tagging function} if $t\big((x,y)\big) \in \interval[o]{x,y}$.
\item A \udef{(free/confined/strict) tagged partition} is a pair $\sSet{J, t}$, where $J\subseteq \R$ is a finite set and $t$ is a (free/confined/strict) tagging function on $J$.
\end{itemize}
\end{definition}
...
\begin{definition}
Let $V$ be a vector space. A function $f: \R \to V$ is called a \udef{step function} if
\begin{itemize}
...
\end{itemize}
We call $J$ a \udef{set of jumps} of $f$. We denote the set of step functions in $(\R\to V)$ by $\SF(\R,V)$ or $\SF(V)$.
\end{definition}
...
\begin{lemma} \label{supersetJumpSet}
...
\subsubsection{Riemann and Stieltjes sums}
\begin{definition}
...
is called the \udef{Stieljes sum} of $f$ with respect to $g$. If $g = \id_\R$, we call it the \udef{Riemann sum} of $f$, which is denoted $\sum f \Delta{x}$

We call the function
\[ I_g: \SF(V,\R) \to V: f \mapsto \sum f \Delta{g} \]
the \udef{Stieltjes summation} function. If $g = \id_\R$, it is called the \udef{Riemann summation} function.
\end{definition}
...
\begin{definition}
Let $V$ be a real ordered vector space, $g: \R\to\R$ a monotonically increasing function. Consider the Stieltjes summation function $I_g$ as partial order-preserving function in $(\R\to V)\not\to V$. We call the consistent extension of $I_g$ the \udef{Riemann-Stieltjes integral} w.r.t.\ $g$. We denote this function $\int \mathrel{.} \diff{g}$.
If a function $f: \R \to V$ is an element of $\dom\big(\int \mathrel{.} \diff{g}\big)$, we say it is \udef{Riemann-Stieltjes integrable} w.r.t.\ $g$.

The consistent extension of the Riemann summation function $I$ is called the \udef{Riemann integral} and denoted $\int \mathrel{.} \diff{x}$. Function $f: \R \to V$ that are elements of $\dom\big(\int \mathrel{.} \diff{x}\big)$ are called \udef{Riemann integrable}.
\end{definition}
...
\begin{lemma} \label{BorelSigmaAlgebraGeneratedByCountableBase}
...
\begin{definition}
Let $\sSet{X, \xi}$ be a Hausdorff topological space. A measure such that all sets in the Borel-$\sigma$-algebra are measurable is called a \udef{Borel} measure.

A \udef{Borel measure space} is a triple $\sSet{X,\xi, \mu}$, where $\sSet{X,\xi}$ is a Hausdorff topological space and $\mu$ a Borel measure.
\end{definition}
...
\subsection{Support}
\begin{definition}
Let $\sSet{X, \xi}$ be a Hausdorff topological space and $\mu$ a Borel measure on $X$. We define the \udef{support} of $\mu$ as
\[ \supp(\mu) \defeq \setbuilder{x\in X}{\forall N\in \neighbourhood(x): \; \mu\big(\interior(N)\big) > 0} \]
\end{definition}
...
\begin{definition}
Let $\sSet{X, \xi}$ be a Hausdorff topological space and $\mu$ a Borel measure on $X$. We call $\mu$ a \udef{strictly positive measure} if $\supp(\mu) = X$.
\end{definition}

\begin{lemma} \label{strictlyPositiveMeasureLemma}
...
\begin{definition}
Let $\sSet{X, \xi}$ be a Hausdorff topological space. A Borel measure $\mu$ on $X$ is called \udef{locally finite} if every $x\in X$ has an open neighbourhood $U_x$ such that $\mu(U_x)$ is finite.
\end{definition}
...
\subsection{Inner and outer regularity}
...
\begin{itemize}
\item \udef{outer regular} if $A$ is outer regular for all $A\in \mathcal{B}$;
\item \udef{inner regular} if $A$ is inner regular for all $A\in \mathcal{B}$;
\item \udef{weakly inner regular} if $A$ is inner regular for all open sets $A\subseteq X$;
\item \udef{regular} if it is both outer regular and inner regular.
\end{itemize}
\end{definition}
...
\begin{definition}
Let $\sSet{X, \xi}$ be a Hausdorff topological space. A Borel measure $\mu$ is called a \udef{Radon measure} if it is locally finite and inner regular.
\end{definition}
...
\begin{lemma} \label{limsupBoundAE}
...
\end{proof}
\begin{proposition}[Reverse Fatou lemma] \label{reverseFatouLemma}
...
\begin{proposition}[Scheffé's lemma] \label{ScheffesLemma}
...
We also say $p,q \in \overline{\R^+}$ are \udef{extended Hölder conjugate} if either
\begin{itemize}
...
\begin{definition}
Let $(X, \mathcal{A}, \mu)$ be a measure space. We define
\begin{itemize}
\item the set $\mathcal{L}^\infty(X,\mu) \defeq \setbuilder{f\in \meas(X,\C)}{\exists C \geq 0: \; |f| \leq C \; \text{a.e.}}$;
\item the real functional
\[ \norm{\cdot}_{\infty}: \mathcal{L}^\infty(X,\mu)\to \R: f\mapsto \inf\setbuilder{C \geq 0}{|f| \leq C \; \text{a.e.}}. \]
\end{itemize}
We call $\norm{f}_\infty$ the \udef{essential supremum} of $f$. We call $f$ \udef{essentially bounded} if $f\in \mathcal{L}^\infty(X,\mu)$.
\end{definition}

\begin{lemma} \label{approximationEssentialSupremum}
Let $(X, \mathcal{A}, \mu)$ be a measure space, $f\in \meas(X, \C)$ such that $\norm{f}_\infty < \infty$ and $\epsilon > 0$. Then $|f| \leq \norm{f}_\infty + \epsilon$ almost everywhere.
\end{lemma}
\begin{proof}
...
\begin{lemma} \label{kernelEssentialSupremum}
Let $(X, \mathcal{A}, \mu)$ be a measure space and $f\in \meas(X, \C)$. Then $\norm{f}_\infty = 0$ \textup{if and only if} $f = 0$ almost everywhere.
\end{lemma}
\begin{proof}
If $f=0$ a.e., then $|f|\leq 0$ a.e. This implies $\norm{f}_\infty = 0$.

Now suppose $\norm{f}_\infty = 0$. Then, by \ref{approximationEssentialSupremum}, $|f| \leq \frac{1}{n}$ a.e.\ for all $n\in \N$. Define $E_n \defeq |f|^{\preimf}\big(\interval[o]{\frac{1}{n}, \infty}\big)$. We have that $\seq{E_n}$ is an increasing sequence of sets and $\mu(E_n) = 0$ for all $n\in \N$.

Now we have, by \ref{premeasureChainContinuous},
\begin{align*}
\mu\setbuilder{x\in X}{f \neq 0} &= \mu\setbuilder{x\in X}{|f| \neq 0} \\
&= \mu\Big(\bigcup_{n\in \N} E_n\Big) = \sum_{n\in\N}\mu(E_n) = 0,
\end{align*}
so $f = 0$ a.e.
\end{proof}
...
\end{lemma}
\begin{proof}
Take arbitrary $\epsilon  > 0$. Then, by \ref{approximationEssentialSupremum},
\[ |f\cdot g| \leq \big(\norm{f}_\infty + \epsilon\big)\big(\norm{g}_\infty + \epsilon\big) = \norm{f}_\infty\norm{g}_\infty + \big(\norm{f}_\infty + \norm{g}_\infty + \epsilon\big)\epsilon \]
almost everywhere, which implies that
\[ \norm{f\cdot g}_\infty \leq \norm{f}_\infty\norm{g}_\infty + \big(\norm{f}_\infty + \norm{g}_\infty + \epsilon\big)\epsilon. \]
By taking $\epsilon \to 0$, we get the result.
\end{proof}

\subsubsection{The $p$ seminorms are seminorms}
...
Take arbitrary $\epsilon > 0$. By \ref{approximationEssentialSupremum}, we have $|g| \leq \norm{g}_\infty + \epsilon$ a.e., so $|fg| \leq \big(\norm{g}_\infty + \epsilon\big)|f|$ a.e.\ and, by \ref{integralsAlmostEverywhereComparableFunctions} and \ref{propertiesIntegralPositiveFunctions}, we have
\[ \int_X |fg| \diff{\mu} \leq \int_X \big(\norm{g}_\infty + \epsilon\big)|f| \diff{\mu} = \big(\norm{g}_\infty + \epsilon\big)\int_X |f|\diff{\mu} = \big(\norm{g}_\infty + \epsilon\big)\norm{f}_1 \]
...
\begin{proof}
First consider the case $p = \infty$. Take arbitrary $\epsilon > 0$. Then $|f|\leq \norm{f}_\infty + \epsilon/2$ a.e.\ and $|g| \leq \norm{g}_\infty+ \epsilon / 2$ a.e., by \ref{approximationEssentialSupremum}, so $|f+g| \leq |f|+|g| \leq \norm{f}_\infty + \norm{g}_\infty + \epsilon$ a.e. This implies $\norm{f+g}_\infty \leq \norm{f}_\infty + \norm{g}_\infty + \epsilon$. Since this inequality holds for arbitrary $\epsilon > 0$, we have $\norm{f+g}_\infty \leq \norm{f}_\infty + \norm{g}_\infty$.
...
Let $\sSet{X,\mathcal{A},\mu}$ be a measure space and $1 \leq p \leq \infty$. Then $\norm{f}_p = 0$ \textup{if and only if} $f = 0$ a.e.
\end{lemma}
...
\begin{lemma} \label{compositionMultiplicationOperators}
...
The \udef{weak $L^p(X,\mu)$ space}, denoted $L^{p,\infty}(X,\mu)$, is the subset of functions $f$ in $\meas(X, \C)$ for which $\norm{f}_{p,\infty}$ is finite.
\end{definition}
