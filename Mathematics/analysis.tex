...
\begin{itemize}
\item The set of continuous functions in $(X\to \R)$ is denoted $\cont(X)$.
\item The set of continuous functions in $(X\to \R)$ which vanish at infinity is denoted $\cont(X)$ (assuming $X$ is normed vector space TODO: more general?).
\item The set of continuous functions in $(X\to \R)$ with compact support is denoted $\cont_c(X)$.
\end{itemize}
\end{definition}
...
If $V= \R^n$, then we define $\pd{f}{x^i}f \defeq \partial_{\vec{e}_i}f$, where $\mathcal{E} = \seq{\vec{e}_i}_{i=1}^n$ is the standard basis of $\R^n$. These directional derivatives are called the \udef{partial derivatives} w.r.t. the basis $\mathcal{E}$.
\end{definition}
...
\begin{proposition} \label{prop:derivativeBilinearForm}
...
We may also write $\diff{f}$ or $f'$ leaving the $a$ implicit.
\end{definition}
...
\begin{lemma} \label{lemma:chainLemma}
...
\end{proposition}
\begin{proof}
We calculate, using \ref{lemma:chainLemma},
...
\subsection{Series}
TODO:move up!!
\begin{theorem}[Tannery's theorem] \label{theorem:tannery}
...
If a function $f:A\subseteq\R\to\R$ is not continuous at $p$, then $p$ is a limit point of $A$ by \ref{lemma:continuityAtIsolatedPoint} and \ref{lemma:notLimitPointSingletonOpen}.
\begin{definition}
...
\begin{theorem}[Stone-Weierstrass] \label{theorem:StoneWeierstrass}
...
\begin{definition}
Let $\mathcal{S}$ be a semi-ring on a set $\Omega$. A \udef{pre-measure} on $\mathcal{S}$ is a map $\mu: \mathcal{R} \to [0,\infty]$ satisfying
\begin{itemize}
\item $\mu(\emptyset) = 0$;
\item $\mu$ is \udef{$\sigma$-additive}: for every sequence $(E_n)$ in $\mathcal{S}$ of pairwise disjoint sets such that $\bigcup_{n\in \N}E_n\in\mathcal{S}$, we have
...
\begin{definition}
Let $(X,\mathcal{T})$ be a topological space. The $\sigma$-algebra on $X$ generated by $\mathcal{T}$ is called the \udef{Borel-$\sigma$-algebra} of $(X,\mathcal{T})$.
\end{definition}
...
\begin{proposition} \label{prop:measurableFromGeneratingSet}
...
\begin{proposition} \label{prop:pointWiseConvergenceMeasurable}
...
\end{proposition}
\begin{proof}
By \ref{prop:measurableFromGeneratingSet} it is enough to show that for all closed sets $C\subset Y$, the inverse image $f^{-1}[C]$ is in $\mathcal{A}_1$.
...
\item there exists a sequence $(E_n)$ in $\mathcal{A}$ with $\Omega = \bigcup_{n\in\N}$ and $\mu(E_n)<\infty$ for all $n\in\N$, then we call $\mu$ \udef{$\sigma$-finite}.
\end{itemize}
A proposition $P(x)$ referencing some $x\in\Omega$ is said to be true \udef{almost everywhere} (or a.e.) if $\setbuilder{x\in\Omega}{\text{$P(x)$ is false}}$ is a measurable set with measure zero.
\end{definition}
...
\begin{align*}
\mu(E_1\cup E_2) &= \mu(E_1\cup E_2 \cup \emptyset \cup \emptyset \cup \ldots) \\
...
\begin{lemma}
If there exists an event $E\in \mathcal{A}$ with $\mu(E)<\infty$, then $\mu(\emptyset) = 0$ is implied by $\sigma$-additivity.
\end{lemma}
\begin{proof}
$\mu(E) = \mu(E \cup \emptyset \cup \emptyset \cup \ldots) = \mu(E) + \infty\cdot\mu(\emptyset)$ which is only not $\infty$ if $\mu(\emptyset) = 0$.
\end{proof}

\begin{lemma} \label{lemma:submeasurespace}
...
\begin{lemma} \label{lemma:measuresPositiveLinear}
...
\begin{proposition} \label{prop:measures}
...
By the $\pi-\lambda$ theorem \ref{corollary:piLambdaTheorem} it is enough to show that $\mathcal{E}$ is a Dynkin system.
\begin{itemize}
...
\begin{definition}
Let $\seq{\Omega, \mathcal{A}, \mu}$ be a measure space. A set $A\subseteq \Omega$ is a \udef{null set} if there exists a measurable set $B\in\mathcal{A}$ such that $A\subseteq B$ and $\mu(B) = 0$.
...
See also Reed/Simon and \ref{prop:BLT}.
...
\begin{lemma}
Let $s:\Omega\to Y\subset \F$ be a simple function into a subset of a field $\F$. Then $s$ can be written as
...
The simple function is positive if and only if $\forall i\in [0,k]: 0\leq\lambda_i$.
\end{lemma}
...
\[ s:\Omega \to [0,+\infty[: x\mapsto \sum_{\lambda \in s[\omega]}\lambda\cdot \chi_{s^{-1}[\lambda]}(x) = \sum_{i=1}^k\lambda_j\cdot\chi_{A_i}(x)  \]
be a measurable simple function. We define the \udef{integral} of $s$ over $\Omega$ w.r.t. $\mu$ as
\[ \int_\Omega s\diff{\mu} \defeq \sum_{\lambda \in s[\omega]}\lambda\cdot \mu(s^{-1}[\lambda]) = \sum_{i=1}^k\lambda_j\mu(A_i). \]
Using the convention that $0\times \infty = 0$. (TODO:clarify)
\end{definition}
Clearly we can also define the integral of $s$ over any $E\in\mathcal{A}$ as the integral of $s|_E$ of the measure space $(E,\mathcal{A}',\mu|_{\mathcal{A}'})$, as defined in \ref{lemma:submeasurespace}.

\begin{proposition} \label{prop:integrationPositiveLinear} \label{prop:integrationOrderPreserving}
Let $(\Omega, \mathcal{A}, \mu)$ be a measure space. The integral of measurable simple functions in $(\Omega\to [0,+\infty[)$ has the properties:
\begin{enumerate}
\item it is positive linear: $\forall c\geq 0$ and $\forall s,t:\Omega\to [0,+\infty[$:
\[ \int_\Omega (c\cdot s + t)\diff{\mu} = c\cdot \int_\Omega s\diff{\mu} + \int_\Omega t\diff{\mu}; \]
\item if $s\leq t$ are positive simple functions on $\Omega$, then
\[ \int_\Omega s\diff{\mu} \leq \int_\Omega t\diff{\mu}; \]
\item if $E_1\subseteq E_2$ are events in $\mathcal{A}$, then
\[ \int_{E_1}s\diff{\mu} \leq \int_{E_2}s\diff{\mu}. \]
\end{enumerate}
\end{proposition}
...
\end{proposition}
\begin{proof}
From the positive linearity of both measures and integration (\ref{lemma:measuresPositiveLinear}, \ref{prop:integrationPositiveLinear}) it is enough to consider $s = \chi_A$ for some $A\in\mathcal{A}$. In this case
...
\[ \nu\left(\bigcup_{n\in\N}E_n\right) = \mu\left(A\cap \bigcup_{n\in\N}E_n\right) = \mu\left(\bigcup_{n\in\N}(A\cap E_n)\right) = \sum_{n\in\N}\mu(A\cap E_n) = \sum_{n\in\N}\nu(E_n). \]
\end{proof}
\begin{corollary} \label{corollary:integralContinuousInDomain}
...
\end{corollary}
\begin{proof}
Define the measure $\nu: E\mapsto \int_E s\diff{\mu}$. Then by \ref{prop:measures}
...
\begin{definition}
Let $(\Omega, \mathcal{A}, \mu)$ be a measure space and let $f:\Omega\to[0,+\infty]$ be a measurable function. Define
\[ \operatorname{SF}(f) \defeq \setbuilder{s\in(\Omega\to[0,+\infty[)}{\text{$s$ is a positive measurable simple function with $s\leq f$}}. \]
We define the \udef{integral} of $f$ on $\Omega$ w.r.t. $\mu$ as
\[ \int_\Omega f \diff{\mu} \defeq \sup\setbuilder{\int_\Omega s \diff{\mu}}{s\in\operatorname{SF}(f)}. \]
We call $f$ \udef{integrable} when $\int_\Omega f \diff{\mu} < \infty$.
\end{definition}
For simple functions this definition corresponds to the previous one by \ref{prop:integrationOrderPreserving}.
...
\begin{lemma} \label{lemma:integralOverSubset}
...
\end{proposition}
The converse is also true and is given by \ref{prop:pointWiseConvergenceMeasurable}.
\begin{proof}
...
\begin{proposition} \label{prop:propertiesIntegralPositiveFunctions}
...
For the other inequality, it is enough to prove that $c\int_\Omega s\diff{\mu} \leq \lim_{n\to\infty}\int_\Omega f_n\diff{\mu}$ for all $s\in\operatorname{SF}(f)$ and $0<c<1$. Fix such a $c$ and $s = \sum_{i=1}^k\lambda_i\chi_{A_i}$. Consider the sets
...
by \ref{corollary:integralContinuousInDomain}. Also 
\[ \int_{E_n} cs\diff{\mu} \leq \int_{E_n} f_n\diff{\mu} \leq \int_{\Omega} f_n\diff{\mu} \]
by the previous points and so the result follows from the fact that limits preserve inequalities, \ref{prop:limitPreservesInequality}.
...
\begin{proposition}[Fatou's lemma] \label{prop:FatouLemma}
...
This shows that $f$ is measurable by \ref{prop:pointWiseConvergenceMeasurable}. Then we can use Beppo Levi's lemma \ref{prop:propertiesIntegralPositiveFunctions} to obtain
...
using monotonicity of the integral \ref{prop:propertiesIntegralPositiveFunctions} and liminf \ref{prop:propertiesIntegralPositiveFunctions} for the last inequality.
\end{proof}
...
Now for all $s:\Omega\to [0,+\infty[$ in $\operatorname{SF}(f)$ we have
\[ 0\leq \int_E s \diff{\mu}\leq \max_x(s(x))\mu(E) = 0  \]
and so the supremum $\int_E f\diff{\mu}$ is zero as well.

Now assume $\int_\Omega f\diff{\mu} = 0$. Consider the sets $E_n = f^{-1}[\,]\frac{1}{n},+\infty]\,]$. Then $\frac{1}{n}\chi_{E_n}\in\operatorname{SF}(f)$. Hence $\frac{1}{n}\mu(E_n)\leq \int_\Omega f\diff{\mu} = 0$, meaning $\mu(E_n) = 0$ for all $n$. So $\mu(E) = \sup_{n\in\N}\mu(E_n) = 0$.

(2) Towards contraposition, assume the set $E = \setbuilder{x\in\R}{f(x) = +\infty}$ has non-zero measure. Then for all real $a>0$, $a/\mu(E)\chi_E\in\operatorname{SF}(f)$ and
...
\end{proposition}
\begin{proof}
By the corollary $g(x)<+\infty$ a.e. and so also $g-h_n<+\infty$ a.e. Applying Fatou's lemma \ref{prop:FatouLemma} gives
\[ \int\liminf g-h_n \leq \liminf\int g-h_n\]
\end{proof}


\subsection{Positive operator-valued functions}
\subsubsection{Bochner integration}

\section{Lebesgues integration for general functions}
\begin{definition}
Let $(\Omega, \mathcal{A}, \mu)$ be a measure space and $A$ TODO. We say that a measurable function $f:\Omega\to A$ is integrable if $|f|$ is integrable.
\end{definition}
...
\begin{lemma}
Let $f: \R^N\to\C$ be an integrable function with $\int_{\R^N}f(x)\diff{x} = 1$. Then $\seq{n^Nf(nx)}$ is a delta sequence.
\end{lemma}

\subsubsection{Integral distributions}
\begin{proposition}
    Let $X$ be an open subset of a normed vector space and $f: X\to\C$ an absolutely integrable function. Then 
    \[ T_f: \testFuncs(X) \to \C: \phi \mapsto \int_X f(x)\phi(x)\diff{x} \]
    is a distribution.
\end{proposition}

\begin{lemma} \label{lemma:uniquenessIntegratedFunction}
Let $f,g: X\to \C$ be absolutely integrable functions. If $T_f = T_g$, then $f(x) = g(x)$ a.e.
\end{lemma}
...
\end{definition}
The weak $\alpha$ derivative is unique if it exists, due to \ref{lemma:uniquenessIntegratedFunction}.
...
\begin{proposition}
A Sobolev space $W^{k,p}(X)$ is a Banach space with norm
\[ \norm{f}_{W^{k,p}(X)} = \begin{cases}
\left(\sum_{|\alpha|\leq k}\norm{D^\alpha f}^p_{L^{p}(X)}\right)^{1/p} & 1\leq p < \infty \\
\max_{|\alpha|\leq k} \norm{D^\alpha f}_{L^{\infty}(X)} & p = \infty.
\end{cases} \]
In particular $W^{k,2}(X)$ is a Hilbert space with inner product
\[ \inner{f,g} \sum_{|\alpha| \leq k} \int_X D^\alpha f(x) \overline{D^\alpha g(x)}\diff{x}. \]
The Hilbert space $W^{k,2}(X)$ is more commonly denoted $H^k(X)$.
\end{proposition}
TODO: generalise ???
