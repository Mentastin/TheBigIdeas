...
\[ \mu\{f\geq \alpha\} = D_f(\alpha) + \mu\{f = \alpha\} \leq \frac{1}{\alpha}\int_\Omega f\diff{\mu}. \]
\end{proposition}
...
\subsection{The $L^p(\Omega,\mu)$ spaces}
...
\subsubsection{$L^0(\Omega, \mu)$ and convergence in measure}
\begin{definition}
...
\begin{enumerate}
\item $d_1: (f,g) \mapsto \inf\setbuilder[\big]{\epsilon > 0}{D_{|f-g|}(\epsilon) \leq \epsilon}$;
...
\begin{definition}
The metric $d_1$ is known as the \udef{Ky Fan metric}.
\end{definition}
