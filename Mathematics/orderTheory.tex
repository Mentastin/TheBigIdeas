...
\end{lemma}
\begin{proof}
(1) We have that $V \subseteq \upset f^{\imf\imf}\big(f^{\preimf\imf}V\big)$ by \ref{upsetPreimageImageGaloisConnection} and $\upset f^{\imf\imf}\big(f^{\preimf\imf}V\big)$ is proper, so the equality follows because $V$ is ultra.
...
\[ (A)_0 \quad\defeq\quad \upset\setbuilder{B\subseteq X}{\text{$A\setminus B$ if finite}}. \]
\end{definition}
...
\begin{align*}
F \leq_w G \qquad &\iff\qquad \liminf F \leq \limsup G; \\
F \leq_s G \qquad &\iff\qquad \limsup F \leq \liminf G.
\end{align*}
\end{lemma}
\begin{proof}
