...
\begin{proposition}
Let $X$ be a set and $F$ a free filter in $\filters_0(\powerset(X))$. If $A\in F$, then $F$ contains the cofinite filter of $A$.
\end{proposition}
...
In order for $L = F_* \vee F_0$ to hold, we must have some $x\in F_0$ such that $x\wedge \ker(F) = \bot$. This implies $x\leq \overline{\ker(F)}$ by \ref{BooleanComplementLargestDisjoint}.
\end{proof}
