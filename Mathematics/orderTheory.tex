...
\begin{definition}
Let $P$ be a poset and $x\in P$. Then $x$ is called \udef{compact} or \udef{finite} if $x\ll x$. We denote the set of compact elements in $P$ by $\compact(P)$.
\end{definition}
...
First we show that $\ddownset\{x\}$ is an ideal. Take $a, b\in \ddownset\{x\}$, so $a \ll x$ and $b\ll x$. By definition of the way-below relation, we have $a,b\in \downset D$. Thus there exist $a',b'\in D$ such that $a\leq a'$ and $b\leq b'$. Now $\{a',b'\}$ has an upper bound in $D$ by directedness. This upper bound is also an upper bound of $\{a,b\}$ and is an element of $\ddownset\{x\}$ because $D\subseteq \ddownset\{x\}$.

Now for the approximation property, we have $x\in \big(\ddownset\{x\}\big)^u$ because $y\in \ddownset\{x\}$ (so $y \ll x$), which implies $y \leq x$. So, since $\big(\ddownset\{x\}\big)^u$ is upwards closed, we calculate
\[ \upset \{x\} \subseteq \big(\ddownset\{x\}\big)^u \subseteq D^u = \upset \{\dirvee D\} = \upset \{x\}. \] 
This implies $\dirvee \big(\ddownset\{x\}\big) = x$.
\end{proof}
...
\begin{lemma}
? Algebraic poset iff for all $x\in P$, there exists a (directed?) set $D\subseteq P$ of compact elements such that $x = \dirvee D$???
\end{lemma}
\begin{proof}
TODO
\end{proof}
...
\begin{lemma}
Let $P$ be a poset. Then $P$ is algebraic \textup{if and only if} $\bigvee\big(\downset \{x\}\cap \compact(P)\big) = x$.
\end{lemma}
