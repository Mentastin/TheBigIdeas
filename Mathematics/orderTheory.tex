...
The same is true for $z f^{-1}[f(x)]$, so we conclude by transitivity.
\end{proof}
TODO: make corollary:
...
\begin{lemma} \label{lemma:definitionUpsetDownset}
...
\begin{lemma} \label{lemma:upDownsetUnionIntersection}
Let $P$ be an ordered set and $\mathcal{A}\subseteq \powerset(P)$. Then
\begin{enumerate}
\item $\upset \bigcup \mathcal{A} = \bigcup \upset \mathcal{A}$ and $\downset \bigcup \mathcal{A} = \bigcup \downset \mathcal{A}$;
\item $\upset \bigcap \mathcal{A} = \bigcap \upset \mathcal{A}$ and $\downset \bigcap \mathcal{A} = \bigcap \downset \mathcal{A}$.
\end{enumerate}
\end{lemma}
\begin{proof}
We calculate TODO
\[ \upset \bigcup \mathcal{A} = \bigcup_{x\in \bigcup\mathcal{A}} \upset x = \bigcup \setbuilder{\upset x}{x\in \bigcup \mathcal{A}} \]
\end{proof}

\begin{lemma} \label{lemma:upDownsetInclusion}
...
\begin{lemma} \label{lemma:unionIntersectionDownUpSets}
...
\item for all $x\in P$: $\upset f^{-1}[\upset x] \subseteq f^{-1}[\upset x]$;
\item for all $x\in P$: $\downset f^{-1}[\downset x] \subseteq f^{-1}[\downset x]$;
\end{enumerate}
\item as are
\begin{enumerate}
...
\[ y\in \upset x \iff x\Yleft y \Rightarrow f(x) \Yleft f(y) \iff f(y) \in \upset f(x). \]

For the reverse, images: take $y\in \upset f^{-1}[\upset x]$
\end{proof}
...
\begin{lemma} \label{lemma:minMaxSingletons}
...
($\wedge$) The union $\bigcap_{i\in I}F_i$ is an up set by \ref{lemma:unionIntersectionDownUpSets}.

Take $a,b \in \bigcap_{i\in I}F_i$. Then $a\wedge b \in \bigcap_{i\in I}F_i$ for all $i\in I$, so $a \wedge b\in \bigcap_{i\in I}F_i$.
...
\begin{lemma}
Let $L$ be a Boolean lattice and $x,y\in L$. Then $x\wedge \overline{y} = \bot$ \textup{if and only if} $x\leq y$.
\end{lemma}
...
\end{proof}
\begin{corollary}
Let $\mathcal{F}$ be a family of sets. If
\begin{itemize}
\item $\mathcal{F}$ is closed under arbitrary intersections; and
\item $\bigcup \mathcal{F}\in \mathcal{F}$;
\end{itemize}
...
For the converse, the isomorphism is given by $x\mapsto \downset x$, as in \ref{prop:orderedSetPowerset}. We just need to show that the meet translates to the intersection, i.e. $\downset \bigwedge Q = \bigcap_{x\in Q}\downset x$. We calculate, using \ref{lemma:minMaxUpsetDownset}, \ref{lemma:boundsFromUpDownSets}, \ref{lemma:upDownsetUnionIntersection} and \ref{lemma:upwardDownwardClosure} (noting that a lattice order is in particular a preorder):
\[ \downset \bigwedge Q = \downset \max(Q^l) = \downset Q^l = \downset \bigcap_{x\in Q} \downset x = \bigcap_{x\in Q} \downset\downset x = \bigcap_{x\in Q} \downset x. \]
\end{proof}
