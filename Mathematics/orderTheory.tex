...
\end{proposition}
\begin{proof}
By definition $D\lhd E$ implies $D\subseteq E$. For the opposite inclusion we need to show that $E\subseteq D^{\mesh}$. Assume, towards a contradiction, that this is not the case. Then there exists $a\in E$ and $b\in D$ such that $a\wedge b = 0$. Now because $D\subseteq E$ and $E$ is $\wedge$-closed, this means that $0\in E$. Because $E$ is upward directed, this means $E = S$, which contradicts the assumption of proper filter.
\end{proof}
...
Let $P$ be an ordered set and $I$ an index set.
\begin{enumerate}
\item If $P$ is a lattice that is meet infinitely distributive, then the lattice $\filters(P)$ of filters on $P$ is also meet infinitely distributive:
...
\item If $P$ is a $\vee$-semilattice, the lattice $\ideals(P)$ of ideals on $P$ is meet infinitely distributive:
...
We calculate
\begin{align*}
F\vee \bigwedge \mathcal{A} &= \Cl_\wedge \left(F\cup \bigcap\mathcal{A}\right) \\
&= \setbuilder{x\wedge y}{x,y \in F\cup \bigcap\mathcal{A}} \\
&= \setbuilder{x\wedge y}{x,y \in \bigcap \setbuilder{F\cup A}{A\in \mathcal{A}}} \\
&\subseteq \bigcap\setbuilder{\setbuilder{x\wedge y}{x\in F\cup B, y \in \bigcap \setbuilder{F\cup A}{A\in \mathcal{A}}}}{B\in\mathcal{A}} \\
&\subseteq \bigcap\setbuilder{\setbuilder{x\wedge y}{x,y \in F\cup A}}{A\in \mathcal{A}} \\
&= \bigcap\setbuilder{\Cl_\wedge(F\cup A)}{A\in \mathcal{A}} = \bigwedge\setbuilder{F\vee A}{A\in \mathcal{A}}.
\end{align*}
This inclusion is the easy one that holds for all lattices (see \ref{infiniteDistributiveInequalities}). We need to reverse both inclusions. We start with the first one and (in the interest of brevity) we call $\mathcal{B} = \bigcap \setbuilder{F\cup A}{A\in \mathcal{A}}$. Now take $z\in \bigcap\setbuilder{\setbuilder{x\wedge y}{x\in F\cup B, y \in \mathcal{B}}}{B\in\mathcal{A}}$. Then for all $B\in\mathcal{A}$ we can find 
\end{proof}
