...
Let $X,Y$ be sets, $f: X\to Y$ a function, $F\in\powerfilters(X)$ and $V\in \powerultrafilters(Y)$. Then
\begin{enumerate}
...
Now we can find an ultrafilter $U$ larger than $F\vee f^{\preimf\imf}(V)$ by \ref{ultrafilterLemma}. Clearly $F\subseteq U$. Also $V \upset f^{\imf\imf}\big(f^{\preimf\imf}(V)\big) \subseteq \upset f^{\imf\imf}(U)$. As the last filter is still proper, we have $V = \upset f^{\imf\imf}(U)$.
\end{proof}
