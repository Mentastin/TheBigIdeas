...
\begin{enumerate}
\item If $P$ is converse well-founded, then $P$ satisfies the ascending chain condition.
\item If $P$ is well-founded, then $P$ satisfies the descending chain condition.
\end{enumerate}
\end{proposition}
\begin{proof}
We prove (2). Point (1) follows by duality.

Assume $P$ well-founded. TODO
\end{proof}
...
Note we do not impose anti-symmetry (TODO check everything still works).
...
$(2) \Rightarrow (1)$ Now assume $P$ is well-founded and linearly ordered and take some non-empty subset $S$ of $P$. We can find some minimal element $x\in S$ (which we will show to be a minimum). For all $y\in S$ we have either $x\mathrel{R}y$ or $y\mathrel{R}x$. In the latter case, $y\in Rx\cap X$, so $y\in xR$ by definition of minimality. Thus $x\mathrel{R}y$ in both cases, so $x\in {^RX}\cap X = \min(X)$.
\end{proof}
