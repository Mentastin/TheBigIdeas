...
\[ (\branch^{-1}\circ\branch)(u)(n) = u[0:n](n) = u|_{0:n}(n) = u(n). \]
Because $n$ was chosen arbitrarily, we have $(\branch^{-1}\circ\branch)(u) = u$.
\item Take $S\in [A^*]$ and $v\in S$. Then $v = \left(\bigcup S\right)[0:\len(v)]$. Indeed their domains match and for all $n\in (0:\len(v))$, we have $\left(\bigcup S\right)(n) = v(n)$. As
\[ (\branch\circ\branch^{-1})(S) = \setbuilder{\left(\bigcup S\right)[0:n]}{n\in \N}, \]
we have that $v\in (\branch\circ\branch^{-1})(S)$. Conversely, take $w\in (\branch\circ\branch^{-1})(S)$. Then $w = \left(\bigcup S\right)[0:\len(w)]$. 
\end{itemize}
\end{proof}
\begin{corollary}
...
We conclude that $S \leq_c \N$ by \ref{injectivityCardinality}, i.e.\ $S$ is countable. By \ref{countableEquivalents}, we conclude that $S =_c \N$, as $S$ was assumed infinite.
