...
\begin{proposition} \label{meetJoinCompleteSubsemilattice}
Let $P$ be a poset, $f: P\to \im(f)\subseteq P$ a function and $A\subseteq \im(f)$. Then
\begin{enumerate}
\item if $f$ is a Moore closure, then
\[ \bigvee_{\im(f)}A = f\Big(\bigvee_P A\Big) \qquad\text{and}\qquad \bigwedge_{\im(f)} A = \bigwedge_P A; \]
\item if $f$ is a dual Moore closure, then
\[ \bigvee_{\im(f)}A = \bigvee_P A \qquad\text{and}\qquad \bigwedge_{\im(f)} A = f\Big(\bigwedge_P A\Big). \]
\end{enumerate}
\end{proposition}
\begin{proof}
(1) We have that $f$ preserves joins by \ref{MooreClosureGaloisConnection}. Since $A\subseteq \im(f)$, we have $A = f^\imf(A)$, so
\[ \bigvee_{\im(f)}A = \bigvee_{\im(f)}f^{\imf}(A) = f\Big(\bigvee_P A\Big). \]
We have that $\incl_{\im(f)}$ preserves meets by \ref{MooreClosureGaloisConnection}, so
...
Now let $B\subseteq P$ be a subset such that the meet $\bigvee B$ exists. Then we calculate, using \ref{supInfConnection}, \ref{boundsOnSetWithLeastBoundIsPrincipalIdeal}, \ref{imagesFromPrincipalImages}, \ref{meetJoinCompleteSubsemilattice} and \ref{DedekindMacNeilleCompletionComplete},
\begin{align*}
...
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ Take $x\in P$. Then we can set $D = \ddownset \{x\}$.

$(2) \Rightarrow (3)$ Immediate.

$(3) \Rightarrow (1)$ Suppose $B$ is a base of $P$. Take arbitrary $x\in P$. Then there exists a directed subset $D\subseteq B$ such that $D\subseteq \ddownset\{x\}$ and $x = \dirvee D$.
