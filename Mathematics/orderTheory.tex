...
we call $B$ a \udef{filter subbasis} of $F$. If $B$ is downward directed, it is called a \udef{filter base} of $F$;
...
\subsection{Meshing and trace}
\begin{lemma} \label{traceFilter}
Let $F$ be a filter on a bounded $\wedge$-semilattice $S$ and $a\subseteq S$ an element. If $\{a\}\mesh F$, then
\[ F|_a \defeq \setbuilder{x\wedge a}{x\in F} \]
is a filter. It is in particular a filter on $\downset a$.
\end{lemma}

\begin{definition}
The filter $F|_a$ of \ref{traceFilter} is called the \udef{trace filter} of $F$ on $a$.
\end{definition}
...
\end{proposition}
In particular, for $F, G\in\filters(L)$, we have that $F\vee G$ is a proper filter if and only if $F\mesh G$.

\begin{proposition} \label{infiniteDistributivityFiltersIdeals}
Let $P$ be an ordered set and $I$ an index set.
\begin{enumerate}
\item If $P$ is a join infinitely distributive lattice, then the lattice $\filters(P)$ of filters on $P$ is meet infinitely distributive:
\[ \forall \mathcal{A}\subseteq \filters(P): \forall F\in \filters(P): \qquad F\vee \bigwedge \mathcal{A} = \bigwedge \setbuilder{F\vee A}{A\in \mathcal{A}}; \]
\item If $P$ is a meet infinitely distributive lattice, the lattice $\ideals(P)$ of ideals on $P$ is meet infinitely distributive:
\[ \forall \mathcal{A}\subseteq \ideals(P): \forall I\in \ideals(P): \qquad I\vee \bigwedge \mathcal{A} = \bigwedge \setbuilder{I\vee A}{A\in \mathcal{A}}. \]
\end{enumerate}
\end{proposition}
\begin{proof}
(1) We calculate, using the expression for $\vee$ applicable to distributive lattices in \ref{latticeFiltersIdeals}:
\begin{align*}
F\vee \bigwedge \mathcal{A} &= \closure_\wedge \left(F\cup \bigcap\mathcal{A}\right) \\
&= \setbuilder{x\wedge y}{x,y \in F\cup \bigcap\mathcal{A}} \\
&= \setbuilder{x\wedge y}{x,y \in \bigcap \setbuilder{F\cup A}{A\in \mathcal{A}}} \\
&\subseteq \bigcap\setbuilder{\setbuilder{x\wedge y}{x,y \in F\cup A}}{A\in \mathcal{A}} \\
&= \bigcap\setbuilder{\closure_\wedge(F\cup A)}{A\in \mathcal{A}} = \bigwedge\setbuilder{F\vee A}{A\in \mathcal{A}}.
\end{align*}
This inclusion is the easy one that holds for all lattices (see \ref{infiniteDistributiveInequalities}). We need to reverse the inclusion.

To that end, take $z\in \bigcap\setbuilder{\setbuilder{x\wedge y}{x,y \in F\cup A}}{A\in \mathcal{A}}$. Then for all $A\in\mathcal{A}$ we can find $x_A, y_A\in F\cap A$ such that $x_A\wedge y_A = z$. Now we show that there exist $x,y\in \bigcap \setbuilder{F\cup A}{A\in \mathcal{A}}$ such that $x\wedge y = z$.

We claim $x = \bigvee_{A\in\mathcal{A}}x_A$ and $y = \bigvee_{B\in\mathcal{A}}y_B$ work. Indeed $x,y$ are elements of $\bigcap \setbuilder{F\cup A}{A\in \mathcal{A}}$ by upwards closure of each $F\cup A$. Then for all $A\in \mathcal{A}$ we have
\[ x_A\wedge y = x_A \wedge \left(\bigvee_{B\in\mathcal{A}}y_B\right) \geq x_A\wedge y_A = z \qquad\text{and}\qquad \text{$x_A \leq z$ and $y \leq z$, so $x_A\wedge y \leq z$.} \]
Thus combining the inequalities gives $x_A \wedge y = z$. We conclude with the calculation
\[ x\wedge y = \left(\bigvee_{A\in\mathcal{A}}x_A\right)\wedge y = \bigvee_{A\in\mathcal{A}}(x_A\wedge y) = \bigvee_{A\in\mathcal{A}}z = z. \]

(2) The proof is identical, except all meets and joins on $P$ (but not $\powerset(P)$!) are flipped.
\end{proof}
TODO compare with it being a frame see also proof in Stone Spaces - Johnstone
...
\begin{definition}
TODO principal filter $\pfilter{x}$.
\end{definition}
...
\begin{proposition}
Let $X$ be a set and $F\in\powerfilters(X)$ a proper filter. Then $F \subseteq F^{\overrightarrow{\mesh}}$.
\end{proposition}
\begin{proof}
...
\begin{definition}
Let $X$ be a set and $F\in\powerfilters(X)$ a filter. We call $F$ an \udef{ultrafilter} if $F = F^{\overrightarrow{\mesh}}$.
\end{definition}
