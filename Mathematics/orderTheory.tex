...
by \ref{supLesserElementsOfLimitPoint}. Now there exists a bijection $f: \gamma \to D$ by assumption and set $D_\alpha \defeq f^\imf(\alpha)$. The family $\{D_\alpha\}_{\alpha\leq\gamma}$ is a familty of subsets of $D$ such that $|D_\alpha| = \alpha < \gamma$ and $\bigcup_{\alpha < \gamma}D_\alpha = D$.


Now by choice we can find an ``upper bound function'' $u$ that maps finite subsets of $D$ to some upper bound of the subset in $D$.
We define $D_\alpha'$ by closing $D_\alpha$ under $u$.
\end{proof}
...
\begin{proposition}
Let $W$ be an infinite well-ordered set. Then $W =_o W\times W$.
\end{proposition}
\begin{proof}
Consider the \udef{Gödel pairing} $\preceq$ on $W\times W$ defined by
\begin{align*}
(a,b)\preceq (c,d) \defequiv &\big(\max\{a,b\} \leq \max\{c,d\}\big)\;\lor \\ &\Big(\big(\max\{a,b\} \sim \max\{c,d\}\big)\land(a\leq c)\Big) \;\lor \\ &\Big(\big(\max\{a,b\} \sim \max\{c,d\}\big)\land(A \sim C)\land(B\leq D)\Big).
\end{align*}
We claim that the Gödel pairing makes $W\times W$ well-ordered.
\end{proof}
