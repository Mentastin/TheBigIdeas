...
Let $\sSet{A,R}$ be an irreflexive and antisymmetric relational structure. Then $A$ contains a maximal element \textup{if and only if} $R$ is not left total.
\end{lemma}
\begin{proof}
Irreflexivity and antisymmetry imply $xR\cap Rx = \emptyset$ for all $x\in A$. 
\begin{align*}
\exists x\in A: \; \text{$x$ is maximal} \quad&\iff\quad \exists x \in A: \; xR\subseteq Rx \\
&\iff\quad \exists x\in A: \; xR = xR\cap Rx = \emptyset \\
...
Let $\sSet{A, R}$ be a relational structure. Then
\begin{itemize}
\item satisfies the \udef{ascending chain condition} (ACC) if $[T_R] = \emptyset$.
\item satisfies the \udef{descending chain condition} (DCC) if $[T_{R^\transp}] = \emptyset$.
\end{itemize}
\end{definition}
...
\item if $R$ is irreflexive and antisymmetric, then the converse also holds.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) Assume that $T_R$ is converse well-founded. Consider a subset $X\subseteq A$ and the restricted associated tree $T' = T_R|_{X^*}$. Now $T'$ has a maximal element $u$ by converse-well-foundedness of $T_R$. Then $u_{-1}R\cap X \neq \emptyset$ and thus $u_{-1}$ must be maximal in $X$ by \ref{maximalMinimalConditions}.

(2) Assume $R$ is irreflexive, antisymmetric and converse well-founded. Then for any $X\subseteq A$, $R|_X^{X}$ is not left-total by \ref{maximalElementNotLeftTotal}. Take some subset $S\subseteq T_R$ and consider the set
...
Let $\sSet{A, R}$ be a antisymmetric and irreflexive relational structure. Then
\begin{enumerate}
...
Assume DC and that $P$ has no maximal element. Define ${<} \defeq {\leq} \setminus\id_P$, which is antisymmetric and irreflexive. Then $<$ is left total by \ref{maximalElementNotLeftTotal}, and we can find a choice sequence by DC. The choice sequence is injective by \ref{choiceSequenceMonotone} and \ref{antisymmetricOrderPreservationInjective}. Thus the image of the choice sequence is an infinite chain.
