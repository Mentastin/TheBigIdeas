...
\item for any left-total binary relation $R$ on a set $A$ and $a\in A$, there exists a sequence $\seq{a_n}_{n\in \N}$ in $A$ such that $a_0 = a$ and
\[ \forall n\in \N: \quad a_n\mathrel{R}a_{n+1}. \]
\end{enumerate}
\end{lemma}
\begin{proof}
$\boxed{(1)\Rightarrow (2)}$ Recursively define $f: \N\to \powerset(A)$ by $f(0) = aR$ and
\[ f(n+1) = \bigcup_{x\in f(n)}xR. \]
Then set $B = \bigcup_{n\in\N}f(n)$. We have that $R|^B_B$ is left-total on $B$. By the axiom of dependent choice we can find some sequence $\seq{b_n}_{n\in \N}$ in $B$.
