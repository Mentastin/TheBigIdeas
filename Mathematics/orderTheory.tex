...
\item subsets such that each pair in the subset has an upper bound in $P$ (the \udef{pairwise compatible system});
\item subsets such that each finite subsubset has an upper bound in $P$ (the \udef{compatible system}).
\end{itemize}
\end{example}

\subsubsection{Hereditary $\mathrm{Z}$-systems}
\begin{definition}
Let $Z$ be a $\mathrm{Z}$-system. Then $Z$ is called \udef{hereditary} if $Z(P) = f^{\preimf\imf}\big(Z(Q)\big)$ for all order-preserving and -reflecting functions $f: P\to Q$.
\end{definition}

\begin{lemma}
Let $Z$ be a $\mathrm{Z}$-system. Then $Z$ is hereditary \textup{if and only if} for all order-preserving and -reflecting functions $f: P\to Q$ we have $A \in Z(P) \iff f^\imf(A) \in Z(Q)$.
\end{lemma}

\begin{lemma} \label{subsetHereditaryZsets}
Let $Z$ be a hereditary $\mathrm{Z}$-system, $P$ a poset and $Q\subseteq P$. Then $Z(Q) = Z(P)\cap \powerset(Q)$.
\end{lemma}
\begin{proof}
We have $Z(Q) \subseteq Z(P) \cap \powerset(Q)$ by \ref{subsetZsets}.

Take some $A\in Z(P) \cap \powerset(Q)$, so $\incl_Q(A) \in Z(P)$. The inclusion function $\incl_A: A\to P$ is order-preserving and order-reflecting, which implies $A\in Z(Q)$.
\end{proof}
...
\begin{enumerate}
\item the join $\bigvee_{D\in \mathcal{D}}\bigvee_L f^\imf(D)$ exists;
...
\begin{proof}
Let $T$ be the set $\setbuilder{A\in M}{\text{$A$ is $(Z,M)$-tapered}}$. The function $D\mapsto \bigvee_L f^\imf(D)$ is a well-defined isotone function on $T$. Since $Z$ is hereditary, we have $\mathcal{D} \in Z(T)$ by \ref{subsetHereditaryZsets}, which implies that $\setbuilder{\bigvee_L f^\imf(D)}{D\in \mathcal{D}} \in Z(L)$. Thus $\bigvee_{D\in \mathcal{D}}\bigvee_L f^\imf(D)$ exists, by $Z$-completeness of $L$ and $\bigvee_L \Big(\bigcup_{D\in \mathcal{D}}f^\imf(D)\Big)$ exists by \ref{joinMeetUnion} and is equal to $\bigvee_{D\in \mathcal{D}}\bigvee_L f^\imf(D)$.
...
Let $\mathcal{D} \subseteq S$ such that $\mathcal{D} \in Z_2(M)$. Since $2$ is hereditary, we have $\mathcal{D} \in Z_2\big(\Closure_{Z_2}(\im(d))\big)$ by \ref{subsetHereditaryZsets}. Then, since each $D\in\mathcal{D}$ is an element of $S$,
...
Let $L$ be a $Z$-complete poset and $f: P \to L$ a $Z$-continuous function. Then there exists a unique $Z$-continuous function $\tilde{f}: \Closure_{Z}\big(\im(d)\big) \to L$ such that
\[ \begin{tikzcd}
...
\subsubsection{OLD - Extension of functions to $Z$-closure}

\begin{lemma} \label{intersectionSystemAmbientCompleteLattice}
Let $L$ be a complete lattice and $P \subseteq L$ a subposet. Let $Z$ be a $\mathrm{Z}$-system. Define
\[ M \defeq \setbuilder[\big]{\downset\{x\} \cap P}{x\in L} \subseteq \powerset(P). \]
Then
\begin{enumerate}
\item $M$ is an intersection system with closure $\Closure_M: \powerset(P) \to M: A \mapsto \downset \big\{\bigvee_L A\big\} \cap P$;
\item If $\incl_P: P\to L$ is $Z$-continuous, then $M$ is $Z$-continuous.
\end{enumerate}
\end{lemma}
It is probably more accurate to write $\Closure_M: \powerset(P) \to M: A \mapsto \downset \{\bigvee_L \incl_{P}^\imf(A)\} \cap P$.
\begin{proof}
(1) It is clear that $M$ is the image of $\Closure_M$. We just need to show that $\Closure_M$ is a Moore closure. It is clear that $\Closure_M$ is order-preserving and expansive. 

For idempotency, it is enough to prove that, for all $x\in L$,
\[ \downset\{x\} \cap P = \downset \big\{\bigvee_L \downset \{x\} \cap P \big\} \cap P. \]
For the first inclusion, take $y\in \downset\{x\} \cap P$. Then $y \leq \bigvee_L \downset \{x\} \cap P$, so $y\in \downset \big\{\bigvee_L \downset \{x\} \cap P \big\}$. Since also $y\in P$, we have the first inclusion.

For the other inclusion, take $y\in \downset \big\{\bigvee_L \downset \{x\} \cap P \big\} \cap P$, so $y \leq \bigvee_L \big(\downset \{x\} \cap P\big) \leq \bigvee_L \downset \{x\} = x$. Thus $y\in \downset\{x\}$ and $y\in P$, so $y\in \downset\{x\} \cap P$.

(2) Clearly the elements of $M$ are downwards closed. Now take $D\in Z(P)$ such that $\bigvee D$ exists in $P$. Since $\incl_P: P\to L$ is $Z$-continuous, we have $\bigvee_P D = \bigvee_L D$, so
\[ \Closure_M(D) = \downset_L \big\{\bigvee_L D\big\} \cap P = \downset_L \big\{\bigvee_P D\big\} \cap P = \downset_P \big\{\bigvee_P D\big\}. \]
\end{proof}

\begin{lemma} \label{ZclosureSimilarToIntersectionSystem}
Let $Z$ be a $\mathrm{Z}$-system, $L$ a complete lattice, $P \subseteq L$ a subposet and $M$ as defined in \ref{intersectionSystemAmbientCompleteLattice}. Then
\[ \tilde{\incl}_P: \Closure_Z\big(\im(d)\big) \to \Closure_Z(P): A \mapsto \bigvee_L A \]
is an order similarity, with inverse
\[ \tilde{\incl}_P^{-1}: \Closure_Z(P) \to \Closure_Z\big(\im(d)\big): x\mapsto \downset\{x\} \cap P. \]
\end{lemma}
\begin{proof}
It is clear that both functions are order-preserving.

We can define the functions 
\[ f: M \to L: A \mapsto \bigvee_L A \]
and
\[ g: L \to M: x\mapsto \downset\{x\} \cap P. \]
We first show that $(f\circ g)(x) = x$ for all $x\in \Closure_Z(P)$. To that end, it is enough to note that this holds for all $x\in P$ and to prove that the set of all $x$ such that this holds is $Z$-closed. 

Set $C \defeq \setbuilder{x\in L}{\bigvee_L \big(\downset\{x\} \cap P\big) = x}$ and take $D\in Z(L)$ such that $D\subseteq C$. We need to show that $\bigvee D \in C$. Since $\bigvee D$ is an upper bound of $\downset\big\{\bigvee D\big\}$ it is also an upper bound of $\downset\big\{\bigvee D\big\} \cap P$, so $\bigvee \big(\downset\{\bigvee D\} \cap P\big) \leq \bigvee D$. 

We now show that $\bigvee \big(\downset\{\bigvee D\} \cap P\big)$ is also an upper bound of $D$. Take $d\in D \subseteq C$, so $d\leq \bigvee D$. This implies $\downset\{d\} \subseteq \downset \{\bigvee D\}$ and $\downset\{d\} \cap P \subseteq \downset \{\bigvee D\} \cap P$. Since $d\in D\subseteq C$, we have
\[ d = \bigvee \big(\downset\{d\} \cap P\big) \leq \bigvee \big(\downset\{\bigvee D\} \cap P\big). \]
Since $\bigvee \big(\downset\{\bigvee D\} \cap P\big)$ is an upper bound of $D$, it is greater than $\bigvee D$.
Putting both inequalities together gives $\bigvee D\in C$.

Next we show that $(g\circ f)(A) = A$ for all $A\in \Closure_Z\big(\im(d)\big)$. To that end, it is enough to note that this holds for all $A\in \im(d)$ and to prove that the set of all $A\in M$ such that this holds is $Z$-closed.

Set $C' \defeq \setbuilder{A\in M}{\downset\{\bigvee A\} \cap P = A}$ and take $\mathcal{D}\in Z(M)$ such that $\mathcal{D}\subseteq C'$. We need to show that $\bigvee_M \mathcal{D} \in C'$. We have, by \ref{meetJoinImageMooreClosure} and \ref{intersectionSystemAmbientCompleteLattice},
\begin{align*}
\bigvee_M \mathcal{D} &= \Closure_M\big(\bigcup \mathcal{D}\big) \\
&= \Closure_M^2\big(\bigcup \mathcal{D}\big) \\
&= \downset\Big\{\bigvee_L \Closure_M\big(\bigcup \mathcal{D}\big)\Big\} \cap P \\
&= \downset\Big\{\bigvee_L \bigvee_M \mathcal{D}\Big\} \cap P,
\end{align*} 
so $\mathcal{D}\in C'$.
\end{proof}

\begin{proposition}
Let $Z$ be a hereditary $\mathrm{Z}$-system, $L$ a complete lattice, $P \subseteq L$ a subposet such that $\incl_P: P\to L$ is $Z$-continuous and $Q$ a $Z$-complete poset. Let $f: P\to Q$ be a $Z$-continuous function. Then there exists a unique $Z$-continuous function $\tilde{f}: \Closure_Z(P) \to Q$ such that
\[ \begin{tikzcd}
P \ar[r, "\incl_P"] \ar[dr, swap, "f"] & \Closure_{Z}(P) \ar[d, dashed, "{\tilde{f}}"] \\ {} & L
\end{tikzcd} \qquad \text{commutes.} \]
This $\tilde{f}$ is defined by $\tilde{f}(x) = \bigvee_Q f^\imf\big(\downset\{x\}\cap P\big)$.
\end{proposition}
The commutation of the diagram just means that $\tilde{f}|_P = f$.
\begin{proof}
The aim is to use \ref{ZcontinuousExtension} to extend $f$ to $\Closure_Z\big(\im(d)\big)$ and then use \ref{ZclosureSimilarToIntersectionSystem} to pull back $f$ to $\Closure_Z(P)$.

Define the intersection system $M$ as in \ref{intersectionSystemAmbientCompleteLattice}. It is $Z$-continuous.

We need to show that $f: \sSet{P,\Closure_M} \to \sSet{Q, (-)^{\leq\geq}}$ is closure-continuous. Indeed, take $A\subseteq L$. Then $A^{\leq\geq}$ is downwards closed and $f^\preimf(A^{\leq\geq})$ is downards closed by \ref{MfunctionIsotone} and \ref{isotoneIffDownsetContinuous}. We also have that $A^{\leq\geq}$ is $Z$-closed. Since $f$ is $Z$-continuous, we have that $f^\preimf\big(A^{\leq\geq}\big)$ is $Z$-closed by \ref{ZcontinuityZclosureContinuity}. Thus $f^\preimf\big(A^{\leq\geq}\big)$ is an element of $\mathcal{C}_{\downarrow Z}$, making $f: \sSet{P, \Closure_{\mathcal{C}_{\downarrow Z}}}\to \sSet{L, (-)^{\leq\geq}}$ closure continuous.
...
\begin{lemma} \label{intersectionSystemAmbientCompleteLattice}
Let $P$ be a poset and $Q \subseteq P$ a subposet. Let $Z$ be a $\mathrm{Z}$-system. Then
\begin{enumerate}
\item the function
\[ \Closure_M: \powerset(Q) \to M: A \mapsto \Closure_{\downarrow Z}\big(\incl_Q^\imf(A)\big) \cap Q \]
is a Moore closure;
\item if $\incl_Q$ is $Z$-continuous, the associated intersection system $M$ is $Z$-continuous.
\end{enumerate}
\end{lemma}
Note that the $Z$-closure $\Closure_{\downarrow Z}\big(\incl_Q^\imf(A)\big)$ is taken in $P$.
\begin{proof}
(1) It is clear that $\Closure_M$ is order-preserving and expansive. For idempotency, we calculate
\[ \Closure_M(A) \subseteq \Closure_M^2(A) = \Closure_{\downarrow Z}\big(\Closure_{\downarrow Z}(A)\cap Q\big)\cap Q \subseteq \Closure_{\downarrow Z}^2(A)\cap Q = \Closure_{\downarrow Z}(A)\cap Q = \Closure_M(A). \]
This implies $\Closure_M^2(A) = \Closure_M(A)$.

(2) Assume $\incl_Q$ is $Z$-continuous. Clearly the elements of $M$ are downwards closed. Now take $D\in Z(Q)$ such that $\bigvee_Q D$ exists in $Q$. The $Z$-continuity of $\incl_Q$ implies that $\bigvee_Q D = \bigvee_P D$. We have $D\in Z(P)$ by \ref{subsetZsets}. This implies, by \ref{closureZsetWithJoin},
\[ \Closure_M(D) = \Closure_{\downarrow Z}\big(\incl_Q^\imf(D)\big) \cap Q = \downset_P\big\{\bigvee_P D\big\} \cap Q = \downset_P\big\{\bigvee_Q D\big\} \cap Q = \downset_Q\big\{\bigvee_Q D\big\}. \]
\end{proof}

\begin{lemma}
Let $Z$ be a $\mathrm{Z}$-system, $P$ a poset and $Q\subseteq P$ a subposet. Suppose that, for all $D\in Z(P)$ such that $\bigvee D$ exists and $\bigvee D \in Q$, we have $\bigvee \big(\downset D \cap Q\big) = \bigvee D$. Then $\Closure_{\downarrow Z}(A) \cap Q \subseteq \Closure_{\downarrow Z}(\downset A\cap Q)$ for all $A\subseteq P$.
\end{lemma}
\begin{proof}
Take $A\subseteq P$. We first show that $\Closure_{\downarrow Z}(A)\subseteq \Closure_{\downarrow Z}(\downset A \cap Q)\cup Q^c$. The result follows simply from this:
\begin{align*}
\Closure_{\downarrow Z}(A) \cap Q &\subseteq \big(\Closure_{\downarrow Z}(\downset A \cap Q)\cup Q^c\big) \cap Q \\
&= \big(\Closure_{\downarrow Z}(\downset A \cap Q) \cap Q\big) \cup (Q^c \cap Q) \\
&= \Closure_{\downarrow Z}(\downset A \cap Q) \cap Q \subseteq \Closure_{\downarrow Z}(\downset A \cap Q)
\end{align*}
We clearly have $A\subseteq \Closure_{\downarrow Z}(\downset A \cap Q)\cup Q^c$, so it is enough to show that $\Closure_{\downarrow Z}(\downset A \cap Q)\cup Q^c$ 
\end{proof}

\begin{lemma} \label{ZclosureSimilarToIntersectionSystem}
Let $Z$ be a hereditary $\mathrm{Z}$-system, $P$ a $Z$-complete poset, $Q \subseteq P$ a subposet with $\incl_Q$ $Z$-continuous. Let $M$ be the $Z$-continuous intersection system defined in \ref{intersectionSystemAmbientCompleteLattice}. Then
\[ \tilde{\incl}_Q: \Closure_Z\big(\im(d)\big) \to \Closure_Z(Q): A \mapsto \bigvee_P A \]
is an order similarity, with inverse
\[ \tilde{\incl}_Q^{-1}: \Closure_Z(Q) \to \Closure_Z\big(\im(d)\big): x\mapsto \Closure_{\downarrow Z}\big(\incl_Q^\imf(\{x\})\big) \cap Q = \downset\{x\} \cap Q. \]
\end{lemma}
\begin{proof}
It is clear that both functions are order-preserving. The equality in the second function is justified by \ref{principalDownsetsZclosed}.

Observe that the first function is well-defined (in the sense that the join always exists) by \ref{ZcontinuousExtension}, for which we need to verify that $\incl_Q: \sSet{Q, \Closure_M} \to \sSet{P, (-)^{\leq\geq}}$ is closure-continuous. For this we use \ref{closureContinuousEquivalents} and calculate, for some $B\subseteq P$
\begin{align*}
\Closure_M\big(\incl_Q^{\preimf}(B)\big) &= \Closure_M\big(B \cap Q\big) \\
&= \Closure_{\downarrow Z}\big(B \cap Q\big) \cap Q \\
&\subseteq \Closure_{\downarrow Z}(B) \cap Q \\
&\subseteq B^{\leq\geq} \cap Q \\
&= \incl_Q^{\preimf}\big(B^{\leq\geq}\big).
\end{align*} 

We can define the function,
\[ f: M \to P: A \mapsto \bigvee_P A, \]
which is defined for all $A$ that has a join in $P$, and the function
\[ g: P \to M: x\mapsto \downset\{x\} \cap Q. \]
We first show that $(f\circ g)(x) = x$ for all $x\in \Closure_Z(Q)$. To that end, it is enough to note that this holds for all $x\in Q$ and to prove that the set of all $x$ such that this holds is $Z$-closed. 

Set $C \defeq \setbuilder{x\in P}{\bigvee_P \big(\downset\{x\} \cap Q\big) = x}$ and take $D\in Z(P)$ such that $D\subseteq C$. We need to show that $\bigvee D \in C$. Since $\bigvee D$ is an upper bound of $\downset\big\{\bigvee D\big\}$ it is also an upper bound of $\downset\big\{\bigvee D\big\} \cap Q$. 

We now show that $\bigvee D$ is in fact the least upper bound of $\downset\big\{\bigvee D\big\} \cap Q$. Indeed, let $u$ be some other upper bound of $\downset\big\{\bigvee D\big\} \cap Q$. Take arbitrary $d\in D$, so $d\leq \bigvee D$. This implies $\downset\{d\} \subseteq \downset \{\bigvee D\}$ and $\downset\{d\} \cap Q \subseteq \downset \{\bigvee D\} \cap Q$, so $u$ is an upper bound of $\downset\{d\} \cap Q$. Since $d\in D\subseteq C$, we have
\[ d = \bigvee \big(\downset\{d\} \cap Q\big) \leq u. \]
As $d$ was taken arbitrarily, we have that $u$ is an upper bound of $D$ and so $\bigvee D \leq u$.

Next we show that $(g\circ f)(A) = A$ for all $A\in \Closure_Z\big(\im(d)\big)$. To that end, it is enough to note that this holds for all $A\in \im(d)$ and to prove that the set of all $A\in M$ such that this holds is $Z$-closed.

Set $C' \defeq \setbuilder{A\in M}{\text{$A$ is $(Z,M)$-tapered and $\downset\{\bigvee_P A\} \cap Q = A$}}$. For every $(Z,M)$-tapered $A\subseteq Q$, we have that $\bigvee_P A = \bigvee_P \incl_Q^\imf(A)$ exists, since $P$ is $Z$-complete. Take $\mathcal{D}\in Z(M)$ such that $\mathcal{D}\subseteq C'$. We need to show that $\bigvee_M \mathcal{D} \in C'$. 

We have that $\bigvee_P \bigvee_M \mathcal{D} = \bigvee_P \incl_Q^\imf\Big(\bigvee_M \mathcal{D}\Big)$ exists by \ref{extensionMcontinuousFunctionZcontinuous} and the fact that $\incl_Q: \sSet{Q, \Closure_M} \to \sSet{P, (-)^{\leq\geq}}$ is closure-continuous. Clearly $\bigvee_M \mathcal{D} \subseteq \downset \big\{\bigvee_P\bigvee_M \mathcal{D}\big\} \cap Q$. For the other inclusion, we have that $\bigvee_P\bigvee_M \mathcal{D} = \bigvee_{D\in \mathcal{D}}\bigvee_P D$ by \ref{extensionMcontinuousFunctionZcontinuous}.
...
\subsection{Atomic and atomistic posets}
\begin{definition}
Let $\sSet{P,\leq}$ be a poset. Then
\begin{itemize}
