...
Let $S$ be a bounded $\wedge$-semilattice, $a\in S$ and $F\in\filters(S)$. Then $a\in F$ \textup{if and only if} $F=F|_a$.
\begin{enumerate}
\item $a\in F|_a$;
...
\end{proof}
\begin{corollary}\label{filterGrillIsPrime}
Let $X$ be a set, $A,B\subseteq X$ and $F \in \powerfilters{X}$ a filter. If $A\cup B\in F^\mesh$, then either $A\in F^\mesh$ or $B\in F^\mesh$.
\end{corollary}
TODO: cfr. ultrafilters. + generalise to Boolean lattices.
\begin{proof}
We have the implications
\begin{align*}
A\cup B\in F^\mesh \iff& (A\cup B)^c = A^c \cap B^c \notin F \\
\implies& \big(A^c \notin F\big)\lor \big(B^c \notin F\big) \\
...
\begin{enumerate}
\item $f^{\preimf\imf}(G)$ is upwards closed; in particular $f^{\preimf\imf}(G)$ is a filter;
