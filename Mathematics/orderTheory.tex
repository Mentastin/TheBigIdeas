...
\begin{enumerate}[(I)]
\setcounter{enumi}{6}
\item[(VII'')] \textbf{Axiom of dependent choice ($\text{DC}$)}: for any left-total binary relation $R$ on a non-empty class $A$, there exists a sequence $\seq{a_n}_{n\in \N}$ in $A$ such that
...
\begin{enumerate}
\item for any left-total binary relation $R$ on a non-empty \emph{set} $A$, there exists a sequence $\seq{a_n}_{n\in \N}$ in $A$ such that
\[ \forall n\in \N: \quad a_n\mathrel{R}a_{n+1}; \]
\item the axiom of dependent choice;
\item for any left-total binary relation $R$ on a class $A$ and $a\in A$, there exists a sequence $\seq{a_n}_{n\in \N}$ in $A$ such that $a_0 = a$ and
\[ \forall n\in \N: \quad a_n\mathrel{R}a_{n+1}. \]
\end{enumerate}
\end{lemma}
\begin{proof}
We prove $(1) \Rightarrow (2) \Rightarrow (3)$. The implication $(3) \Rightarrow (1)$ is immediate as every set is a class and non-empty means there exists some $a\in A$.

$\boxed{(1)\Rightarrow (2)}$ Assume (1) and let $R$ be a left-total relation on some non-empty class $A$. Take some $a\in A$. For each 
