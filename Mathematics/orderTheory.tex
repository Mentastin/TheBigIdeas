...
\[ \bigvee A = \bigvee (\Closure\circ \incl)^\imf(A) = \Closure\Big(\bigcup \incl^{\imf}(A)\Big) = \Closure\Big(\bigcup A\Big). \]
Using $\Closure\circ\incl = \id$.

\begin{definition}
Let $P$ be a poset and $f: P\to \im(f) \subseteq P$ a function. We call $f$
\begin{itemize}
\item a \udef{Moore closure operator} if
\begin{itemize}
\item $f$ is order-preserving;
\item $x \leq f(x)$ for all $x\in P$;
\item $f = f^2$;
\end{itemize}
\item \udef{Moore dual closure operator} if
\begin{itemize}
\item $f$ is order-preserving;
\item $f(x) \leq x$ for all $x\in P$;
\item $f = f^2$;
\end{itemize}
\end{itemize}
\end{definition}
...
\end{proposition}
\begin{proof}
(1) Suppose $f$ is a Moore closure operator. Take $x\in P$ and $y\in P\im(f)$. Then there exists $y'\in P$ such that $y = f(y')$, so $f(y) = f^2(y') = f(y') = y$. Now
...
We have $f(x) = (\iota_{\im(f)}\circ f)(x) \leq x$ for all $x\in P$ by \ref{preorderGaloisCondition}.
...
\end{proposition}
\begin{proof}
Let $A\subseteq P$ be a subset such that the meet $\bigwedge A$ exists. Then, by \ref{boundsOnSetWithLeastBoundIsPrincipalIdeal}, \ref{boundsFromPrincipalImages} and \ref{DedekindMacNeilleCompletionComplete},
\[ m\Big(\bigwedge A\Big) = \downset\Big\{\bigwedge A\Big\} = A^\geq = \bigcap_{a\in A}\downset\{a\} = \bigcap_{a\in A}m(a) = \bigcap m^\imf(A) = \bigwedge_{\DM(P)}m^\imf(A). \]

Now let $B\subseteq P$ be a subset such that the meet $\bigvee B$ exists. Then we calculate, using \ref{supInfConnection}, \ref{boundsOnSetWithLeastBoundIsPrincipalIdeal}, \ref{imagesFromPrincipalImages}, \ref{meetJoinImageMooreClosure} and \ref{DedekindMacNeilleCompletionComplete},
\begin{align*}
...
&= A^{\leq\geq} \\
&= (\downset A)^{\leq\geq} \\
&= \Big(\bigcup_{x\in A} \downset\{x\}\Big)^{\leq\geq} \\
&= \Big(\bigcup m^\imf(A)\Big)^{\leq\geq} \\
...
\begin{align*}
\bigwedge \{F_i\}_{i\in I} &= \bigcap_{i\in I}F_i & &\hspace{-3em}= \setbuilder{\bigcup_{i\in I}A_i}{\forall i\in I: A_i\in F_i} \\
\bigvee \{F_i\}_{i\in I} &= \upset \Closure_\wedge\left(\bigcup_{i\in I}F_i\right) & &\hspace{-3em}= \setbuilder{\bigcap_{j\in J}A_j}{\forall J\subseteq I \text{finite}: \forall j\in J: A_j\in F_j}.
\end{align*}
...
Let $P$ be a poset and $x,y\in P$ such that $x\ll y$. The $x\leq y$.
\end{lemma}
\begin{proof}
...
which is a principal up set iff it has a minimum, which is exactly the statment (3). The equivalences then follow from \ref{GaloisConnectionLatticePreservation}, since in this case $\ideals(P)$ is a complete $\wedge$-semilattice by \ref{filterSetIdealSetWedgeSemilattice}.
\end{proof}
\begin{corollary}
...
\begin{definition}
A poset $P$ is called an \udef{algebraic poset} if it has a base of compact elements.

An \udef{algebraic lattice} is a \emph{complete} lattice that is an algebraic poset.
\end{definition}

\begin{example}
Let $X$ be a set. Then $\sSet{\powerset(X), \subseteq}$ is an algebraic lattice.
\end{example}
...
\subsubsection{Continuous and algebraic closure}
\begin{definition}
Let $P$ be a poset and $f: P\to \im(f) \subseteq P$ a function. We call $f$
\begin{itemize}
\item a \udef{continuous closure operator} if
\begin{itemize}
\item $x \leq f(x)$ for all $x\in P$;
\item $f = f^2$;
\item $\bigvee_{P} f^\imf\big(\ddownset\{x\}\big)$ exists and $f(x) \leq \bigvee_{P} f^\imf\big(\ddownset\{x\}\big)$ for all $x\in P$;
\end{itemize}
\item an \udef{algebraic closure operator} if
\begin{itemize}
\item $x \leq f(x)$ for all $x\in P$;
\item $f = f^2$;
\item $\bigvee_{P} f^\imf\big(\ddownset\{x\}\cap \compacts(P)\big)$ exists and $f(x) \leq \bigvee_{P} f^\imf\big(\ddownset\{x\}\cap \compacts(P)\big)$ for all $x\in P$.
\end{itemize}
\end{itemize}
\end{definition}
TODO: dual closure operators. Note that $\ddownset \{x\}\cap \compacts(P) = \downset \{x\}\cap \compacts(P)$ by \ref{downsetCompactElement}.

\begin{lemma}
Let $P$ be a poset and $f: P\to \im(f) \subseteq P$ a function.
\begin{enumerate}
\item if $f$ is a continuous closure operator, then $f(x) = \bigvee_{P} f^\imf\big(\ddownset\{x\}\big)$ for all $x\in P$;
\item if $f$ is an algebraic closure operator, then $f(x) = \bigvee_{P} f^\imf\big(\ddownset\{x\}\cap \compacts(P)\big)$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) One inequality is given by the definition. For the other, note that
\[ \downset f^\imf\big(\ddownset \{x\}\big) \subseteq \downset f^\imf\big(\downset \{x\}\big) = \downset \{f(x)\}, \]
so $\bigvee_Pf^\imf\big(\ddownset \{x\}\big) \leq \bigvee \downset\{f(x)\} = f(x)$.

(2) One inequality is given by the definition. For the other, note that
\[ \downset f^\imf\big(\ddownset \{x\}\cap \compacts(P)\big) \subseteq \downset f^\imf\big(\ddownset \{x\}\big) \subseteq \downset f^\imf\big(\downset \{x\}\big) = \downset \{f(x)\}, \]
so $\bigvee_P f^\imf\big(\ddownset \{x\}\cap \compacts(P)\big) \leq \bigvee \downset\{f(x)\} = f(x)$.
\end{proof}

\begin{lemma} \label{continuousClosurePreservesOrder}
Let $P$ be a poset and $f: P\to \im(f) \subseteq P$ a continuous closure operator. Then $f$ preserves order, so $f$ is a Moore closure operator.
\end{lemma}
\begin{proof}
Let $x \leq y$ in $P$. Then $\ddownset\{x\}\subseteq \ddownset\{y\}$ by \ref{downsetEmbeddingContinuousPosets} and so $f^\imf\big(\ddownset\{x\}\big)\subseteq f^\imf\big(\ddownset\{y\}\big)$. This implies
\[ f(x) = \bigvee_P f^\imf\big(\ddownset\{x\}\big)\leq \bigvee_P f^\imf\big(\ddownset\{y\}\big) = f(y). \]
\end{proof}

\begin{proposition} \label{continuousClosurePreservesWayBelow}
Let $L$ be a continuous lattice and $f: L\to \im(f) \subseteq L$ a continuous closure operator. Then $f$ preserves the way-below relation.
\end{proposition}
\begin{proof}
Let $x \ll y$ in $L$. We want to prove that $f(x) \ll f(y)$. To that end, let $D\subseteq \im(f)$ be a non-empty directed set with supremum such that $f(y) \leq \dirvee_{\im(f)}D$. Using the fact that $f$ is a continuous closure operator and \ref{meetJoinImageMooreClosure} (because $f$ is also a Moore closure operator, \ref{continuousClosurePreservesOrder}, and $L$ is complete, so $\dirvee_L D$ exists), we have
\[ y \leq f(y) \leq \dirvee_{\im(f)}D = f\Big(\dirvee_L D\Big) = \bigvee_L f^\imf\Big(\ddownset\big\{\dirvee_L D\big\}\Big). \]
Since $x\ll y$ and $f^\imf\Big(\ddownset\big\{\dirvee_L D\big\}\Big)$ is directed (using \ref{imageDirectedSet} and the fact that $L$ is continuous), this implies
\[ x\in \downset f^\imf\Big(\ddownset\big\{\dirvee_L D\big\}\Big) \subseteq \downset f^\imf\Big(\downset\big\{\dirvee_L D\big\}\Big) \subseteq \downset f\Big(\dirvee_L D\Big) = \downset \dirvee_{\im(f)}D. \]
This implies $f(x) \in \downset \dirvee_{\im(f)}D$ and so $f(x) \ll f(y)$.
\end{proof}
\begin{corollary}
Let $L$ be a continuous lattice and $f: L\to \im(f) \subseteq L$ a continuous closure operator. Then $\im(f)$ is a continuous lattice.
\end{corollary}
\begin{proof}
By \ref{meetJoinImageMooreClosure}, $\im(f)$ is a complete lattice.

For all $x\in \im(f)$, we have
\[ x = f(x) = f^2(x) = f\Big(\bigvee_{L} f^\imf\big(\ddownset\{x\}\big)\Big) = \bigvee_{\im(f)} f^\imf\big(\ddownset\{x\}\big). \]
Since $f$ preserves the way-below relation, we have $f^\imf\big(\ddownset\{x\}\big) \subseteq \ddownset\{f(x)\} = \ddownset\{x\}$. Also, $f^\imf\big(\ddownset\{x\}\big)$ is directed by \ref{imageDirectedSet}, so $\im(f)$ continuous by \ref{posetContinuousIffHasBase}.
\end{proof}

\begin{lemma} \label{algebraicClosureIsContinuous}
Every algebraic closure operator is a continuous closure operator.
\end{lemma}
\begin{proof}
...
so $\bigvee_P f^\imf\big(\ddownset\{x\}\big) = f(x)$.
\end{proof}

\begin{proposition} \label{imageAlgebraicClosureAlgebraicLattice}
Let $L$ be a continuous lattice and $f: L\to \im(f)\subseteq L$ an algebraic closure operator. Then $\im(f)$ is an algebraic lattice.
\end{proposition}
\begin{proof}
Since $f$ preserves the way-below relation, \ref{continuousClosurePreservesWayBelow} and \ref{algebraicClosureIsContinuous}, $f^\imf\big(\ddownset\{x\}\cap \compacts(L)\big)$ is a set of compact elements. Thus $\im(f)$ is algebraic by \ref{algebraicJoinSemilattice}.
\end{proof}
