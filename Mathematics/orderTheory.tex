...
TODO review definition for (loose) mini/maximality.
...
\begin{definition}
Let $\sSet{P, \Yleft}$ be an ordered \emph{set}.
Then $P$ is called
\begin{itemize}
...
\end{itemize}
Additionally, $P$ is called
\begin{itemize}
\item \udef{well-ordered} if every non-empty subset has a minimum. In this case $\Yleft$ is said to be a \udef{well-ordering}.
\end{itemize}
A set is called \udef{well-orderable} if it admits a well-ordering.
\end{definition}
The requirement that $P$ be a set makes it easier to quantify over all subsets.
...
\item $P$ is loosely well-founded and connex.
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ Assume $P$ well-ordered. Then every non-empty subset has a minimum, which is also loosely minimal by \ref{maximumIsMaximal}. So $P$ is well-founded.
...
$(2) \Rightarrow (1)$ Now assume $P$ is well-founded and connex and take some non-empty subset $S$ of $P$. Now $S$ is also connex, so each minimal element of $S$ is a minimum by \ref{maximumIsMaximal}.
\end{proof}
...
\begin{definition}
Let $\sSet{U, \Yleft}$ be an ordered set. We say $U$ satisfies
\begin{itemize}
\item the principle of \udef{well-founded induction} if for all subsets $P\subseteq U$, we have the implication
\[ \text{if}\quad  \forall x\in U: \Big(\big(\downset\{x\}\setminus\{x\}\subseteq P\big)\implies x\in P\Big), \quad \text{then}\quad P=U; \]
\item the principle of \udef{loosely well-founded induction} if for all subsets $P\subseteq U$, we have the implication
\[ \text{if}\quad  \forall x\in U: \Big(\big(\downset\{x\}\setminus\upset\{x\}\subseteq P\big)\implies x\in P\Big), \quad \text{then}\quad P=U. \]
\end{itemize}
...
\[ \forall x\in U: \quad f(x) = h(f|_{\downset\{x\}\setminus\{x\}}); \]
\item the principle of \udef{loosely well-founded recursion} if for all non-empty sets $E$ and functions $h: (U\not\to E)\to E$, there exists exactly one function $f: U\to E$ that satisfies
\[ \forall x\in U: \quad f(x) = h(f|_{\downset\{x\}\setminus\upset\{x\}}). \]
\end{itemize}
\end{definition}

\begin{theorem}[Well-founded induction and recursion]
Let $\sSet{U, \Yleft}$ be an ordered set. Then the following are equivalent:
\begin{enumerate}
...
\begin{proof}
We show the first set of equivalences. The proof of the second set is identical, except with $\downset\{x\}\setminus\upset\{x\} = \Yleft\{x\}\setminus\{x\}\Yleft$ instead of $\downset\{x\}\setminus\{x\}$.
...
\[ \forall x\in U: \Big(\big(\downset\{x\}\setminus\{x\}\perp P\big)\implies x\notin P\Big), \]
which by \ref{maximalMinimalEquivalents} implies that $P$ has no minimal elements (as $\downset\{x\}\setminus\{x\} = \Yleft\{x\}\setminus\{x\}$).
...
$\boxed{(2) \Rightarrow (1)}$ Assume, towards a contradiction, the induction step for some $P\subseteq U$ and $\exists x\in U\setminus P$. Then the set $P^c$ is not empty and by well-foundedness we can find a minimal element $x^*\in P^c$. By minimality $\downset\{x^*\}\setminus\{x^*\}\perp P^c$, so $\downset\{x^*\}\setminus\{x^*\}\subseteq P$. By the induction hypothesis $x^*\in P$, which is a contradiction.

$\boxed{(2) \Rightarrow (3)}$ Let $\sSet{U, \Yleft}$ be an ordered set and $P\subseteq U$. Consider the characteristic function $\chi_P: U\to \{0,1\}$ of $P$.

We may take $E = \{0,1\}$ and the $h$ that maps partial functions $p$ to zero iff $0\in\im(p)$. Then the statement
\[ \forall x\in U: \quad\chi_P(x) = h(\chi_P|_{\downset\{x\}\setminus\{x\}}) \]
...
$\boxed{(3) \Rightarrow (2)}$ Consider $U' \defeq U\cup \{t_U\}$, where $t_U$ is greater than any element in $U$. Then $U'$ is well-founded and $\downset\{t_U\}\setminus \{t_U\} = U$.

Consider the set $P\subseteq U'$ that contains $u\in U'$ if and only if there exists exactly one function $f_u:\downset\{u\}\to E$ which satisfies
\[ f_u(x) = h(f_u|_{\downset\{x\}\setminus\{x\}}) \qquad \forall x\in\downset\{u\}. \]
...
Fix an arbitrary $u\in U'$ and assume the induction hypothesis, which posits that such a unique function $f_v: \downset\{v\}\to E$ exists for all $v\in \downset\{u\}\setminus\{u\}$. We show the existence and uniqueness of $f_u:\downset\{u\}\to E$.

Note first that for $w\Yleft v\in \downset\{u\}\setminus\{u\}$ we have $f_w = f_v|_{\downset\{w\}}$, as both functions satisfy the conditions that give the unique definition.

For existence: first define $f_u' \defeq \bigcup_{v\in \downset\{u\}\setminus\{u\}} f_v$. 
This is a relation; we show that it is functional and has domain $\downset\{u\}\setminus\{u\}$.
Indeed
\begin{align*}
\dom(f'_u) &= \bigcup_{v\in \downset\{u\}\setminus\{u\}} \dom(f_v) \\
&= \bigcup_{v\in \downset\{u\}\setminus\{u\}} \downset\{v\} \\
&= \bigcup_{v\in \downset\{u\}\setminus\{u\}} \{v\} \\
&= \downset\{u\}\setminus\{u\}.
\end{align*}
To show that $f'_u$ is functional, take $v\in \downset\{u\}\setminus\{u\}$ and let $v\mathrel{(f'_u)}w_1$ and $v\mathrel{(f'_u)}w_2$.
...
Now let $f_u$ be the function $f'_u$ with the domain extended to $\downset\{u\}$ by adding $f_u(u) = h(f'_u)$.

(In the case of loosely well-founded recursion, we need to add $f_u(w) = h(f'_u)$ for all $w\in\downset\{u\}\cap\upset\{u\}$)

To show $f_u$ satisfies the defining property, take $v\in \downset\{u\}$. If $v\neq u$, then
\[ f_u(v) = f_v(v) = h(f_v|_{\downset\{v\}\setminus\{v\}}) = h(f_u|_{\downset\{v\}\setminus\{v\}}). \]
Otherwise $v=u$, in which case $\downset\{u\}\cap\upset\{u\} = \downset\{v\}\cap\upset\{v\}$ and then
\[ f_u(v) = h(f'_u) = h(f_u|_{\downset\{u\}\setminus\{u\}}) = h(f_u|_{\downset\{v\}\setminus\{v\}}). \]
This concludes the existence part of the proof.

For uniqueness, assume that another function $g: \downset\{u\}\to E$ satisfies the property. For $v\neq u$, we have $g|_{\downset\{u\}\setminus\{u\}} = f'_u = f_u|_{\downset\{u\}\setminus\{u\}}$. For $v=u$, we have
\[ g(u) = h(g|_{\downset\{u\}\setminus\{u\}}) = h(f'_u) = f_u(u). \]
This proves uniqueness.
\end{proof}
\begin{corollary}
Let $\sSet{U,\Yleft}$ be an ordered set.
\begin{enumerate}
...
\[ u <_l v \quad\iff\quad u_k < v_k \;\text{where $k = \min\setbuilder{n\in\N}{n \leq \len(u), n\leq \len(v), u_n \neq v_n}$}. \]
...
$\boxed{\Rightarrow}$ We need to show that $(\lambda_f)^{\imf}(T_{R})$ is a subset of $T_S$
\end{proof}
