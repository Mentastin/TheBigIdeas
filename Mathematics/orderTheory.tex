...
\end{lemma}
\begin{proof}
(1) Take $a\in \{y\}^\cons$, then there exists $b\in P$ such that $a,y \leq b$.

Now $x\leq y\leq b$, so $b$ is an upper bound of $\{x,a\}$ and thus $x\cons a$, which means that $a\in \{x\}^\cons$.

(2) Immediate from (1), using \ref{polarsGaloisConnection}.
...
Let $P$ be a poset, $Q$ a Dedekind-complete poset, $(x,y)\in P\times Q$ and $f: P\not\to Q$ a partial order-preserving function. Then $\{(x,y)\} \in \{f\}^\cons$ \textup{if and only if}
...
\bigvee f^\imf\big(\upset\{x\} \cap \dom(f)\big) \geq y.
\end{cases} \]
\end{proof}

\begin{proposition}
Let $P$ be a poset, $Q$ a Dedekind-complete poset and $f: P\not\to Q$ a partial order-preserving function. Then the consistent extension $\tilde{f}$ of $f$ is defined by
\[ \tilde{f}(x) = y \defequiv \bigvee f^\imf\big(\downset \{x\} \cap \dom(f)\big) = y = \bigwedge f^\imf\big(\upset \{x\} \cap \dom(f)\big). \]
\end{proposition}
\begin{proof}
From \ref{consistentExtenionsIsotoneFunctionToDedekindPoset}. TODO.
\end{proof}
