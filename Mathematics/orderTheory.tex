...
\item $\sSet{T,\sqsubseteq}$ is converse well-founded \textup{if and only if} $[T] = \emptyset$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) We show that $\sqsubseteq$ is
\begin{itemize}
\item \emph{transitive}: let $u \sqsubseteq v \sqsubseteq w$. Then $u = v[0:m]$ and $v = w[0:n]$. TODO \ref{initialSliceRestriction}.
\end{itemize}
\end{proof}
