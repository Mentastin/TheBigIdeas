...
The antecedent of the implication is called the \udef{induction hypothesis}.
\end{definition}
...
If $U$, $V$ are well-ordered sets and $U =_o V$, then the order similarity is unique.
\end{corollary}
\begin{proof}
Assume $f,g: U\to V$ are order similarities. Let the proposition $P(y)$ be $f(y) = g(y)$. Take arbitrary $x\in U$ and assume $\forall y<x: f(y) = g(y)$. Then
\begin{align*}
f(x) &= f^\imf\Big(\min\big(\setbuilder{y}{y\geq x}\big)\Big) \\
&= f^\imf\Big(\min\big(\setbuilder{y}{y < x}^c\big)\Big) \\
&= \min\big(\setbuilder{f(y)}{y< x}^c\big) \\
&= \min\big(\setbuilder{g(y)}{y< x}^c\big) \\
&= g^\imf\Big(\min\big(\setbuilder{y}{y < x}^c\big)\Big) \\
&= g^\imf\Big(\min\big(\setbuilder{y}{y\geq x}\big)\Big) = g(x).
\end{align*}
\end{proof}
...
\[ f(x) = h(f|_{\downset\{x\}}) \qquad \forall x\in U. \]
\end{theorem}
\begin{proof}
Like when proving recursion on $\N$, we will consider ``approximations'' of the function $f$, i.e.\ functions $\downset\{t\} \to E$ which satisfy the requirement for all $x<t$.

This is the subject of the following lemma:
\begin{lemma*}
Let $U$ be a well-ordered set, $E$ some non-empty set and $h: (U\not\to E) \to E$ some function.

For all $t\in U$, there is exactly one function $\sigma_t:\downset\{t\}\to E$ which satisfies
\[ \sigma_t(x) = h(\sigma_t|_{\downset\{x\}}) \qquad \forall x < t. \]
\end{lemma*}
...
\[ \forall u<t: \exists! \sigma_u\in (\downset\{u\}\to E): \forall x<u: \sigma_u(x) = h(\sigma_u|_{\downset\{x\}}). \]
...
\begin{itemize}[leftmargin=2.5cm]
\item[\boxed{t = 0_U}] Then $\downset\{t\} = \emptyset$ and we must have $\sigma_t = \emptyset$.
...
\[ \sigma_u|_{\downset\{x\}} = \sigma_v|_{\downset\{x\}}, \]
and by the induction hypothesis
\[ \sigma_u(x) = h(\sigma_u|_{\downset\{x\}}) = h(\sigma_v|_{\downset\{x\}}) = \sigma_v(x). \]
This is a contradiction, proving we do indeed have a chain.

Finally we verify
\begin{itemize}
\item the domain of $\sigma_t$ is $\downset\{t\}$; indeed
\[ \dom(\sigma_t) = \bigcup\{\dom(\sigma_u)\;|\; u< t \} = \bigcup\{ \downset\{u\}\;|\; u<t \} \]
which is an initial segment and thus equal to $\downset\{v\}$ for some $v$. By lemma (TODO ref)
\[ v\leq t \leq S_U(v). \]
...
\end{cases} \]
We can then apply the lemma to $\operatorname{Succ}(U)$ and $h'$. Because $\downset\{t_U\} = U$, this gives a unique function $\sigma_{t_U}: U\to E$. We take this as our $f$.
\end{proof}
