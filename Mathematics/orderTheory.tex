...
is a Moore closure;
\item the inclusion function $\incl_Q: \sSet{Q, \Closure_M} \to \sSet{P, (-)^{\leq\geq}}$ is closure-continuous;
\item if $\incl_Q$ is $Z$-continuous, the intersection system $M$ is $Z$-continuous.
\end{enumerate}
\end{lemma}
...
(2) We use \ref{closureContinuousEquivalents} and calculate, for some $B\subseteq P$
\begin{align*}
...
(3) Assume $\incl_Q$ is $Z$-continuous. Clearly the elements of $M$ are downwards closed. Now take $D\in Z(Q)$ such that $\bigvee_Q D$ exists in $Q$. The $Z$-continuity of $\incl_Q$ implies that $\bigvee_Q D = \bigvee_P D$. We have $D\in Z(P)$ by \ref{subsetZsets}. This implies, by \ref{closureZsetWithJoin},
...
Let $Z$ be a hereditary $\mathrm{Z}$-system, $P$ a $Z$-complete poset, $Q \subseteq P$ a subposet with $\incl_Q$ $Z$-continuous. Let $M$ be the $Z$-continuous intersection system defined in \ref{intersectionSystemAmbientCompleteLattice} and $A\in \Closure_Z\big(\im(d)\big)$. Then $\Closure_{\downarrow Z}(A) = \downset \{\bigvee_P A\}$, where the $\downarrow Z$-closure is taken in $P$.
\end{lemma}
\begin{proof}
First note that the join $\bigvee_P A = \bigvee \incl_Q^\imf(A)$ exists for all $A\in \Closure_Z\big(\im(d)\big)$ by \ref{ZcontinuousExtension} and \ref{intersectionSystemAmbientCompleteLattice}.

Next note that $\Closure_{\downarrow Z}\big(\downset \{x\}\big) = \downset_P \{x\} = \downset_P \{\bigvee_P \downset \{x\}\}$ for all $x\in Q$, so $\Closure_{\downarrow Z}(A) = \downset \{\bigvee_P A\}$ holds for all $A\in \im(d)$.

Finally, we need to show that the set
\[ C \defeq \setbuilder{A\in \Closure_Z\big(\im(d)\big)}{\Closure_{\downarrow Z}(A) = \downset \{\bigvee_P A\}} \]
is a $Z$-closed subset of $M$. To that end, take $\mathcal{D} \subseteq C$ such that $\mathcal{D}\in Z(M)$. We then calculate, using \ref{meetJoinImageMooreClosure},
\begin{align*}
\Closure_{\downarrow Z}\Big(\bigvee_M \mathcal{D}\Big) &= \big(\Closure_{\downarrow Z}\circ \Closure_M\big)\Big(\bigcup \mathcal{D}\Big) \\
...
&= \big(\Closure_{\downarrow Z}\circ \Closure_M\big)\Big(\bigcup \mathcal{D}\Big) \\
&= \Closure_{\downarrow Z}\Big(\bigvee_M \mathcal{D}\Big),
\end{align*}
so $\Closure_{\downarrow Z}\Big(\bigvee_M \mathcal{D}\Big) = \Closure_{\downarrow Z}\Big(\bigcup \mathcal{D}\Big)$.
By \ref{closureContinuityJoin} we have
\begin{align*}
\Closure_{\downarrow Z}\Big(\bigvee_M \mathcal{D}\Big) &= \Closure_{\downarrow Z}\Big(\bigcup_{D\in \mathcal{D}} D\Big) \\
...
&= \downset \{\bigvee_P\bigvee_{M}\mathcal{D} \},
\end{align*}
where we have used the fact that the function $A\mapsto \bigvee_P A$ is isotone, so $\setbuilder{\bigvee_P D}{D\in \mathcal{D}} \in Z(P)$, \ref{closureZsetWithJoin} and \ref{extensionMcontinuousFunctionZcontinuous}.
\end{proof}
...
\[ \tilde{\incl}_Q^{-1}: \Closure_Z(Q) \to \Closure_Z\big(\im(d)\big): x\mapsto \Closure_{\downarrow Z}\big(\{x\}\big) \cap Q = \downset\{x\} \cap Q. \]
\end{lemma}
\begin{proof}
...
Observe that the first function is well-defined (in the sense that the join always exists) by \ref{ZcontinuousExtension} and \ref{intersectionSystemAmbientCompleteLattice}.
...
Next we show that $(g\circ f)(A) = A$ for all $A\in \Closure_Z\big(\im(d)\big)$. This follows from \ref{extensionDomainDownsetCompletion}:
\[ (g\circ f)(A) = \downset \{\bigvee_P A\} \cap Q = \Closure_{\downset Z}(A) \cap Q = \Closure_M(A) = A. \]
\end{proof}

TODO: require $\Closure_{\downarrow Z, Q}(A) = \Closure_{\downarrow Z}(A) \cap Q$.

\begin{proposition}
Let $Z$ be a hereditary $\mathrm{Z}$-system, $P,L$ $Z$-complete posets and $Q \subseteq P$ a subposet such that $\incl_Q: Q\to P$ is $Z$-continuous and $Q$ a $Z$-complete poset. Let $f: Q\to L$ be an isotone $Z$-continuous function. Then there exists a unique $Z$-continuous function $\tilde{f}: \Closure_Z(Q) \to L$ such that
\[ \begin{tikzcd}
...
We need to show that $f: \sSet{P,\Closure_M} \to \sSet{Q, (-)^{\leq\geq}}$ is closure-continuous. Indeed, take $A\subseteq L$. Then $A^{\leq\geq}$ is downwards closed and $f^\preimf(A^{\leq\geq})$ is downwards closed by \ref{isotoneIffDownsetContinuous}. We also have that $A^{\leq\geq}$ is $Z$-closed. Since $f$ is $Z$-continuous, we have that $f^\preimf\big(A^{\leq\geq}\big)$ is $Z$-closed by \ref{ZcontinuityZclosureContinuity}. Thus $f^\preimf\big(A^{\leq\geq}\big)$ is an element of $\mathcal{C}_{\downarrow Z}$, making $f: \sSet{P, \Closure_{\mathcal{C}_{\downarrow Z}}}\to \sSet{L, (-)^{\leq\geq}}$ closure continuous.
