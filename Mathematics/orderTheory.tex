...
\begin{lemma} \label{consistentExtenionsIsotoneFunctionToDedekindPoset}
Let $P$ be a poset, $Q$ a Dedekind-complete poset, $(x,y)\in P\times Q$ and $f\in (P\not\to Q)_\leq$ a partial order-preserving function. Then $\{(x,y)\} \in \{f\}^\cons$ \textup{if and only if}
\[ \bigvee f^\imf\big(\downset \{x\} \cap \dom(f)\big) \leq y \leq \bigwedge f^\imf\big(\upset \{x\} \cap \dom(f)\big). \]
\end{lemma}
\begin{proof}
Since any restriction of an order-preserving function is order-preserving, we have that $\{(x,y)\} \cons f$ iff $f\cup \{(x,y)\}$ is order-preserving. This is equivalent to
\[ \begin{cases}
\forall x'\in \downset\{x\} \cap \dom(f): \; f(x') \leq y \\
\forall x'\in \upset\{x\} \cap \dom(f): \; f(x') \geq y.
\end{cases} \]
Since $Q$ is Dedekind complete, this is equivalent to 
\[ \begin{cases}
\bigvee f^\imf\big(\downset\{x\} \cap \dom(f)\big) \leq y \\
\bigwedge f^\imf\big(\upset\{x\} \cap \dom(f)\big) \geq y.
\end{cases} \]
\end{proof}
...
\item $\tilde{f}$ is the the consistent extension of $f$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
\end{align*}
where we have used \ref{consistencyFromAtomsOrderPreservingPartialFunctions}, \ref{consistentExtenionsIsotoneFunctionToDedekindPoset} and points (1) and (2).
...
By \ref{consistentExtenionsIsotoneFunctionToDedekindPoset}, we have $\{(x,a)\} \in \{f\}^\cons = \{g\}^\cons$ and $\{(x,b)\}\in \{f\}^\cons = \{g\}^\cons$. Since $x\in \dom(g)$, this implies $a = g(x) = b$ and so $x\in\dom(\tilde{f})$, with $g(x) = \tilde{f}(x)$.
\end{proof}
