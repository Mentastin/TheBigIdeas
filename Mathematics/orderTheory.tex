...
TODO: minimal / maximal: $X^{R\cup \overline{R}^\transp}$ (if anti-asymmetric = $X^{\overline{R}^\transp}$).
...
\end{corollary}
\begin{proof}
(1) $A^l \subseteq A^l \implies A \subseteq (A^l)^u$.
...
Let $S\subset L$. Then
\begin{itemize}
\item the commonality $S^\perp$ is called the \udef{disjoint complement} of $S$;
\item the commonality $S^{\mesh}$ is called the \udef{grill} of $S$.
\end{itemize}
\end{definition}
...
\section{Grills}
\begin{definition}
Let $X$ be a set. Let $\mathcal{A} \subseteq \powerset(X)$. The \udef{grill} of $\mathcal{A}$ is defined as the commonality $\mathcal{A}^{\mesh}$.
\end{definition}
...
(3) TODO (general for commonalities using $\mesh^\transp = \mesh$ (?)).
