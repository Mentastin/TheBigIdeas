...
\[  f(x\vee y) \geq f(x)\vee f(y) \quad\iff\quad \text{$f$ is order-preserving}. \]
\item If $P,Q$ are $\wedge$-semilattices, then
\[  f(x\wedge y) \leq f(x)\wedge f(y) \quad\iff\quad \text{$f$ is order-preserving}. \]
\end{enumerate} 
\end{lemma}
\begin{proof}
(1) Assume $f(x\vee y) \geq f(x)\vee f(y)$. Then $x\leq y$ implies $y = x\vee y$, which implies $f(x) \leq f(x)\vee f(y) \leq f(x\vee y) = f(y)$.

For the converse: From $x \leq x\vee y$ we get $f(x) \leq f(x\vee y)$. Similarly $f(y) \leq f(x\vee y)$. Together this gives $f(x\vee y) \geq f(x)\vee f(y)$.
