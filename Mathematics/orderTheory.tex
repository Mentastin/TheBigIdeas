...
\begin{proposition}
TODO: for directed set $\lhd$ proper filter, inclusions are equivalent.
\end{proposition}
...
\begin{proposition}
Let $f:X\to Y$ be a function between sets and $F\in \filters(\powerset(X))$ a filter. Then $\upset f[F]$ is a filter on $\powerset(Y)$. If $F$ is an ultrafilter on $\powerset(X)$, then $\upset f[F]$ is an ultrafilter on $\powerset(Y)$.
\end{proposition}
