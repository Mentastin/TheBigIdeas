...
An \udef{initial slice} operation is a function $(-)[0:n]: A^*\to A^*$ for some $n\in\N$.

A \udef{tree} $T$ on $A$ is a set of strings in $A$ (i.e.\ $T\subseteq A^*$) that is closed under all initial slice operations.
\end{definition}
In other words, for any string in a tree $T$, all initial segments are also in $T$.

\begin{lemma}
Let $T$ be a non-empty tree on a set $A$. Then $\seq{}\in T$.
\end{lemma}
\begin{proof}
As $T$ is non-empty, we can find $u\in T$. Now $u[0:0] = \seq{}\in T$ by closure under initial slice operations.
\end{proof}

\begin{definition}
Let $T$ be a tree on an alphabet $A$. We define the \udef{tree order} $\sqsubseteq$ on $T$ by
\[ u \sqsubseteq v \qquad\defequiv\qquad \exists n\in \N: \; u = v[0:n]. \]
We call
\begin{itemize}
\item an infinite chain in $\sSet{T,\sqsubseteq}$ a \udef{branch} of $T$;
\item the set of all branches of $T$ the \udef{body} of $T$, denoted $[T]$;
...
(3) Assume there exists a branch $B\in [T]$. Then clearly $B$ does not have a maximal element, so $\sSet{T,\sqsubseteq}$ is not converse well-founded.
\end{proof}
...
We have that $R|^B_B$ is left-total on $B$. By the axiom of dependent choice we can find some sequence $\seq{b_n}_{n\in \N}$ in $B$.


Recursively define $f: \N\to \powerset(A)$ by $f(0) = aR$ and
\[ f(n+1) = \bigcup_{x\in f(n)}xR. \]
Then set $B = \bigcup_{n\in\N}f(n)$. We have that $R|^B_B$ is left-total on $B$. By the axiom of dependent choice we can find some sequence $\seq{b_n}_{n\in \N}$ in $B$. Then there exists $n\in \N$ such that $b_0\in f(n)$.
