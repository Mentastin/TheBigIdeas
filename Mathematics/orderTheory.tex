...
\begin{lemma} \label{prop:BooleanComplementLargestDisjoint}
Let $L$ be a Boolean lattice and $x\in L$. Then $\overline{x} = \max(\{x\}^\perp)$.
\end{lemma}
\begin{proof}
By definition $\overline{x} \in \{x\}^\perp$. We just need to show that $y\leq \overline{x}$ for all $y\in \{x\}^\perp$, or, equivalently, $\overline{x} \vee y = \overline{x}$. By the uniqueness of the complement, \ref{prop:distributiveComplementUnique}, it is enough to prove that $\overline{x} \vee y$ is a complement. Indeed
\begin{align*}
...
\end{proof}
\begin{corollary}
Let $L$ be a Boolean lattice and $x,y\in L$. Then $x\perp y \iff x \leq \overline{y}$.
\end{corollary}
\begin{corollary}
