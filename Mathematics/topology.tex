...
\[ \inh_\xi(\mathcal{A}) = \bigcap_{\mathcal{A}\lhd F}\lim_\xi F. \]
\end{itemize}
\end{definition}
TODO def inherence not correct.
...
$(3) \Rightarrow (7)$ We may take $F = \upset\{x\}$ and $G = \upset \{y\}$.

$(7) \Rightarrow (3)$ Fix $x\in X$. For all $y\in X$ we take such a filter $G$. Now we have $x\notin \ker\vicinity_\xi(y)$. Indeed

so for all $y\in\{x\}^c$, we have $\{x\}^c \in \vicinity_\xi(y)$ and thus $y\in \inh_\xi(\{x\}^c)$. This means $\{x\}^c \subseteq \inh_\xi(\{x\}^c)$ and thus $\adh_\xi(\{x\}) = \inh_\xi(\{x\}^c)^c \subseteq \{x\}$. The result follows from \ref{singletonAdherence}.
\end{proof}
