...
(3) If $f$ is continuous, it is automatically continuous at $1$. Now assume $f$ continuous at $1$ and let $x\in G$. Then
...
\end{proposition}
\begin{proof}
Let $\sSet{G,\cdot, 1, \xi}$ be a convergence group. We want to show that $\adh_\xi^2(A) = \adh_\xi(A)$ for arbitrary $A\subseteq G$. Take $x\in \adh_\xi^2(A)$. We now want to show that $A \in \vicinity_\xi(x)$
\end{proof}
