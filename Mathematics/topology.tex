...
\end{enumerate}
\end{lemma}
TODO cfr \ref{adherenceElementConvergentFilter}
...
\begin{lemma} \label{adherenceElementConvergentFilter}
Let $\sSet{X,\xi}$ be a convergence space, $F\overset{\xi}{\longrightarrow} x$ a proper filter and $A\in F$. Then $x\in\adh_\xi(A)$.
\end{lemma}
\begin{proof}
We have $\vicinity(x)\subseteq F$ and $F \amesh F$ by \ref{properFilterLemma}. This implies $\vicinity(x)\amesh F$, so $A\in \vicinity(x)^\mesh$. This implies $x\in \adh_\xi(A)$.
\end{proof}
...
$\boxed{\Leftarrow}$ Assume every ultrafilter in $A$ converges in $X$. Then every ultrafilter in $A$ converges in $\adh_\xi(A)$ by \ref{adherenceElementConvergentFilter}.
