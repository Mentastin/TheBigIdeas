...
\begin{lemma} \label{lemma:subspaceClosure}
...
\begin{lemma} \label{lemma:denseSubsetOfDenseSubspaceIsDense}
Let $Y$ be a dense subspace of a topological space $X$. Let $S$ be a dense subset of $Y$. Then $S$ is dense in $X$.
\end{lemma}
\begin{proof}
Let $\overline{S}$ be the closure of $S$ in $X$. Then by \ref{lemma:subspaceClosure} we have $Y = Y\cap \overline{S}$, so $\overline{S} \supseteq Y \supseteq S$, which, taking the closure, implies $\overline{S} \supseteq \overline{Y} \supseteq \overline{S}$. Thus $\overline{S} = \overline{Y} = X$.
\end{proof}
...
\end{proposition}
\begin{corollary} \label{corollary:imageCompactIsClosedBounded}
...
\begin{lemma} \label{lemma:HolderLipschitzContinuity}
...
\begin{lemma} \label{lemma:LipschitzcontinuousContinuous}
...
\begin{proposition} \label{prop:contractionFixedPoint}
...
By completeness it has a limit. Now $T$ is continuous by \ref{lemma:LipschitzcontinuousContinuous}. Then
...
For uniqueness it is enough to note that any fixed point of $f$ is a fixed point of $f^n$, \ref{lemma:fixedPointsMultipleComposition}.
\end{proof}
