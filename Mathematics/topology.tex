...
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be convergence spaces. The continuous preconvergence on $(X\to Y)$ is the weakest preconvergence such that the evaluation map $\evalMap|_{(X\to Y)\times X}: (X\to Y)\times X \to Y$ is continuous.
\end{lemma}
\begin{proof}
...
Now assume $\sSet{Y,\zeta}$ is regular and let $H \overset{\cont_c(X,Y)}{\longrightarrow} f$.
\end{proof}
...
\section{Modifications on the way to topological}
\subsection{Finite depth modification}
\begin{definition}
...
\subsection{Pseudotopological modification}
\begin{definition}
...
\section{Countability modifications}
\subsection{Sequential modification}
\begin{definition}
$\Seq(\xi)$
\end{definition}

\section{(Generalised) compactness modifications}
