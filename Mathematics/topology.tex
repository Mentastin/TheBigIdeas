...
\begin{proposition}
Let $X$ be a set, $F\in \powerfilters(X)$ and $H\in\powerfilters(X^2)$ a symmetric filter. Then
\[ H\subseteq F\otimes F \implies \bigcap_{x\in X} Hx \subseteq F \implies H;H\subseteq F\otimes F. \]
\end{proposition}
\begin{proof}
We have the implications
\begin{align*}
H\subseteq F\otimes F \iff& \forall A\in H: \exists B\in F: B\times B\subseteq A \\
\iff& \forall A\in H: \exists B\in F: \forall x\in B:  B\subseteq Ax \\
\implies& \forall A\in H: \exists B\in F: \exists x\in X:  B\subseteq Ax \\
\implies& \forall A\in H: \exists B\in F: B\subseteq \bigcup_{x\in X} Ax \\
\iff& \bigcap_{x\in X} Hx \subseteq F,
\end{align*}
and
\begin{align*}
\bigcap_{x\in X} Hx \subseteq F
\end{align*}
\end{proof}

\begin{proposition}
Let $X$ be a set. The functions
\[ \powerfilters(X^2) \to \powerset\big(\powerfilters(X)\big): H\mapsto \setbuilder{Hx}{x\in X} \quad\text{and}\quad \powerset\big(\powerfilters(X)\big) \to \powerfilters(X^2): \mathcal{F} \mapsto \bigcap\setbuilder{F\otimes F}{F\in\mathcal{F}} \]
...
\subsection{Smallness and boundedness}
\begin{definition}
Let $\sSet{X,\mathcal{U}}$ be a uniform space, $H\in \mathcal{U}$ and $F\in\powerfilters(X)$. Then $F$ is called \udef{$H$-small} if $H\subseteq F\otimes F$.
\end{definition}

\subsubsection{Total boundedness}
\begin{definition}
Let $\sSet{X,\mathcal{U}}$ be a uniform space and $F\in\powerfilters(X)$. We call $F$ \udef{totally bounded} if for all $H\in\mathcal{U}$, there exists a finite subset $S\subseteq X$ such that
\[ \bigcap_{x\in S}Hx \subseteq F. \]
\end{definition}

\begin{proposition}
Let $\sSet{X,\mathcal{U}}$ be a uniform space and $F\in\powerfilters(X)$. Then $F$ is pre-compact \textup{if and only if} $F$ is totally bounded.
\end{proposition}

\begin{definition}
Let $\sSet{X,\mathcal{U}}$ be a uniform space and $\mathcal{C}$ a cover of $X$. We call $\mathcal{C}$ a \udef{uniform cover} of $\sSet{X,\mathcal{U}}$ if
\[ \forall H\in \mathcal{U}: \exists \seq{C_i}_{i\in I}\in \mathcal{C}^I: \; \bigcup_{i\in I}C_i\times C_i \in H. \]
We call $\mathcal{C}$ a \udef{finite uniform cover} if for all $H\in \mathcal{U}$, the index set $I$ can be taken to be finite.
\end{definition}

\begin{proposition}
Let $\sSet{X,\mathcal{U}}$ be a uniform space. Then $X$ is pre-compact \textup{if and only if} every uniform cover has a finite uniform subcover.
\end{proposition}
\begin{proof}
$\boxed{\Rightarrow}$ Assume $X$ is pre-compact and let $\mathcal{C}$ be a uniform cover of $X$. Now assume, towards a contradiction, that $\mathcal{C}$ does not have a finite subset that covers $X$. Then
\end{proof}

\subsubsection{Boundedness}
\begin{definition}
Let $\sSet{X,\mathcal{U}}$ be a uniform space and $F\in\powerfilters(X)$. We call $F$ \udef{totally bounded} if for all $H\in\mathcal{U}$, there exists a finite subset $S\subseteq X$ and $n\in \N$ such that
\[ \bigcap_{x\in S}(H^n)x \subseteq F. \]
Here $H^n$ means $\underbrace{H;\ldots; H}_{\text{$n$ times}}$.
\end{definition}
...
\begin{proposition}
Let $\sSet{X,\mathcal{U}}$ be a uniform space and $F\in\powerfilters(X)$. Then $F$ is precompactoid iff $F$ is totally bounded.
\end{proposition}
\begin{proof}
TODO!!!!!
\end{proof}
...
\url{https://math.stackexchange.com/questions/1399310/precompactness-and-boundedness-in-uniform-space}
...
\begin{definition}
Let $\sSet{X,d}$ be a metric space and $A\subseteq X$ a subset. The \udef{diameter} of $A$ is defined by
...
\begin{definition}
Let $\sSet{G, \cdot, 1, \xi}$ be a convergence group. Consider the function $\Delta: G\times G\to G: (x,y) \mapsto x\cdot y^{-1}$. Then the \udef{uniform structure} of the group is given by
...
\end{enumerate}
Finally, for all $F\in \powerfilters(G)$ and $x\in G$, we have
...
\subsubsection{Precompact or totally bounded subsets}
\begin{definition}
A precompact subset of a convergence group is also called a \udef{totally bounded} subset.
\end{definition}
