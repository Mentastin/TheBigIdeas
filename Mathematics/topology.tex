...
\[ \setbuilder{H\in \powerfilters\big((X\to Y)^2\big)}{\forall S\in \mathcal{S}:\; \evalMap(H, \Delta_S)\in \mathcal{U}} \]
...
\[ \forall S\in \mathcal{S}:\; \evalMap(\Delta_K, \Delta_S)\in \mathcal{U}. \]
\end{itemize}
In particular:
\begin{itemize}
...
\begin{proposition}
Let $X$ be a set and $\sSet{Y, \mathcal{U}}$ a uniform space. Then $\setbuilder{\{x\}}{x\in X}$-uniform convergence equals pointwise convergence.
\end{proposition}
...
\[ \forall \epsilon >0: \exists \delta >0: \forall x,y\in X: \; d_X(x,y) \leq \delta \implies d_Y(f(x), f(y)) \leq \epsilon. \]
\end{corollary}
\begin{proof}
...
\[ \forall \epsilon >0: \exists N\in\N: \forall m,n \geq N: d(x_m, x_n)\leq \epsilon. \]
\end{lemma}
\begin{proof}
...
\[ \forall \epsilon>0: \exists N\in\N: \forall n\geq N: d(x,x_n)\leq \epsilon. \]
\end{corollary}
