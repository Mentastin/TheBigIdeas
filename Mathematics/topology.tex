...
\[ \evalMap|_{(A\to B)_c\times A}: (A\to B)_c\times A \to B \]
is continuous.
\end{lemma}
\begin{proof}
Take $K\in \powerfilters((A\to B)_c\times A)$ such that $K$ converges to $(f,x)$. Then 
\[ \pi_1^{\imf\imf}(K)\overset{(A\to B)_c}{\longrightarrow} f \qquad\text{and}\qquad \pi_2^{\imf\imf}(K) \overset{\xi}{\longrightarrow} x. \]
By definition of continuous convergence,
\[ \evalMap^{\imf\imf}(\pi_1^{\imf\imf}(K) \otimes \pi_2^{\imf\imf}(K)) \overset{\zeta}{\longrightarrow} f(x). \]
By \ref{filterFactorisationInequality} and monotonicity of the convergence,
\[ \evalMap^{\imf\imf}(\pi_1^{\imf\imf}(K) \otimes \pi_2^{\imf\imf}(K)) \subseteq \evalMap^{\imf\imf}(K) \overset{\zeta}{\longrightarrow} f(x) = \evalMap(f,x). \]
\end{proof}
...
\[ \curry_1: \cont_c(X\times Y, Z) \to \cont_c\big(X, \cont_c(Y,Z)\big) \]
is well-defined and a homeomorphism.
\end{proposition}
\begin{proof}
That this is well-defined, is the last part of \ref{universalPropertyContinuousConvergence}.

Let
\[ \alpha: \big(\cont_c(X\times Y, Z)\times X\big)\times Y \to \cont_c(X\times Y, Z)\times (X\times Y) \]
be the associator homeomorphism constructed in \ref{associatorTernaryProducts}. Then
\[ \evalMap|_{\cont_c(X\times Y, Z)\times (X\times Y)}\circ \alpha: \big(\cont_c(X\times Y, Z)\times X\big)\times Y \to Z \]
is continuous by \ref{evalMapContinuous}.

Now it is straightforward to calculate that $\curry_1^{-1}\big(\curry_1^{-1}(\curry_1)\big) = \evalMap|_{\cont_c(X\times Y, Z)\times (X\times Y)}\circ \alpha$, which is continuous. So $\curry_1$ is continuous by \ref{universalPropertyContinuousConvergence}.
\end{proof}
\begin{corollary}
...
\subsection{Conditional compactness}
\begin{definition}
