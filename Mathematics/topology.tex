...
\subsection{Continuous modification of functions}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space and $\sSet{Y,\zeta}$ a Hausdorff convergence space, $A\subseteq X$ and $f: A\to Y$ a function. The \udef{continuous modification} of $f$ is defined as
\[ \tilde{f}: \setbuilder{(x,y) \in X\times Y}{\exists F\overset{\xi|_A}{\longrightarrow} x \quad\land\quad \forall F\overset{\xi|_A}{\longrightarrow} x: \; f^{\imf\imf}(F)\overset{\zeta}{\longrightarrow} y}. \]
\end{definition}

\begin{lemma}
Let $\sSet{X,\xi}$ be a convergence space and $\sSet{Y,\zeta}$ a Hausdorff convergence space, $A\subseteq X$ and $f: A\to Y$ a function. Let $\tilde{f}$ be the continuous modification of $f$. Then
\begin{enumerate}
\item $\tilde{f}$ is a functional relation;
\item $\tilde{f}: \dom(\tilde{f})\to Y$ is a continuous function;
\item if $f$ is continuous, then $f \subseteq \tilde{f}$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) Take $x\in X$, $y,y'\in Y$ and suppose $x\mathrel{\tilde{f}}y$ and $x\mathrel{\tilde{f}}y'$. Then there exists $F\to x$ and both $f^{\imf\imf}(F)\to y$ and $f^{\imf\imf}(F)\to y'$ hold. Since $Y$ is Hausdorff, this implies $y = y'$.

(2) Take $x\in \dom(\tilde{f})$ and $F\to x$. Then $f^{\imf\imf}(F)\to f(x)$ by definition.

(3) For all $x\in \dom(f)$, we have 
\end{proof}
