...
\[ \adh_\xi(\mathcal{A}) = \bigcup_{\mathcal{A}\amesh F}\lim_\xi F; \]
\item the \udef{inherence} of $\mathcal{A}$ as
\[ \inh_\xi(\mathcal{A}) = \setbuilder{x\in X}{\forall F\in \powerfilters(X): \; x\in \lim_\xi F \implies F\mesh\mathcal{A}}. \]
...
\end{lemma}
\begin{proof}
Take $x\in \ker(F)$. Then $\pfilter{x}\amesh F$, so $x\in \lim_\xi \pfilter{x} \subseteq \adh_\xi(F)$.
\end{proof}
...
Let $\sSet{X,\xi}$ be a convergence space and $F\subseteq \powerfilters(X)$ a proper filter. Then
\begin{enumerate}
...
We have
\begin{align*}
\inh_\xi(\mathcal{A})^c &= \setbuilder{x\in X}{\exists F\in\powerfilters(X): \; \big(x\in \lim_\xi F\big)\land\big(F\perp\mathcal{A}\big)} \\
&= \setbuilder{x\in X}{\exists F\in\powerfilters(X): \; \big(x\in \lim_\xi F\big)\land\big(\forall A\in\mathcal{A}: A\notin F\big)} \\
&= \setbuilder{x\in X}{\exists F\in\powerfilters(X): \; \big(x\in \lim_\xi F\big)\land\big(\forall A\in\mathcal{A}: A^c\subseteq F^\mesh\big)} \\
&= \setbuilder{x\in X}{\exists F\in\powerfilters(X): \; \big(x\in \lim_\xi F\big)\land\big((-^c)^{\imf}(\mathcal{A}) \amesh F\big)} \\
...
\end{proposition}
\begin{proof}
(1) First take $x\in \adh_\xi(A)$. Then, by \ref{differentUnionsForAdherence}, there exist a proper filter $F$ such that $\upset\{A\}\subseteq F$ and $F\to x$. We have $\vicinity_\xi(x)\subseteq F$, so $F^\mesh \subseteq \vicinity_\xi(x)^\mesh$ and $F\subseteq F^\mesh$ by \ref{properSubsemilatticeLemma}. Then
\[ \{A\} \subseteq F\subseteq F^\mesh\subseteq \vicinity_\xi(x)^\mesh, \]
so $A\in \vicinity_\xi(x)^\mesh$.

For the converse, we have
\[ A\in \vicinity_\xi(x)^\mesh = \left(\bigcap_{F\to x} F\right)^\mesh = \bigcup_{F\to x}F^\mesh, \]
by TODO ref
Thus there
\end{proof}
...
\end{itemize}
\end{definition}
In particular we call the convergence space $X$ compact if every ultrafilter converges.
...
\begin{definition}
Let $\sSet{X,d}$ be a metric space. The \udef{metric uniformity} $\mathcal{U}_d$ is the uniformity given by
