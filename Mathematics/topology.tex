...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Take arbitrary $F,G \in \filters(\powerset(X))$ and $x\in X$. The convergence space is called
\begin{itemize}
\item \udef{finitely deep} if $\lim_\xi(F\cap G) = \lim_\xi F \cap \lim_\xi G$;
...
\begin{definition}
Let $X$ be a set and $\xi,\zeta$ preconvergences on $X$. We say $\xi$ is \udef{finer} than $\zeta$, denoted $\xi \leq \zeta$, if $\lim_\xi F \subseteq \lim_\zeta F$ for all $F\in\filters(\powerset(X))$. We also say $\zeta$ is \udef{coarser} than $\xi$.
\end{definition}
...
Let $X$ be a set.
\begin{itemize}
\item The \udef{empty preconvergence} on $X$ is $E_{\filters(\powerset(X)), X}$, i.e.\ the limit of all filters is the empty set.
...
\item The \udef{chaotic convergence} on $X$ is $U_{\filters(\powerset(X)), X}$, i.e.\ the limit of all filters is $X$.
\end{itemize}
\end{definition}
...
A subset $V\subseteq X$ is called a \udef{vicinity} of $x$ for $\xi$ if $V\in \vicinity_\xi(x)$.
\end{definition}

\begin{lemma}
Let $\sSet{X,\xi}$ be a convergence space, $x\in X$ and $V\in\vicinity_\xi(x)$ a vicinity of $x$. Then $x\in V$.
\end{lemma}
This also means $\{x\}\in\vicinity_\xi(x)^{\mesh}$.
\begin{proof}
We have $\vicinity_\xi(x) \subseteq \pfilter{x}$, so $V\in \pfilter{x}$ and thus $\{x\} \subseteq V$ or $x\in V$. 
\end{proof}
...
\subsubsection{Pretopological convergence}
...
\begin{proposition}
Let $\sSet{X,\xi}$ be a convergence space and $F\subseteq \filters(\powerset(X))$ a filter. Then $\ker(F) \subseteq \adh_\xi(F)$
\end{proposition}
\begin{proof}
TODO
\end{proof}
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space and $A\subseteq X$. A point $x\in X$ is called an \udef{accumulation point} of $A$ if $x\in \adh_\xi(A\setminus\{x\})$.
\end{definition}
...
\begin{definition}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be convergence spaces. A function $f: X\to Y$ is called \udef{continuous} if it preserves limits: for all $D \in \directed(\powerset(X))$ and $x\in X$:
\[ D \overset{\xi}{\longrightarrow} x \implies f[D] \overset{\zeta}{\longrightarrow} f(x). \]
...
\begin{lemma}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be convergence spaces. A function $f: X\to Y$ is continuous \textup{if and only if} for all $D \in \directed(\powerset(X))$
\[ f\left[\lim_\xi D \right] \subseteq \lim_\zeta f[D]. \]
\end{lemma}
\begin{proof}
...
We calculate
\begin{align*}
f[\adh_\xi(A)] &= f\left[\bigcup \setbuilder{\lim_\xi F}{A \lhd F} \right] \\
&= \bigcup f\left[ \setbuilder{\lim_\xi F}{A \lhd F} \right] = \bigcup \setbuilder{f\left[\lim_\xi F\right]}{A \lhd F} \\
&\subseteq \bigcup \setbuilder{\lim_\zeta f[F]}{A \lhd F} \subseteq \bigcup \setbuilder{\lim_\zeta f[F]}{f[A] \lhd f[F]} \\
&\subseteq \bigcup \setbuilder{\lim_\zeta G}{f[A] \lhd G} = \adh_\zeta(f[A]),
\end{align*}
where we have used that $A \lhd B$ implies $f[A] \lhd f[B]$ (TODO ref).
\end{proof}

\begin{proposition} \label{continuityConstructions}
...
(3) Let $F\to x\in X$. Then $f[F] \to f(x)$ by continuity of $f$ and $g[f[F]] \to g(f(x))$ by continuity of $g$. So $g\circ f$ is continuous.
\end{proof}
...
\subsection{Initial and final convergences}
\begin{definition}
Let $X, Y$ be sets and $F\subseteq (X\to Y)$ a set of functions.
\begin{itemize}
\item Given a convergence $\zeta$ on $Y$, the coarsest convergence on $X$ that makes all functions in $F$ continuous is the \udef{initial convergence} $\mu$ w.r.t. $F$:
\[ \mu = \bigvee \setbuilder{\sigma}{F \subseteq \cont(\sigma, \zeta)}; \]
\item Given a convergence $\xi$ on $X$, the finest convergence on $Y$ that makes all functions in $F$ continuous is the \udef{final convergence} $\nu$ w.r.t. $F$:
...
\begin{proposition}[Characteristic property] \label{characteristicPropertyInitialFinalConvergence}
...
\end{corollary}
\begin{proof}
$\Rightarrow$ is clear: the composition of continuous functions is continuous (\ref{continuityConstructions}).
...
TODO filter converges iff all its components do (pointwise)

\begin{lemma} \label{continuousEmbeddingProduct}
Let $\sSet{X,\xi}$ and $\sSet{Y,\sigma}$ be convergence spaces. Then for all $y\in Y$, $X\to X\times Y: x\mapsto (x,y)$ is continuous.
\end{lemma}
\begin{proof}
From \ref{characteristicPropertyInitialFinalConvergence}, using the fact that $\id_X$ and $\underline{y}$ are continuous.
\end{proof}
...
\begin{definition}
A convergence space $\sSet{X,\xi}$ is called \udef{regular} if $x \mapsto \adh_\xi(\{x\})$ is continuous.
\end{definition}

\subsection{Separation properties}
\subsubsection{Distinguishability and separation}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space and $x,y\in X$. We call $x$ and $y$ \udef{distinguishable} if ${\lim_\xi}^{-1}(x) \neq {\lim_\xi}^{-1}(y)$.
\end{definition}

In orther words, $x,y \in X$ are distinguishable if there exists a filter $F \in \filters(\powerset(X))$ such that
...
Let $X$ be a set, $\xi$ a convergence of finite depth on $X$ and $x,y\in X$. Then $x$ and $y$ are indistinguishable \textup{if and only if}
...
\subsubsection{$R_0$ or symmetric}
\begin{definition}
...
\end{enumerate}
The following are consequences of the above. If $\xi$ is of finite depth, then they are also equivalent:
\begin{enumerate} \setcounter{enumi}{4}
...
$(5) \Rightarrow (6)$ Indistinguishability is an equivalence relation.

$(6) \Rightarrow (7)$ Equivalence relations are symmetric.

$(7) \Rightarrow (1)$ Assume $\xi$ of finite depth and $x,y$ distinguishable. Then $\pfilter{x}\not\to y$ or $\pfilter{y}\not\to x$ by \ref{distinguishabilityPrincipalUltrafilters}. Because of (8), the ``or'' becomes an ``and'' and we conclude with \ref{separatednessPrincipalUltrafilters}.
\end{proof}
...
\subsubsection{$R_1$ or reciprocal}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then $\xi$ is called \udef{$R_1$} or \udef{reciprocal} if
\[ {\lim_\xi}^{-1}(x)\perp {\lim_\xi}^{-1}(y) \]
any two distinguishable points $x,y\in X$.
\end{definition}
...
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ If $\lim_\xi^{-1}(x)\perp \lim_\xi^{-1}(y)$, then $x$ and $y$ are definitely distinguishable, e.g.\ by $\pfilter{x}$.

$(2) \Rightarrow (3)$ All pairs of points are either distinguishable, or indistinguishable. In the first case we have $\lim_\xi^{-1}(x)\perp \lim_\xi^{-1}(y)$, in the second $\lim_\xi^{-1}(x) = \lim_\xi^{-1}(y)$.

$(3) \Rightarrow (4)$ From $F\to x$ and $F\to y$, we get $F\in \lim_\xi^{-1}(x)$ and $F\in \lim_\xi^{-1}(y)$, so $\lim_\xi^{-1}(x)\mesh \lim_\xi^{-1}(y)$. Using (3) this implies $\lim_\xi^{-1}(x) = \lim_\xi^{-1}(y)$.
...
$(3) \Leftrightarrow (4)$ $R_1$ implies $R_0$ and $T_0+T_0$ is equivalent with $T_1$.
\end{proof}
...
Now let $X$ be a finite set and let $F$ be a proper filter in $\filters(\powerset(X))$. Then $F$ is principal by \ref{finiteFiltersPrincipal} and not free because it is proper. So we can take $x\in \ker F$ and $F \subseteq \pfilter{x}$. Thus $\lim F \subseteq \lim \pfilter{x} \supseteq \{x\}$, meaning the convergence is $T_2$.
\end{proof}
\begin{corollary}
...
\begin{definition}
A convergence space $\sSet{X, \xi}$ is called \udef{first countable} or \udef{$C1$}
\end{definition}
...
\[ \mathcal{F} \defeq \setbuilder{F\in \filters(\powerset(X))}{F\otimes F \in \mathcal{J}} \]
...
\begin{definition}
Let $\sSet{X, \mathcal{J}}$ be a uniform structure. We say a filter $F \in \filters(\powerset(X))$ converges \udef{uniformly} to $x\in X$ if it is Cauchy and converges to $x$ in the induced Cauchy structure. We write $F\overset{u}{\to} x$.
...
\begin{proposition}
Let $\sSet{X, \mathcal{J}}$ be a (finite depth TODO!) uniform structure, $F\in \filters(\powerset(X))$ and $x\in X$. Then $F\overset{u}{\to} x$ \textup{if and only if} $F \otimes \pfilter{x} \in \mathcal{J}$.
\end{proposition}
\begin{proof}
TODO
...
(3) We calculate
\begin{align*}
\vicinity_\xi(x) &= \bigcap \setbuilder{F\in \filters(\powerset(X))}{x\in \lim_\xi F} \\
&= \bigcap \setbuilder{F\in \filters(\powerset(X))}{1\in \lim_\xi F\cdot x^{-1}} \\
&= \bigcap \setbuilder{G\cdot x\in \filters(\powerset(X))}{1\in \lim_\xi G} \\
&= \bigcap \setbuilder{G\in \filters(\powerset(X))}{1\in \lim_\xi G}\cdot x = \vicinity_\xi(1)\cdot x.
\end{align*}
...
\end{proposition}
\begin{proof}
Let $\sSet{G,\cdot, 1, \xi}$ be a convergence group. We want to show that $\adh_\xi^2(A) = \adh_\xi(A)$ for arbitrary $A\subseteq G$. Take $x\in \adh_\xi^2(A)$. We now want to show that $A \in \vicinity_\xi(x)^{\mesh}$
\end{proof}
...
\begin{definition}
Let $\sSet{G, \cdot, 1, \xi}$ be a convergence group. The family
\[ \mathcal{F} \defeq \setbuilder{F\in \filters(\powerset(G))}{F\cdot F^{-1} \overset{\xi}{\to} 1} \]
makes $\sSet{G, \mathcal{F}}$ a Cauchy space.
\end{definition}

\begin{lemma}
Let $\sSet{G, \cdot, 1, \xi}$ be a convergence group. Then $\sSet{G, \mathcal{F}}$ is a Cauchy space that is compatible with the convergence structure. If $G$ is of finite depth, then so is the Cauchy space $\sSet{G, \mathcal{F}}$.
\end{lemma}
\begin{proof}
We have $\pfilter{x}\cdot \pfilter{x}^{-1} \supseteq \pfilter{1}$, so $\pfilter{x}\cdot \pfilter{x}^{-1} \to 1$ and thus $\pfilter{x} \in \mathcal{F}$.
...
The claim of finite depth follows TODO????????????????????????????
\end{proof}

TODO: consequence of uniformalisibility?
\begin{lemma}
Let $\sSet{G, \cdot, 1, \xi}$ be a convergence group.
\end{lemma}
...
\end{proposition}
\begin{proof}
Let $\sSet{G, \cdot, 1, \xi}$ be a locally compact convergence group and let $F$ be a Cauchy filter. Then $F\cdot F^{-1} \to 1$, so there exists a compact set $K\subseteq F\cdot F^{-1}$
\end{proof}