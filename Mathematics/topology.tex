...
\begin{definition}
Let $X$ be a set, $\xi$ a preconvergence on $X$ and $A\subseteq X$. The \udef{vicinity filter} of $\xi$ at $A$ is
\[ \vicinity_\xi(A) \defeq \bigcap \setbuilder{F\in \powerfilters(X)}{A\mesh \lim_\xi F}. \]
A subset $V\subseteq X$ is called a \udef{vicinity} of $A$ for $\xi$ if $V\in \vicinity_\xi(A)$.

In particular we define the vicinity filter at a point $x\in X$ as $\vicinity_\xi(x) \defeq \vicinity_\xi(\{x\})$.
\end{definition}
...
Let $\sSet{X,\xi}$ be a convergence space and $A,B\subseteq X$. Then the following are equivalent:
\begin{enumerate}
\item $A,B$ are separated by vicinities;
\item $\lim^{-1}(A)\perp \lim^{-1}(B)$.
\end{enumerate}
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ Assume $A,B$ are separated by vicinities $U$ and $V$. Assume, towards a contradiction, that $\lim^{-1}(A)\mesh \lim^{-1}(B)$, i.e.\ there exists a filter $F$ that converges to $x\in A$ and $y\in B$. Then $U,V\in F$ and thus $U\cap V = \emptyset \in F$, meaning $F$ is not a proper filter.

$(2) \Rightarrow (1)$ Now assume $A,B$ are not separated by vicinities. Then $\vicinity_\xi(A)\mesh \vicinity_\xi(B)$, meaning $\vicinity_\xi(A)\vee \vicinity_\xi(B)$ is a proper filter by \ref{joinProperFilter}. Now by \ref{infiniteDistributivityFiltersIdeals} we can calculate
\begin{align*}
\vicinity_\xi(A)\vee \vicinity_\xi(B) &= \left(\bigwedge \setbuilder{F\in\powerfilters(X)}{A\mesh \lim F}\right)\vee \left(\bigwedge \setbuilder{G\in\powerfilters(X)}{B\mesh \lim G}\right) \\
&= \bigwedge_{\substack{F,G\in \powerfilters(X)\\ A\mesh \lim F \\ B\mesh \lim G}} F\vee G,
\end{align*}
and in order for the right-hand side to be proper, $F\vee G$ must be a proper filter for some $F,G$. Now $\lim F\vee G \supseteq \lim F \cup \lim G$, so $F\vee G\in\lim^{-1}(A)\cap \lim^{-1}(B)$. Thus $\lim^{-1}(A)\mesh \lim^{-1}(B)$.
\end{proof}
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then $\xi$ is called \udef{$R_1$}, \udef{reciprocal} or \udef{preregular} if all distinguishable points are separated by vicinities.
\end{definition}
...
\end{proposition}
\begin{proof}
$(1) \Leftrightarrow (2)$ \ref{separationByVicinitiesEquivalences}.
...
\item for all $A\subseteq X$ and $x\notin \adh_\xi(A)$: $x$ and $\adh_\xi(A)$ are separated by vicinities.
\end{enumerate}
\end{proposition}
\begin{proof}
