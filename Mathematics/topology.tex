...
\begin{lemma}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be convergence spaces and $H\in \powerfilters(X\to Y)$ such that $H\overset{(X\to Y)_c}{\longrightarrow} f$ and $\cont(X,Y)\in H$. Then $f\in\cont(X,Y)$.
\end{lemma}
\begin{proof}
...
Set $X = \{a,b\}$ and $Y = \{1,2\}$. Let $X$ have the convergences
\[ \begin{cases}
\pfilter{a} \to a \\
\pfilter{b} \to b \\
\pfilter{b} \to a
\end{cases} \]
and let $Y$ have the convergences
\[ \begin{cases}
\pfilter{1} \to 1 \\
\pfilter{1} \to 2 \\
\pfilter{2} \to 2.
...
\end{cases}$. Then it is straightforward to verify that $\pfilter{g} \overset{(X\to Y)_c}{\longrightarrow} f$, but $f$ is not continuous, indeed $\pfilter{b} \to a$, but $f^{\imf\imf}[\pfilter{b}] = \pfilter{2} \not\to f(a) = 1$.
\end{example}
