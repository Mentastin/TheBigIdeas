\chapter{Convergence}
\begin{definition}
Let $X$ be a set and let $R$ be a relation between filters in $\powerset(X)$ and elements of $X$. Then
\begin{itemize}
\item we denote the image function of $R$ as $\lim_R: \filters(\powerset(X)) \to \powerset(X): F\mapsto FR$; we call $\lim_R F$ the \udef{$R$-limit} of $F$;
\item $R$ is called a \udef{preconvergence} on $X$ if $\lim_R$ is order-preserving when $\filters(\powerset(X))$ is ordered by inclusion:
\[ F \subseteq G \implies \lim_R F \subseteq \lim_R G; \]
\item $R$ is called a \udef{convergence} if it is a preconvergence and it is \udef{centered}: for all $x\in X: x \in \lim_R \upset\{x\}$;
\item a (pre)convergence $R$ is called \udef{finitely deep} if 
\[ \forall F,G \in \filters(\powerset(X)): \quad \lim_R(F\cap G) = \lim_R F \cap \lim_R G. \]
\end{itemize}
If $R$ is a convergence, then we call $\sSet(X, R)$ a \udef{convergence space}.
\end{definition}

\begin{lemma}
Let $X$ be a set and $R$ a preconvergence on $X$. Then $R$ is finitely deep \textup{if and only if}
\[ \forall F,G \in \filters(\powerset(X)): \quad \lim_R(F\cap G) \supseteq \lim_R F \cap \lim_R G. \]
\end{lemma}
\begin{proof}
The inclusion $\lim_R(F\cap G) \subseteq \lim_R F \cap \lim_R G$ follows from \ref{orderPreservingFunctionLatticeOperations} because a preconvergence is order-preserving.
\end{proof}
...
\item The \udef{discrete preconvergence} $\iota_X$ on $X$ is defined by $x \in \lim_\iota F \iff F = \upset\{x\}$.
\item The \udef{chaotic preconvergence} on $X$ is $U_{\filters(\powerset(X)), X}$, i.e. the limit of all filters is $X$.
\end{itemize}
\end{definition}

\subsection{The lattices of preconvergences and convergences}
\begin{definition}
Let $X$ be a set and $R,S$ preconvergences on $X$. We say $R$ is \udef{finer} than $S$, denoted $R \leq S$, if $\lim_R F \subseteq \lim_S F$ for all $F\in\filters(\powerset(X))$. We also say $S$ is \udef{coarser} than $R$.
\end{definition}

\begin{lemma}
Let $X$ be a set and $R$ a preconvergence on $X$. Then
\begin{enumerate}
\item $E_{\filters(\powerset(X)), X} \leq R \leq U_{\filters(\powerset(X)), X}$;
\item $R$ is a convergence \textup{if and only if} $\iota_X \leq R$.
\end{enumerate}
\end{lemma}
...
\begin{definition}
Let $X$ be a set and $R$ a preconvergence on $X$. Then $R$ is called \udef{Kolmogorov} or \udef{$T_0$} if every pair of points in $X$ is distinguishable, that is for all $x,y\in X$:
\[ x\neq y \implies {\lim_R}^{-1}[\{x\}] \neq {\lim_R}^{-1}[\{y\}]. \]
\end{definition}

\subsubsection{$T_1$ or Fréchet}
\begin{definition}
Let $X$ be a set and $R$ a preconvergence on $X$. Then $R$ is called \udef{Fréchet} or \udef{$T_1$} if for all $x\in X$: $\lim_R \upset\{x\} \subseteq \{x\}$.
\end{definition}
Clearly a \emph{convergence} is $T_1$ if and only if
\[ \forall x\in X: \quad \lim_R\upset\{x\} = \{x\}. \]

\begin{lemma}
If $X$ is a set and $R$ is a $T_1$ convergence on $X$, then $R$ is $T_0$.
\end{lemma}
\begin{proof}
...
\begin{definition}
Let $X$ be a set and $R$ a preconvergence on $X$. Then $R$ is called \udef{Hausdorff} or \udef{$T_2$} if every $R$-limit contains at most one point.
\end{definition}
