...
Finally consider $U\cdot \pi_2^{\imf\imf}(U')^{-1}$, which converges to $xb^{-1}$. If we can show $xb^{-1}\in A$, then $x\in A\cdot B$ and we are done. As $A$ is closed, it is enough to show that $A\in \big(U\cdot \pi_2^{\imf\imf}(U')^{-1}\big)^\mesh$:
\[ A\in \big(U\cdot \pi_2^{\imf\imf}(U')^{-1}\big)^\mesh \subseteq \vicinity(xb^{-1})^\mesh \quad\implies\quad xb^{-1}\in \adh(A) = A. \]
To complete the proof, take $C = D\cdot E^{-1} \in U\cdot \pi_2^{\imf\imf}(U')^{-1}$. We can then write $C \supseteq \cdot^{\imf}(F)$, where $F\in U'$. WLOG we can take $F\subseteq A\times E$, by replacing it with $F\cap (A\times E)$. Then $C \supseteq \cdot^{\imf}(F) \cdot E^{-1}$ contains some elements of $A$: if $(a,x)\in F$, then $a\in C$, because $x^{-1}\in E^{-1}$.
\end{proof}
