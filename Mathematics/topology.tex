...
\section{Uniform relations}
\begin{definition}
Let $X$ be a sets. A \udef{uniformity relation} on $X$ is a relation $R_\mathcal{U}$ on $\powerfilters(X)^2$ such that
\begin{itemize}
...
\item $R_\mathcal{U}$ is symmetric and transitive when restricted to $\powerfilters(X)\setminus\{\powerset(X)\}$.
\end{itemize}
We call the pair $\sSet{X,R_\mathcal{U}}$ a \udef{uniform relation space}.
\end{definition}

\begin{lemma} \label{uniformityMeshingFilters}
Let $\sSet{X,R_\mathcal{U}}$ be a uniform relation space and $F,G\in\powerfilters(X)$. If $F\amesh G$, then $F\mathrel{\mathcal{U}} G$.
\end{lemma}
\begin{proof}
If $F\amesh G$, then $F\vee G \neq \powerset(X)$ by \ref{joinProperFilter}. By upwards closure and symmetry, $F\mathrel{R_\mathcal{U}} (F\vee G)$ and $G\mathrel{R_\mathcal{U}} (F\vee G)$. By transitivity and symmetry, $F\mathrel{R_\mathcal{U}} G$.
\end{proof}
...
\begin{lemma} \label{filterFactorisationLemma}
Let $X$ be a set and $H\in \mathcal{F}$. Then
\begin{enumerate}
\item $H;H^\transp = p_1^{\imf\imf}(H)\otimes p_1^{\imf\imf}(H)$;
\item $p_1^{\imf\imf}(H)\otimes p_2^{\imf\imf}(H) = p_1^{\imf\imf}(H)\otimes p_1^{\imf\imf}(H); H; p_2^{\imf\imf}(H)\otimes p_2^{\imf\imf}(H)$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) If $H = \powerset(X^2)$, then the lemma is clearly true. Now assume $H$ proper. For all $A,B\in H$ we claim that
\[ p_1^\imf[A\cap B]\times p_1^\imf[A\cap B] \subseteq A;B^\transp \subseteq p_1^\imf[A]\times p_1^\imf[B]. \]
This would imply $H;H^\transp = p_1^{\imf\imf}(H)\otimes p_1^{\imf\imf}(H)$.

The second inclusion is immediate from $A;B^\transp \subseteq p_1^\imf[A]\times p_2^\imf[B^\transp] = p_1^\imf[A]\times p_1^\imf[B]$.

For the first inclusion we have
\[ p_1^\imf[A\cap B]\times p_1^\imf[A\cap B] = (A\cap B);(A\cap B)^\transp \subseteq A;B^\transp. \]

(2) The inclusion $\supseteq$ follows from \ref{componentInclusionsFilterComposition}.

For the other inclusion, take $A,B,C,D,E\in H$. We need to find $F,G\in H$ such that
\[ p_1^\imf(F)\times p_2(G) \subseteq p_1^\imf(A)\times p_1^\imf(B); C;p_2^\imf(D)\times p_2^\imf(E). \]
Take $F= A\cap B\cap C \cap D\cap E = G$ and take $(a,a')\in F$ and $(b', b)\in G$ such that $(a,b)\in p_1^\imf(F)\times p_2(G)$. Then we have $(a,a)\in p_1^\imf(A)\times p_1^\imf(B)$, so $(a,a')\in p_1^\imf(A)\times p_1^\imf(B); C$. Now $(a,a')\in D$ and $(b',b)\in E$, so $(a', b)\in p_2^\imf(D)\times p_2^\imf(E)$. Thus finally we have
\[ (a,b) \in p_1^\imf(A)\times p_1^\imf(B); C;p_2^\imf(D)\times p_2^\imf(E). \]
\end{proof}
...
We call a uniformity $\mathcal{U}$ on $X$ \udef{diagonal} if $\upset\{\Delta\} \in \mathcal{U}$, where $\Delta = \setbuilder{(x,x)}{x\in X}$.
\end{definition}

\begin{lemma} \label{uniformFilterSetFactorisable}
Let $X$ be a set, $\mathcal{F}$ a uniform filter set for $X$ and $H\in \mathcal{F}$. Then
\begin{enumerate}
\item $p_1^{\imf\imf}(H)\otimes p_1^{\imf\imf}(H) \in \mathcal{F}$;
\item $p_2^{\imf\imf}(H)\otimes p_2^{\imf\imf}(H) \in \mathcal{F}$;
\item $p_1^{\imf\imf}(H)\otimes p_2^{\imf\imf}(H) \in \mathcal{F}$.
\end{enumerate} 
\end{lemma}
\begin{proof}
(1) Because $H;H^\transp\in\mathcal{F}$, this follows straight from point (1) of \ref{filterFactorisationLemma}.

(2) Similar, by applying point (1) of \ref{filterFactorisationLemma} to $H^\transp$.

(3) Follows from point (2) of \ref{filterFactorisationLemma}.
\end{proof}

\begin{proposition}
Let $X$ be a set and $\mathcal{F}$ a uniform filter set on $X$. We define a uniform structure $\Theta(\mathcal{F})$ on $X$ by
\[ \forall F,G\in\powerfilters(X)\setminus\{\powerset(X)\}:\qquad F\mathrel{\Theta(\mathcal{F})}G \quad\defequiv\quad F\otimes G\in \mathcal{F}.  \]
The function
\[ \Theta: \{\text{uniform filter sets on $X$}\} \to \{\text{uniformities on $X$}\} \]
is an order isomorphism.
\end{proposition}
\begin{proof}
We claim $\Theta$ has an inverse, $\Theta^{-1}$ defined by
\[ \forall H\in \powerfilters(X^2):\qquad H\in\Theta^{-1}(\mathcal{U}) \quad\defequiv\quad p_1^{\imf\imf}[H]\mathrel{\mathcal{U}}p_2^{\imf\imf}[H]. \]

Clearly $\Theta$ and $\Theta^{-1}$ preserve order.

We verify both $\Theta$ and $\Theta^{-1}$ are well defined.

The prove $\Theta(\mathcal{F})$ is a uniformity, we verify the conditions:
\begin{itemize}
\item From $\pfilter{x}\otimes \pfilter{x}\in\mathcal{F}$, we get $\pfilter{x}\mathrel{\Theta(\mathcal{F})}\pfilter{x}$.
\item Assume $F\mathrel{\Theta(\mathcal{F})}G$ and $G\subseteq G'$. Then $F\otimes G\in \mathcal{F}$ and $F\otimes G\subseteq F\otimes G'$. By upwards closure, $F\otimes G'\in\mathcal{F}$.
...
\end{itemize}
The prove $\Theta^{-1}(\mathcal{U})$ is a uniform filter set, we verify the conditions:
\begin{itemize}
\item From $\pfilter{x}\mathrel{\mathcal{U}}\pfilter{x}$, we get $\pfilter{x}\otimes \pfilter{x}\in\Theta^{-1}(\mathcal{U})$.
\item If $H\in\Theta^{-1}(\mathcal{U})$ and $H\subseteq H'$, then $p_1^{\imf\imf}[H]\subseteq p_1^{\imf\imf}[H']$ and $p_2^{\imf\imf}[H]\subseteq p_2^{\imf\imf}[H']$. Thus by upwards closure of $\mathcal{U}$, we have $p_1^{\imf\imf}[H']\mathrel{\mathcal{U}} p_1^{\imf\imf}[H']$ and so $H'\in\Theta^{-1}(\mathcal{U})$.
\item Take $H\in \Theta^{-1}(\mathcal{U})$. Then
\[ p_1^{\imf\imf}[H]\mathrel{\mathcal{U}} p_1^{\imf\imf}[H] \iff p_2^{\imf\imf}[H^\transp]\mathrel{\mathcal{U}} p_1^{\imf\imf}[H^\transp] \iff p_1^{\imf\imf}[H^\transp]\mathrel{\mathcal{U}} p_2^{\imf\imf}[H^\transp] \iff H^\transp\in \Theta^{-1}(\mathcal{U}). \]
\item Take $H_1, H_2\in \Theta^{-1}(\mathcal{U})$. If $H_1;H_2 = \powerset(X^2)$, then $H_1;H_2\in\Theta^{-1}(\mathcal{U})$ by upwards closure. If $H_1;H_2 \neq \powerset(X^2)$, then $p_2^{\imf\imf}[H_1]\amesh p_1^{\imf\imf}[H_2]$ by \ref{meshingFilterComposition} and thus $p_2^{\imf\imf}[H_1]\mathrel{\mathcal{U}} p_1^{\imf\imf}[H_2]$ by \ref{uniformityMeshingFilters}. So we have, by definition of $\Theta^{-1}(\mathcal{U})$
\[ p_1^{\imf\imf}[H_1]\;\mathrel{\mathcal{U}} \;p_2^{\imf\imf}[H_1] \;\mathrel{\mathcal{U}} \;p_1^{\imf\imf}[H_2]\; \mathrel{\mathcal{U}} \;p_2^{\imf\imf}[H_2]. \]
By \ref{componentInclusionsFilterComposition} and upward closure, we get $p_1^{\imf\imf}[H_1;H_2]\mathrel{\mathcal{U}}p_2^{\imf\imf}[H_1;H_2]$, which means $H_1;H_2\in \Theta^{-1}(\mathcal{U})$.
\end{itemize}

Finally we show $\Theta$ and $\Theta^{-1}$ are inverses of each other. First take a uniform filter set $\mathcal{F}$. Then, using \ref{uniformFilterSetFactorisable}, we have
\begin{align*}
H\in \mathcal{F} &\iff p_1^{\imf\imf}[H]\otimes p_2^{\imf\imf}[H] \in \mathcal{F} \\
&\iff p_1^{\imf\imf}[H]\mathrel{\Theta(\mathcal{F})} p_2^{\imf\imf}[H] \\
&\iff H\in \Theta^{-1}(\Theta(\mathcal{F})).
\end{align*}
Similarly, for any uniformity $\mathcal{U}$ on $X$,
\begin{align*}
F\mathrel{\mathcal{U}}G &\iff p_1^{\imf\imf}[F\otimes G]\mathrel{\mathcal{U}}p_2^{\imf\imf}[F\otimes G] \\
&\iff F\otimes G \in \Theta^{-1}(\mathcal{U}) \\
&\iff F\mathrel{\Theta(\Theta^{-1}(\mathcal{U}))} G.
\end{align*}
\end{proof}
...
\begin{definition}
Let $\sSet{X, \mathcal{U}}$ be a uniform space. Then
\[ \entourage_\mathcal{U} \defeq \bigcap \setbuilder{F\otimes G}{F\mathrel{\mathcal{U}}G} \]
is called the \udef{entourage filter} of $\mathcal{U}$ and the elements of $\entourage_\mathcal{U}$ are called \udef{entourages}.

We call the uniform space \udef{topological} if $\mathcal{U} = \Theta\big(\upset\{\entourage_\mathcal{U}\}\big)$.
\end{definition}

\begin{lemma}
Let $\sSet{X,\mathcal{U}}$ be a uniform space. Then $\mathcal{U}$ is topological \textup{if and only if}
\[ F\mathrel{\mathcal{U}}G \qquad\iff\qquad \entourage_\mathcal{U}\subseteq F\otimes G. \]
\end{lemma}
...
\end{enumerate}
Any filter in $\powerfilters(X\times X)$ satisfying these properties is the entourage filter of a topological uniformity.
\end{lemma}
\begin{proof}
(1) We have
\[ \mathcal{E} = \bigcap \setbuilder{F\otimes G}{F\mathrel{\mathcal{U}}G} \subseteq \bigcap \setbuilder{\pfilter{x}\otimes \pfilter{x}}{x\in X} = \upset\big\{\setbuilder{(x,x)}{x\in X}\big\}. \]

(2) We have
\[ \mathcal{E}^\transp = \bigcap \setbuilder{F\otimes G}{F\mathrel{\mathcal{U}}G}^\transp = \bigcap \setbuilder{G\otimes F}{G\mathrel{\mathcal{U}}F} = \mathcal{E}, \]
...
\begin{align*}
\entourage = \bigcap \setbuilder{F\otimes G}{F\mathrel{\mathcal{U}}G} &\subseteq \bigcap \setbuilder{(F_1\otimes G_1);(F_2\otimes G_2)}{F_1\mathrel{\mathcal{U}}G_1, F_2\mathrel{\mathcal{U}}G_2}.
\end{align*}
Take $A\in \entourage$. For each Cauchy filter $F$, we can find $B_{F},C_{F}\in F\otimes F$ such that $B_{F};C_{F} \subseteq A$. Take $B \defeq \bigcup_{F\,\text{Cauchy}} B_F$
...
Take $A\in \entourage$. For each Cauchy filter $F$, we can find $C_{F},D_{F}\in F\otimes F$ such that $C_{F};D_{F} \subseteq A$. Consider $E_F \defeq (C_F\cap D_F)\cup \Delta \in F\otimes F$. Then $E_F;E_F\subseteq A$ by (1). By upwards closure, we have that $E \defeq \bigcup_{F\,\text{Cauchy}} E_F$ is an element of all Cauchy filters $F$. We now show that $E;E\subseteq A$. Indeed, take $(a,b)\in E;E$. Then there exists $x$ such that $(a,x),(x,b)\in E$. There exist $E_F, E_G$ such that $(a,x)\in E_F$ and $(x,b)\in E_G$. Then $(a,x)\in C_G^\transp, D_G$, so $(x,x)\in C_G;D_G \subseteq A$.


Define $E_{F,G}\defeq C_{F,G}\cap D_{F,G}$, so that $E_{F,G};E_{F,G}\subseteq A$.


Now $\bigcup_{F\mathrel{\mathcal{U}}G}C_{F,G};D_{F,G} \subseteq A$ and also $\bigcup_{F\mathrel{\mathcal{U}}G}C_{F,G};D_{F,G}\in F$

Now take $A\in \entourage$. Then $A\cap A^\transp\in \entourage$, so $(A\cap A^\transp);(A\cap A^\transp)\in\entourage;\entourage$. We now have $(A\cap A^\transp);(A\cap A^\transp) \subseteq A$, indeed take $(a,b)\in (A\cap A^\transp);(A\cap A^\transp)$, then there exist $(a,x), (x,b)\in A\cap A^\transp$
\end{proof}

TODO: any entourage filter satisfies these. Also any entourage filter is the entourage filter of a topological uniform space.
...
&\subseteq \entourage_d; \pfilter{x}\otimes \pfilter{x} \\
...
\begin{align*}
F\mathrel{\mathcal{U}}\pfilter{x} \implies& \entourage_\mathcal{U}\subseteq F\otimes \pfilter{x} \\
...
\begin{enumerate}
\item If $f$ is uniformly continuous, then $\entourage_\mathcal{V} \subseteq \upset (f\times f)^{\imf\imf}[\entourage_\mathcal{U}]$;
\item If $\mathcal{V}$ is topological, then opposite implication also holds.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) By uniform continuity we have
\[ \entourage_\mathcal{V} \subseteq \bigcap\setbuilder{\upset f^{\imf\imf}[F]\otimes \upset f^{\imf\imf}[G]}{F\mathrel{\mathcal{U}} G} \]
Now take $A\in \bigcap\setbuilder{\upset f^{\imf\imf}[F]\otimes \upset f^{\imf\imf}[G]}{F\mathrel{\mathcal{U}} G}$. This means that
\[ \forall F, G \,\text{Cauchy filters}: \exists B_F\in F, C_G\in G: \; f^\imf[B_F]\otimes f^\imf[C_G] \subseteq A. \]
Then $\bigcup_{F \,\text{Cauchy}}B_F$ and $\bigcup_{F \,\text{Cauchy}}C_F$ are elements of all Cauchy filters and
\[ A \supseteq f^{\imf}\left[\bigcup_{F \,\text{Cauchy}}B_F\right] \otimes f^{\imf}\left[\bigcup_{F \,\text{Cauchy}}C_F\right] = (f\times f)^{\imf}\left[\left(\bigcup_{F \,\text{Cauchy}}B_F\right)\otimes \left(\bigcup_{F \,\text{Cauchy}}C_F\right)\right]. \]
This means that $A\in \upset (f\times f)^{\imf\imf}[\entourage_\mathcal{U}]$.

(2) Assume $F\mathrel{\mathcal{U}} G$, so $\entourage_\mathcal{U}\subseteq F\otimes G$. By assumption $\entourage_\mathcal{V}\subseteq \upset (f\times f)^{\imf\imf}[\entourage_\mathcal{U}]\subseteq \upset f^{\imf\imf}[F]\otimes \upset f^{\imf\imf}[G]$. Because $\mathcal{V}$ is topological, we have $\upset f^{\imf\imf}[F]\mathrel{\mathcal{U}} \upset f^{\imf\imf}[G]$, making $f$ uniformly continuous.
\end{proof}

\subsubsection{Uniformities from convergences on product spaces}
\begin{lemma} \label{uniformityFromConvergenceProductSpace}
Let $X$ be a set and $\psi$ a convergence on $X^2$ such that
\begin{itemize}
\item if $H\overset{\psi}{\longrightarrow} (x,x)$, then $H^\transp \overset{\psi}{\longrightarrow} (x,x)$;
\item if $H_1\overset{\psi}{\longrightarrow} (x,x)$ and $H_2\overset{\psi}{\longrightarrow} (x,x)$, then $H_1;H_2 \overset{\psi}{\longrightarrow} (x, x)$.
\end{itemize}
Then the set
...
is the uniformity of \udef{$\mathcal{S}$-uniform convergence}.
\item If $\mathcal{S}$ is a set of subsets of $X$ and $K\subseteq (X\to Y)$. Then $K$ is called \udef{$\mathcal{S}$-equicontinuous} if
\[ \forall S\in \mathcal{S}:\; \upset\big\{(\evalMap, \evalMap)^\imf(\Delta_K, \Delta_S)\big\}\in \mathcal{U}. \]
\end{itemize}
In particular:
\begin{itemize}
...
\begin{lemma}
Let $X$ be a set, $\sSet{Y, \mathcal{U}}$ a diagonal uniform space and $\mathcal{S}$ a set of subsets of $X$. The uniformity of $\mathcal{S}$-uniform convergence is a uniformity on $(X\to Y)$.
\end{lemma}
\begin{proof}
We verify the conditions:
\begin{itemize}
\item Take $f\in(X\to Y)$. For any $S\in \mathcal{S}$, we have
\begin{align*}
(\evalMap, \evalMap)^{\imf\imf}\big((\pfilter{f}\otimes \pfilter{f})\otimes \{\Delta_S\}\big)^\ttransp &= \upset \Big\{ (\evalMap, \evalMap)^{\imf}\big(\{(f,f)\}\times \Delta_S\big)^\ttransp \Big\} \\
&= \upset \Big\{ \bigcup_{x\in S}(\evalMap, \evalMap)^{\imf}\big(\{(f,f)\}\times \{(x,x)\}\big)^\ttransp \Big\} \\
&= \upset \Big\{ \bigcup_{x\in S}\big{(\evalMap, \evalMap)\big((f,x),(f,x)\big)\big\} \Big\} \\
&= \upset \Big\{ \bigcup_{x\in S}\big{\big(f(x), f(x)\big)\big\} \Big\} \\
\end{align*}
\end{itemize}
\end{proof}
...
\begin{proposition}[Universal property of continuous convergence]
Let $\sSet{X, \xi},\sSet{Y,\sigma}$ and $\sSet{Z, \zeta}$ be convergence spaces. A function $f: Z \to \cont_c(X,Y)$ is continuous \textup{if and only if} $\curry_1^{-1}(f): Z\times X\to Y$ is continuous.
\end{proposition}
...
\end{proposition}
\begin{proof}
We have that $F\mathrel{\mathcal{U}^N}G$ iff $F\otimes G$ converges to a point in $d^\preimf\{0\}$ in the initial convergence $\psi$ of $d$. Now $d^\preimf\{0\} = \setbuilder{(x,x)}{x\in X}$. In order to apply \ref{uniformityFromConvergenceProductSpace}, we just need to verify the two points. 
\begin{itemize}
