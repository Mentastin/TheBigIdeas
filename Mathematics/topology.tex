...
\item If $F\in \powerfilters(X)$, $a\in A$, $F\overset{\xi}{\longrightarrow} a$, then $F|_A\overset{\xi|_A}{\longrightarrow} a$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
\end{proposition}
\begin{proof}
(1) Take some ultrafilter $F$ in $A$. Then $\upset F$ is an ultrafilter in $X$, so it converges to some $x\in X$ by compactness. Now $A \in (\upset F)^{\mesh} \subseteq \vicinity(x)^{\mesh}$, so $x\in \adh(A) = A$. Thus $F \to x$ in $A$.

(2) We need to show that $\adh(A)\subseteq A$. Take $x\in \adh(A)$, meaning there exists a filter $F\to x$ such that $A\in F$ by \ref{adherenceInherenceCharacterisation}. By the ultrafilter lemma, \ref{ultrafilterLemma}, $F$ is contained in an ultrafilter $G$ and $G\to x$. Now $G|_A$ is an ultrafilter in $A$, so $G|_A$ converges to a point in $A$, and $G$ converges to the same point in $X$. By Hausdorff this point must be $x$, so $x\in A$.
\end{proof}
\begin{corollary}
...
\begin{enumerate}
\item If $A\subseteq X$ is compact, then $f^\imf[A]$ is compact.
\item 
\end{enumerate}
\end{proposition}
\begin{proof}
...
\begin{itemize}
\item a \udef{covering system of convergence filters} if $\mathcal{C}\mesh F$ for each convergent filter $F\in\powerfilters(X)$;
\item a \udef{covering system of vicinities} if $\mathcal{C}\mesh \vicinity_\xi(x)$ for all $x\in $;
\item a \udef{covering system of neighbourhoods} if $\mathcal{C}\mesh \neighbourhood_\xi(x)$ for all $x\in $;
\item an \udef{open cover} if $\mathcal{C} \subseteq \topology_\xi$ and $\bigcup \mathcal{C} = X$.
\end{itemize}
\end{definition}

\begin{lemma}
Let $\sSet{X,\xi}$ be a convergence space and $\mathcal{C}\subseteq \powerset(X)$ a family of sets. Then $\mathcal{C}$ is a 
\end{lemma}
...
\item 
\end{enumerate}
$X$ is compact \textup{if and only if} every open cover has a finite subcover.
\end{proposition}
\begin{proof}
$\Rightarrow$ Assume $X$ compact. 

$\Leftarrow$ Assume $X$ not compact. 
\end{proof}
