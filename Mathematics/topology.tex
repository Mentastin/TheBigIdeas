...
We call the uniform space \udef{topological} if
\[ F\mathrel{\mathcal{U}}G \qquad\iff\qquad \entourage_\mathcal{U}\subseteq F\otimes G. \]
\end{definition}

Note that even in the topological case, $\entourage_\mathcal{U}$ is not, in general, an element of the uniform filter set!

\begin{lemma}
Let $X$ be a set and $\mathcal{E}\in \powerfilters(X\times X)$. Then $\mathcal{E}$ is the entourage filter of a topological uniform structure on $X$ \textup{if and only if}
\begin{itemize}
\item $\mathcal{E}\subseteq \upset\{\setbuilder{(x,x)}{x\in X}\}$;
\item $\mathcal{E}^\transp = \mathcal{E}$;
\item $\mathcal{E};\mathcal{E} = \mathcal{E}$.
\end{itemize}
\end{lemma}
\begin{proof}
First assume $\mathcal{E}$ is the entourage filter of a topological 
\[ \mathcal{E} = \]
\end{proof}
