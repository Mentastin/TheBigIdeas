...
\[ x\in \adh_\xi(A) \defequiv A\in \vicinity_\xi(x)^{\overrightarrow{\mesh}}; \]
...
\begin{enumerate}
\item $\displaystyle x\in \adh_\xi(A) \iff A \in \bigcup_{F\in {\lim_\xi}^{-1}(x)} F^{\overrightarrow{\mesh}} \iff A \in \bigcup_{F\in {\lim_\xi}^{-1}(x)} F$;
\item $\displaystyle \adh_\xi(A) = \bigcup_{A \in F^{\overrightarrow{\mesh}}}\lim_\xi F = \bigcup_{\substack{\text{$G$ proper filter} \\ A\in G}} \lim_\xi G$;
...
\begin{lemma}
Let $X$ be a set, $\xi$ a convergence on $X$ and $A \subseteq X$ a set. Then
\[ \adh_\xi A = \bigcup_{A \in F^{\overrightarrow{\mesh}}}\lim_\xi F = \bigcup_{\substack{\text{$G$ proper filter} \\ A\in G}}. \]
\end{lemma}
\begin{proof}
...
\begin{itemize}
\item Given a (pre)convergence $\zeta$ on $Y$, the coarsest (pre)convergence on $X$ that makes $f$ continuous is the \udef{initial (pre)convergence} for $f: X\to \sSet{Y,\zeta}$.
\item Given a (pre)convergence $\xi$ on $X$, the finest (pre)convergence on $Y$ that makes $f$ continuous is the \udef{final (pre)convergence} for $f: \sSet{X, \xi} \to Y$.
\end{itemize}
\end{proposition}
...
\chapter{Pretopological and topological convergence}
...
A convergence is topological if $\inh_\xi(A)$ is open for all $A\subseteq X$.
