...
Let $X$ be a set and and let $\Xi$ be a set of (pre)convergences on $X$. The sets of preconvergences and of convergences are complete bounded lattices and for all $F \in \filters(\powerset(X))$:
...
\begin{definition}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be (pre)convergence spaces. A function $f: X\to Y$ is called \udef{continuous} if it preserves limits: for all $D \in \directed(\powerset(X))$ and $x\in X$:
...
Note that for filters $F\in\filters(\powerset(X))$, $f[F]$ is in general a directed set, but not necessarily a filter.

\begin{lemma}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be (pre)convergence spaces. A function $f: X\to Y$ is continuous \textup{if and only if} for all $D \in \directed(\powerset(X))$
...
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be preconvergence spaces and $f: X\to Y$ a function. Then $f$ is continuous \textup{if and only if} $\forall F\in \filters(\powerset(Y))$ and $y\in Y$
...
\begin{proposition}
Let $Y$ be a set, $F\in \filters(\powerset(Y))$ and $y\in Y$.
\begin{enumerate}
...
\begin{lemma} \label{convergenceProductFilter}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be convergence spaces, $F\in \filters(\powerset(X\times Y))$, $G\in \filters(\powerset(X))$ and $H\in \filters(\powerset(Y))$. Then
\begin{enumerate}
...
\begin{lemma}
Let $G$ be a group. And $\mathcal{G} \subseteq \filters(\powerset(G))$ a family of filters. There exists a convergence $\xi$ on $G$ such that $\mathcal{G} = \lim^{-1}_\xi(1)$ \textup{if and only if}
\begin{enumerate}
\item $\pfilter{1} \in \mathcal{G}$;
\item if $F \in \mathcal{G}$ and $G\supseteq F$, then $G\in \mathcal{G}$;
\item if $F,G\in \mathcal{G}$, then $F\cdot G^{-1}\in \mathcal{G}$.
\end{enumerate}
\end{lemma}
\begin{proof}
It is clear that $F \to x$ iff $F\cdot x^{-1} \in \mathcal{G}$ determines a convergence. We just need to show that $u: (x,y) \mapsto x\cdot y^{-1}$ is continuous.

Let $F\otimes G \to (x,y)$, so $f$

{convergenceProductFilter}
\end{proof}
...
Now we use that the vicinity filters converge, so $\vicinity_\xi(x)\otimes \vicinity_\xi(1) \to (x,1)$ in $X\times X$ by \ref{convergenceProductFilter}. By continuity $\vicinity_\xi(x)\cdot \vicinity_\xi(1) \to x\cdot 1 = x$, so $\vicinity_\xi(x) \subseteq \vicinity_\xi(x)\cdot \vicinity_\xi(1)$. This implies that there exists $V\in \vicinity_\xi(x)$ and $W\in\vicinity_\xi(1)$ such that $V\cdot W \subseteq A$.
...
\[ \mathcal{F} \defeq \setbuilder{F\in \filters(\powerset(G))}{F\cdot F^{-1} \overset{\xi}{\longrightarrow} 1}. \]
Then
\begin{enumerate}
\item $\sSet{G, \mathcal{F}}$ is a Cauchy space;
\item if $F\in \filters(\powerset(G))$ Cauchy converges to $x$, then it converges to $x$;
\item if $F\in \filters(\powerset(G))$ converges to $x$, then $F\in \mathcal{F}$; if $G$ is a Kent space, then $F$ Cauchy converges to $x$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
(3) Assume $F\to x$. Then by \ref{convergenceProductFilter}, $F\otimes F \to (x,x)$. By continuity of $(x,y)\mapsto x\cdot y^{-1}$, we have $F\cdot F^{-1}\to x\cdot x^{-1} = 1$, so $F$ is a Cauchy filter.
