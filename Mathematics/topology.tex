...
\item a \udef{local cover of convergence filters} at $x\in X$ if $\mathcal{C}\mesh F$ for each filter $F\in\powerfilters(X)$ that converges to $x$;
...
Let $\sSet{X,\xi}$ be a convergence space and $x\in X$. Then
\begin{enumerate}
\item the set of local covers of convergence filters at $x$ is given by $\setbuilder{F}{F\overset{\xi}{\longrightarrow} x}^\mesh$;
\item the set of covers of convergence filters is given by $\setbuilder{F}{\exists y\in X: F\overset{\xi}{\longrightarrow} y}^\mesh$.
\end{enumerate}
\end{lemma}
...
\begin{enumerate}
\item $A$ is compact;
\item every cover of $A$ of convergent filters has a finite subset that covers $A$.
\end{enumerate}
\end{proposition}
In other words, every cover of convergent filters has a finite subset whose union contains $A$.
\begin{proof}
...
\[ A \in \upset \iota^{\imf\imf}[F] \subseteq \big(\upset \iota^{\imf\imf}[F]\big)^{\mesh} \subseteq \vicinity(x)^{\mesh}, \]
...
\end{proof}
\begin{corollary} \label{compactClosedIntersectionCompact}
...
Notice that if $A$ is relatively compact, it is $\adh(A)$-compactoid. The converse does not hold.
...
\section{Convergence modification}
The convergence modification is obtained from a preconvergence space by adding all convergence of the form $\pfilter{x}\to x$.

\begin{definition}
Let $\sSet{X,\xi}$ be a preconvergence space. The convergence modification of $\xi$ is denoted $\convMod(\xi)$.
\end{definition}
...
\chapter{Continuity and convergence morphisms}
...
(3) We have, by \ref{initialFinalConvergence},
\begin{align*}
F \overset{\nu}{\longrightarrow} y &\iff \exists i\in I: \exists x\in g_i^\preimf(\{y\}):  \exists G \overset{\xi_i}{\longrightarrow} x: \; \upset g_i^{\imf\imf}[G] \subseteq F \\
&\iff \exists i\in I: \exists x\in g_i^\preimf(\{y\}): \; \upset g_i^{\imf\imf}\big[\vicinity_{\xi_i}(x)\big] \subseteq F \\
&\iff \bigcap_{\substack{i\in I \\ x\in g^\preimf_i\{y\}}} \upset g_i^{\imf\imf}\big[\vicinity_{\xi_i}(x)\big] \subseteq F.
\end{align*}
...
\begin{enumerate}
\item $F|_A = \upset(\iota^{\imf\imf}\circ \iota^{\preimf\imf})(F)$;
...
\section{Modifications and (co)reflections}
\subsection{Modifications on the way to topological}
\subsubsection{Finite depth modification}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then the \udef{Kent modification} $\kentMod(\xi)$ is the smallest convergence in $\upset \{\xi\}$ that is a Kent space.
\end{definition}


\subsubsection{Finite depth modification}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then the \udef{finite depth modification} $\finDepthMod(\xi)$ is the smallest convergence in $\upset \{\xi\}$ that is of finite depth.
\end{definition}
...
\subsubsection{Pseudotopological modification}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then the \udef{pseudotopological modification} $\pseudotopMod(\xi)$ is the smallest convergence in $\upset \{\xi\}$ that is pseudotopological.
\end{definition}
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then the \udef{pretopological modification} $\pretopMod(\xi)$ is the smallest convergence in $\upset \{\xi\}$ that is pretopological.
\end{definition}
...
\subsubsection{Topological modification}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then the \udef{topological modification} $\topMod(\xi)$ is the smallest convergence in $\upset \{\xi\}$ that is topological.
\end{definition}
...
\begin{proposition}
Let $\sSet{X,\xi}, \sSet{Y,\zeta}$ be non-empty convergence spaces. Then $\cont_c(X,Y)$ is pseudotopological \textup{if and only if} $Y$ is pseudotopological.
\end{proposition}
\begin{proof}
First assume $\cont_c(X,Y)$ is pseudotopological. Then $Y$ is homeomorphic to a subspace of $\cont_c(X,Y)$ by \ref{embeddingInContinuousConvergence} and thus pseudotopological by \ref{pretopologicalInitialConvergence}.
...
Consider the set $\setbuilder{C_{F,x}}{F\to x}$, which is a cover of convergent filters. Then by compactness of $K$ and \ref{compactFiniteSubcover}, we can find a finite subset $C_{x_0, F_0},\ldots C_{x_n, F_n}$ such that $C_{x_0, F_0} \cup \ldots \cup C_{x_n, F_n} \supseteq K$. Now 
...
G&\to x
\end{aligned}\right\}\qquad\implies\qquad \evalMap^{\imf\imf}(F\otimes G)\to y. \]
\end{lemma}
...
\begin{proposition}
Let $\sSet{X,\xi},\sSet{Y, \zeta}$ be convergence spaces and $H\subseteq (X\to Y)$.

If $H$ evenly continuous and $\evalMap_x^\imf(H)$ compactoid in $Y$ for all $x\in X$, then $H$ is compactoid in $(X\to Y)_c$.
\end{proposition}
\begin{proof}
Let $U\in\powerultrafilters\big((X\to Y)\big)$ be an ultrafilter that contains $H$. Take arbitrary $x\in X$. Then $\evalMap_x^\imf(H) = \evalMap^\imf(H\times \{x\})\in \upset\evalMap^{\imf\imf}(U\otimes \pfilter{x})$. Because $\evalMap_x^\imf(H)$ was assumed compactoid, $\upset\evalMap^{\imf\imf}(U\otimes \pfilter{x})$ converges to some $y\in Y$.

Now let $f: X\to Y$ be a function that maps each $x$ to such a $y$. By construction, $U$ converges pointwise to $f$. As $H$ is evenly continuous and $U$ contains $H$, we have that $U$ converges in continuous convergence to $f$.
\end{proof}
\begin{corollary}
If $H\subseteq \cont(X,Y)$ and $Y$ is regular, then $H$ is compactoid in $\cont_c(X, Y)$ under the same assumptions.
\end{corollary}
\begin{proof}
We need to show that in this case the function $f$ may be chosen continous. As $f$ is a limit of the continuous convergence, its continuity is given by \ref{continuousLimitsContinuous}.
\end{proof}

\begin{proposition}
Let $\sSet{X,\xi}$ be a convergence space, $\sSet{Y, \zeta}$ a Hausdorff pseudotopological convergence space and $H\subseteq (X\to Y)$. If $H$ is compactoid in $(X\to Y)_c$, then
\begin{enumerate}
...
\begin{proposition}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be non-empty convergence spaces. Then $\cont_c(X,Y)$ is regular \textup{if and only if} $\sSet{Y,\zeta}$ is.
\end{proposition}
\begin{proof}
The direction $\Rightarrow$ is given by \ref{embeddingInContinuousConvergence}: $Y$ is homeomorphic to a subspace of $\cont_c(X,Y)$ and must therefore be regular by \ref{regularityInitialConvergence}.
