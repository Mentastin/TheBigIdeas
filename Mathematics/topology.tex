...
\begin{proposition}
Let $\sSet{X,\xi}$ be a convergence space and $A\subseteq X$ a closed subset. Let $B\subseteq A$ and $x\in A$. Then
\begin{enumerate}
\item $\adh_{\xi|_A}(B) = \adh_\xi(B)$.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) We have $\iota^\imf(B) = B$, so $\adh_{\xi|_A}(B) \subseteq \adh_\xi(B)$ by \ref{adherenceInherenceContinuity}.
\end{proof}
...
\begin{proposition}
Let $\sSet{X,\xi},\sSet{Y, \zeta}$ be convergence spaces and $H\subseteq \cont(X, Y)$.
...
\end{proof}
\begin{corollary}
If $Y$ is regular, then $H$ is compactoid in $\cont_c(X, Y)$ under the same assumptions.
\end{corollary}
\begin{proof}
...
\begin{example}
TODO example where converse does not hold.
\end{example}
