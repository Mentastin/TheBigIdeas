...
\begin{definition}
Let $\sSet{X,\xi}, \sSet{,\zeta}$ be convergence spaces, $A\subseteq X$ and $f: A\to Y$ a function.
Consider the relation
\[ C_f \defeq \setbuilder{(x,y)\in X\times Y}{\exists F\in\powerfilters(X): \; \big(F \overset{\xi}{\longrightarrow}x\big) \land \big(f^\imf(F)\overset{\zeta}{\longrightarrow} y\big)}. \]
The function restriction of $C_f$ is called the \udef{continuous extension} of $f$.
\end{definition}

\begin{lemma}
Let $\sSet{X,\xi}, \sSet{,\zeta}$ be convergence spaces, $A\subseteq X$ and $f: A\to Y$ a function. Let $g: B \to Y$ be the continuous extension of $f$. If $f$ is continuous, then $f \subseteq g$.
\end{lemma}
