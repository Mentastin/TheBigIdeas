...
If $\lim_\xi F \neq \emptyset$, we say the directed set $F$ \udef{converges}. If there exists a filter that converges to a point $x\in X$, then we call $x$ a \udef{limit point}.
\end{definition}
In a convergence space every point is a limit point.
...
\item If $\{\sSet{X_i, \xi_i}\}_{i\in I}$ is a set of \emph{pretopological} \emph{pre}convergence spaces, then $\nu$ is pretopological.
...
Now consider the convergence space $\coprod_{\text{$F$ converges in $\xi$}}\sSet{X, \xi_F}$, which is topological by \ref{pretopologicalFinalConvergence} (TODO why???). Then the convergence $\xi$ is equal to the final convergence on $X$ w.r.t. to the projection $p_2: \coprod_{\text{$F$ converges in $\xi$}}\sSet{X, \xi_F} \to X$.
\end{proof}
