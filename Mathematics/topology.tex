...
\begin{lemma}
Let $X,Y$ be sets, $F,G\in \powerfilters(X)$ and $H,I\in\powerfilters(Y)$. Then
\[ (F\cap G)\otimes (H\cap I) \subseteq (F\otimes H)\cap (G\otimes I). \]
\end{lemma}
...
\end{lemma}
\begin{proof}
Because $\{(x,x)\}\subseteq \Delta_C$ for some $C\in\mathcal{C}$ and $\pfilter{x}\otimes\pfilter{x}\in\mathcal{F} = \upset \big\{\{(x,x)\}\big\}$.
\end{proof}
...
\begin{lemma}
Let $X$ be a set, $\sSet{Y, \mathcal{U}}$ a diagonal uniform space and $\mathcal{S}$ a set of subsets of $X$. The uniformity of $\mathcal{S}$-uniform convergence is a diagonal uniformity on $(X\to Y)$.
\end{lemma}
\begin{proof}
We verify the conditions:
\begin{itemize}
\item Diagonality: For any $S\in \mathcal{S}$, we have
\begin{align*}
...
\begin{lemma}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be convergence spaces. If $X$ and $Y$ are Kent spaces, then so is $(X\to Y)_c$. ???
\end{lemma}
\begin{proof}
Let $H\overset{(X\to Y)_c}{\longrightarrow} f$. We need to show that $\evalMap^{\imf\imf}\big((H\cap \pfilter{f})\otimes F\big) \overset{\zeta}{\longrightarrow} f(x)$ for all $F\overset{\xi}{\longrightarrow} x$. Because $X$ is a Kent space, it is enough to show $\evalMap^{\imf\imf}\big((H\cap \pfilter{f})\otimes (F\cap \pfilter{x})\big) \overset{\zeta}{\longrightarrow} f(x)$ for all $F\overset{\xi}{\longrightarrow} x$.

We first prove $(H\cap \pfilter{f})\otimes (F\cap \pfilter{x}) \supseteq (H\otimes F) \cap (\pfilter{f}\otimes \pfilter{x})$. Take $A \in  (H\otimes F) \cap (\pfilter{f}\otimes \pfilter{x})$. Then there exist $B\in H, C\in F$ such that $B\times C \subseteq A$. Define $B' \defeq B\cup\{f\}$ and $C'\defeq C\cup \{x\}$. Then $B'\in H, C'\in F$
\end{proof}
\begin{corollary}
Let $\sSet{X,\xi}$, $\sSet{Y,\zeta}$ be convergence spaces and $H\in \powerfilters(X\to Y)$. If $Y$ is a Kent space, then $H\overset{(X\to Y)_c}{\longrightarrow} f$ implies $f\in\cont(X,Y)$.
\end{corollary}
\begin{proof}

\end{proof}
The hypothesis of $Y$ being a Kent space is necessary.
\begin{example}
...
\begin{lemma}
Let $\sSet{X, \xi}$ be a convergence space and $\sSet{Y, \mathcal{U}}$ a uniform space. Then the uniformity of continuous convergence induces continous convergence on $(X\to Y)$.
\end{lemma}
\begin{proof}
Let $H$ be a filter in $\powerfilters(X\to Y)$ and $f\in (X\to Y)$. Then
\begin{align*}
...
&\iff \forall F\overset{\xi}{\longrightarrow}x: \; \evalMap^{\imf\imf}(H\otimes F)\otimes \pfilter{f}(x) \in\mathcal{U} \\
&\iff \forall F\overset{\xi}{\longrightarrow}x: \; \evalMap^{\imf\imf}(H\otimes F)\otimes \evalMap^{\imf\imf}(\pfilter{f}\otimes \pfilter{x}) \in\mathcal{U} \\
&\iff \forall F\overset{\xi}{\longrightarrow}x: \; (\evalMap, \evalMap)^{\imf\imf}\big((H\otimes F)\otimes (\pfilter{f}\otimes \pfilter{x})\big) \in\mathcal{U} \\
&\iff \forall F\overset{\xi}{\longrightarrow}x: \; (\evalMap, \evalMap)^{\imf\imf}\big((H\otimes \pfilter{f})\otimes (F \otimes \pfilter{x})\big)^\ttransp \in\mathcal{U} \\
\end{align*}
?? TODO
\end{proof}

\begin{proposition}
Let $\sSet{X, \xi}$ be a convergence space, $\sSet{Y, \mathcal{U}}$ a uniform space and $\mathcal{S}$ a set of compact subsets if $X$. The continuous convergence uniformity is stronger than the $\mathcal{S}$-uniformity.
\end{proposition}
\begin{proof}
