...
Now take $x\in\adh_\xi(\mathcal{A}\cup \mathcal{B})$, then there exists $F\to x$ such that $F\subseteq (\mathcal{A}\cup \mathcal{B})^\mesh = \mathcal{A}^\mesh\cap \mathcal{B}^\mesh$, where the last equality is due to \ref{grillUpsetOrderSimilarity} (and the fact that joins and meets in the lattice of upwards closed sets are just given by the union and intersection, see \ref{meetJoinRClosedSets}).

It is enough to prove that either $F\subseteq \mathcal{A}^\mesh$ or $F\subseteq \mathcal{B}^\mesh$. Assume, towards a contradiction, that this is not the case, i.e.\ there exist $A\in\mathcal{A}$, $B\in \mathcal{B}$ and $C,D\in F$ such that $A\perp C$ and $B\perp D$. Then $C\cap D\perp A$ and $C\cap D \perp B$, so $C\cap D \notin \mathcal{A}^\mesh\cap \mathcal{B}^\mesh$. But $C\cap D \in F$ because $F$ is a filter.
