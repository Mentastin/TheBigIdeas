...
\begin{definition}
Let $\sSet{X, \xi}, \sSet{Y, \zeta}$ be topological spaces and $f: X\to Y$ a function. Then
\begin{itemize}
...
\end{lemma}
\begin{proof}
(1) First assume $\xi$ is compatible with the weak order. Set $x = \liminf F$. Then, by \ref{filterInequalityCriterion} and compatibility with the weak order, we have
\[ \liminf F \leq x \implies \liminf F \leq \limsup\pfilter{x} \implies F\leq_w \pfilter{x} \implies \forall y\in\lim F: \; y\leq x. \]
With a similar argument for the limsup, we have $\limsup F\leq y \leq \liminf F$ for all $y\in \lim F$. Since $\liminf F \leq \limsup F$, the result follows.

For the converse, take $F,G\in \powerfilters(X)$ such that $F\leq_w G$. Take $x\in \lim F$ and $y\in \lim G$. By assumption, $x = \liminf F$ and $y = \limsup G$. By \ref{filterInequalityCriterion} we have $x\leq y$.

(2) In this case, \ref{filterInequalityCriterion} gives $\liminf F\leq y \leq \limsup F$ for all $y\in \lim F$.

For the converse, take $F,G\in \powerfilters(X)$ such that $F\leq_s G$. Take $x\in \lim F$ and $y\in \lim G$. By assumption, $x \leq \limsup F$ and $y \geq \liminf G$. By \ref{filterInequalityCriterion} we have $\limsup F\leq \liminf G$, so $x\leq \limsup F\leq \liminf G \leq y$.
\end{proof}
\begin{corollary}
The weakest convergence compatible with the weak order is given by $\lim F = \liminf F =\limsup F$, if they exist and coincide, for all $F\in \powerfilters(X)$.
\end{corollary}

\subsubsection{Order convergence}
\begin{definition}
Let $\sSet{X,\leq}$ be a poset. The \udef{order convergence} $\mathfrak{o}$ is defined by
\[ \lim_\mathfrak{o}F = \begin{cases}
...
\begin{lemma}
Let $\sSet{X,\leq}$ be a poset. The order convergence on $X$ is a convergence.
\end{lemma}
\begin{proof}
The order convergence is monotone by construction: if $F\subseteq F'$, then $\bigcap_{A\in F}\upset A \supseteq \bigcap_{A\in F'}\upset A$ and thus $\Big(\bigcap_{A\in F}\upset A\Big)^l \subseteq \Big(\bigcap_{A\in F'}\upset A\Big)^l$. A similar inclusion holds for $\Big(\bigcap_{A\in F}\downset A\Big)^u$.

To show that the order convergence is centered, take $x\in X$. Then
\begin{align*}
\lim_\mathfrak{o}\pfilter{x} &= \Big(\bigcap_{A\in \pfilter{x}}\upset A\Big)^l \cap \Big(\bigcap_{A\in \pfilter{x}}\downset A\Big)^u \\
&= \Big(\upset \{x\}\Big)^l \cap \Big(\downset \{x\}\Big)^u \\
...
\begin{proposition}
Let $\sSet{X,\leq}$ be a finitely Dedekind complete poset. Then the order convergence $\mathfrak{o}$ is the weakest convergence compatible with the weak order.
\end{proposition}
In particular the proposition holds for all lattices.
\begin{proof}
Let $\xi$ be an arbitrary convergence that is compatible with the weak order.

Take arbitrary $y\in \lim_\xi(F)$. Take arbitrary $x\in \bigcap_{A\in F}\upset A$. Then $F \leq_w \pfilter{x}$ by \ref{weakRelationPrincipalFilter}, so $y\leq x$ by compatibility with the weak order. Since this holds for all such $x$, we have $y\in \Big(\bigcap_{A\in F}\upset A\Big)^l$. Similarly $y\in \Big(\bigcap_{A\in F}\downset A\Big)^u$.

Thus $\lim_\xi F \subseteq \Big(\bigcap_{A\in F}\upset A\Big)^l \cap \Big(\bigcap_{A\in F}\downset A\Big)^u$, so any convergence compatible with the weak order is stronger than the order convergence.

We now need to show that the convergence is compatible with the weak order. Take $F,G\in\powerfilters(X)$ such that $F\leq_w G$. We have
\begin{align*}
F \leq_w G \iff& \forall A\in F, B\in G: \exists x\in A, y\in B: \; x\leq y \\
\iff& \forall A\in F, B\in G: \exists x\in A, y\in B: \; x\leq y \\
\end{align*}
TODO
\end{proof}
...
\begin{example}
Consider the ``bug-eyed line'' $\R\uplus \{0'\}$, where ($x< 0'$ iff $x< 0$) and ($0'<x$ iff $0<x$). This poset is clearly not finitely Dedekind complete, since $\{0,0'\}$ is upper bounded by all positive numbers, but does not have a least upper bound.

Set $F \defeq \upset\setbuilder[\big]{\interval[o]{0,\epsilon}}{\epsilon > 0} = \upset\setbuilder[\big]{\interval[o]{0',\epsilon}}{\epsilon > 0'}$. Then
\begin{align*}
\lim_\mathfrak{o}F &= \Big(\bigcap_{A\in F}\upset A\Big)^l \cap \Big(\bigcap_{A\in F}\downset A\Big)^u \\
&= \Big(\bigcap_{\epsilon > 0}\interval[o]{0,\infty}\Big)^l \cap \Big(\bigcap_{\epsilon > 0}\interval[o]{-\infty, \epsilon}\Big)^u \\
&= \Big(\interval[o]{0,\infty}\Big)^l \cap \Big(\interval[o]{-\infty, 0}\cup \{0,0'\}\Big)^u \\
&= \Big(\interval[o]{-\infty, 0} \cup \{0,0'\}\Big) \cap \Big(\interval[o]{0,\infty}\Big) \\
&= \emptyset.
\end{align*}
Set $F \defeq \setbuilder[\big]{\interval[o]{0,\epsilon}}{\epsilon > 0} \cup \setbuilder[\big]{\interval[o]{-\epsilon, 0}}{\epsilon > 0}$. Then
\begin{align*}
\lim_\mathfrak{o}F &= \Big(\bigcap_{A\in F}\upset A\Big)^l \cap \Big(\bigcap_{A\in F}\downset A\Big)^u \\
&= \Big(\interval[o]{0,\infty}\Big)^l \cap \Big(\interval[o]{-\infty, 0}\Big)^u \\
&= \Big(\interval[o]{-\infty, 0} \cup \{0,0'\}\Big) \cap \Big(\interval[o]{0,\infty} \cup \{0,0'\}\Big) \\
&= \{0,0'\}.
\end{align*}
So the filter property is important.

Set $F \defeq \setbuilder[\big]{\interval[o]{-\epsilon,\epsilon}}{\epsilon > 0}$. Then
\begin{align*}
\lim_\mathfrak{o}F &= \Big(\bigcap_{A\in F}\upset A\Big)^l \cap \Big(\bigcap_{A\in F}\downset A\Big)^u \\
&= \Big(\interval[o]{0,\infty} \cup \{0,0'\}\Big)^l \cap \Big(\interval[o]{-\infty, 0} \cup \{0,0'\}\Big)^u \\
&= \Big(\interval[o]{-\infty, 0}\Big) \cap \Big(\interval[o]{0,\infty}\Big) \\
&= \emptyset.
\end{align*}
\end{example}
...
\begin{itemize}
\item there exist sets $A,B\subseteq X$ such that $\bigvee A = x = \bigwedge B$ (and these extrema exist); and
\item $\interval{a,b}\in F$ for all $a\in A$ and $b\in B$.
\end{itemize}
If this holds, then $A$ may be chosen to be upwards directed and $B$ downwards directed.
\end{proposition}
\begin{proof}
\ref{weakRelationPrincipalFilter}
\[ \lim F \subseteq \Big(\bigcap_{A\in F}\upset A\Big)^l \cap \Big(\bigcap_{A\in F}\downset A\Big)^u \subseteq \Big(\bigcap_{A\in F}\upset A \cap \bigcap_{A\in F}\downset A\Big)^l \cap \Big(\bigcap_{A\in F}\upset A \cap \bigcap_{A\in F}\downset A\Big)^u \]
if $\bigcap_{A\in F}\upset A \cap \bigcap_{A\in F}\downset A \neq \emptyset$, then $\lim F$ is at most a singleton.

Suppose $\bigcap_{A\in F}\upset A \cap \bigcap_{A\in F}\downset A = \emptyset$, then $\bigcap \upset A \subseteq \bigcup (\downset A)^c$.
\end{proof}

\begin{proposition}
Let $\sSet{X,\leq}$ be a poset and $F\subseteq \powerfilters(X)$ a filter. Then $F\overset{\mathfrak{o}}{\longrightarrow} x$ \textup{if and only if} $F$ is the tails filter of some net $\seq{x_i}_{i\in I}$ and there exist monotone nets $\seq{a_i}_{i\in I}$ and $\seq{b_i}_{i\in I}$ such that
\begin{itemize}
\item $a_i \leq x_i \leq b_i$;
\item $\sup_{i\in I}a_i = x$;
\item $\inf_{i\in I}b_i = x$.
\end{itemize}
\end{proposition}
\begin{proof}
TODO
...
\begin{proposition}
In a lattice, $\vee, \wedge$ are continuous (?)
\end{proposition}
...
TODO show that all modiciations are functors: $f:X\to Y$ continuous implies $f: M(X)\to M(Y)$ continuous.
...
\begin{definition}
Let $\sSet{X, \xi}$ be a convergence space and $\mathcal{B}$ a set of filters. The \udef{$\mathcal{B}$-based modification} $\basedMod_\mathcal{B}(\xi)$ is the coclosure into the lattice of $\mathcal{B}$-based convergences.
\end{definition}
...
\begin{definition}
Let $X$ be a set. A convergence $\xi$  on $x$ is called \udef{sequentially based} if it is based in the set of sequential filters in $X$.

The \udef{sequentially based modification} of $\xi$ is denoted $\seqMod(\xi)$.
\end{definition}
...
\begin{proposition}
Let $\sSet[\big]{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth and $\lambda \in \Lambda_\mathfrak{C}$. Then $\sSet[\big]{\hat{X}, C_\lambda}$ is a complete Hausdorff Cauchy space.
\end{proposition}
TODO: we can relax the requirement of finite depth to recipocity?
\begin{proof}
...
Let $\sSet[\big]{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth, $A\subseteq X$ and $\lambda \in \Lambda_\mathfrak{C}$. Then $\adh_{C_\lambda}\big(A^\lambda\big) = \adh_{C_\lambda}\big(k^{\imf}(A)\big)$.
\end{lemma}
\begin{proof}
...
\begin{proposition}
Let $\sSet[\big]{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth and $\lambda \in \Lambda_\mathfrak{C}$. Then
\begin{enumerate}
...
Let $\sSet{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth, $\lambda \in \Lambda_\mathfrak{C}$ and $\sSet{Y,\mathfrak{D}}$ a complete regular Hausdorff Cauchy of finite depth. Let $f: X\to Y$ be a Cauchy continuous map. Then $f$ can be extended to a unique Cauchy continuous map $\hat{f}: \hat{X}\to Y$.
\end{proposition}
...
\begin{lemma}
Let $\sSet{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth. Consider the function
...
\end{definition}
\begin{lemma}
Let $\sSet{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth. The Cauchy structure $\mathfrak{C}_W$ of the Wyler completion is given by
...
\begin{lemma}
Let $\sSet{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth. The only non-trivial Cauchy filters in $\mathfrak{C}_W$ that contain $\hat{X}\setminus \im(k)$ are the principal ultrafilters.
\end{lemma}
\begin{proof}
...
Let $\sSet{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth and let $\sSet[\big]{\sSet{Y,\mathfrak{D}}, k': X\to Y}$ be a completion of $X$. Then there exist a unique Cauchy continuous function $g: \hat{X} \to Y$ such that
...
\begin{proposition}
Let $\sSet{X, \mathfrak{C}}$ be a Hausdorff Cauchy space of finite depth and $\sSet{Y,\mathfrak{D}}$ a complete Hausdorff Cauchy of finite depth. Let $f: X\to Y$ be a Cauchy continuous map. Then $f$ can be extended to a unique Cauchy continuous map $\hat{f}: \sSet{\hat{X}, \mathfrak{C}_W} \to Y$.
\end{proposition}
...
\end{proposition}
This follows from \ref{topologicalUniformSpaceRegular}, \ref{groupUniformStructure} and \ref{convergenceGroupTopologicalIffUniformlyTopological}. Below is an alternate, more elementary, proof.
\begin{proof}
