...
\begin{proposition}
Let $X$ be a set. A filter $F\in \powerfilters(X)$ is sequential \textup{if and only if} $F$ contains a countable set and admits a countable base $\mathcal{B}$ such that $B\setminus B'$ is finite for all $B,B'\in \mathcal{B}$.
\end{proposition}
\begin{proof}
$\boxed{\Rightarrow}$ Assume $F = \TailsFilter(\seq{x_n})$ for some sequence $\seq{x_n}$. Then every set in $\Tails(\seq{x_n})$ is countable and an element of $F$.
...
$\Leftarrow$ Assume $F$ contains a countable set $A$ and consider some enumeration (i.e. surjective function) $f:\N\to\mathcal{B}$.

Then consider the following recursively defined function on $\N$:
\[ n\mapsto \begin{cases}
B_0 = A \\
B_n = f(n-1)\cap B_{n-1} & (n> 0).
\end{cases} \]
It is clear that $B_n$ is an element of $F$ for all $n\in \N$. Indeed the sets $B_n$ form a base of $F$.
