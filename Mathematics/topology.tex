...
\item a \udef{Kent space} if $F \to x \implies F\cap \upset\{x\} \to x$.
\end{itemize}
\end{definition}
...
\item The \udef{discrete convergence} $\iota_X$ on $X$ is defined by $x \in \lim_\iota F \iff F = \upset\{x\}$ if $F$ is a proper filter. If $F = \powerset(X)$, then $\lim_\iota F = X$.
...
\begin{proof}
We have $\vicinity_\xi(x) \subseteq \upset\{x\}$, so $V\in \upset\{x\}$ and thus $\{x\} \subseteq V$ or $x\in V$. 
\end{proof}
...
Let $\sSet{X,\xi}$ be a convergence space and $x\in X$. Then $\adh_\xi(\{x\}) = \lim_\xi \upset\{x\}$.
\end{lemma}
\begin{proof}
If $F$ is a filter such that $\{x\} \lhd F$, then $F = \upset\{x\}$ by \ref{ultrafilterContainment}. So $\adh_\xi \{x\} = \lim_\xi \upset\{x\}$.
\end{proof}
...
\begin{itemize}
\item A point $x\in X$ is called \udef{isolated} if ${\lim_\xi}^{-1}(x) = \Big\{ \upset\{x\} \Big\}$.
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space and $A,B\subseteq X$. We say $A$ and $B$ are \udef{separated} if $\adh_\xi(A) \perp \adh_\xi(B)$.

We call two points $x,y\in X$ \udef{separated} if $\{x\}$ and $\{y\}$ are separated.
\end{definition}

\begin{lemma}
Let $\sSet{X, \xi}$ be a convergence space and $x,y\in X$. Then $x$ and $y$ are separated \textup{if and only if} $\lim_\xi\upset\{x\} \perp \lim_\xi\upset\{y\}$.
\end{lemma}
\begin{proof}
By \ref{singletonAdherence}.
\end{proof}

\begin{proposition}
Let $X$ be a set, $\xi$ a convergence of finite depth on $X$ and $x,y\in X$. Then $x$ and $y$ are separated \textup{if and only if}
\[ \upset\{x\} \not\to y \qquad \text{and}\qquad \upset\{y\}\not\to x. \]
\end{proposition}
\begin{proof}
The direction $\Rightarrow$ is clear: separatedness means $x\in \lim_\xi\upset\{x\} \perp \lim_\xi\upset\{y\} \ni y$.

The direction $\Leftarrow$ is proved by contradiction. Assume $x$ and $y$ are not separated, so there exists $z\in X$ such that $\upset\{x\} \to z$ and $\upset\{y\}\to z$.

TODO!!!!!!!!!!!!!!!!!
\end{proof}
In other words, $x$ and $y$ are not separated iff $\upset\{x\} \to y$ or $\upset\{y\} \to x$.
...
We call the convergence space $\xi$ \udef{Kolmogorov} or \udef{$T_0$} if every pair of distinct points in $X$ is distinguishable.
\end{definition}
...
\[ \upset\{x\} \to y \qquad \text{and}\qquad \upset\{y\}\to x. \]
\end{proposition}
\begin{proof}
The direction $\Rightarrow$ is clear: from $\upset\{x\} \to x$, we get $\upset\{x\}\to y$ by indistinguishability.

The direction $\Leftarrow$ is proved by contradiction. Assume $x$ and $y$ are distinguishable, so there exists a filter $F$ such that $F\to x$ but $F\not\to y$. Then $F\cap \upset\{x\} \to y$ by finite depth. Now $F\cap \upset\{y\} \subseteq F$, so $F\to y$. This is a contradiction.
\end{proof}
...
Let $\sSet{X,\xi}$ be a convergence space.
\begin{enumerate}
\item Every pair of separated points is distinguishable.
\item If $\xi$ is $R_0$, then the converse also holds.
\end{enumerate}
\end{lemma}

\begin{proposition} \label{R0conditions}
Let $X$ be a set and $\xi$ a (topological??) convergence on $X$. Then the following are equivalent:
\begin{enumerate}
\item $\xi$ is an $R_0$ convergence;
\item $\upset\{x\} \to y$ \textup{if and only if} $x$ and $y$ are indistinguishable;
...
\item every open set is a union of closed sets.
\end{enumerate}
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ All points that are indistinguishable from $x$ are in $\adh_\xi(\{x\})$ by construction. Assume $y$ is distinguishable from $x$. Then $y\in \adh_\xi(\{y\})\perp \adh_\xi(\{x\})$, so $y\notin \adh_\xi(\{x\})$.

$(2) \Rightarrow (3)$ By \ref{singletonAdherence} we have $\lim_\xi \upset\{x\} = \adh_\xi(\{x\})$.

$(3) \Rightarrow (4)$ Indistinguishability is an equivalence relation.

$(4) \Rightarrow (5)$  TODO requires topological assumption?
...
\item $\forall x\in X: \; \lim_\xi \upset\{x\} \subseteq \{x\}$;
\item every singleton in $X$ is closed;
\item every finite subset of $X$ is closed.
\end{enumerate}
\end{proposition}
\begin{proof}
$(1) \Leftrightarrow (2)$ Separated points are distinguishable.

$(2) \Leftrightarrow (3)$ From \ref{R0conditions} and \ref{singletonAdherence} we have that $\lim_\xi\upset\{x\}$ is the set of points that are indistinguishable from $x$ iff $\xi$ is $R_0$. This set is $\{x\}$ iff $\xi$ is $T_0$.

$(3) \Leftrightarrow (4)$ Convergences are centered.

$(3) \Leftrightarrow (5)$ Immediate from \ref{singletonAdherence}.

$(5) \Leftrightarrow (6)$ Adherence preserves finite union, ref TODO.
\end{proof}
...
\end{proposition}
\begin{proof}
Suppose $\ker F$ in non-empty and take $y \in \ker F$. Then $F \subseteq \upset \{y\}$ and so
\[ x \in \lim F \subseteq \lim \upset\{y\} \subseteq \{y\}. \]
This means that $y = x$.
\end{proof}
\begin{corollary}
The kernel of a convergent proper filter is empty or a singleton.
\end{corollary}


TODO: big question marks:
\begin{proposition}
Let $X$ be a set and $\xi$ a convergence on $X$. If $\xi$ is $T_1$, then $\forall x,y\in X: \exists F,G\in \filters(\powerset(X))$ such that
\[ x\in \lim_\xi F \;\land\; y\in \lim_\xi G \;\land\; x\notin \lim_\xi G \;\land\; y\notin \lim_\xi F. \]
If $\xi$ is topological, the converse also holds.
\end{proposition}
\begin{proof}
$\Rightarrow$ We may take $F = \upset\{x\}$ and $G = \upset \{y\}$.
...
\begin{definition}
Let $X$ be a set. Let $\mathcal{F}$ be a family of filters in $\directed(\powerset(X))$ such that
\begin{itemize}
\item $\upset\{x\} \in \mathcal{F}$ for all $x\in X$;
...
\begin{definition}
Let $X$ be a set. A \udef{uniform convergence structure} (or just \udef{uniform structure}) is a convergence on $X\times X$ such that transpose and composition are continuous.
\end{definition}

\subsection{Induced convergence}
\begin{definition}
...
Conversely, assume $(x,y) \mapsto xy^{-1}$ continuous. TODO
\end{proof}