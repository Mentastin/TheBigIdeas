...
Let $X$ be a set and $H\in\powerfilters(X^2)$ a filter. Then the function $H;-: \powerfilters(X^2) \to \powerfilters(X^2)$ preserves intersections.
\end{proposition}
\begin{proof}
Let $\mathcal{F}\subseteq \powerfilters(X^2)$ be a set of filters. Then we calculate
\begin{align*}
...
\begin{proposition}
Let $X$ be a set, $H\in \powerfilters(X^2)$ and $y\in X$. Then
\begin{enumerate}
\item $H;(\pfilter{y}\otimes \pfilter{y}) = Hy\otimes\pfilter{y}$;
\item $H = \bigcap_{x\in X}H;(\pfilter{x}\otimes \pfilter{x}) = \bigcap_{x\in X}Hx\otimes\pfilter{x}$;
\item $H;H = \bigcap_{x\in X}H;(\pfilter{x}\otimes \pfilter{x});H = \bigcap_{x\in X}Hx\otimes xH$.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) For all relations $R$ on $X$, we have $R;\{(y,y)\} = Ry \times \{y\}$. As there exist bases of both filters in these forms, the result is proven.

(2) We calculate, using \ref{filterCompositionResidual},
\[ H = H; \upset\{\id\} = H; \Big( \bigcap_{x\in X} \pfilter{x}\otimes \pfilter{x}\Big) = \bigcap_{x\in X} H;(\pfilter{x}\otimes \pfilter{x}). \]
