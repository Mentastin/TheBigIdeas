...
\end{definition}
\begin{lemma}
Let $\sSet{X,\leq}$ be a poset. The convergence defined above on $X$ is a convergence.
\end{lemma}
\begin{proof}
It is monotone by construction: if $F\subseteq F'$, then $\bigcap_{A\in F}\upset A \supseteq \bigcap_{A\in F'}\upset A$ and thus $\Big(\bigcap_{A\in F}\upset A\Big)^l \subseteq \Big(\bigcap_{A\in F'}\upset A\Big)^l$. A similar inclusion holds for $\Big(\bigcap_{A\in F}\downset A\Big)^u$.
...
\begin{lemma}
Let $X$ be a set and $A,B\subseteq X$ such that $A\mesh B$. Then
\begin{enumerate}
\item $A^u \subseteq \upset B$;
\item $A^l \subseteq \downset B$.
\end{enumerate}
\end{lemma}
...
\begin{proposition}
Let $\sSet{X,\leq}$ be a poset. Then any convergence that is compatible with the weak order is stronger than this convergence.
\end{proposition}
\begin{proof}
...
Thus $\lim_\xi F \subseteq \Big(\bigcap_{A\in F}\upset A\Big)^l \cap \Big(\bigcap_{A\in F}\downset A\Big)^u$, so any convergence compatible with the weak order is stronger than this convergence.
\end{proof}

\begin{example}
Consider $P \defeq \setbuilder{(a,b)\in \R^2}{(a=0)\lor (b=0)}$ with ordering
\[ (a,b) \leq (c,d) \;\defequiv\; \begin{cases}
b \leq c & (a = 0 = c) \\
b < c & \big((a\neq 0)\lor (c \neq 0)\big).
\end{cases} \]
...
\item $\Big(\bigcap_{\epsilon > 0}\upset A_\epsilon\Big)^l = \setbuilder{(0,y)}{y > 0}\cup \R\times\{0\}$ and $\Big(\bigcap_{\epsilon > 0}\downset A_\epsilon\Big)^u = \setbuilder{(0,y)}{y < 0}\cup \R\times\{0\}$;
\item $\Big(\bigcap_{\epsilon > 0}\upset A_\epsilon\Big)^l \cap\Big(\bigcap_{\epsilon > 0}\downset A_\epsilon\Big)^u = \R\times\{0\}$.
\end{itemize}
Thus $P$ has no weakest convergence compatible with weak ordering.
\end{example}

\begin{example}
Let $X$ be an infinite set. Consider $P = \{\top, \bot\}\uplus X$ with the following order: $x\leq y$ if and only if either $x = \bot$ or $y = \top$.

Consider any free filter $F$ on $X$. Then
\[ \Big(\bigcap_{A\in F}\upset A\Big)^l \cap \Big(\bigcap_{A\in F}\downset A\Big)^u = X. \]
\end{example}
...
\begin{lemma}
Let $\sSet{X,\leq}$ be a poset. The order convergence is a convergence that is compatible with the weak order.
\end{lemma}
\begin{proof}
Suppose $F, G\in \powerfilters(X)$ and $x\in \lim_\mathfrak{o} F$. 
\end{proof}

\begin{proposition}
Let $\sSet{X,\leq}$ be a poset and $F\subseteq \powerfilters(X)$ a filter. Then $F\overset{\mathfrak{o}}{\longrightarrow} x$ \textup{if and only if} there exist $M,N\subseteq X$ such that
\begin{itemize}
\item $\bigvee M = x = \bigwedge N$;
\item $\upset \setbuilder{\interval[c]{m,n}}{m\in M, n\in N} \subseteq F$
\end{itemize} 
\end{proposition}
\begin{proof}
