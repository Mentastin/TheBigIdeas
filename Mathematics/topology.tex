\chapter{Convergence}
\section{Convergence on directed sets}
\begin{definition}
Let $\sSet{P,\leq}$ be an ordered set. Consider a relation $\xi$ on $(\directed(P), P)$ and denote the image function by $\lim_\xi: \directed(P) \to \powerset(P)$. Then $\xi$ is called a \udef{convergence} on $P$ if for all $Q, Q' \in \directed(P)$:
\begin{itemize}
\item $\bigvee Q \subseteq \lim_\xi Q$;
\item if $Q'$ refines $Q$, then $\lim_\xi Q \subseteq \lim_\xi Q'$.
\end{itemize}
We usually write $Q\overset{\xi}{\to} x$ instead of $Q\xi x$. We may also write $Q \to x$ is the convergence is clear from the context.
\end{definition}

\begin{definition}
\begin{itemize}
\item $\bigwedge Q \subseteq \lim Q$
\item $x \leq \lim \{x\}$.
\item $\forall x\in Q: \lim Q = \lim (Q\cap \upset x)$ (Kent convergence).
\end{itemize}
\end{definition}
...
\begin{itemize}
\item we denote the image function of $\xi$ as $\lim_\xi: \filters(\powerset(X)) \to \powerset(X): F\mapsto F\xi$; we call $\lim_\xi F$ the \udef{$\xi$-limit} of $F$;
\item $\xi$ is called a \udef{preconvergence} on $X$ if $\lim_\xi$ is order-preserving when $\filters(\powerset(X))$ is ordered by inclusion:
\[ F \subseteq G \implies \lim_\xi F \subseteq \lim_\xi G; \]
\item $\xi$ is called a \udef{convergence} if it is a preconvergence and it is \udef{centered}: for all $x\in X: x \in \lim_\xi \upset\{x\}$;
\item a (pre)convergence $\xi$ is called \udef{finitely deep} if 
...
If $\lim_\xi F \neq \emptyset$, we say the filter $F$ \udef{converges}.
\end{definition}
...
\begin{definition}
Let $X$ be a set and $\xi$ a preconvergence on $X$. Then $\xi$ is called \udef{Kolmogorov} or \udef{$T_0$} if every pair of points in $X$ is distinguishable, that is for all $x,y\in X$:
...
\begin{definition}
Let $X$ be a set and $\xi$ a preconvergence on $X$. Then $\xi$ is called \udef{Fréchet} or \udef{$T_1$} if for all $x\in X$: $\lim_\xi \upset\{x\} \subseteq \{x\}$.
\end{definition}
