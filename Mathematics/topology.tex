...
\begin{definition}
Let $X$ be a set and $\xi,\zeta$ preconvergences on $X$. We say $\xi$ is \udef{finer} than $\zeta$, denoted $\xi \leq \zeta$, if $\lim_\xi F \subseteq \lim_\zeta F$ for all $F\in\powerfilters(X)$. We also say $\zeta$ is \udef{coarser} than $\xi$.
\end{definition}
...
\subsection{Distinguishability and separation}
\subsubsection{Distinguishable points}
\begin{definition}
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space and $A,B\subseteq X$. We say $A$ and $B$ are \udef{separated by convergent filters} if $\a dh_\xi(A) \perp B$ and $A\perp \adh_\xi(B)$.
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then $\xi$ is called \udef{$R_1$}, \udef{reciprocal} or \udef{preregular} if
...
\begin{proposition}
Let $\sSet{X,\xi}$ be a pretopological convergence space and $x,y\in X$. Then ${\lim_\xi}^{-1}(x)\perp {\lim_\xi}^{-1}(y)$ \textup{if and only if} there exist disjoint vicinities of $x$ and $y$ (i.e.\ $\neg\big(\vicinity_\xi(x)\mesh \vicinity_\xi(x)\big)$).
\end{proposition}
\begin{proof}
Assume disjoint vicinities $V_x,V_y$ . If $F\in\powerfilters(X)$ converges to $x$ and $y$, then $V_x\in F$ and $V_y\in F$. In this case $V_x\cap V_y = \emptyset \in F$. So $F = \powerset(X)$ and thus not a proper filter.

Now assume there are no disjoint vicinities, i.e.\ $\vicinity_\xi(x)\mesh \vicinity_\xi(y)$. This means that $y$ is an accumulation point of $\vicinity_\xi(x)$. By \ref{subfilterToAccumulationPoint} there exists a filter $F\supseteq \vicinity_\xi(x)$ that converges to $y$. By monotonicity $F$ converges to $x$. Thus $F\in {\lim_\xi}^{-1}(x)\cap {\lim_\xi}^{-1}(y) \neq \emptyset$.
\end{proof}
...
Now let $X$ be a finite set and let $F$ be a proper filter in $\powerfilters(X)$. Then $F$ is principal by \ref{finiteFiltersPrincipal} and not free because it is proper. So we can take $x\in \ker F$ and $F \subseteq \pfilter{x}$. Thus $\lim F \subseteq \lim \pfilter{x} \supseteq \{x\}$, meaning the convergence is $T_2$.
\end{proof}
\begin{corollary}
...
\subsubsection{$R_2$ or regular}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then $\xi$ is called \udef{regular} or \udef{$R_2$} if 
for all $F\in\powerfilters(X)$, $F\overset{\xi}{\longrightarrow} x$ implies $\adh_\xi[F] = \setbuilder{\adh_\xi(A)}{A\in F} \overset{\xi}{\longrightarrow} x$.
\end{definition}
...
\item for all $A\subseteq X$ and $x\notin \adh_\xi(A)$: $\lim^{-1}(x) \perp \lim^{-1}(\adh_\xi(A))$.
\end{enumerate}
\end{proposition}
\begin{proof}
$(1) \Rightarrow (2)$ One direction is given by definition. The other follows by monotonicity because $\adh_\xi[F] \preceq F$.


\end{proof}

\begin{proposition}
Let $\sSet{X,\xi}$ be a pretopological convergence space, $A\subseteq X$ and $x\in X$. Then $\lim^{-1}(x) \perp \lim^{-1}(\adh(A))$ \textup{if and only if} there exist disjoint vicinities of $x$ and $\adh_\xi(A)$.
\end{proposition}
...
\subsubsection{$T_3$ or regular Hausdorff}
\begin{definition}
...
\subsubsection{$R_3$ or normal}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then $\xi$ is called \udef{normal} or \udef{$R_3$} if
\[ \forall A,B\subseteq X: \qquad \adh_\xi(A)\perp\adh_\xi(B) \implies {\lim}^{-1}(\adh_\xi(A))\perp{\lim}^{-1}(\adh_\xi(B)). \]
\end{definition}
...
\subsubsection{$T_4$ or normal Hausdorff}
\begin{definition}
...
\begin{definition}
A convergence space $\sSet{X, \xi}$ is called \udef{first countable} or \udef{$C1$} if for all $x\in X$ the vicinity filter $\vicinity_\xi(x)$ has a countable base.
\end{definition}
...
\begin{lemma} \label{groupConvergenceConstruction}
...
Let $F \to (x,y) \in G\times G$, so by \ref{convergenceProductFilter} there exist $F_1\to x$ and $F_2 \to y$ such that $F_1\otimes F_2 \leq F$. Then $F_1 - x \in \mathcal{G}$ and $F_2 - y \in \mathcal{G}$. From point (3) (and commutativity) we get $F_1 - F_2 - (x - y) \in \mathcal{G}$, so $F_1 - F_2 \to x-y$. Now $F_1 - F_2 = u[F_1\otimes F_2] \leq u[F]$ (TODO ref!), so $u[F] \to x-y$ and thus $u$ is continuous.
\end{proof}
...
\begin{enumerate}
\item $G$ is metrisable;
\item the topology on $G$ is induced by a left translation invariant metric;
\item the topology on $G$ is induced by a right translation invariant metric;
\item $G$ is Hausdorff (TODO necessary?), first countable and topological;
\item $\{1\}$ is closed, and $\vicinity_\xi(1)$ has a countable base.
\end{enumerate}
\end{theorem}
