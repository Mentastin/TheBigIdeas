...
\subsubsection{Separate and joint continuity of binary functions}
\begin{proposition}
Let $f: X\times Y \to Z$ be a binary function and $\mathcal{F}$ a uniform filter set on $Z$. We say $F\in\powerfilters(X)$ converges \udef{$f$-uniformly} to $x\in X$ if
\[ \exists G\in \powerfilters(Y): \forall y\in Y: \; TODO \]
\end{proposition}
...
\subsubsection{Lipschitz and Hölder uniformities}
\begin{definition}
Let $\sSet{X,d}$ be a metric space. The \udef{$M$-Lipschitz uniformity}
\end{definition}
