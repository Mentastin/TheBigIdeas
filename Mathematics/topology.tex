...
\begin{proposition} \label{continuousMappingVicinities}
Let $\sSet{X,\xi}$ and $\sSet{Y,\zeta}$ be convergence spaces, $f: X\to Y$ a continuous function and $x\in X$. If $\xi$ is pretopological, then
\begin{enumerate}
\item $\upset f[\vicinity_\xi(x)] \supseteq \vicinity_\zeta(f(x))$;
\item if $U\in \vicinity_\zeta(f(x))$, then there exists $V\in \vicinity_\xi(x)$ such that $f[V] \subseteq U$.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) We have that $\vicinity_\xi(x)$ converges to $x$, so by continuity $f[\vicinity_\xi(x)]$ converges to $f(x)$ and thus $\upset f[\vicinity_\xi(x)] \supseteq \vicinity_\zeta(f(x))$.

(2) From (1) we know that $U\in \upset f[\vicinity_\xi(x)]$. The result follows imemdiately.
\end{proof}
...
$(2) \Rightarrow (3)$ Assume $x$ and $\adh_\xi(A)$ are not separated by vicinities. We then need to show that $x\in \adh_\xi(A)$. By \ref{separationByVicinitiesEquivalences} there exists
\end{proof}
...
Let $\sSet{X,\xi}$ be a topological space. Then $\xi$ is regular \textup{if and only if} for any open set $O\subseteq X$ and $x\in O$ there exists an open set $U$ such that $x\subseteq U\subseteq \overline{U} \subseteq O$.
\end{proposition}
\begin{proof}
Regular means that for any closed set $C$ there exist disjoint open sets $U,V$ such that $x\in U$ and $C\subseteq V$.

The equivalence can be seen by setting $O = C^c$. Then $\overline{U}\subseteq V^c \subseteq C^c = O$.
\end{proof}
...
\end{lemma}
\begin{proof}
Consider the function $f: G\times G \to G: (x,y)\mapsto xy$. Then by \ref{continuousMappingVicinities} and \ref{productVicinity} we have
