...
\begin{proposition} \label{principalUpDownSetsClosed}
Let $\sSet{P,\leq}$ be a poset, $\xi$ a convergence compatible with the weak order and $x,y\in P$. Then
\begin{enumerate}
...
\end{proposition}
\begin{proof}
(1) Take arbitrary $y\in \adh_\xi\big(\upset\{x\}\big)$. Then, from \ref{principalAdherenceInherence}, there exists $F\in \powerfilters(P)$ such that $F\to y$ and $\upset\{x\} \in F$. This implies $\pfilter{x} \leq_w F$, so $x\leq y$ and $y\in \upset \{x\}$. Thus $\adh_\xi\big(\upset\{x\}\big) \subseteq \upset\{x\}$, which means that $\upset\{x\}$ is closed.

(2) The argument for $\downset\{x\}$ is dual.

(3) We have that $\interval{x,y} = \upset\{x\} \cap \downset\{y\}$ is closed from (1),(2) and \ref{propertiesTopology}.
\end{proof}

\begin{lemma} \label{principalUpDownsetWeaklyOrderedFilters}
Let $\sSet{P,\leq}$ be a poset $x\in P$ and $F,G\in \powerfilters(P)$. If $F\leq_w G$, then $\downset\{x\} \in G$ implies $\downset\{x\} \in F^\mesh$.
\end{lemma}
\begin{proof}
Suppose $\downset\{x\} \in G$. Take arbitrary $A\in F$. Then there exists $a\in A$ and $b\in \downset \{x\}$ such that $a\leq b \leq x$. Thus $a \leq x$ and $a\in \downset\{x\}$, which implies $A \mesh \downset\{x\}$.
\end{proof}

\begin{proposition}
Let $\sSet{P,\leq}$ be a poset and $\xi$ a convergence on $P$. Then the following are equivalent:
\begin{enumerate}
\item $\xi$ is compatible with the weak order;
\item $\upset\{x\}$ is closed for all $x\in P$;
\item $\downset\{x\}$ is closed for all $x\in P$.
\end{enumerate}
\end{proposition}
TODO: is this true????
\begin{proof}
$(2) \Rightarrow (1)$ Let $F,G\in\powerfilters(P)$ be such that $F\to x$, $G\to y$ and $F\leq_w G$. We need to prove $x \leq y$. Suppose this is not true, so $x\nleq y$. Then $y\in \big(\upset\{x\}\big)^c$. Since $\big(\upset\{x\}\big)^c$ is open, we have $\big(\upset\{x\}\big)^c\in G$ from \ref{openClosedCriteria}. Then \ref{complementInIsotoneGrill} gives $\upset\{x\} \notin G^\mesh$ and \ref{principalUpDownsetWeaklyOrderedFilters} implies $\upset\{x\} \notin F$. TODO: does this give a contradiction?
\end{proof}

\begin{lemma} \label{latticeOperationsWeaklyOrderedFilters}
Let $\sSet{P,\leq}$ be a poset and $F,G\in\powerfilters(P)$ such that $F\leq_w G$. Then
\begin{enumerate}
...
Let $\sSet{P,\leq}$ be a poset and $\xi$ a convergence such that
\begin{itemize}
\item $\xi$ is Hausdorff;
\item either $\vee: P\times P \to P$ or $\wedge: P\times P \to P$ is continuous.
\end{itemize}
Then $\xi$ is compatible with the weak order.
\end{proposition}
\begin{proof}
Suppose $\vee: P\times P \to P$ is continuous.

Take arbitrary $F,G\in\powerfilters(P)$ such that $F\leq_w G$, $F\to x$ and $G\to y$. Then \ref{latticeOperationsWeaklyOrderedFilters} gives $\vee^{\imf\imf}(F\otimes G) \amesh G$. Since $\vee^{\imf\imf}(F\otimes G) \to x\vee y$, \ref{HausdorffFilterPartition} yields $x\vee y = y$ and so $x\leq y$.

The argument if $\wedge: P\times P \to P$ is continuous, is dual.
\end{proof}
...
Take $y\in \lim_\xi(F)$. Take arbitrary $x\in \bigcap_{A\in F}\upset A$. Then $F \leq_w \pfilter{x}$ by \ref{weakRelationPrincipalFilter}, so $y\leq x$ by compatibility with the weak order. Since this holds for all such $x$, we have $y\in \Big(\bigcap_{A\in F}\upset A\Big)^\geq$. Similarly $y\in \Big(\bigcap_{A\in F}\downset A\Big)^\leq$.
\end{proof}
