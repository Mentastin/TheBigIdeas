...
\[ \vicinity_\xi(A) \defeq \bigcap \setbuilder{F\in \powerfilters(X)}{x\in \lim_\xi F}. \]
...
\subsection{Distinguishability, separation and regularity}
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Then $\xi$ is called \udef{regular} or \udef{$R_2$} if it is based in $\setbuilder{\adh_\xi(A)}{A\in\powerset(X)}$.
\end{definition}
...
\end{proposition}
\begin{proof}
$(1) \Leftrightarrow (2)$ Assume (1), then there exists a filter $G$ based in $\setbuilder{\adh_\xi(A)}{A\in\powerset(X)}$ such that $G\to x$ and $G\subseteq F$. We just need to show that $G\subseteq \adh[F]$. Indeed take $A\in G$, then $A = \adh(B)$ for some $B\subseteq X$. Now $B\in F$
...
\end{proposition}
\begin{proof}
$(1) \Leftrightarrow (2)$ By \ref{pretopologicalRegularity}.

$(2) \Leftrightarrow (3)$ The equivalence can be seen by setting $O = C^c$. Then $\overline{U}\subseteq V^c \subseteq C^c = O$.
\end{proof}
...
\subsection{Comparison with reals and metrisability}
...
is well-defined everywhere because $f^{-1}[\{0\}]\perp g^{-1}[\{0\}]$ and functionally separated $A$ and $B$.
\end{proof}
