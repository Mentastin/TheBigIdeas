...
If $\entourage_\mathcal{U}\in \Theta^{-1}(\mathcal{U})$, the uniform filter set associated with $\mathcal{U}$, then we call the uniform space \udef{topological}.
\end{definition}

\begin{lemma}
Let $\sSet{X, \mathcal{U}}$ be a topological uniform space. Then $F\mathrel{\mathcal{U}}G$ \textup{if and only if} $\entourage_\mathcal{U}\subseteq F\otimes G$.
\end{lemma}
\begin{proof}
TODO
\end{proof}
