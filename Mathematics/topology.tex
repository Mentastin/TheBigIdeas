...
\subsection{(Pre)topological initial and final convergence}
...
\item Let $\{g_i: \sSet{X_i, \xi_i} \to Y\}_{i\in I}$ be set of functions from \emph{pretopological} preconvergence spaces and $\nu$ the final preconvergence on $Y$ w.r.t. this set. Then $\nu$ is pretopological and
...
Let $Y$ be a set, $F\in \powerfilters(Y)$ and $y\in Y$.
Let $\{f_i: Y\to \sSet{Z_i, \zeta_i}\}_{i\in I}$ be set of functions to \emph{topological} preconvergence spaces and $\mu$ the initial preconvergence on $Y$ w.r.t. this set. Then $\mu$ is topological and $\topology_\mu$ is generated by $\setbuilder{f_i^\preimf(U)}{i\in I, U\in \topology_{\zeta_i}}$.
\end{proposition}
\begin{proof}
By \ref{pretopologicalInitialFinalConvergence}, $\mu$ is pretopological. Each $U\in \vicinity_{\zeta_i}(f(x)) = \neighbourhood_{\zeta_i}(f(x))$ contains an open set, so $f_i^\preimf(U)$ also contains an open set by \ref{preimageOpenClosed}. Thus $\vicinity_\mu(x)$ is based in open sets. This means that the initial convergence is topological.
...
The final convergence w.r.t. a set of functions from topological spaces is not necessarily topological (see \ref{convergenceSpaceQuotientOfTopologicalSpace}).

\begin{proposition}
Let $Y$ be a set, $F\in \powerfilters(Y)$ and $y\in Y$.
Let $\{g_i: \sSet{X_i, \xi_i} \to Y\}_{i\in I}$ be set of functions from topological convergence spaces and $\nu$ the final \emph{topological} convergence on $Y$ w.r.t. this set. Then
...
(2) The convergence group $H$ is pretopological by \ref{pretopologicalInitialFinalConvergence} and thus topological by \ref{pretopologicalGroupConvergence}.
\end{proof}

\begin{lemma}
Let $\sSet{G, \xi}$ and $\sSet{H, \zeta}$ be convergence groups. Let $f: A\to B$ be a continuous group homomorphism and $N\lhd G$ a normal subgroup such that $N\subseteq \ker f$. Then there exists a unique continuous homomorphism $f': A/N \to B$ such that
\[ \begin{tikzcd}
