...
\end{definition}
\begin{lemma}
A uniform convergence is a convergence.
\end{lemma}

\subsubsection{Induced uniform space}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. Let $\Phi(\xi)$ be a relation on $\powerfilters(X)$ defined by
...
\begin{lemma}
The uniform space associated to a convergence space is a uniform space.
\end{lemma}
...
\begin{align*}
&\Phi: \{\text{convergences on $X$}\} \to \{\text{uniformities on $X$}\}: \xi \mapsto \text{uniformity associated with $\xi$} \\
&\Gamma: \{\text{uniformities on $X$}\} \to \{\text{convergences on $X$}\}: \mathcal{U} \mapsto \text{convergence associated with $\mathcal{U}$}
\end{align*}
form a Galois connection $(\Phi, \Gamma)$. Additionally,
\begin{enumerate}
\item $\im(\Phi)$ is the set of complete uniformities on $X$;
\item $\im(\Gamma)$ is the set of reciprocal convergences on $X$.
\end{enumerate}
\end{proposition}
In the notation of the proposition, we have
\begin{align*}
F \mathrel{\Phi(\xi)} G \qquad&\iff\qquad \exists x\in X: \; \big(F\overset{\xi}{\longrightarrow} x\big) \land \big(G\overset{\xi}{\longrightarrow} x\big) \\
F \overset{\Gamma(\mathcal{U})}{\longrightarrow}x \qquad&\iff\qquad F\mathrel{\mathcal{U}}\pfilter{x}.
\end{align*}
\begin{proof}
Let $\mathcal{U}$ be a uniformity on $X$ and $\xi$ a convergence on $X$. Then we need to prove $\Phi(\xi) \subseteq \mathcal{U}$ \textup{if and only if} $\xi \subseteq \Gamma(\mathcal{U})$.
