...
\begin{lemma}
Let $V$ be a tolerance relation on $X$ and $U = \setbuilder{xV}{x\in X}$. Then
\[ \operatorname{star}_U(xV) = x(V;V;V). \]
\end{lemma}
\begin{proof}
We calculate
\begin{align*}
\operatorname{star}_U(xV) &= \bigcup \setbuilder{yV}{xV\mesh yV} \\
...
\begin{definition}
Let $X$ be a set. A \udef{uniform cover} of $X$ is a filter in $\sSet{\powerset^2(X), <^*}$.
\end{definition}


\subsection{Uniform convergences and equicontinuity}
\begin{proposition}
Let $f: X\times Y \to \sSet{Z, \mathcal{U}}$ be a binary function to a uniform space. Let $\Delta_Y = \setbuilder{(y,y)\in Y^2}{y\in Y}$ be the diagonal on $Y$. We say
\begin{itemize}
\item $F\in\powerfilters(X)$ converges \udef{$f$-pointwise} on $Y$ to $x\in X$ if
\[ \forall y\in Y: \quad f(F, y)\mathrel{\mathcal{U}} f(x,y); \]
\item $F\in\powerfilters(X)$ converges \udef{$f$-uniformly} on $Y$ to $x\in X$ if
\[  f(F, \Delta_Y) \mathrel{\mathcal{U}} f(x,\Delta_Y); \]
\item $H\subseteq X$ is \udef{$f$-pointwise equicontinuous} on $Y$ if
\[  \forall G\to y\in Y: \quad f(\Delta_H, G) \mathrel{\mathcal{U}} f(\Delta_H, y); \]
\item $H\subseteq X$ is \udef{$f$-uniformly equicontinuous} on $Y$ if
\[  f(\Delta_H, \Delta_Y) \mathrel{\mathcal{U}} f(\Delta_H, \Delta_Y). \]
\end{itemize}
\end{proposition}
WRONG!
