...
\end{lemma}
\begin{proof}
One direction is immediate. The other follows by monotonicity because $\upset \adh_\xi^\imf(F) \subseteq F$.
\end{proof}
...
\end{proposition}
\begin{proof}
Let $\sSet{X,\xi}$ be an $R_1$ convergence space. We use \ref{R1Conditions}.
\end{proof}
...
\[ \lim_\xi F \subseteq \lim_\xi \adh_{\omega_\xi}^\imf(F) \]
for all $F\in \powerfilters(X)$.
\end{definition}
