...
\subsubsection{Convergence selections}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. A \udef{convergence selection} or \udef{$\xi$-selection} is a function $F: X\to \powerfilters(X)$ such that $F(x)$ is a proper filter and $F(x) \overset{\xi}{\longrightarrow} x$ for all $x\in X$.

Let $A$ be a set and $f: A\to X$ a function. An \udef{$f$-convergence selection} is a function $F: A\to \powerfilters(X)$ such that $F(a)$ is a proper filter and $F(a) \overset{\xi}{\longrightarrow} f(a)$ for all $a\in A$.
\end{definition}
Note that a convergence selection is just an $\id$-convergence selection.


\begin{lemma}
Let $\sSet{X,\xi}$ be a convergence space. The function $X\to \powerfilters(X): x\mapsto \pfilter{x}$ is a convergence selection.
\end{lemma}

\begin{lemma} \label{fSelectionLargerThanVicinities}
Let $\sSet{Y,\zeta}$ be a convergence space, $X$ a set, $f: X\to Y$ a function and $F$ an $f$-convergence selection. Then $\vicinities_\xi\circ f \subseteq F$ pointwise.
\end{lemma}
In other words, $\vicinities_\xi\big(f(x)\big) \subseteq F(x)$ for all $x\in X$.
\begin{proof}
Take arbitrary $x\in X$. Since $F(x)$ converges to $f(x)$, we have $\vicinities_\xi\big(f(x)\big) \subseteq F(x)$.
\end{proof}

\begin{proposition} \label{convergenceAdherenceSelectionAdherence}
Let $\sSet{Y,\zeta}$ be a convergence space, $X$ a set, $f: X\to Y$ a function and $F$ an $f$-convergence selection. Let $A\subseteq Y$ and $G\in \powerfilters(Y)$. Then
\begin{enumerate}
\item $f^\imf\big(\adh_F(A)\big) \subseteq \adh_\xi(A)$;
\item $\adh_\xi^\imf(G) \subseteq \upset f^{\imf\imf}\big(\adh_F^\imf(G)\big)$.
\end{enumerate}
\end{proposition}
\begin{proof}
(1) From \ref{fSelectionLargerThanVicinities}, we have $F^\mesh \subseteq (\vicinities_\xi\circ f)^\mesh$, so
\begin{align*}
x\in \adh_F(A) \iff& A\in F(x)^\mesh \\
\implies& A\in \vicinities_\xi\big(f(x)\big)^\mesh \\
\iff& f(x)\in \adh_\xi(A).
\end{align*}
This implies $f^\imf\big(\adh_F(A)\big) \subseteq \adh_\xi(A)$.

(2) Take arbitrary $B\in G$. We need to show that $\adh_\xi(B) \in \upset f^{\imf\imf}\big(\adh_F^\imf(G)\big)$. Indeed, $f^\imf\big(\adh_F(B)\big) \subseteq \adh_\xi(B)$, from (1).
\end{proof}

\begin{proposition} \label{adherenceSelectionContour}
Let $\sSet{Y,\zeta}$ be a convergence space, $X$ a set, $f: X\to Y$ a function, $F$ an $f$-convergence selection and $G\in \powerfilters(X)$. Then
\[ \adh_\xi^\imf\big(F(G)\big) \subseteq \upset f^{\imf\imf}(G). \]
\end{proposition}
\begin{proof}
Since $\im(F) \subseteq \powerfilters(Y)\setminus\{\powerset(Y)\}$, we have $F(x) \subseteq F^\mesh(x)$ for all $x\in X$ from \ref{properFiltersSelfMesh}. Then \ref{inclusionPreservantionOfContours} implies $F(G) \subseteq F^\mesh(G)$. We calculate
\begin{align*}
F(G) \subseteq F^\mesh(G) \iff& F(G) \subseteq \big(F(G^\mesh)\big)^\mesh \\
\iff& F(G) \amesh F(G^\mesh) \\
\iff& \adh_F^\imf\big(F(G)\big) \amesh G^\mesh \\
\iff& \adh_F^\imf\big(F(G)\big) \subseteq G^{\mesh\mesh} \\
\iff& \adh_F^\imf\big(F(G)\big) \subseteq G \\
\implies& \upset f^{\preimf\imf}\big(\adh_\xi^\imf\big(F(G)\big)\big) \subseteq G \\
\iff& \adh_\xi^\imf\big(F(G)\big) \subseteq \upset f^{\imf\imf}(G),
\end{align*}
using \ref{adherenceContourDuality}, \ref{upwardClosureGrill} and \ref{convergenceAdherenceSelectionAdherence} (with \ref{upsetPreimageImageGaloisConnection}).
\end{proof}
...
\begin{lemma}
Let $\sSet{X,\xi}$ be a convergence space. Then $\vicinities_\xi$ is a convergence selection \textup{if and only if} $\xi$ is pretopological.
\end{lemma}
...
\end{proposition}
This proposition uses the ultrafilter lemma.
\begin{proof}
...
\end{corollary}
TODO: redo using \ref{adherenceContourDuality}.
\begin{proof}
We have
\begin{align*}
...
Now take $A,B\in \neighbourhoods_\xi(x)$. Then there exist $O_A, O_B \in \topology_\xi$ such that $x\in O_A$ and $x\in O_B$. Now $x\in O_A\cap O_B$ and $O_A\cap O_B \in \topology_\xi$ by \ref{propertiesTopology}. Since $O_A\cap O_B\subseteq A\cap B$, we have $A\cap B\in \neighbourhoods_\xi(x)$.
\end{proof}
...
\begin{definition}
Let $\sSet{P,\leq}$ be a poset. A convergence $\xi$ in $P$ is called \udef{compatible with the order} or \udef{order-compatible} if, for all $F,G\in\powerfilters(P)$ and $x,y\in P$ such that $F\overset{\xi}{\longrightarrow} x$ and $G \overset{\xi}{\longrightarrow} y$, we have
\[ F \leq_w G \qquad\implies\qquad x \leq y. \]
\end{definition}

\begin{proposition} \label{convergenceCompatibleWithOrderIFFGraphClosed}
Let $\sSet{P,\leq}$ be a poset and $\xi$ a convergence on $P$. Then $\xi$ is compatible with the order \textup{if and only if} $\graph(\leq)$ is a closed subset of $P\times P$.
\end{proposition}
\begin{proof}
First assume $\xi$ is compatible with the order and take $(x,y) \in \adh_{\xi\otimes \xi}\big(\graph(\leq)\big)$, so \ref{productAdherenceFilterLemma} implies that there exist $F,G\in \powerfilters(P)$ such that $F\to x, G\to y$ and $\graph(\leq) \in F\otimes G$. Since \ref{properFiltersSelfMesh} gives $F\otimes G \subseteq (F\otimes G)^\mesh$, we have $F \leq_w G$ from \ref{relationsOnFiltersRestated}. This implies $x\leq y$, so $(x,y)\in \graph(\leq)$. Thus $\adh_{\xi\otimes \xi}\big(\graph(\leq)\big) \subseteq \graph(\leq)$, which means that $\graph(\leq)$ is closed.

Now assume $\graph(\leq)$ is a closed subset of $P\times P$. Take arbitrary $F,G\in\powerfilters(P)$ and $x,y\in P$ such that $F\overset{\xi}{\longrightarrow} x$, $G \overset{\xi}{\longrightarrow} y$ and $F \leq_w G$. Then $F\otimes G \to (x,y)$ and $\graph(\leq) \in (F\otimes G)^\mesh$ from \ref{relationsOnFiltersRestated}, so $(x,y) \in \adh_{\xi\otimes \xi}\big(\graph(\leq)\big) = \graph(\leq)$ and thus $x \leq y$.
\end{proof}

\begin{proposition} \label{compatibleWeakOrderHausdorff}
Let $\sSet{X,\leq}$ be a poset and $\xi$ a convergence compatible with the order. Then $\xi$ is Hausdorff.
\end{proposition}
\begin{proof}
Take $F\in\powerfilters(X)$ and $x,y\in \lim_\xi F$. Since $\leq_w$ is reflexive by \ref{reflexivityFilterRelations}, we have $F\leq_w F$, which implies $x\leq y$ and $y\leq x$ by compatibility with the order. Thus $x=y$ by antisymmetry.
\end{proof}
...
\begin{enumerate}
\item if $\lim_\xi F\subseteq \{\liminf F\}\cap\{\limsup F\}$ for all $F\in\powerfilters(X)$, then $\xi$ is compatible with the order;
...
\begin{proposition}
Let $\sSet{P,\leq}$ be a poset and $\xi$ a convergence on $P$ that is compatible with the order. Let $A\subseteq P$ be a compact subset. Then $\downset A$ and $\upset A$ are closed. 
\end{proposition}
\begin{proof}
We can write
\[ \downset A = \proj_1^\imf\big((P\times A) \cap \graph(\leq)\big) \]
From \ref{compactClosedSets} and \ref{compatibleWeakOrderHausdorff}, we know that $A$ is closed, so \ref{productOpenClosed} and \ref{propertiesTopology} imply that $P\times A$ is closed. Using \ref{convergenceCompatibleWithOrderIFFGraphClosed} and \ref{propertiesTopology}, we have that $(P\times \{x\}) \cap \graph(\leq)$ is a closed subset of $P\times P$. It is also a closed subset of $P\times \{x\}$, from \ref{subspaceAdherence}.

Since $A$ is compact, \ref{projectionClosedFunction} implies that $\proj_1^\imf\big((P\times A) \cap \graph(\leq)\big)$ is closed.

The argument for $\upset A$ is dual.
\end{proof}
\begin{corollary} \label{principalUpDownSetsClosed}
Let $\sSet{P,\leq}$ be a poset, $\xi$ a convergence compatible with the order and $x,y\in P$. Then
\begin{enumerate}
\item $\upset\{x\}$ is closed;
\item $\downset\{x\}$ is closed;
\item $\interval{x,y}$ is closed.
\end{enumerate}
\end{corollary}
\begin{proof}
(1,2) Immediate from the proposition.

(3) We have that $\interval{x,y} = \upset\{x\} \cap \downset\{y\}$ is closed from (1),(2) and \ref{propertiesTopology}.
\end{proof}
\begin{proof}[Alternate proof of (1)]
Take arbitrary $y\in \adh_\xi\big(\upset\{x\}\big)$. Then, from \ref{principalAdherenceInherence}, there exists $F\in \powerfilters(P)$ such that $F\to y$ and $\upset\{x\} \in F$. This implies $\pfilter{x} \leq_w F$, so $x\leq y$ and $y\in \upset \{x\}$. Thus $\adh_\xi\big(\upset\{x\}\big) \subseteq \upset\{x\}$, which means that $\upset\{x\}$ is closed.
\end{proof}

\begin{lemma} \label{latticeOperationsWeaklyOrderedFilters}
Let $L$ be a lattice and $F,G\in\powerfilters(L)$ such that $F\leq_w G$. Then
\begin{enumerate}
\item $\vee^{\imf\imf}(F\otimes G) \amesh G$;
\item $\wedge^{\imf\imf}(F\otimes G) \amesh F$.
\end{enumerate}
\end{lemma}
\begin{proof}
(1) Take arbitrary $A,B\in G$ and $C\in F$. Since $F\leq_w G$, there exists $x\in C$ and $y\in A\cap B$ such that $x\leq y$. Then $y = x\vee y \in \vee^\imf(C\times A)$ and $y\in B$, so $\vee^\imf(C\times A)\mesh B$ and $\vee^{\imf\imf}(F\otimes G) \amesh G$.

(2) Dual.
\end{proof}

\begin{proposition} \label{HausdorffContinuousLatticeOperationsOrderCompatible}
Let $L$ be a lattice and $\xi$ a convergence such that
\begin{itemize}
\item $\xi$ is Hausdorff;
\item either $\vee: L\times L \to L$ or $\wedge: L\times L \to L$ is continuous.
\end{itemize}
Then $\xi$ is compatible with the order.
\end{proposition}
\begin{proof}
Suppose $\vee: L\times L \to L$ is continuous.

Take arbitrary $F,G\in\powerfilters(L)$ such that $F\leq_w G$, $F\to x$ and $G\to y$. Then \ref{latticeOperationsWeaklyOrderedFilters} gives $\vee^{\imf\imf}(F\otimes G) \amesh G$. Since $\vee^{\imf\imf}(F\otimes G) \to x\vee y$, \ref{HausdorffFilterPartition} yields $x\vee y = y$ and so $x\leq y$.

The argument if $\wedge: L\times L \to L$ is continuous, is dual.
\end{proof}

\subsubsection{Convergence potentially compatible with order}
\begin{definition}
...
Let $\sSet{X,\leq}$ be a poset, $\xi$ a convergence on $X$ that is compatible with the order and $F\in \powerfilters(X)$. Then
\[ \lim_\xi F \subseteq \Big(\bigcap_{A\in F}\upset A\Big)^\geq \cap \Big(\bigcap_{A\in F}\downset A\Big)^\leq. \]
\end{proposition}
\begin{proof}
Let $\xi$ be an arbitrary convergence that is compatible with the order.

Take $y\in \lim_\xi(F)$. Take arbitrary $x\in \bigcap_{A\in F}\upset A$. Then $F \leq_w \pfilter{x}$ by \ref{weakRelationPrincipalFilter}, so $y\leq x$ by compatibility with the order. Since this holds for all such $x$, we have $y\in \Big(\bigcap_{A\in F}\upset A\Big)^\geq$. Similarly $y\in \Big(\bigcap_{A\in F}\downset A\Big)^\leq$.
\end{proof}
...
\begin{enumerate}
\item $X$ has a weakest convergence compatible with order;
\item $\xi$ is Hausdorff;
\item $\xi$ is compatible with the order.
\end{enumerate}
\end{proposition}
Is this true????
\begin{proof}
...
\end{align*}
It is enough to show that both $\zeta_x$ and $\zeta_y$ are compatible with the order, since in this case $\zeta_x \vee \zeta_y$ is not Hausdorff and thus not compatible with the order by \ref{compatibleWeakOrderHausdorff}.
...
The convergence $\zeta_y$ is compatible with order by a similar argument.
...
\end{itemize}
Thus $P$ has no weakest convergence compatible with the ordering.
\end{example}
...
\begin{example}
Order convergence not topological. See \url{https://arxiv.org/pdf/1705.09883}
\end{example}

\begin{lemma} \label{orderConvergenceCompatibleWeakOrder}
Let $\sSet{X,\leq}$ be a poset. The order convergence on $X$ is compatible with the order.
\end{lemma}
\begin{proof}
...
\begin{proposition} \label{orderConvergenceRegular}
Let $\sSet{P,\leq}$ be a poset. Then the order convergence on $P$ is regular.
\end{proposition}
See also \cite{VANDERWALT2011138}.
\begin{proof}
Take $F\in\powerfilters(P)$ and $x\in P$ such that $F\overset{\mathfrak{o}}{\longrightarrow} x$. Let $M,N$ be the sets from \ref{orderConvergenceIntervalBase}.

For all $a\in M$ and $b\in N$, the interval $\interval{a,b}$ is closed, from \ref{principalUpDownSetsClosed}. Thus $\interval{a,b} \in \adh^\imf(F)$, and \ref{orderConvergenceIntervalBase} implies that $\adh^\imf(F)\overset{\mathfrak{o}}{\longrightarrow} x$.
\end{proof}

\begin{note}
I had thought that the following may be true:
\begin{lemma*}
Let $P$ be a poset and $A\subseteq P$ a subset. Then $A$ is order-dense in $P$ \textup{if and only if} for all $x\in P$ there exist subsets $M,N \subseteq A$ such that $\bigvee M = x = \bigwedge N$.
\end{lemma*}
\begin{proof}
First assume $A$ is order-dense in $P$ and take arbitrary $x\in P$. Then there exists $F \overset{\mathfrak{o}}{\longrightarrow} x$ such that $A\in F$. From \ref{orderConvergenceIntervalBase}, we know that there exist $M_0, N_0 \subseteq X$ such that $\bigvee M_0 = x = \bigwedge N_0$ and $\interval{a,b} \in F$ for all $a\in M_0, b\in N_0$.

Since $A\in F$, we have $A\mesh \interval{a,b}$. 
\end{proof}


It turns out it is not. Consider $P = \R^+\times \{0,1\} \setminus \{(0,0)\}$ with lexicographic ordering: 
\[ (x, \alpha) \leq (y, \beta) \quad\defequiv\quad (x < y) \lor \big((x = y) \land (\alpha \leq \beta)\big). \]
We have
\[ \bigwedge \setbuilder{(n^{-1},1)}{n\in \N} = (0,1), \]
so
\[ F \defeq \upset\setbuilder{\interval{(0,1), (n^{-1},1)}}{n\in \N} \overset{\mathfrak{o}}{\longrightarrow} (1,0). \]
But also $F \amesh \big\{\R^+\times \{0\}\big\}$, so $F \vee \upset\big\{\R^+\times \{0\}\big\}$ is a proper filter and $(0,1) \in \adh_\mathfrak{o}\big(\{0\}\times\R\big)$


BETTER COUNTEREXAMPLE: Consider complete lattice $\interval{0,1}$ with dense subset $\interval[oc]{0,1}$.
\end{note}
...
\begin{proposition}
Let $X$ be a set and $L$ a complete lattice. Then the order convergence on $(X\to L)$ is the pointwise convergence.
\end{proposition}
It is important that $L$ be complete. Dedekind-complete is not enough!
\begin{proof}
TODO
\end{proof}
...
0 & (\text{otherwise}).
\end{cases} \]
Then $\seq{f_n}$ converges to $\constant{0}$ pointwise as a sequence in both $(\R\to\R)$ and $(\R\to\overline{\R})$. It converges in order in $(\R\to\overline{\R})$, but \emph{not} in $(\R\to\R)$.

By definition of the order convergence, we must have that $\seq{m(f_n)}$ does not converge to $m(\constant{0})$ in $\DM(\R\to \R)$. In particular, we do not have $\DM(\R\to \R) \cong (\R\to\overline{\R})$. Rather, we have $\DM(\R\to\R) \cong (\R\to\R)\cup\{\bot,\top\}$ from \ref{DMCompletionDedekindCompleteLattice}.
\end{example}
...
(2) We calculate, using \ref{pretopologicalFinalConvergence}, \ref{grillIntersectionUnion} and \ref{meshConnectionSetsOfSets}, \ref{upsetPreimageImageGaloisConnection} (with the fact that $\vicinities_{\xi_i}(x)$ is upwards closed, since it is a filter)
\begin{align*}
...
\begin{lemma} \label{vicinitiesPretopologicalSubspaceConvergence}
...
\end{corollary}
\begin{corollary} \label{productAdherenceFilterLemma}
Let $\sSet{X_i, \xi_i}$ be a (pre)convergence space for all $i\in I$ and $A\subseteq \bigtimes_{i\in I} X_i$. Then
\[ \seq{x_i}_{i\in I} \in \adh_{\prod_{i\in I}\xi_i}(A) \iff \exists \seq{F_i}_{i\in I}\in \powerfilters\Big(\bigcup_{i\in I}X_i\Big)^I: \Big(\forall i\in I: \big(F_i\in\powerfilters(X_i)\big) \land \big(F_i \overset{\xi_i}{\longrightarrow} x_i\big)\Big) \land \Big(A\in \bigotimes_{i\in I}F_i\Big). \]
In particular, suppose $\sSet{X,\xi},\sSet{Y, \zeta}$ are convergence spaces and $A\subseteq X\times Y$. Then $(x,y) \in \adh_{\xi\otimes \zeta}(A)$ \textup{if and only if} there exist $F\to x$ and $G\to y$ such that $A\in F\otimes G$.
\end{corollary}
\begin{proof}
From \ref{principalAdherenceInherence}, we have
\[ \seq{x_i}_{i\in I} \in \adh_{\prod_{i\in I}\xi_i}(A) \iff \exists F \in\powerfilters\Big(\bigtimes_{i\in I}X_i\Big): \Big(F \overset{\prod_{i\in I}\xi_i}{\longrightarrow} \seq{x_i}\Big) \land (A\in F). \]
Due to the proposition, we can replace $F$ by a product of convergent filters and this is equivalent to the statement.
\end{proof}
...
Let $I$ be a set and $X_i$ a set for all $i\in I$. Suppose $A_i \subseteq X_i$ is a subset and $F_i \in powerfilters(X_i)$ for all $i\in I$. Then $\upset\proj_i^{\imf\imf}\big(\bigotimes_{i\in I}F_i\big) = F_i$ for all $i\in I$.
\end{lemma}
\begin{proof}
Take $i\in I$. The equality is clear by considering basissets of $\upset \proj_i^{\imf\imf}\big(\bigotimes_{i\in I}F_i\big)$.
\end{proof}
...
\end{proposition}
TODO: analogous proposition for sequential spaces by replacing vicinities with neighbourhoods.
\begin{proof}
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space and $A,B\subseteq X$. We say $A$ and $B$ are \udef{separated by convergent filters} if for all convergence selections $F:X\to \powerfilters(X)$, the contours satisfy $\neg(F(A)\amesh F(B))$.
...
\end{lemma}
\begin{proof}
$(1) \Rightarrow (2)$ Assume $\lim^{-1}(x)\amesh \lim^{-1}(y)$, i.e.\ there exists some filter $F\in \lim^{-1}(x)\cap \lim^{-1}(y)$. Then the constant convergence selection $x,y\mapsto F$ has $F(x) = F(y)$, meaning $F(x)\amesh F(y)$ and thus $x,y$ are not separated by convergent filters.

$(2) \Rightarrow (1)$ Assume $\lim^{-1}(x)\perp \lim^{-1}(y)$. Pick an arbitrary convergence selection $F$. If $F(x)\amesh F(y)$, then $F(x)\vee F(y)$ is proper by \ref{joinProperFilter} and in $\lim^{-1}(x)\cap \lim^{-1}(y)$ by monotonicity, which is a contradiction.
\end{proof}
...
\end{proposition}
\begin{proof}
$(1)$ Assume $A,B\subseteq X$ are separated by vicinities $U$ and $V$. Assume, towards a contradiction, that $F(A)\amesh F(B)$ for some convergence selection $F$. Then for all $a\in A$, $U\in F(a)$, so $U\in F(A)$. Similarly $V\in F(B)$. This means $U\mesh V$, which is a contradiction.

$(2)$ If $\xi$ is pretopological, then $\vicinities_\xi$ is a convergence selection. Thus $\neg(\vicinities_\xi(A)\amesh \vicinities_\xi(B))$, meaning there exist disjoint vicinities of $A,B$.
\end{proof}
Thus in the pretopological case all convergence selections $F$ satisfy $\neg(F(A)\mesh F(B))$ iff the vicinity filter satisfies $\neg(\vicinities(A)\mesh \vicinities(B))$.
...
\begin{lemma} \label{HausdorffFilterPartition}
Let $\sSet{X,\xi}$ be a Hausdorff convergence space, $F,G\in \powerfilters(X)$ and $x,y\in X$ such that $F\to x$ and $G\to y$. If $F\amesh G$, then $x = y$.
\end{lemma}
\begin{proof}
We have that $F\vee G$ is proper, by \ref{joinProperFilter}. Also $F\subseteq F\vee G$ and $G\subseteq F\vee G$ imply $F\vee G \to x$ and $F\vee G \to y$. Since $\xi$ is Hausdorff, we have $x = y$.
\end{proof}
...
\end{proposition}
\begin{proof}
(1) Fix an arbitrary convergence selection $F: X\to \powerfilters(X)$ and $V_x\in \vicinities(x)$ for all $x\in X$.
...
\begin{proposition} \label{regularityContourConvergence}
Let $\sSet{X,\xi}$ be a convergence space. Then the following are equivalent:
\begin{enumerate}
\item $\xi$ is regular;
\item for every set $A$, every function $f: A\to X$ and every function $F: A\to \powerfilters(X)$ such that $F(a) \overset{\xi}{\longrightarrow} f(a)$ for all $a\in A$, we have
\[ \lim F(G) \subseteq \lim \upset f^{\imf\imf}(G) \]
for all $G\in \powerfilters(A)$;
\item for every set $A$, every function $f: A\to X$ and every function $F: A\to \powerultrafilters(X)$ such that $F(a) \overset{\xi}{\longrightarrow} f(a)$ for all $a\in A$, we have
\[ \lim F(G) \subseteq \lim \upset f^{\imf\imf}(G) \]
for all $G\in \powerfilters(A)$.
\end{enumerate}
\end{proposition}
The implication $(3) \Rightarrow (1)$ uses the ultrafilter lemma.
\begin{proof}
$(1) \Rightarrow (2)$ Assume $\xi$ is regular and pick arbitrary $A,f, F$ and $G$ as in the statement. 
From \ref{adherenceSelectionContour}, we have $\adh_\xi^\imf\big(F(G)\big) \subseteq \upset f^{\imf\imf}(G)$ and so
\[ \lim F(G) = \lim \adh_\xi^\imf\big(F(G)\big) \subseteq \lim \upset f^{\imf\imf}(G), \]
using regularity.

$(2) \Rightarrow (3)$ Clear.

$(3) \Rightarrow (1)$ Assume (3). Let $H\in \powerfilters(X)$ be a an arbitrary filter. Set $A \defeq X \times H$. For all $(x, B)\in X\times H$ such that $x\in \adh_\xi(B)$, there exists an ultrafilter $U_{x,B}$ such that $B\in U_{x,B}$ and $U_{x,B} \overset{\xi}{\longrightarrow} x$, from \ref{differentUnionsForAdherence}.

For $(x, B)\in X\times H$ such that $x\in \adh_\xi(B)$, we set $F(x,B) \defeq U_{x,B}$. For $(x, B)\in X\times H$ such that $x\notin \adh_\xi(B)$, we set $F(x,B) \defeq \pfilter{x}$. We set $f(x,B) \defeq x$. It is clear by construction that $F$ is an $f$-selection.

Consider
\[ G \defeq \upset \setbuilder[\Big]{\setbuilder[\big]{(x,C)}{x\in\adh_\xi(C), C\in H\cap \downset\{B\}}}{B\in H}. \]
We first show that $G$ is a filter. Since it is upwards closed by construction, it is enough to show that it is downwards directed. Take arbitrary $B,B'\in H$. Then
\begin{align*}
\setbuilder[\big]{(x,C)}{x\in\adh_\xi(C), C\in H\cap \downset\{B\}} \cap &\setbuilder[\big]{(x,C)}{x\in\adh_\xi(C), C\in H\cap \downset\{B'\}} \\
&\subseteq \setbuilder[\big]{(x,C)}{x\in\adh_\xi(C), C\in H\cap \downset\{B\cap B'\}},
\end{align*}
indeed, for any element $(x,C)$ of the left-hand side, it is enough to observe that $C \subseteq B\cap B'$. Since $B\cap B'\in H$, this implies that $G$ is downwards directed.

Next we observe that
\begin{align*}
\upset f^{\imf\imf}(G) &= \upset \setbuilder[\big]{\setbuilder{x}{\exists C\in H\cap \downset \{B\}: x\in \adh_\xi(C)}}{B\in H} \\
&= \upset \setbuilder[\big]{\setbuilder{x}{x\in \adh_\xi(B)}}{B\in H} \\
&= \upset \adh_\xi^\imf(H).
\end{align*}
Finally, we calculate, using the fact that $C\subseteq B$ implies $B\in U_{x,C}$ from the upwards closude of $U_{x,C}$,
\begin{align*}
F(G) &= \bigcup_{B\in H}\bigcap_{\substack{C\in H\cap \downset\{B\} \\ x\in \adh_\xi(C)}} U_{x,C} \\
&\supseteq \bigcup_{B\in H}\{B\} \\
&= H.
\end{align*}
Thus, by assumption, we have $\lim H \subseteq \lim F(G) \subseteq \lim \adh_\xi^\imf(H)$. Since $H$ was chosen arbitrarily, we have that $\xi$ is regular.
\end{proof}
\begin{corollary}
Let $\sSet{X,\xi}$ be a regular convergence space, $F$ a convergence selection and $G\in \powerfilters(X)$. Then $\lim F(G) \subseteq \lim G$.
\end{corollary}
...
\section{Diagonality}
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space. We call $\xi$ a \udef{diagonal} convergence if
\[ \lim_\xi F \subseteq \lim_\xi\big(f(F)\big) \]
for all $\xi$-selections $f: X\to \powerfilters(X)$.
\end{definition}
TODO: this concept is somehow dual to regularity.

\begin{proposition}
A pretopological convergence is topological \textup{if and only if} it is diagonal.
\end{proposition}
\begin{proof}
TODO!
\end{proof}
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space and $A\subseteq X$. We call $A$ \udef{functionally closed} if there exists a continuous function $f: X\to \R$ and a closed set $C\subseteq \R$ such that $A = f^{\preimf}(C)$.
\end{definition}
...
\begin{definition}
Let $\sSet{X,\xi}$ be a convergence space and $A,B\subseteq X$. Then $A$ and $B$ are \udef{functionally separated} if there exists a continuous function $f: X\to [0,1]\subseteq \R$ with $f^\imf(A) = \{0\}$ and $f^\imf(B) = \{1\}$.
\end{definition}
...
\begin{lemma}
Let $\seq{a_n}_{n\in \N}$ be a sequence in a normed abelian group. Then the series $\sum_{i=0}^\infty a_i$ is absolutely convergence \textup{if and only if} $\sum_{i=0}^\infty \norm{a_i} < \infty$.
\end{lemma}
\begin{proof}
Every bounded monotone sequence in $\R$ converges. TODO ref.
\end{proof}
